...
class SqlParser {
    ...
    public $handle = null;
    public static $querysections = array('alter', 'create', 'drop', 'select', 'delete', 'insert', 'update','from','where','limit','order','set');
    ...
    public static $setExtras=array('set','');
    public $query = '';
    ...
    public static function & ParseString($sqlQuery,$cleanWhitespace = true) {
        ...
        // post-process result
        if(isset($handle->query['set'])){
            $setAr=self::Tokenize($handle->query['set']);
// throw new Exception(var_export($setAr,true));
            $isleft=true;
            $lastkey='';
            $params=array();
            foreach($setAr as $setField){
                if(trim($setField)=='')	continue;
                if($setField=='=') $isleft=false;
                else if($setField==',') $isleft=true;
                else if($isleft) $lastkey=$setField;
                else $params[$lastkey]=$setField;
            }
            $handle->query['set_params']=$params;
            $handle->query['tablename']=trim(str_ireplace('update','',$handle->query['update']));

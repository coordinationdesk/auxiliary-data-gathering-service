...
*/ ------------------------------
ALTER TABLE t_dsqueue_x_cirqueue ADD globalstatus_id INT2 DEFAULT 7 NOT NULL ;
...
BEGIN
	...
	--      Starting Chek       --
	------------------------------
	IF pre_stat = 'ELIGIBLE' AND post_stat = 'UPLOADING' THEN
		RETURN 0 ;
...
BEGIN
	...
	FOR row IN
		...
		IF cq_stat = 'ERROR' THEN
			IF (SELECT type_id FROM t_datastoragequeue WHERE id = ds_id) = archive_type_id THEN
...
DECLARE
  	status INTEGER ;
BEGIN
  	status := p_dsqueuexcirqueue_statetrans(OLD.globalstatus_id, NEW.globalstatus_id) ;
	RETURN NEW ;
END ;
$$ LANGUAGE plpgsql ;
...
VALUES	(
		...
		20,
		'CIRCULATION_1-7-3',
		'Added globalstatus_id to t_dsqueue_x_cirqueue table. Added p_dsqueuexcirqueue_statetrans procedure and changed p_datastoragequeue_statetrans and p_datastoragequeue_update.'
) ;

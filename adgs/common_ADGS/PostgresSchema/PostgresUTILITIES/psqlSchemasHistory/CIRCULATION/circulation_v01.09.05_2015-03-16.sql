...
BEGIN
	...
	IF dsq_type = 'RETRIEVE' THEN
		...
		ELSIF pre_stat = 'ELIGIBLE' AND post_stat = 'DOWNLOADING' THEN
			RETURN 0 ;
		ELSIF pre_stat = 'DOWNLOADING' AND (post_stat = 'DISTRIBUTION' OR post_stat = 'ELIGIBLE' OR post_stat = 'COMPLETED' OR post_stat = 'LTA_DOWNLOADING' OR post_stat = 'NOTFOUNDERROR') THEN
			RETURN 0 ;
...
DELETE FROM t_schema WHERE version = 'circulation-1.9.5' ;
...
VALUES	(
		...
		20,
		'CIRCULATION_1-9-5',
		'Changed stored procedures p_datastoragequeue_statetrans.'
) ;

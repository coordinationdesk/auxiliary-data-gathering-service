...
usage() {
	echo "usage: GetCamsOriginDate.sh <JobOrderFile>"
	echo "-------------------------------------------------------------------"
	echo ""
	exit 1
}
...
init_log() {
	...
	echo "" | tee -a $LOG_FILE
	echo "############################################################" | tee -a $LOG_FILE
	echo "###  GET CAMS ORIGIN DATE" | tee -a $LOG_FILE
...
export LOG_FILE=$EXE_DIR/GetCamsOriginDate.log
init_log
...
# Extract manifest file from DBL
TMPOUT=`tar xvf $INP_FILE *.manifest` | tee -a $LOG_FILE

MNF_FILE=`ls z_cams_c_ecmf_*_prod.manifest`

# Chek if exists the manifest file 
check_file Input $MNF_FILE

# Get modification date of the input file
echo -e "Get the modification date of the input file ..." | tee -a $LOG_FILE
ORDATE=`date -r $MNF_FILE +"UTC=%Y-%m-%dT%H:%M:%S.000000"`

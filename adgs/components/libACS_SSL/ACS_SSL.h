...
public:
	...
	public:
		ACS_AES_handle(ACS_AES_Cipher cipher, const ACS_AES_Key&, const ACS_AES_IV&, bool enc_oper, bool paddingSet=true) ;
		...
		size_t getKeyLength() const { return _keyLength ; } // In BYTES, so e.g. 16 for AES-128bit
		size_t getBlockLength() const { return _blockLength ; } // In BYTES (always 16 bytes for every AES)
		...
		static size_t getKeyLength(ACS_AES_Cipher cipher) ;	  // Static: can be run BEFORE instancing the class, to prepare the input parameters
		static size_t getBlockLength(ACS_AES_Cipher cipher) ; // Static: can be run BEFORE instancing the class, to prepare the input parameters
	...
	enum EncryptionAlgo
	{
		...
		ENC_AES128CBC		= 0x0500,		/* 2048:  AES_128_CBC */

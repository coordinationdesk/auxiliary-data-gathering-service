...
Application::PostLogHooks *Application::PostLogHooks::_instance = 0 ;
...
{
    ACS_COND_THROW(_instance!=0,Application::Exception("Another Builder already exists.")); // PRQA S 3081
    _instance=this;
}
...
{
    ACS_COND_THROW(_instance==0,Application::Exception("No application Builder has been built.")); // PRQA S 3081
...
// exit with a false or true response
    ...
    if ((0 != _log) && (_log->creationTime() + time_t(ms/1000) < ::time(0))) {
        return true ;
    }
...
// PRQA S 4631 L1
    ...
    _instance=0; // no more application instances (ther can be only one at time)
...
int Application::run(const int argc, char const * const * argv, char const * const * env) // PRQA S 4020
{
    ...
    ACS_COND_THROW(app == 0,Application::Exception("The Application has not been instantiated.")); // PRQA S 3081
    ...
    lastStopModeChangeTime_()
{
    ...
    if(_instance!=0) // any instance till now ?
    {
        // not a GOOD THING
...
bool Application::checkStopManagerThread() const
{
    ...
    if ( ! stopManagerThread_->running() )
    {
        ...
        if (!errorEmitted)
        {
            ...
            {
                exception* e;
                stopManagerThread_->runException(e);
                if(e)
                {

...
std::string AwsS3Wrapper::uploadSmallObject(const string& local_object, const string& bucket_name_in, const string& s3key_in)
{
	...
	*/
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Set PutObjectRequest for bucket name: \"" << bucket_name << "\"  key: \"" << s3_remote_key << "\"") ;

	const size_t to_read(1024) ; // FIXME REPLACE WITH: 1048576
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Bucket name: \"" + bucket_name_in + "\"  local_object: \"" + local_object + "\"" \
		+ "  s3_remote_key: \"" << s3_remote_key << "\"  bytes transferred: " << r_tot << "  over " << iter_tot << " iterations" ) ;
											...
											const multimap <Enum_S3MetadataTag, std::string>& tags_M) 
{
	...
	*/
	...
	shared_ptr<Aws::Utils::Threading::PooledThreadExecutor> pooledThreadExecutor = \
		Aws::MakeShared<Aws::Utils::Threading::PooledThreadExecutor>("executor", get_upload_pool_size());
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Starting TransferManager for bucket name: \"" << bucket_name << "\"  key: \"" << s3_remote_key << "\"") ;
...
off64_t AwsS3Wrapper::downloadObject(const string& bucket_name_in, const string& s3key_in, const string& local_object)
{
	...
	*/
	...
	Aws::Utils::Threading::DefaultExecutor defaultExecutor ;
	shared_ptr<Aws::Utils::Threading::PooledThreadExecutor> pooledThreadExecutor = \
		Aws::MakeShared<Aws::Utils::Threading::PooledThreadExecutor>("executor", get_download_pool_size());
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Bucket name: \"" + bucket_name_in + "\"  s3_remote_key: \"" << s3_remote_key 
		...
		for (size_t i=0; i<path_V.size(); i++)
		{
			...
			if ( (true == recursive) || (i == (path_V.size()-1)) )
			{
				if ( (false == intermed_path.empty()) && (false == isDirectoryObject(bucket_name_in, intermed_path)) ) 
				{
					...
					ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Setting PutObjectRequest for bucket name: \"" << bucket_name << "\"  key: \"" << s3_remote_key << "\"") ;
...
/**********************************************************************************
  ...
  *	                                                                              *
  **********************************************************************************/
ostream &operator<<(ostream &os, const Aws::String &s) 	// output to stream
{
	const string out( (not s.empty())? string(s.c_str(), s.length()) : "") ; 
...
acs::exostream &operator<<(acs::exostream &os, const Aws::String &s) 	// output to stream
{
	const string out( (not s.empty())? string(s.c_str(), s.length()) : "") ; 
...
ostream &operator<<(ostream &os, const AwsS3Wrapper::AwsS3WrapperConfiguration &v) 	// output to stream
{
	os	<< "provider = \"" 			<< AwsS3Wrapper::toStr(v.get_aws_s3_provider())	<< "\"\n"
		...
		<< "endpointIsSecure = "	<< boolalpha << v.get_endpoint_is_secure() 		<< "\n" ;	 

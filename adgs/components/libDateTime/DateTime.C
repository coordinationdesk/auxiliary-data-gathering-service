...
void DateTime::JD50::exportXML(XMLOstream &xst) const
{
	...
	os << setiosflags(ios::fixed) << setprecision(17) << _jd ; // PRQA S 4400
...
#warning BOOST does not defines nanoseconds
        ...
        if (pt.is_special()) {
            ostringstream os ;
            os << " invalid time: " << year << "-" << month << "-" << day << ":" <<  hour << ":" << min << ":" << sec << "." << frac ;
            ACS_THROW(InvalidTimeException(os.str())) ; // PRQA S 3081
        }
		...
		if (genException) {
       		ostringstream os ;
        	os << " invalid time: " << year << "-" << month << "-" << day << ":" <<  hour << ":" << min << ":" << sec << "." << frac ;
        	ACS_THROW(InvalidTimeException(x,os.str())) ;
		}
		return false ;
	}
}
...
const string &DateTime::unfilledDate() throw()
{
...
int DateTime::month2Num(const string &month) // PRQA S  4020
{    
...
const string &DateTime::num2Month(int m)
{
    ...
    if ((m<1) || (m>12)) { // PRQA S 4400
        ...
        ACS_THROW(InvalidTimeException("Cannot convert int("+os.str()+") to month")) // PRQA S 3081
    }
    return months[m-1] ;
}
...
pair<unsigned short, unsigned int> DateTime::weekOfYear() const // PRQA S 4020
{
    DateTime::UTCD utcd;
    toUTCD(utcd) ; 
    unsigned short woy = utcd.weekNumber() ; // PRQA S 3000, 3010
    switch(woy) { // PRQA S 3000
    case 1:
        {
            if (utcd.month == 12) { // PRQA S 3000, 4400 2
                return make_pair(woy, utcd.year+1) ;
            }
            else {
...
DateTime DateTime::dateOfWeek(const pair<unsigned short, unsigned int> &p)  
{
    ...
    int o = p.first * 7 + 1 - ( UTCD::dayOfTheWeek(4, 1, p.second) + 3 ) ; // PRQA S 3000, 3010, 4400
    int diy = UTCD::dayOfYear(31,12,p.second) ; // PRQA S 3010
    unsigned short year = p.second ; // PRQA S 3010
    if (o < 1) {
        --year ;
    ...
    UTCD::dayAndMonthFromDOY(o, year, day, month); // PRQA S 3000, 3010
...
namespace { // PRQA S 2134 L5
    ...
    inline int toNum(const string &month) { // PRQA S 2134, 4020
    ...
    DateTime::Y2ns_type from_D_month_Y_h_m_s_f(const string &s) { // PRQA S 4020
        ...
        char *end = 0 ;
        const char *bgn = &s[0] ;
        y2ns.D = strtoul(bgn, &end, 10) ; // PRQA S 3010, 4400 L5
        ...
        y2ns.ns = DateTime::convertDigits (ns, nsl, 9);
    ...
    DateTime::Y2ns_type from_YMD_hmsf(const string &s) { // PRQA S 4020
        ...
        char *end = 0 ;
        y2ns.Y = strtoul(s.substr(0,4).c_str(), &end, 10) ; // PRQA S 3010, 4400 L7
        ACS_COND_THROW(*end != 0, exIllegalValueException("Unrecognized year")) ; // PRQA S 3081 L7
        y2ns.M = strtoul(s.substr(4,2).c_str(), &end, 10) ;
        ACS_COND_THROW(*end != 0, exIllegalValueException("Unrecognized month")) ;
        y2ns.D = strtoul(s.substr(6,2).c_str(), &end, 10) ;
    ...
    DateTime::Y2ns_type from_Y_M_D_h_m_s_f(const string &s) { // PRQA S 4020
        ...
        char *end = 0 ;
        const char *bgn = &s[0] ;
        y2ns.Y = strtoul(bgn, &end, 10) ; // PRQA S 3010, 4400 L5
    ...
    DateTime::Y2ns_type from_D_mon_Y_h_m_s_f(const string &s) { // PRQA S 4020
        ...
        end = const_cast<char *>(&s[6]) ;  // PRQA S 3081 L5     
        ACS_COND_THROW(not is_dash(*end), exIllegalValueException("Unrecognized month or too short")) ; // PRQA S 3081
        bgn = end+1 ;
        y2ns.Y = strtoul(bgn, &end, 10) ;
    ...
    DateTime::Y2ns_type from_Y_mon_D_h_m_s_f(const string &s) { // PRQA S 4020
        ...
        end = const_cast<char *>(&s[8]) ;  // PRQA S 3081 L5     
        ACS_COND_THROW(not is_dash(*end), exIllegalValueException("Unrecognized month or too short")) ; // PRQA S 3081
        bgn = end+1 ;
        y2ns.D = strtoul(bgn, &end, 10) ;
...
// NB: this is a very tricky method: it deals with precision computation, rounding and negative values
// eg: a jd50 value x and -x should be always symmetric wrt to the origin (Jan 1, 1950 - midnight).
// so 1 ns after is just 1 ns after midnight, while 1 ns BEFORE (negative) is the last ns of Dec 31, 1949
DateTime::Y2ns_type DateTime::jd50_to_Y2ns(long double jd50, int digits, bool truncate) 
{ 
    ...
    digits = max(0,digits) ; // take care of domain: digits >= 0
	try {
        interval_day i = fromAbsJD50(jd50) ;
        Y2ns_type c ; // PRQA S 4102
        if (jd50 >=0) 
        {
            c.ns = rintl(i.ps/1000.0L) ;
            ...
            if (digits < 9) // take care of domain: digits < 9
            {
                ...
                {
                    c.ns = rintl (c.ns / exp10l (9 - digits) ); // PRQA S 3084
                }
                c.ns = c.ns * exp10_v[9 - digits] ; // PRQA S 3084
            }
        ...
        { 
            ...
            if (digits < 9) // take care of domain: digits < 9 
            {
                ...
                if (truncate)
                {
                    ...
                    // was ns = truncl (ns / exp10l (9 - digits) );

                }
                else
                {
                    negns = rintl (negns / exp10l (9 - digits) );
                }
                negns = negns * exp10_v[9 - digits] ; // PRQA S 3084
            }
		...
		ostringstream os ;
		os << "Invalid JD50: " << setprecision(12) << jd50 << " [JD50]" ; // PRQA S 4400
		ACS_THROW(InvalidTimeException(x,os.str())) ; // PRQA S 3081
	}
}
...
DateTime::Y2ns_type DateTime::jd50_to_Y2D(long double jd50) 
{
	try {
		...
		long dd = jd50 ; // keep the sign // PRQA S 3011
...
boost::posix_time::ptime DateTime::jd50_to_ptime(long double jd50) 
{
	try {
        ...
        long nsecs = ((s-int(s)) * 1000000000 + 0.5) ;
...
DateTime::DateTime(const boost::posix_time::ptime &pt) throw() : 
    jd50_(ptime_to_jd50(pt)), 
    _filled(!pt.is_special())
{
}

bool DateTime::isOlder(time_t secs) const { return *this < DateTime(time(0) - secs) ; } // PRQA S 3081
...
istream &operator >> (istream &is, DateTime& d)
{

...
SQLDialect::SQLCast::SQLCast(const SQLCast &r) :
	_castType(r._castType),
	_arg1(r._arg1),
	_arg2(r._arg2)
{  }

SQLDialect::SQLCast::~SQLCast() throw() {} 

SQLDialect::SQLCast & SQLDialect::SQLCast::operator=(const SQLCast &r)
{   
    if (this != &r ) {
	    _castType = r._castType ;
	    _arg1 = r._arg1 ;
...
SQLDialect::~SQLDialect() throw() 
{
// PRQA S 4631 5
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
}

const string &SQLDialect::dialectName() const throw() // PRQA S 4120
{
	return _dialectName ;
}
...
// W8115_OFF: multiple returns
    ...
    */
	map<SQLToken, string const>::const_iterator it=_sqlTokenMap.find(s);
...
// W8115_OFF: multiple returns
    /**
        ...
        Useful for dumping values, also if no translation is available.
    */
	map<SQLJoinToken, string const>::const_iterator it=_sqlJoinTokenMap.find(s);
...
// W8115_OFF: multiple returns
    ...
    */
	...
	map<SQLOperator, string const>::const_iterator it=_sqlOperatorMap.find(s);
...
string SQLDialect::translateColumnName(const string &s_in) const // PRQA S 4020, 4214
{
    ...
    map <string, string>::const_iterator it = _columnTranslationMap.find( StringUtils::lowercase(colname) );
...
bool SQLDialect::listenToMessages (dbConnection *conn, vector<string>& messages_V, const uint64_t& timeout_usec) const 
{
	/* This method should be re-implemented. The base class is not able to handle this. */
...
{
	string outString ;
	char oct[16] ;  // PRQA S 4403
		
	for (size_t i=0; i<l; i++) // each char 
	{
		char c = *(buffer + i) ; // PRQA S 3084
		
		sprintf(oct, "\\%03o", int(c) ); // convert it in the form \%OOO // PRQA S 3081
		
		outString += string(oct) ; // add to the finale result // PRQA S 3081
	}

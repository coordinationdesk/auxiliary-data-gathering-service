...
public:	
	...
	public:
		...
		~SQLCast() throw() ;
		SQLCast (const SQLCast &) ;
		SQLCast& operator= (const SQLCast &) ;

		SQLCastType getSQLCastType() const throw() { return _castType ; }
		const std::string &getArg1() const throw() { return _arg1 ; }
		const std::string &getArg2() const throw() { return _arg2 ; }
...
protected:    
    ...
    private:
        GuardFileDesc() ;   // not implemented
        GuardFileDesc(const GuardFileDesc &) ;   // not implemented
        GuardFileDesc & operator= (const GuardFileDesc &) ;   // not implemented
...
public:	
    ...
    virtual ~SQLDialect() throw() ; 


    const std::string &dialectName() const throw() ;


    virtual void setFastConnect() const = 0 ;

...
public:
	SQLDialectAppSpecific() ;
	virtual ~SQLDialectAppSpecific() throw() ; 
	virtual void init(SQLDialect &) ;
private:
	SQLDialectAppSpecific(const SQLDialectAppSpecific &) ;
	SQLDialectAppSpecific & operator=(const SQLDialectAppSpecific &) ;
} ;
...
public:
	...
	virtual ~SQLDialectFactory() throw() ; 
...
private:
    ...
    SQLDialectFactory(const SQLDialectFactory &) ;
    
	SQLDialectFactory &operator=(const SQLDialectFactory &) ;
...
private:
	...
	static std::map <std::string, DBXSH_PTR_NSPACE::shared_ptr<Plugin> > _dialectPlugin ;

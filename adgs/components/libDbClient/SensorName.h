...
#ifndef _SensorName_H_
#define _SensorName_H_
...
//
//
// class SensorName Name
//
//

class SensorName:   // PRQA S 2153
    public NameIdAssociator<unsigned int>, 
    public ParamConstructibleSingletonMap<SensorName, std::string>
{
    friend SensorName* ParamConstructibleSingletonMap<SensorName, std::string>::instance(const std::string &); // PRQA S 2107
public:
	virtual ~SensorName() throw() {}  // PRQA S 2131
	
protected:
	explicit SensorName(const std::string &key) :
        NameIdAssociator<unsigned int>("T_Sensors", "sensorid", "sensorname", key),
        ParamConstructibleSingletonMap<SensorName, std::string>()
        {
            NameIdAssociator<unsigned int>::load() ;
        }

private:
	SensorName () ; 			// declared but not implemented
	SensorName (const SensorName &) ; 			// declared but not implemented
	SensorName &operator=(const SensorName &) ; 	// declared but not implemented
} ; 


_ACS_END_NAMESPACE

#endif // _SensorName_H_

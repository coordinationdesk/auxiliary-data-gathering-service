...
std::string dbAppResSet::candidate_conf_file(const std::string &key, const std::string &conf, std::string &newkey) // PRQA S 4020
{
    ...
    if (key.empty()) { // note: this block infinite recursive call
        ...
        ACS_COND_THROW(not File::isReadable(fullp), // PRQA S 3081
...
// any of them introduces at least a conditional expression
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "stack: \n" << exStackTrace::getStack(true) ) ;
	...
	ACS_COND_THROW(not file.good(),exFileOpenException("Cannot open \"" + conf_file +"\"" ,errno)) ;
...
map<string, dbAppResSet*> dbAppResSetFactory::_instances ; 
...
dbAppResSet* dbAppResSetFactory::instance(const std::string& conf, const std::string& key) // PRQA S 4020
{
	...
	const map<string, dbAppResSet*>::iterator i =  _instances.find(key) ; 
	...
	if(!conf.empty()) {
		tmp = new dbAppResSet(key, conf) ; // new conf // PRQA S 3081
	} else {
		tmp = new dbAppResSet(key, dbAppResSet::getDefaultDbConfName()) ; // default conf // PRQA S 3081
	}	
...
dbAppResSetFactory::dbAppResSetFactory()
{

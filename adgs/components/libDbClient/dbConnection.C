...
dbConnection::Memento::~Memento() throw()
{
}
...
dbConnection::~dbConnection() throw() 
{
// PRQA S 4631 L1
	try {
...
void dbConnection::beginWork(const string &transStatLabel)
{
	...
	ACS_COND_THROW(transactionState() != None,CommitException("Transaction is already started or cannot be started now")) ; // PRQA S 3081
	try {
		...
		ACS_COND_THROW(!query.execForStatus(), // PRQA S 3081
...
void dbConnection::clearAggregateTransactionState()
{
...
long dbConnection::getLastError() const throw() 
{
    /* Return Error Code */ 
    return _lastError ;
}

const string &dbConnection::getLastErrorText() const throw() 
{
    /* Return Error Code */ 

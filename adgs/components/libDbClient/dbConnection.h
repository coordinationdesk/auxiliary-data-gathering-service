...
public:
	...
	virtual ~dbConnection() ACS_NOEXCEPT ;
...
public:
	 ...
	 */
    const std::string &getLastErrorText() const ACS_NOEXCEPT ;
...
private:
	 ...
	 */
    long getLastError() const ACS_NOEXCEPT ;
public:    
	 ...
	 */
    void setMaxQueryDurationSecs(int s) ACS_NOEXCEPT { _maxQueryDurationSecs = s ; } 
	/**
	 * returns the interval in secs that would cause a warning if any query should last longer.
	 */
    int  getMaxQueryDurationSecs() const ACS_NOEXCEPT { return _maxQueryDurationSecs ; } 
private:
	 ...
	 */
	void clearAggregateTransactionState() ;

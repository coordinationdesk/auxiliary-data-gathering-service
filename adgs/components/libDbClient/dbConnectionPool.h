...
public:
	...
	~dbConnectionPoolElem() ACS_NOEXCEPT ;
...
public:
	virtual ~ConnPools() ACS_NOEXCEPT ;
...
private:  
    std::map<std::string, DBXSH_PTR_NSPACE::shared_ptr<dbConnectionPool> > pools_ ;  
...
public:
	...
	~dbConnectionWrapper() ACS_NOEXCEPT ;
...
public:
	virtual ~dbConnectionPool() ACS_NOEXCEPT ;
	...
	void setMaxConnectionAttempts(unsigned short) ACS_NOEXCEPT ;
	void setMaxConnection(size_t) ;
	void setAging(int) ;
    size_t getMaxConnection() const ACS_NOEXCEPT ;
    const std::string &getKey() const ACS_NOEXCEPT ;
    static math::StatNode &statN(const std::string &key) ;
private:
	void destroyElement(dbConnectionPoolElem *) ;
	void wrongConn() ACS_NOEXCEPT ;
	void goodConn() ACS_NOEXCEPT ;

...
public:
	dbSqlStringParamItem(const std::string &k, const SQLString &s) : dbParamItem(k), _s(s) {}
	virtual ~dbSqlStringParamItem() throw() {} // PRQA S 2131

	virtual dbParamItem *clone() const { return new dbSqlStringParamItem(*this) ; } // PRQA S 2131, 2502
	virtual std::string getValue() const ;

    dbSqlStringParamItem(const dbSqlStringParamItem &r) : dbParamItem(r), _s(r._s) {}
    dbSqlStringParamItem & operator=(const dbSqlStringParamItem &r) 
    { if (this != &r) { dbParamItem::operator=(r) ;  _s = r._s; } return *this ; }
private:
    dbSqlStringParamItem() ; // not implemented
private:
	SQLString _s ;
} ;
...
pair<std::string, std::string> dbParam::obtainTableAndField(const std::string &key, const std::string &paramName) // PRQA S 4020
{
...
dbParam::dbParam(const string &tableName, const string &fieldName, Operation operation, const string &value, const string &value2) :
	_sqlString(),
    _leftConditionPart(),
	...
	_key("") 
{
	ACS_COND_THROW((_operation == IN) || (_operation == NOT_IN), // PRQA S 3081
							  InvalidOperation("Invalid operation (IN) with this ctor") ) ;
}

dbParam::dbParam(const string &tableName, const string &fieldName, Operation operation) :
	_sqlString(),
    _leftConditionPart(),
	_tableName(tableName),
	_fieldName(SQLDialectFactory::sqlDialect("").translateColumnName(fieldName)),
...
dbParam::dbParam(const string &tableName, 
...
dbParam::dbParam(const pair<string, string > &tblAndCol, 
				 Operation operation, 
				 const string &value, 
				 const string &value2) :
	_sqlString(),
...
dbParam::dbParam(Operation operation, 
	...
	_key("") 
{
	...
	ACS_COND_THROW((_operation == IN) || (_operation == NOT_IN), // PRQA S 3081 6
							  InvalidOperation("Invalid operation with this ctor") ) ;
	ACS_COND_THROW(((_operation == BETWEEN) || (operation == NOT_BETWEEN) ) && (value1.empty() || value2.empty()),
							  InvalidOperation("BETWEEN/NOT_BETWEEN operation requires two values !") ) ;
	ACS_COND_THROW(((operation != ISNULL) && (operation != ISNOTNULL) ) && (value1.empty() ),
...
dbParam::dbParam(const std::string &key, const string &tableName, const string &fieldName, Operation operation, const string &value, const string &value2) :
	_sqlString(),
    _leftConditionPart(),
	_tableName(tableName),
...
dbParam::dbParam(const std::string &key, 
...
dbParam::dbParam(const std::string &key, const string &tableName, 
				 const string &fieldName, 
				 Operation operation, 
				 const dbGeoObject &v) :
	_sqlString(),
...
dbParam::dbParam(const std::string &key, const pair<string, string > &tblAndCol, 
				 Operation operation, 
				 const string &value, 
				 const string &value2) :
	_sqlString(),
...
dbParam::dbParam(const std::string &key, Operation operation, 
				 SQLString const &leftConditionPart, 
				 const std::string &value1, 
				 const std::string &value2 ) :  
	_sqlString(),
...
dbGenericColumnType::Type dbParam::getType(const std::string &table, const std::string &column, dbConnection &conn) // PRQA S 4020
{
// W8115_OFF: multiple returns
...
// stream output
ostream &operator<<(ostream &str, const dbParam &par)
{
	// read input
	str << par._tableName << "\n" 
		<< par._fieldName << "\n" 
...
void dbParam::ReadSingleParameter(istream &str, string &res)
{
	stringbuf line ;
	str.get(line) ;
...
// stream input
istream &operator>>(istream &str, dbParam &par)
{
	// read from a stream
...
std::string dbParam::getSqlString() const // PRQA S 4020, 4214, 5500, 5510
{
...
// any attemp to lower CC would increase complexity for maintenance
	...
	case dbParam::OUTSIDE:
        {
            const std::string &dname = SQLDialectFactory::dialectName(_key) ;
            if ( dname == "Informix9")
            {
			    sqlString += getOperatorName(_key, _operation) ; 
			    sqlString += sqlParameterName ; 
...
std::vector<dbParam::Operation> dbParam::operationsInMask(int mask)
{

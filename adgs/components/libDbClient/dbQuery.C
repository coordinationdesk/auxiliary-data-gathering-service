...
{
public:
    StaleConnectionGuard() = delete;
    StaleConnectionGuard(const StaleConnectionGuard&) = delete;
    StaleConnectionGuard& operator=(const StaleConnectionGuard&) = delete;
    StaleConnectionGuard(StaleConnectionGuard&&) = delete;
    StaleConnectionGuard& operator=(StaleConnectionGuard&&) = delete;
    ...
    virtual ~StaleConnectionGuard() ACS_NOEXCEPT
    {
    ...
    _connection (conn) // PRQA S 2528
{
    ...
    if (! _connection.isOpen(checkIsGood) )
    {
        ...
        ACS_THROW( exIllegalValueException( "dbTemporaryTable::dbTemporaryTable  Connection not open!" ) ); // PRQA S 3081
    }
    ...
    timeval tim; // PRQA S 4102
    gettimeofday(&tim, 0);
    srand(tim.tv_sec + tim.tv_usec) ;  // set seed // PRQA S 3000, 3010
    
}


dbTemporaryTable::~dbTemporaryTable() throw()
{
// PRQA S 4631 L2
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "tableCreated: " << boolalpha << _tableCreated << "  selfRemove: " << _selfRemove) ;
...
void dbTemporaryTable::setColumnName( const string& columnName ) { // PRQA S 4121
...
string dbTemporaryTable::createTable() 
{
    ...
    StatNode &stat = statN(_connection.getKey())["createTable [ms]"] ; // PRQA S 4412

    if (_tableCreated)
    {
        (stat["error #"]+=1)["table exists #"]+=1 ;
        ACS_THROW( exIllegalValueException( "dbTemporaryTable::dbTemporaryTable  Table already created !" ) ); // PRQA S 3081
    }
    ...
    while (!_tableCreated && retrySupported && (++attempts < maxAttempts)) ; // PRQA S 3230
    
    if (!_tableCreated)
    {
        (stat["error #"]+=1)["retries exceeded #"]+=1 ;
        ostringstream o ;
        o << "dbTemporaryTable::createTable()  Unable to create temporary table after: " << attempts << " attempts" ;
    ...
    timer.stop() ;
    stat += double(timer.elapsed()) ; // PRQA S 3081
    stat["ok #"]+=1 ;
...
namespace {
    ...
    void getSeparators(const string &dialect, dbTemporaryTable::ctypeEnum c, string &L, string &R) {
        ...
        default:
            {
                ostringstream os ;
                os << "CType not handled (" << int(c)<< ')' ; // PRQA S 3081 2
                ACS_THROW( exIllegalValueException( os.str() ) );
            }
        } // switch
    }
    
    string getInsertQuery(const string &dialect, const string &tblname) // PRQA S 4020
    {
        ...
        if ( dialect == "Informix9" ) {
            ...
            ACS_THROW( exIllegalValueException( "SQL Dialect not handled: " + dialect ) ); // PRQA S 3081
        }
    }

    string getInsertQueryTail(const string &dialect)  // PRQA S 4020
    {
       //
    ...
    string getInsertQueryElem(const string &dialect,  // PRQA S 4020
                              size_t i, 
                              const string &L, 
                              const string &R,
                              const string &el,
                              const string &table)
    {
        ...
        if ( dialect == "Informix9" ) {
            return ((i != 0) ? ", " : "") + L + el + R ; // PRQA S 3054, 3380, 3383
        }
        else if ( dialect == "Postgres8" ) {
            return ((i != 0)? ", " : "") + string("(") + L + el + R + ")"; // PRQA S 3054, 3081, 3380, 3383
        }
        else if ( dialect == "Oracle11" ) {
...
//
void dbTemporaryTable::insertValues(const vector<string> & v) // PRQA S 4020, 5510
{
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;

    Timer timer ;
    timer.start() ;
    StatNode &stat = statN(_connection.getKey())["insertValues [ms]"] ; // PRQA S 4412

    if (!_tableCreated)
    {
        (stat["error #"]+=1)["no table #"]+=1 ;
        ACS_THROW( exIllegalValueException( "Table not created" ) ); // PRQA S 3081
    }
    ...
    if ( dialect == "Informix9" ) 
    {
        max_length = 60*1024;    /* 2011-02-01  -  Max: 64Kb */ // PRQA S 4400
    ...
    else if ( dialect == "Postgres8" ) 
    {
        ...
        max_length = 128*1024 ;    // PRQA S 4400
        getSeparators(dialect,_ctype, sepL, sepR) ;
    }
    else if ( dialect == "Oracle11" ) 
    {
    ...
    while (n<vsize) 
    {
        ...
        while ( (n<vsize) && (i<max_length) && ((i+v[n].length())<max_length) ) // PRQA S 3084
        {
            ...
            ACS_COND_THROW( (strLen > 0 ) && ( ssize_t(v[n].length()) > strLen), // PRQA S 3081
        ...
        try {
            ...
            ACS_THROW(exIllegalValueException(x,"Error executing SQL command. Query string is: " +  s.getSQL()) ) ; // PRQA S 3081
        }
...
//
void dbTemporaryTable::removeTable() // PRQA S 4020
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "tableCreated: " << boolalpha << _tableCreated ) ;
    Timer timer ;
    timer.start() ;
    StatNode &stat = statN(_connection.getKey())["removeTable [ms]"] ; // PRQA S 4412
    ...
    else if ( dialect == "Oracle11" ) 
    {
        ...
        ACS_THROW( exIllegalValueException( "dbTemporaryTable::insertValues  SQL Dialect not handled: " + dialect ) ); // PRQA S 3081
    }
    ...
    try {
        ...
        {
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Executing sql command #" << (i+1) << "/" << sqlQuery.size() << ": !" << sqlQuery[i] << "!")  // PRQA S 3084
        ...
        (stat["error #"]+=1)["execForStatus #"]+=1 ; // PRQA S 4412
        ACS_THROW(exIllegalValueException(x,"Error executing SQL command. Query string is: " +  s.getSQL()) ) ; // PRQA S 3081
    }
...
//
std::string dbTemporaryTable::createTableSQL() const // PRQA S 4214
{    
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
...
{
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
    ACS_COND_THROW( !_tableCreated, exIllegalValueException( "dbTemporaryTable::dbTemporaryTable  Table not created" ) ); // PRQA S 3081
...
//
string dbTemporaryTable::randomName(const string& dialectName)
{    
/*
...
*/
    ...
    ACS_COND_THROW( 0 != gettimeofday(&tv, NULL), exIllegalValueException( "dbTemporaryTable::randomName  Error calling gettimeofday") ) ; // PRQA S 3081
    ...
    sprintf(rn, "TT_%06X%05X%06X", r[0], r[1], r[2]) ; // PRQA S 4400

    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method finished: !" << rn << "!") ;
    ...
    dbTemporaryTable::getDbType(enum ctypeEnum ctype, const string& dialectName, ssize_t* pTypeLength) 
{
    ...
    case INTEGER_TYPE: 
        {
            if ( dialectName == "Informix9" ) {
                ...
                ACS_THROW( exIllegalValueException("dbTemporaryTable::getDbType:  Dialect not handled: " + dialectName) ); // PRQA S 3081
            }
     ...
     case STRING: 
        {
            if ( dialectName == "Informix9" )
            {    l = 1024; /* Max: 32739 */ t << "LVARCHAR(" << l << ")" ; } // PRQA S 4400
            else if ( dialectName == "Postgres8" )
            {    l = 1024; /* Max: <~ 1GB */ t << "VARCHAR(" << l << ")" ; } // PRQA S 4400
            else if ( dialectName == "Oracle11" )
            {    l = 1024; /* Max: 4000 */ t << "VARCHAR2(" << l << ")" ; } // PRQA S 4400
            else {
                ACS_THROW( exIllegalValueException("dbTemporaryTable::getDbType:  Dialect not handled: " + dialectName) ); // PRQA S 3081
            }
    ...
    default:
        {
            ostringstream msg ;
            msg << "ctype not handled: " << int(ctype) ; // PRQA S 3081
    ...
    _stat(doStat)
{
    ...
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Conn ptr: " << hex << &_connection) ;
}
...
bool dbQuery::execForStatus() 
{
    ...
    vector<dbSpecificColumnType::Type> fakeVector ;
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Conn ptr: " << hex << &_connection << dec // PRQA S 3380
            << " connection: " << (!_connection.isOpen(false) ? "not " : "") << "open"
            << "  About to execute query: \"" << _sqlString.getSQL() << "\"" );
    StatNode &stat = _stat ? (statN(_connection.getKey())+=1)["execOneRow"]+=1 : statN(_connection.getKey())["execOneRow"] ; // PRQA S 4412
     ...
     for (timer.start();;Timer::randomDelay(_defaultMsecCycleDelay)) {
        ...
        try {
            ...
            if (eType != NoError) {
                ...
                _errorString = _success ? _connection.getLastErrorText() : string("") ; // PRQA S 3081, 3380, 3385
                ...
                if (eType == PrepareError) {
                    ...
                    ACS_THROW(QueryExecuteException(string("dbQuery: ") + " " + _errorString + " Query is : " + _sqlString.getSQL() )) ; // PRQA S 3081
                }
                else { // should be an ExecuteError or EmptyCursor
                    updateStat2(stat,"error #","lock #") ; 
                    ostringstream os;
                    os << "Conn ptr: " << hex << &_connection << " " ;
                    ACS_THROW(QueryLockException( os.str() + string("dbQuery:") + " Lock Error " + _errorString + " Query is : " + _sqlString.getSQL() )) ; // PRQA S 3081
                }
            }
            /*
                Get Row
            ...
            else
            {
                ...
                */
                ostringstream os ;
                os << commandStatus ;
                ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Conn ptr: " << hex << &_connection << dec
                        << " CommandStatus: " << os.str()
                        << " Error: " << _connection.errorText(itcrow)) ;
                updateStat2(stat,"error #","lock #") ; 
                ACS_THROW(QueryLockException(string("dbQuery:") + " Lock Error " + // PRQA S 3081
                                                   " Got status: " + os.str() +  
                                                   " Error: " + _connection.errorText(itcrow) +
            ...
            if (_connection.getLockMode() == dbConnection::WaitForSeconds) {
                ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Elapsed: " << timer.elapsed() << " Step duration: " << stepTimer.elapsed())  ;
                 ...
                 if (long(timer.elapsed()) > _connection.getLockWaitSeconds() * 1000) { // PRQA S 3081, 4400
                    updateStat2(stat,"error #","timeout (total steps) #") ; 
                     throw ;
                }
                else {
                    updateStat(stat,"retries #") ; 
                    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Still: " << _connection.getLockWaitSeconds() * 1000 -  timer.elapsed() << " [msec] remaining") ; // PRQA S 3000, 3084, 4400
                }
            }
            else {
...
// an empty set, EVEN IF SUCCESSFULL
    if(!itcrow.get())
    {
        ...
        ACS_THROW(QueryNoDataFound(string("dbQuery: no data found. Query is: ") + _sqlString.getSQL())) ; // PRQA S 3081
    }
    ...
    try {
        ...
        ACS_THROW(QueryExecuteException(x,string("Conversion to dbRow error. Query string is: ") +  _sqlString.getSQL()) ) ; // PRQA S 3081
    }
...
void dbQuery::privExecForStatus(dbSet &fakeSet, 
                                ...
                                bool acceptEmptyRow) 
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "connection:  ptr " << &_connection << "  " << (!_connection.isOpen(false) ? "not " : "") << "open" // PRQA S 3380
           << "  About to execute query: \"" << _sqlString.getSQL() << "\"") ;
    
    StatNode &stat = _stat ? (statN(_connection.getKey())+=1)["execForStatus"]+=1 : (statN(_connection.getKey()))["execForStatus"] ; // PRQA S 4412
     ...
     for (timer.start();;timer.randomDelay(_defaultMsecCycleDelay)) {
        ...
        try {
            ...
            if (! _success) {
                ...
                ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "query.execForStatus: \"" <<  _connection.getLastErrorText() << "\"" // PRQA S 3380
                        << "_preventConnCheckCounter: " << ((_connection.transactionState() == dbConnection::Error) ? prev_counter : 0)) ;
                // throw an exception
                ACS_THROW(QueryExecuteException(string("dbQuery: ") + " " +  // PRQA S 3081
            ...
            stepTimer.stop() ;
            repeatTimer.stop() ;
            if (shouldRepeat && (repeatTimer.elapsed() < 60000) ) { // PRQA S 4400
                ...
                // how long ?? 
                if (_connection.getSingleStepWaitSeconds() * 500 > long(stepTimer.elapsed())) { // PRQA S 3081, 4400
                    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Step duration: " << stepTimer.elapsed() 
                           << " SSTep Wait: " << _connection.getSingleStepWaitSeconds()*1000 // PRQA S 4400
                           << " throwing exception" ) ;
...
void dbQuery::privExecForSet(dbSet &set, 
                            bool& retVal,
                            const vector<dbSpecificColumnType::Type>& suggestedTypes,
                            bool acceptEmptySet) 
{
    ACS_COND_THROW(!&_connection, QueryException("Null connection")) ; // PRQA S 3081

    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "acceptEmptySet: " << boolalpha << acceptEmptySet << noboolalpha // PRQA S 3380
            << " connection: ptr " << &_connection << "  " << (!_connection.isOpen(false) ? "not " : "") << "open"
            << "  About to execute query: \"" << _sqlString.getSQL() << "\"" ) ;
    StatNode &stat = _stat ? (statN(_connection.getKey())+=1)["execForSet"]+=1 : (statN(_connection.getKey()))["execForSet"]; // PRQA S 4412
     ...
     for (timer.start();;timer.randomDelay(_defaultMsecCycleDelay)) {
        ...
        try {
            ...
            if (!_success) {
                ...
                ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, " query.execForSet: \"" << _errorString << "\"" // PRQA S 3380
                       << " _preventConnCheckCounter: " << ((_connection.transactionState() == dbConnection::Error) ? prev_counter : 0)) ;
                ...
                if (eType == PrepareError) {
                    ...
                    updateStat2(stat,"error #","lock #") ;
                    ACS_THROW(QueryLockException(string("dbQuery:") + " Lock Error " + _errorString + " Query is : " + _sqlString.getSQL() )) ; // PRQA S 3081
                }
            }
            ...
            */
            ...
            if (_connection.getLockMode() == dbConnection::WaitForSeconds) {
                ...
                if (_connection.getSingleStepWaitSeconds() * 500 > long(stepTimer.elapsed())) { // PRQA S 3081, 4400
                 ...
                 if (int(timer.elapsed()) > _connection.getLockWaitSeconds() * 1000) { // PRQA S 3081, 4400
    ...
    try {
        ...
        ACS_THROW(QueryExecuteException(x,string("Query Execution error. Command is: ") +  _sqlString.getSQL())) ; // PRQA S 3081
    }
...
{   
    ...
    vector < pair<string, string> > tableList ; 
    string tableName ;
    ...
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called for " << (isBlob? "BLOB": "CLOB" )) ; // PRQA S 3380

    if ( columnAndFileName.first.empty() && (commandAndFileName.first.getType() == SQLString::QUERY_UNDEFINED) )
    {
        ostringstream m ;
        m << "dbQuery::downloadBlobClob() " << __LINE__ << " Required field unset";
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }

    if ( not columnAndFileName.first.empty() && ( commandAndFileName.first.getType() != SQLString::QUERY_UNDEFINED) )
    {
        ostringstream m ;
        m << "dbQuery::downloadBlobClob() " << __LINE__ << " Both column and command set" ;
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    ...
    {
        // Column is set
        tableList = _sqlString.getTableList() ;

        // check sqlstring type: must be LOBJ_DOWNLOAD
        ACS_COND_THROW( _sqlString.getType() != SQLString::LOBJ_DOWNLOAD, // PRQA S 3081
                                    QueryException( "dbQuery::downloadBlobClob() Wrong SQLString type" )
                                  ) ;
        if ( tableList.size() != 1 ) // only one table allowed
        {
            ostringstream m ;
            m << "dbQuery::downloadBlobClob() ONE and ONLY ONE table must be provided for blob ops. (current size: " << tableList.size() << "should be: 1)" ;
            ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
        }

        ACS_COND_THROW( columnAndFileName.first.empty() || columnAndFileName.second.empty(), // PRQA S 3081
                                   QueryException( "dbQuery::downloadBlobClob() Column and FileName both required for blob ops and not set" )
                                 ) ;
    ...
    else
    {
        ...
        ACS_COND_THROW( sqlCommand.getType() == SQLString::QUERY_UNDEFINED, // PRQA S 3081
                                    QueryException( "dbQuery::downloadBlobClob() Command not set" )
                                  ) ;
         ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "dbQuery::downloadBlobClob(): fileName: " << fileName ) ;
    
    }
    
    // PRE-CHECK
    // Check if filename already exists
    ACS_COND_THROW( not fileName.empty() && File::exists( fileName ), // PRQA S 3081
                                QueryException("dbQuery::downloadBlobClob() PRE-CHECK FAILED: File " + fileName + " already exists!") 
                              ) ;
    ...
    if ( not fileName.empty() )
    {
        ...
        {
            ACS_THROW(QueryException(x, string("dbQuery::downloadBlobClob() PRE-CHECK FAILED: File ") + fileName + " cannot be written!" )) ; // PRQA S 3081
        }
    }

    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "PRE-CHECK SUCCESSFULL for file: " << fileName) ;
  
    StatNode &stat = _stat ? (statN(_connection.getKey())+=1)["LO download"]+=1 :(statN(_connection.getKey()))["LO download"] ; // PRQA S 4412
    Timer timer ;
    timer.start() ;
    try {
        if ( not columnAndFileName.first.empty() )
...
{  
    ...
    {
        ostringstream m ;
        m << "dbQuery::uploadBlobClob() " << __LINE__ << " Required field not set";
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    ...
    {
        ostringstream m ;
        m << "dbQuery::uploadBlobClob() " << __LINE__ << " Both column and command set" ;
         ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    ...
    {    
        ...
        ACS_COND_THROW( _sqlString.getType() != SQLString::LOBJ_UPLOAD, // PRQA S 3081
                                    QueryException( "dbQuery::uploadBlobClob() Wrong SQLString type" )
                                  ) ;
 
        if ( tableList.size() != 1 ) // only oe table allowed
        {
            ostringstream m ;
            m << "dbQuery::uploadBlobClob() ONE and ONLY ONE table must be provided for blob ops. (current size: " << tableList.size() << "should be: 1)" ;
             ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
        }

        // column and file name MUST be set
        ACS_COND_THROW( columnAndFileName.first.empty() || columnAndFileName.second.empty(), // PRQA S 3081
                                    QueryException( "dbQuery::uploadBlobClob() Column and FileName both required for blob ops and not set" )
                                   ) ;
    ...
    else
    {
        ...
        ACS_COND_THROW( sqlCommand.getType() == SQLString::QUERY_UNDEFINED, // PRQA S 3081
                                   QueryException("dbQuery::uploadBlobClob() Command not set")
                                 ) ;
    ...
    // Check if filename already exists
    ACS_COND_THROW( not fileName.empty() && (! File::exists( fileName )), // PRQA S 3081
                                QueryException("dbQuery::uploadBlobClob() PRE-CHECK FAILED: File " + fileName + " doesn't exist!")
                              ) ;
    if( not fileName.empty() )
    {
        // OK -- File exists.
        ...
        // Read test
        {
            ifstream file_op( fileName.c_str(), ios::in );
            ACS_COND_THROW( ! file_op.is_open(), // PRQA S 3081
                                       QueryException("dbQuery::uploadBlobClob() PRE-CHECK FAILED: File " + fileName + " not readable !")
                                     ) ;
            file_op.close() ;
        }           
    }
    ...
    StatNode &stat = _stat ? (statN(_connection.getKey())+=1)["LO upload"]+=1 : (statN(_connection.getKey()))["LO upload"]; // PRQA S 4412
    Timer timer ;
    timer.start() ;
...
{
    vector < pair<string, string> > tableList = _sqlString.getTableList() ;
    pair <string, string> columnAndFileName = _sqlString.getColumnAndFileName() ;
    ...
    {
         ACS_THROW(QueryException( "dbQuery::zeroBlob() Wrong SQLString type" )) ; // PRQA S 3081
    }

    if ( tableList.size() != 1 ) // only one table
    {
        ostringstream m ;
        m << "dbQuery::zeroBlob() ONE and ONLY ONE table must be provided for blob ops. (current size: " << tableList.size() << "should be: 1)" ;
          ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    
    // no column set: error
    ACS_COND_THROW( columnAndFileName.first.empty(), // PRQA S 3081
                               QueryException( "dbQuery::zeroBlob() Column is required for setting blob to zero and not set" )
                             ) ;
...
void dbQuery::zeroClob() 
{
    ...
    if ( _sqlString.getType() != SQLString::LOBJ_UPLOAD )
    {
         ACS_THROW(QueryException( "dbQuery::zeroClob() Wrong SQLString type" )) ; // PRQA S 3081
    }

    if ( tableList.size() != 1 ) // only one table
    {
        ostringstream m ;
        m << "dbQuery::zeroClob() ONE and ONLY ONE table must be provided for blob ops. (current size: " << tableList.size() << "should be: 1)" ;
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }

    // no column set: error
    ACS_COND_THROW( columnAndFileName.first.empty(),  // PRQA S 3081
                               QueryException("dbQuery::zeroClob() Column is required for setting blob to zero and not set") 
                             ) ;
...
void dbQuery::zeroBlobClob(const string &tableName, const string& columnName, const string& whereCondition ) 
{
    ...
    try
    {
        ...
        ACS_COND_THROW(!status,QueryException("dbQuery::zeroBlobClob: Invalid status after execForStatus" )) ; // PRQA S 3081
    }
    catch (exception& e)
    {
        ACS_THROW(QueryException( e, "dbQuery::zeroBlobClob: Cannot set to NULL on table \"" + tableName + "\"" )) ; // PRQA S 3081
    }
...
bool dbQuery::blobIsNull()  
{
    ...
    {
         ACS_THROW(QueryException( "dbQuery::blobIsNull() Wrong SQLString type" )) ; // PRQA S 3081
    }

    if ( tableList.size() != 1 ) // only one table
    {
        ostringstream m ;
        m << "dbQuery::blobIsNull() ONE and ONLY ONE table must be provided for blob ops. (current size: " << tableList.size() << "should be: 1)" ;
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    ...
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished with status: " << boolalpha << retValue ) ;
...
bool dbQuery::clobIsNull()  
{
    ...
    if ( _sqlString.getType() != SQLString::LOBJ_ISNULL )
    {
         ACS_THROW(QueryException( "dbQuery::clobIsNull() Wrong SQLString type" )) ; // PRQA S 3081
    }

    if ( tableList.size() != 1 ) // only one table in the query
    {
        ostringstream m ;
        m << "dbQuery::clobIsNull() ONE and ONLY ONE table must be provided for clob ops. (current size: " << tableList.size() << "should be: 1)" ;
        ACS_THROW(QueryException( m.str() )) ; // PRQA S 3081
    }
    // no column: exception 
    ACS_COND_THROW( columnAndFileName.first.empty(), // PRQA S 3081
                                QueryException("dbQuery::clobIsNull() Column is required for getting clob size and not set")
                               )  ;
...
#pragma GCC diagnostic pop
    ...
    {
        ACS_THROW(QueryException( e, "dbQuery::blobClobIsNull : Cannot execute query" )) ; // PRQA S 3081
    }
...
*/
bool dbQuery::dbxExecForStatus(const string& sqlString, bool &shouldRepeat) // PRQA S 4020
{
    ...
    */
    ...
    {
        ostringstream os ; 
        os << "dbQuery::dbxExecForStatus. Cannot run getSQLCommand. Got status: " << commandStatus << " ";
    ...
    if (commandStatus != SQL_SUCCESS )
    {
        ...
        ostringstream os ; 
        os << "dbQuery::dbxExecForStatus. Cannot run executeImmediate. Got status: " << commandStatus << " " ;
        _connection.errorText(command, os.str()) ; _connection.setErrorCode(commandStatus) ;
        return false ;
    }
...
*/
dbQuery::ErrorType dbQuery::dbxExecForSet(const string &sqlString, dbSQLCursor &cursor) // PRQA S 4020
{
    /*
    ...
    */
    ...
    {
           ostringstream os ; 
        os << "dbQuery::dbxExecForSet. Cannot run getSQLCommand. Got status: " << commandStatus << " " ; 
            _connection.errorText(os.str()) ;  _connection.setErrorCode(commandStatus) ;
        return PrepareError ;
    }
    
    sqlcomm->setOption(eCommRowsetSize,20) ;    // FIXME ENRICO SERVE ??? // PRQA S 4400
    
    commandStatus = SQLDialect::SQLReturn(getKey(), sqlcomm->prepare (const_cast<char *>(sqlString.c_str()),0) ) ; // PRQA S 3081
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "command prepared, status: " << commandStatus) ;
    if (commandStatus != SQL_SUCCESS )
    {
        ostringstream os ; 
        os << "dbQuery::dbxExecForSet. Cannot run prepare. Got status: " << commandStatus << " " ; 
    ...
    {
        ostringstream os ; 
        os << "dbQuery::dbxExecForSet. Cannot run execute. Got status: " << commandStatus << " " ;
        _connection.errorText(sqlcomm, os.str()) ; _connection.setErrorCode(commandStatus) ;
        return ExecuteError ;
    }
    ...
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "returning cursor." << (!cursor ? " !!! EMPTY !!!": "")) ; // PRQA S 3380

    return !cursor ? EmptyCursor : NoError ; // PRQA S 3382
}
...
/*
    ...
    \ret: true success.
*/
bool dbQuery::execForTablesMetaData(const string &tableName, dbSQLCursor &cursor) // PRQA S 4020
{
    /*
        If there was an error during query: false is returned, and errorCode is Set.
        If query is successfully, true is returned.
    ...
    */
    ...
    {
           ostringstream os ; 
        os << "dbQuery::execForTablesMetaData. Cannot run getSQLMetaData. Got status: " << commandStatus << " " ; 
        _connection.errorText(os.str()) ;  _connection.setErrorCode(commandStatus) ;
        return false ;
    }
    ...
    {
        ostringstream os ; 
        os << "dbQuery::execForTablesMetaData. Cannot run execute. Got status: " << commandStatus << " " ;
        _connection.errorText(sqlMetaData, os.str()) ; _connection.setErrorCode(commandStatus) ;
        return false ;
    }
...
void dbQuery::updateStat(StatNode &stat, const std::string &s) const // PRQA S 4211
{
...
void dbQuery::updateStat2(StatNode &stat, const std::string &s1, const string & s2) const // PRQA S 4211
{    
    if (not _stat) { return ; } // no statistics required
    // this is a method to update the statistic accumulator passed 
    // increments values
...
void dbQuery::updateStat(StatNode &stat, Timer &timer) const // PRQA S 4211
{
    if (not _stat) { return ; } // no statistics required
    // this is a method to update the statistic accumulator passed 
    // computes average and set min and max
    stat["lapse [ms]"]+=double(timer.elapsed()); // PRQA S 3081
    
    if (!_queryStatLabel.empty()) {
        stat["Queries"][_queryStatLabel + " [ms]"] +=double(timer.elapsed()); // PRQA S 3081
    }
}        


void dbQuery::updateStat(StatNode &stat, size_t rows) const // PRQA S 4211
{
    if (not _stat) { return ; } // no statistics required
    // this is a method to update the statistic accumulator passed 
    // computes average and set min and max
...
void dbQuery::execForStatusRetry(StopController &sc, size_t msec, unsigned short retriesMax) // PRQA S 4020 
{
    for (unsigned int attempt = 0 ; ; ++attempt ) {
        try {
            if (!execForStatus())
            {
                ACS_THROW(QueryExecuteException("error executing query: " // PRQA S 3081
        ...
        catch (exception &e) 
        {
            ...
            if ((attempt >= retriesMax) || sc.timeoutOrCancel(msec) ) {
                ...
                if (attempt >= retriesMax) {
                    msg <<"maximum number of db connection retries ("
                        << retriesMax << ") performed";
                }
                else { // stop requested
                    msg << "stop requested" ;
                }
                ACS_THROW(QueryExecuteException(e ,msg.str())); // PRQA S 3081
            }
        }
    }
}
...
void dbQuery::execOneRowRetry(StopController &sc, // PRQA S 4020
                                size_t msec, 
...
void dbQuery::execForSetRetry(StopController &sc,   // PRQA S 4020
                              size_t msec, 
                              unsigned short retriesMax, 
                              dbSet& set, 
                              const std::vector<dbSpecificColumnType::Type>& suggestedTypes )
...
const string &dbQuery::queryStatLabel() const { return _queryStatLabel ; } // PRQA S 4120 EOF
...
size_t dbQuery::getAffectedRows() const { return _affectedRows ; } // PRQA S 3000

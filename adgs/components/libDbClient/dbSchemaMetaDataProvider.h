...
public:
	virtual ~dbSchemaMetaDataProvider() throw() ;
	...
	dbSchemaMetaDataProvider(dbSchemaMetaDataProvider &&) = delete ;		
	dbSchemaMetaDataProvider &operator=(dbSchemaMetaDataProvider &&) = delete ;	
	 ...
	 * \endcode
	 */
	std::map<std::string , std::vector< std::pair<std::string , std::string > > > getDependantTables(std::string const & tableName, dbConnection &)  ;
...
private:
	...
	typedef std::map < std::set < std::string > , std::string > IndexSet;

	struct FieldMetaData // PRQA S 2173, 2175
	{
		friend class SQLDialect ; // PRQA S 2107
		...
		unsigned long 				 foreignField ;
        bool                        isUnique ;
		// PRQA L:L1
		//methods
		FieldMetaData() ;
		~FieldMetaData() throw() ;
		FieldMetaData(FieldMetaData const &) ;
		FieldMetaData const & operator=(FieldMetaData const &) ;
	};
	...
	struct TableMetaData // PRQA S 2173, 2175
	{
		...
		TableMetaData() ;
		~TableMetaData() throw() ;
		TableMetaData(TableMetaData const &) ;
		TableMetaData const & operator=(TableMetaData const &) ;
	};

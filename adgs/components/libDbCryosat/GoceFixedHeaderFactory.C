...
GoceFixedHeaderFactory::GoceFixedHeaderFactory() : dbFixedHeaderFactory("GOCE") 
{
}

GoceFixedHeaderFactory::~GoceFixedHeaderFactory() throw()
{
}


void GoceFixedHeaderFactory::getWriter(dbInventoryObject const &inv, string const &system, XMLOstream &os, dbFixedHeaderWriter *&fhw) const
{
	fhw = new EarthExplorerFixedHeaderWriter("GOCE" , inv, system, os) ;
}




string GoceFixedHeaderFactory::getFilename(dbInventoryObject const &inventoryObj) const 
{
	bool set = false ;
	...
	{
		ACS_THROW(exIllegalValueException("fileClass must be set for satellite GOCE")) ; // PRQA S 3081
	}

	string filetype=inventoryObj.getFileType(&set) ;
	if(!set ||filetype.empty())
	{
		ACS_THROW(exIllegalValueException("filetype must be set for satellite GOCE")) ; // PRQA S 3081
	}

	DateTime validityStart=inventoryObj.getValidityStart(&set) ;
	if(!set)
	{
		ACS_THROW(exIllegalValueException("validityStart must be set for satellite GOCE")) ; // PRQA S 3081
	}

	DateTime validityStop=inventoryObj.getValidityStop(&set) ;
	if(!set)
	{
		ACS_THROW(exIllegalValueException("validityStop must be set for satellite GOCE")) ; // PRQA S 3081
	}

	string fileVersion=inventoryObj.getFileVersion(&set) ;
	if(!set ||fileVersion.empty())
	{
		ACS_THROW(exIllegalValueException("fileVersion must be set for satellite GOCE")) ; // PRQA S 3081
	}
...
int GoceFixedHeaderFactory::getSatelliteId() const
{

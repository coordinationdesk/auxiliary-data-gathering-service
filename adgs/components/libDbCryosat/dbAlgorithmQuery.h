...
/*
	...
	Revision 2.3  2012/11/16 13:01:12  marpas
...
#ifndef _dbAlgorithmQuery_H_
#define _dbAlgorithmQuery_H_
...
//
//
// class dbAlgorithmQuery
//
//
//
class dbAlgorithmQuery : // PRQA S 2109, 2153
	public dbGeneralQuery
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:
	typedef dbAlgorithm _queriedType ;
	
	dbAlgorithmQuery(dbAlgorithmQuery const &) ;
	dbAlgorithmQuery &operator=(dbAlgorithmQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbAlgorithmQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbAlgorithm *operator[](unsigned long i) ; 
	virtual const dbAlgorithm *operator[](unsigned long i) const ; 

protected:
	explicit dbAlgorithmQuery(const std::string &dbkey);
private:
    dbAlgorithmQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbAlgorithmQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbAlgorithmQuery_H_

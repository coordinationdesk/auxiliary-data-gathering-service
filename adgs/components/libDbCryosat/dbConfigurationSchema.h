...
#ifndef _dbConfigurationSchema_H_
#define _dbConfigurationSchema_H_
...
public:
	...
	virtual ~dbConfigurationSchema() throw() ;
	dbConfigurationSchema &operator=(const dbConfigurationSchema &) ; // operator =
	...
	void replaceSchemaFile(const std::string &path, dbConnection &) ;
	// save the file on disk
	void saveSchemaFile(const std::string &path, const std::string &filename, dbConnection &) ;

	// This method return a hint for the filename
	std::string getSchemaFileName() const ;
	// set/get the original file name
	void setSchemaUploadFileName(const std::string &) ;
	std::string getSchemaUploadFileName(bool *set=0) const;
...
protected:
	...
	dbConfigurationSchema(const dbConfigurationSchema &) ;
...
private:
	...
	std::string _schemaUploadFileName ;
	bool _schemaUploadFileNameSet ;

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbConfigurationSchema) ;
} ;


_ACS_END_NAMESPACE

#endif // _dbConfigurationSchema_H_

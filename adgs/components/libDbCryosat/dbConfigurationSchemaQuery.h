...
#ifndef _dbConfigurationSchemaQuery_H_
#define _dbConfigurationSchemaQuery_H_
...
class dbConfigurationSchema ;

//
//
//
// class dbConfigurationSchemaQuery
//
//
//
class dbConfigurationSchemaQuery : // PRQA S 2109, 2153
	public dbGeneralQuery
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:
	typedef dbConfigurationSchema _queriedType ;
	
	dbConfigurationSchemaQuery(dbConfigurationSchemaQuery const &) ;
	dbConfigurationSchemaQuery &operator=(dbConfigurationSchemaQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbConfigurationSchemaQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbConfigurationSchema *operator[](unsigned long i) ; 
	virtual const dbConfigurationSchema *operator[](unsigned long i) const ; 

protected:
	explicit dbConfigurationSchemaQuery(const std::string &dbkey);
private:
    dbConfigurationSchemaQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbConfigurationSchemaQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbConfigurationSchemaQuery_H_

...
#ifndef _dbDistributionItemQuery_H_
#define _dbDistributionItemQuery_H_
...
//
//
// class dbDistributionItemQuery
//
//
//
class dbDistributionItemQuery : // PRQA S 2109, 2153
	public dbGeneralQuery
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:
	typedef dbDistributionItem _queriedType ;
	
	dbDistributionItemQuery(dbDistributionItemQuery const &) ;
	dbDistributionItemQuery &operator=(dbDistributionItemQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbDistributionItemQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbDistributionItem *operator[](unsigned long i) ; 
	virtual const dbDistributionItem *operator[](unsigned long i) const ; 

protected:
	explicit dbDistributionItemQuery(const std::string &dbkey);
private:
    dbDistributionItemQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbDistributionItemQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbDistributionItemQuery_H_

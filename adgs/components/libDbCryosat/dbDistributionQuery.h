...
#ifndef _dbDistributionQuery_H_
#define _dbDistributionQuery_H_
...
public:
	...
	typedef dbDistribution _queriedType ;
	
	dbDistributionQuery(dbDistributionQuery const &) ;
	dbDistributionQuery &operator=(dbDistributionQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ;
	
	static  std::string staticClassName() ;
	virtual ~dbDistributionQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbDistribution *operator[](unsigned long i) ; 
	virtual const dbDistribution *operator[](unsigned long i) const ; 

protected:
	explicit dbDistributionQuery(const std::string &dbkey);

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbDistributionQuery)
} ;

} // namespace

#endif // _dbDistributionQuery_H_

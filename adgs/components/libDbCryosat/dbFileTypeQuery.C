...
/*
	...
	Revision 2.5  2012/11/16 13:01:13  marpas
	...
	Revision 2.2  2009/01/30 14:28:15  marpas
	db statistics improved
	...
	Revision 1.24  2005/10/24 16:44:01  marpas
	dbexpress compatibility
	
	Revision 1.23  2005/10/11 08:12:08  enrcar
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbFileTypeQuery)

string dbFileTypeQuery::staticClassName()
{
	return "dbFileTypeQuery" ;
}


// constructor
dbFileTypeQuery::dbFileTypeQuery(const string &dbkey) :
    dbGeneralQuery(dbkey) 
{
}


dbFileTypeQuery::dbFileTypeQuery(dbFileTypeQuery const &other) :
    dbGeneralQuery(other.getKey()) 
{
	*this=other ;
}

dbFileTypeQuery &dbFileTypeQuery::operator=(dbFileTypeQuery const &other)
{
	if(this != &other)
	{
...
dbGeneralQueryIF * dbFileTypeQuery::clone() const
{
	return new dbFileTypeQuery(*this) ;
}


dbFileTypeQuery::~dbFileTypeQuery() throw() 
{
}



unsigned long dbFileTypeQuery::doQuery(dbConnection &conn, bool incremental, bool enabled) // PRQA S 4020
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbFileType::getFields() );
    queryString.addTableList( dbFileType::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "FileTypeQuery") ;
	...
	{
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB,"Added a new FileType") ;
		// record = new dbFileType ;
		ACS_SMARTPTR<dbFileType> record(dynamic_cast<dbFileType*>( dbPersistentFactoryChain::instance()->newObject("dbFileType", conn.getKey()))) ; // PRQA S 3081
...
dbFileType *dbFileTypeQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbFileType *>(item(i)) ; // PRQA S 3081
}

const dbFileType *dbFileTypeQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbFileType *>(item(i)) ; // PRQA S 3081
}

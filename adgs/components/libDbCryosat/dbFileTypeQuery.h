...
#ifndef _dbFileTypeQuery_H_
#define _dbFileTypeQuery_H_
...
//
//
// class dbFileTypeQuery
//
//
//
class dbFileTypeQuery : public dbGeneralQuery // PRQA S 2109
{
public:
	friend class dbCryosatFactory ; // PRQA S 2107
	typedef dbFileType _queriedType ;
	
	dbFileTypeQuery(dbFileTypeQuery const &) ;
	dbFileTypeQuery &operator=(dbFileTypeQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbFileTypeQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbFileType *operator[](unsigned long i) ; 
	virtual const dbFileType *operator[](unsigned long i) const ; 

protected:
	explicit dbFileTypeQuery(const std::string &dbkey);
private:
	dbFileTypeQuery(); // not implemented

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbFileTypeQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbFileTypeQuery_H_

/*
	...
	Revision 1.24  2006/01/04 16:03:45  enrcar
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbInventoryCommentQuery)

string dbInventoryCommentQuery::staticClassName()
{
	return "dbInventoryCommentQuery" ;
}


// constructor
dbInventoryCommentQuery::dbInventoryCommentQuery(const string &dbkey) :
    dbGeneralQuery(dbkey)
{
}


dbInventoryCommentQuery::dbInventoryCommentQuery(dbInventoryCommentQuery const &other):
    dbGeneralQuery(other.getKey()) 
{
	*this=other ;
}

dbInventoryCommentQuery &dbInventoryCommentQuery::operator=(dbInventoryCommentQuery const &other)
{
	if(this != &other)
	{
		Lock lock1(*this) ;
...
dbGeneralQueryIF * dbInventoryCommentQuery::clone() const
{
	return new dbInventoryCommentQuery(*this) ;
}


dbInventoryCommentQuery::~dbInventoryCommentQuery() throw() {}



unsigned long dbInventoryCommentQuery::doQuery(dbConnection &conn, bool incremental, bool enabled)
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbInventoryComment::getFields() );
    queryString.addTableList( dbInventoryComment::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "dbInventoryCommentQuery::doQuery") ;
	...
	{
		dbInventoryComment *record ;

		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Added a new InventoryComment : ") ; 
        
		// record = new dbInventoryComment ;
		record=dynamic_cast<dbInventoryComment*>( dbPersistentFactoryChain::instance()->newObject("dbInventoryComment", conn.getKey())) ;
		try{
...
dbInventoryComment *dbInventoryCommentQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbInventoryComment *>(item(i)) ;
}

const dbInventoryComment *dbInventoryCommentQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbInventoryComment *>(item(i)) ;
}

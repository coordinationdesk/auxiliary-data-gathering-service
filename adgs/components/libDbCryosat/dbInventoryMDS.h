...
/*
	...
	Revision 2.6  2012/06/15 09:54:29  marpas
...
#ifndef _dbInventoryMDS_H_
#define _dbInventoryMDS_H_
...
//
//
class dbInventoryMDS : // PRQA S 2109, 2153
	public dbSmartPersistent
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:

	virtual ~dbInventoryMDS() throw() ;
	dbInventoryMDS &operator=(const dbInventoryMDS &) ; // operator =
	...
	int getMeasDataSetId(bool &null, dbConnection &) const ;
	void unsetMeasDataSetId();

	void setCount(int, dbConnection &) ;
	int getCount(bool &null, dbConnection &) const ;
	void unsetCount();
...
protected:
	explicit dbInventoryMDS(dbConnection &) ;
	dbInventoryMDS(const dbInventoryMDS &) ;

private:
	dbInventoryMDS() ; // not implemented

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbInventoryMDS)
} ;


_ACS_END_NAMESPACE

#endif // _dbInventoryMDS_H_

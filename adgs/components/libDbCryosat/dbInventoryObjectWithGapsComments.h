...
/*
	...
	Revision 5.7  2016/07/04 15:10:43  damdec
	...
	Revision 2.7  2012/06/15 09:54:30  marpas
	...
	Revision 2.3  2007/03/16 14:00:19  marpas
	work in progress
...
#ifndef _dbInventoryObjectWithGapsComments_H_
#define _dbInventoryObjectWithGapsComments_H_
...
//
//
class dbInventoryObjectWithGapsComments : // PRQA S 2109, 2153
	public dbInventoryObjectWithGaps
{
	friend class dbCryosatFactory; // PRQA S 2107
public:

	virtual ~dbInventoryObjectWithGapsComments() throw() ;
	dbInventoryObjectWithGapsComments &operator=(const dbInventoryObjectWithGapsComments &) ; // operator =
...
#pragma GCC diagnostic pop
	...
	unsigned long	commentsNum(dbConnection &)	;
	dbInventoryComment * comment(unsigned long ind, dbConnection &) ; 
	void addComment(const std::string &comment, const DateTime &date, const std::string &author, dbConnection &) ; 
	bool hasComments() const throw() ;
...
protected:
	...
	dbInventoryObjectWithGapsComments(const dbInventoryObjectWithGapsComments &) ;

private:

	unsigned long loadComments(dbConnection &) ; 
	void saveComments(dbConnection &conn) ;
	void deleteComments2Save() throw() ;

private:
	
	bool _commentsNumSet ; 
	int _commentsNum ; 
	dbInventoryCommentQuery *_inventoryComments ; 
	bool _commentsLoaded ; 
	
	std::vector<dbInventoryComment *> _comments2Save ;
	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbInventoryObjectWithGapsComments);
} ;
...
#endif // _dbInventoryObjectWithGapsComments_H_

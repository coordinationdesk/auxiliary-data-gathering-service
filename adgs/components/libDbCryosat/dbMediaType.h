...
#ifndef _dbMediaType_H_
#define _dbMediaType_H_
...
public:
	...
	virtual ~dbMediaType() throw() ;
	dbMediaType &operator=(const dbMediaType &) ; // operator =
	...
	void setIdMediaType(const int &) ;
	int getIdMediaType(bool *set=0) const ;
	void unsetIdMediaType();

	void setMediaType(const std::string &) ;
	const std::string &getMediaType(bool *set=0) const ;
	void unsetMediaType();

	void setUncompressedCapacity(int) ;
	int getUncompressedCapacity(bool *set=0) const ;
	void unsetUncompressedCapacity();
...
protected:
	dbMediaType() ;
	dbMediaType(const dbMediaType &) ;


private:
	int _id_mediaType ;
	bool _id_mediaTypeSet ;
	
	std::string _mediaType ;
	bool _mediaTypeSet ;

	int _uncompressedCapacity ;
	bool _uncompressedCapacitySet ;

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbMediaType)
} ;


_ACS_END_NAMESPACE

#endif // _dbMediaType_H_

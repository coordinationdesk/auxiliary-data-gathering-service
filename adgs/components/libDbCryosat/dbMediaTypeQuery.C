...
/*
	...
	Revision 5.4  2016/05/05 13:13:25  marpas
	...
	Revision 1.24  2006/01/04 16:22:39  enrcar
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbMediaTypeQuery)

string dbMediaTypeQuery::staticClassName()
{
	return "dbMediaTypeQuery" ;
}


// constructor
dbMediaTypeQuery::dbMediaTypeQuery(const string &dbkey) :
    dbGeneralQuery(dbkey) 
{
}


dbMediaTypeQuery::dbMediaTypeQuery(dbMediaTypeQuery const &other) :
    dbGeneralQuery(other.getKey()) 
{
}

dbMediaTypeQuery &dbMediaTypeQuery::operator=(dbMediaTypeQuery const &other)
{
	if(this != &other)
	{
...
dbGeneralQueryIF * dbMediaTypeQuery::clone() const
{
	return new dbMediaTypeQuery(*this) ;
}


dbMediaTypeQuery::~dbMediaTypeQuery() throw() {}



unsigned long dbMediaTypeQuery::doQuery(dbConnection &conn, bool incremental, bool enabled) // PRQA S 4020
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbMediaType::getFields() );
    queryString.addTableList( dbMediaType::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "dbMediaTypeQuery::doQuery") ;
	...
	{
		// record = new dbMediaType ;
		ACS_SMARTPTR<dbMediaType> record(dynamic_cast<dbMediaType*>( dbPersistentFactoryChain::instance()->newObject("dbMediaType", conn.getKey()))) ; // PRQA S 3081
...
dbMediaType *dbMediaTypeQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbMediaType *>(item(i)) ; // PRQA S 3081
}

const dbMediaType *dbMediaTypeQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbMediaType *>(item(i)) ; // PRQA S 3081
}

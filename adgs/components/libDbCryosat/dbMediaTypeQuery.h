...
#ifndef _dbMediaTypeQuery_H_
#define _dbMediaTypeQuery_H_
...
//
//
// class dbMediaTypeQuery
//
//
//
class dbMediaTypeQuery : // PRQA S 2109
	public dbGeneralQuery
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:
	typedef dbMediaType _queriedType ;
	
	dbMediaTypeQuery(dbMediaTypeQuery const &) ;
	dbMediaTypeQuery &operator=(dbMediaTypeQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbMediaTypeQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbMediaType *operator[](unsigned long i) ; 
	virtual const dbMediaType *operator[](unsigned long i) const ; 

protected:
	explicit dbMediaTypeQuery(const std::string &dbkey);
private:
    dbMediaTypeQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbMediaTypeQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbMediaTypeQuery_H_

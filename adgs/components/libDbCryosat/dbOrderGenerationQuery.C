/*
	...
	Revision 1.24  2006/01/04 16:25:25  enrcar
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbOrderGenerationQuery)

string dbOrderGenerationQuery::staticClassName()
{
	return "dbOrderGenerationQuery" ;
}


// constructor
dbOrderGenerationQuery::dbOrderGenerationQuery(const string &dbkey) :
    dbGeneralQuery(dbkey)
{
}


dbOrderGenerationQuery::dbOrderGenerationQuery(dbOrderGenerationQuery const &other):
    dbGeneralQuery(other.getKey()) 
{
	*this=other ;
}

dbOrderGenerationQuery &dbOrderGenerationQuery::operator=(dbOrderGenerationQuery const &other)
{
	if(this != &other)
	{
		Lock lock1(*this) ;
...
dbGeneralQueryIF * dbOrderGenerationQuery::clone() const
{
	return new dbOrderGenerationQuery(*this) ;
}


dbOrderGenerationQuery::~dbOrderGenerationQuery() throw() {}


unsigned long dbOrderGenerationQuery::doQuery(dbConnection &conn, bool incremental, bool enabled)
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbOrderGeneration::getFields() );
    queryString.addTableList( dbOrderGeneration::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "dbOrderGenerationQuery::doQuery") ;
	...
	{
		dbOrderGeneration *record ;

		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Added a new OrderGeneration : ") ;
        
		// record = new dbOrderGeneration ;
		record=dynamic_cast<dbOrderGeneration*>( dbPersistentFactoryChain::instance()->newObject("dbOrderGeneration", conn.getKey())) ;
		try{
...
dbOrderGeneration *dbOrderGenerationQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbOrderGeneration *>(item(i)) ;
}

const dbOrderGeneration *dbOrderGenerationQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbOrderGeneration *>(item(i)) ;
}

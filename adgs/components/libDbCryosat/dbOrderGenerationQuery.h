...
#ifndef _dbOrderGenerationQuery_H_
#define _dbOrderGenerationQuery_H_
...
public:
	...
	typedef dbOrderGeneration _queriedType ;
	
	dbOrderGenerationQuery(dbOrderGenerationQuery const &) ;
	dbOrderGenerationQuery &operator=(dbOrderGenerationQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ;
	
	static  std::string staticClassName() ;
	virtual ~dbOrderGenerationQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbOrderGeneration *operator[](unsigned long i) ; 
	virtual const dbOrderGeneration *operator[](unsigned long i) const ; 

protected:
	explicit dbOrderGenerationQuery(const std::string &dbkey);

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbOrderGenerationQuery)
} ;

} // namespace

#endif // _dbOrderGenerationQuery_H_

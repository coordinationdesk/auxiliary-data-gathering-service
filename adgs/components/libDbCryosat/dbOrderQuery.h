...
#ifndef _dbOrderQuery_H_
#define _dbOrderQuery_H_
...
//
//
class dbOrderQuery : public dbGeneralQuery // PRQA S 2109
{
	friend class dbCryosatFactory ; // PRQA S 2107
public:
	typedef dbOrder _queriedType ;
	
	dbOrderQuery(dbOrderQuery const &) ;
	dbOrderQuery &operator=(dbOrderQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbOrderQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbOrder *operator[](unsigned long i) ; 
	virtual const dbOrder *operator[](unsigned long i) const ; 

protected:
	explicit dbOrderQuery(const std::string &dbkey);
private:
	dbOrderQuery(); // not implemented

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbOrderQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbOrderQuery_H_

...
/*
	...
	Revision 2.5  2013/02/13 18:23:46  marpas
	qa warnings removed
	
	Revision 2.4  2013/02/13 18:04:23  marpas
...
#ifndef _dbOrderTypeQuery_H_
#define _dbOrderTypeQuery_H_
...
//
//
// class dbOrderTypeQuery
//
//
//
class dbOrderTypeQuery : public dbGeneralQuery // PRQA S 2109
{
public:
	friend class dbCryosatFactory ; // PRQA S 2107
	typedef dbOrderType _queriedType ;
	
	dbOrderTypeQuery(dbOrderTypeQuery const &) ;
	dbOrderTypeQuery &operator=(dbOrderTypeQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbOrderTypeQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbOrderType *operator[](unsigned long i) ; 
	virtual const dbOrderType *operator[](unsigned long i) const ; 

protected:
	explicit dbOrderTypeQuery(const std::string &dbkey);
private:
	dbOrderTypeQuery(); // not implemented

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbOrderTypeQuery) 
} ;


_ACS_END_NAMESPACE

#endif // _dbOrderTypeQuery_H_

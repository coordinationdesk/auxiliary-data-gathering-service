...
#ifndef _dbProcessorQuery_H_
#define _dbProcessorQuery_H_
...
public:
	...
	typedef dbProcessor _queriedType ;
	
	dbProcessorQuery(dbProcessorQuery const &) ;
	dbProcessorQuery &operator=(dbProcessorQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ;
	
	static  std::string staticClassName() ;
	virtual ~dbProcessorQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbProcessor *operator[](unsigned long i) ; 
	virtual const dbProcessor *operator[](unsigned long i) const ; 

protected:
	explicit dbProcessorQuery(const std::string &dbkey);

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbProcessorQuery) ;
} ;

} // namespace

#endif // _dbProcessorQuery_H_

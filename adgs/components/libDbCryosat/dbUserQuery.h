...
#ifndef _dbUserQuery_H_
#define _dbUserQuery_H_
...
public:
	...
	typedef dbUser _queriedType ;
	
	dbUserQuery(dbUserQuery const &) ;
	dbUserQuery &operator=(dbUserQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ;
	
	static  std::string staticClassName() ;
	virtual ~dbUserQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbUser *operator[](unsigned long i) ; 
	virtual const dbUser *operator[](unsigned long i) const ; 

protected:
	explicit dbUserQuery(const std::string &dbkey);

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbUserQuery)
} ;

} // namespace

#endif // _dbUserQuery_H_

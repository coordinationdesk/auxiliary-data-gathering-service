/*
	...
	Revision 1.25  2006/01/13 13:46:49  enrcar
	updated to new dbexpress
	
	Revision 1.24  2006/01/04 14:55:26  enrcar
	updated to new DBEXPRESS interface
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbUserTypeQuery)

string dbUserTypeQuery::staticClassName()
{
	return "dbUserTypeQuery" ;
}


// constructor
dbUserTypeQuery::dbUserTypeQuery(const string &dbkey) :
    dbGeneralQuery(dbkey) 
{
}


dbUserTypeQuery::dbUserTypeQuery(dbUserTypeQuery const &other) :
    dbGeneralQuery(other.getKey()) 
{
	*this=other ;
}

dbUserTypeQuery &dbUserTypeQuery::operator=(dbUserTypeQuery const &other)
{
	if(this != &other)
	{
		Lock lock1(*this) ;
...
dbGeneralQueryIF * dbUserTypeQuery::clone() const
{
	return new dbUserTypeQuery(*this) ;
}


dbUserTypeQuery::~dbUserTypeQuery() throw() {}



unsigned long dbUserTypeQuery::doQuery(dbConnection &conn, bool incremental, bool enabled)
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbUserType::getFields() );
    queryString.addTableList( dbUserType::getTable() );
	...
	dbQuery query(conn, queryString, "dbUserTypeQuery::doQuery") ;
	...
	for (size_t i=0; set.getNextRow(row); i++) 
	{
		...
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Added a new UserType : ") ;
		// record = new dbUserType ;
		record=dynamic_cast<dbUserType*>( dbPersistentFactoryChain::instance()->newObject("dbUserType", conn.getKey())) ;
		try{
...
dbUserType *dbUserTypeQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbUserType *>(item(i)) ;
}

const dbUserType *dbUserTypeQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbUserType *>(item(i)) ;
}

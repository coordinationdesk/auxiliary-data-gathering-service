...
#ifndef _dbUserTypeQuery_H_
#define _dbUserTypeQuery_H_
...
public:
	...
	typedef dbUserType _queriedType ;
	
	dbUserTypeQuery(dbUserTypeQuery const &) ;
	dbUserTypeQuery &operator=(dbUserTypeQuery const &) ;
	
	virtual dbGeneralQueryIF *clone() const ;
	
	static  std::string staticClassName() ;
	virtual ~dbUserTypeQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbUserType *operator[](unsigned long i) ; 
	virtual const dbUserType *operator[](unsigned long i) const ; 

protected:
	explicit dbUserTypeQuery(const std::string &dbkey);

private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbUserTypeQuery)
} ;

} // namespace

#endif // _dbUserTypeQuery_H_

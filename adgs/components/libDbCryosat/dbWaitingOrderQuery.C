...
/*
	...
	Revision 1.4  2006/01/04 16:36:33  enrcar
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbWaitingOrderQuery)

string dbWaitingOrderQuery::staticClassName()
{
	return "dbWaitingOrderQuery" ;
}


// constructor
dbWaitingOrderQuery::dbWaitingOrderQuery(const string &dbkey) :
    dbGeneralQuery(dbkey) 
{
}


dbWaitingOrderQuery::dbWaitingOrderQuery(dbWaitingOrderQuery const &r) :
    dbGeneralQuery(r.getKey()) 
{
	*this=r ;
}

dbWaitingOrderQuery &dbWaitingOrderQuery::operator=(dbWaitingOrderQuery const &other)
{
	if(this != &other)
	{
...
dbGeneralQueryIF * dbWaitingOrderQuery::clone() const
{
	return new dbWaitingOrderQuery(*this) ;
}


dbWaitingOrderQuery::~dbWaitingOrderQuery() throw() {}

unsigned long dbWaitingOrderQuery::doQuery(dbConnection &conn, bool incremental, bool enabled) // PRQA S 4020
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbWaitingOrder::getFields() );
    queryString.addTableList( dbWaitingOrder::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "dbWaitingOrderQuery::doQuery") ;
	...
	{
		// record = new dbWaitingOrder ;
		ACS_SMARTPTR<dbWaitingOrder> record(dynamic_cast<dbWaitingOrder*>( dbPersistentFactoryChain::instance()->newObject("dbWaitingOrder", conn.getKey()))) ; // PRQA S 3081
...
dbWaitingOrder *dbWaitingOrderQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbWaitingOrder *>(item(i)) ; // PRQA S 3081
}

const dbWaitingOrder *dbWaitingOrderQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbWaitingOrder *>(item(i)) ; // PRQA S 3081
}

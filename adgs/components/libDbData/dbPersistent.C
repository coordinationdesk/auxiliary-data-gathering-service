...
dbPersistent::~dbPersistent() throw() {}
...
dbPersistent::dbPersistent(dbPersistent&& other) noexcept :
	dbStateWrapper(std::move(other)),
	_id(other._id),
	_modified(other._modified),
	_fromDatabase(other._fromDatabase),
	_mutex()
{
	// Reset other
	other._id = 0;
	other._modified = false;
	other._fromDatabase = false;
}

dbPersistent & dbPersistent::operator=(dbPersistent && other) noexcept {
	dbStateWrapper::operator=(std::move(other));
	_id = other._id;
	_modified = other._modified;
	_fromDatabase = other._fromDatabase;

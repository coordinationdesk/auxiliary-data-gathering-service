...
dbSmartAggregatePersistent::~dbSmartAggregatePersistent() throw() 
{
    destroy() ; 
}
...
std::string dbSmartAggregatePersistent::getUserIdentifier() const throw() //  PRQA S 4020, 4214
{
    try {
...
std::string dbSmartAggregatePersistent::getUserIdentifier(dbConnection &conn) const throw() // PRQA S 4020
{	
	try
	{
	    ThreadSafe::Lock guard(*this); // PRQA S 3050 
...
void dbSmartAggregatePersistent::destroy() throw()
{
	ThreadSafe::Lock guard(*this); // PRQA S 3050 

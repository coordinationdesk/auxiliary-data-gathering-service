...
typedef __int16_t INT16, *pINT16;
typedef __uint16_t UINT16, *pUINT16;
//typedef int INT32, *pINT32;
//typedef unsigned int UINT32, *pUINT32;
typedef __int32_t INT32 ;
typedef __int32_t *pINT32;

typedef __uint32_t UINT32, *pUINT32;
typedef __int64_t INT64;
typedef __int64_t *pINT64;
typedef __uint64_t UINT64, *pUINT64;
typedef float FLOAT, *pFLOAT;
typedef double DFLOAT, *pDFLOAT;

// PRQA S 1040 EOF
#pragma pack(push, before_typedef, 1)   // PRQA S 1040 EOF
...
typedef struct FMTBcd
{
  BYTE iPrecision;
  BYTE iSignSpecialPlaces;
  BYTE iFraction[32];       // PRQA S 4403
} *pFMTBcd;
...
struct DataType { // PRQA S 2173, 2175
	DataType () :logType(), subType() {}
	DataType (UINT16 ltype, UINT16 stype) : logType(ltype), subType(stype)  {}
	UINT16 logType; // PRQA S 2100 2
	UINT16 subType;
};
...
typedef struct SQLDriver **ppSQLDriver;
typedef struct SQLConnection **ppSQLConnection;
typedef struct SQLCommand **ppSQLCommand;
typedef struct SQLCursor **ppSQLCursor;
typedef struct SQLMetaData **ppSQLMetaData;
...
struct SQLUnknown   // PRQA S 2171, 2173, 2175
{
    SQLUnknown() throw() {}
	virtual ~SQLUnknown() throw() {} // PRQA S 2131
...
#pragma pack(push, before_typedef, 1)
	struct GUID
	{
		...
		UINT16 Data2;
		UINT16 Data3;
		BYTE Data4[8];  // PRQA S 4403
	};
#pragma pack(pop, before_typedef)

	COM_METHOD(INT32) QueryInterface (const GUID& riid, void **ppvObject) = 0;
	COM_METHOD(UINT32) AddRef (void) = 0;
	COM_METHOD(UINT32) Release (void) = 0;
protected:
    SQLUnknown(const SQLUnknown &) throw() {}
    SQLUnknown& operator= (const SQLUnknown &) throw() { return *this ; } // PRQA S 4072
} ;
...
#pragma pack(push, before_typedef, 1)
typedef struct TRACEDesc
{
	CHAR szTrace[1024]; // PRQA S 4403
	TRACECat eTraceCat;
	INT32 iCBInfo;
	UINT16 uTotalMsgLen;
} *pTRACEDesc;
...
typedef CBRType (API_CALL_TYPE * pfCallBack) (CBType ecbType, pVOID pCbInfo);


/////////////////////////////////////////////////////////////////////////////
// SQLDriver interface

enum eSQLDriverOption
{
	eDrvBlobSize     = 0,
	eDrvCallBack     = 1,
	eDrvCallBackInfo = 2,
...
struct SQLDriver : public SQLUnknown // PRQA S 2171, 2173, 2175
{
    SQLDriver() throw() : SQLUnknown() {}
	virtual ~SQLDriver() throw() {} // PRQA S 2131
...
#endif
private:
    SQLDriver(const SQLDriver &r) ; // not implemented
    SQLDriver& operator= (const SQLDriver &r) ;  // not implemented
} ;
...
typedef struct TransactionDesc
{
	UINT32 uTransID;
	UINT32 uGID;
...
struct SQLConnection : public SQLUnknown    // PRQA S 2171, 2173, 2175
{
    SQLConnection() throw() : SQLUnknown() {}
	virtual ~SQLConnection() throw() {} // PRQA S 2131
    
	SQL_METHOD connect (pCHAR pszServerName, pCHAR pszUserName, pCHAR pszPassword) = 0; // PRQA S 2502
...
private:
    SQLConnection(const SQLConnection &r) ; // not implemented
    SQLConnection& operator= (const SQLConnection &r) ; // not implemented
} ;
...
struct SQLCommand : public SQLUnknown   // PRQA S 2171, 2173, 2175
{
    SQLCommand() throw() : SQLUnknown() {}
	virtual ~SQLCommand() throw() {} // PRQA S 2131
...
#endif
	SQL_METHOD setParameter (UINT16 uParameterNumber, UINT16 uChildPos, STMTParamType ePType, UINT16 uLogType, UINT16 uSubType, INT32 lMaxPrecision, INT32 lMaxScale, UINT32 ulLength, pVOID pBuffer, BOOL bIsNull) = 0;
	SQL_METHOD getParameter (UINT16 uParameterNumber, UINT16 uChildPos, pVOID pData, UINT32 ulLength, pINT32 plInd) = 0;
	...
	SQL_METHOD close () = 0;        // PRQA S 2502
	SQL_METHOD getErrorMessage (pBYTE pszError) = 0;
	SQL_METHOD getErrorMessageLen (pUINT16 puErrorLen) = 0;
private:
    SQLCommand(const SQLCommand &) ; // not implemented
    SQLCommand& operator= (const SQLCommand &) ; // not implemented
} ;
...
struct SQLCursor : public SQLUnknown   // PRQA S 2171, 2173, 2175
{
    SQLCursor() throw() : SQLUnknown() {}
	virtual ~SQLCursor() throw() {} // PRQA S 2131
	...
	SQL_METHOD getBlob (UINT16 uColumnNumber, pVOID pData, pBOOL pbIsNull, UINT32 ulLength) = 0;
private:
    SQLCursor(const SQLCursor &) ; // not implemented
    SQLCursor& operator= (const SQLCursor &) ; // not implemented
} ;
...
struct SQLMetaData : public SQLUnknown   // PRQA S 2171, 2173, 2175
{
    SQLMetaData() throw() : SQLUnknown() {}
	virtual ~SQLMetaData() throw() {} // PRQA S 2131
	...
	SQL_METHOD getErrorMessageLen (pUINT16 puErrorLen) = 0;
private:
    SQLMetaData(const SQLMetaData &) ; // not implemented
    SQLMetaData& operator= (const SQLMetaData &) ; // not implemented
} ;

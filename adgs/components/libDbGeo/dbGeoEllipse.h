...
#ifndef _dbGeoEllipse_H_
#define _dbGeoEllipse_H_
...
class dbGeoEllipse : public dbGeoObject
{

public :
	dbGeoEllipse() ;
	dbGeoEllipse(const dbGeoPnt &, double ma, double mi, double az) ;
	virtual ~dbGeoEllipse() throw() ;
	 
	dbGeoEllipse(const dbGeoEllipse &) ;
	dbGeoEllipse &operator=(const dbGeoEllipse &) ;
	...
	void majorAxis(double) ;
	void minorAxis(double) ;
	void azimuth(double) ;
...
private:
    ...
    double _minor ;
    double _azimuth ;
	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbGeoEllipse) ;
} ;


std::istream  &
operator >> (std::istream &is, dbGeoEllipse& e) ;

_ACS_END_NAMESPACE


#endif // _dbGeoEllipse_H_

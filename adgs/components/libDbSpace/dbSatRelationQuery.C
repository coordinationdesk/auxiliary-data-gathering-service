...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbSatRelationQuery)

std::string dbSatRelationQuery::staticClassName()
{
	return "dbSatRelationQuery" ;
}


// constructor
dbSatRelationQuery::dbSatRelationQuery(const string &dbkey) :
    dbGeneralQuery(dbkey)
{
}


dbSatRelationQuery::~dbSatRelationQuery() throw() {}

dbSatRelationQuery::dbSatRelationQuery(const dbSatRelationQuery &r) :
    dbGeneralQuery(r)
{
}

dbSatRelationQuery & dbSatRelationQuery::operator=(const dbSatRelationQuery &v)
{
...
dbGeneralQueryIF *dbSatRelationQuery::clone() const
{
	return new dbSatRelationQuery(*this) ;
}

unsigned long dbSatRelationQuery::doQuery(dbConnection &conn, bool incremental, bool enabled) // PRQA S 4020
{
	if(incremental)
	{
    ...
    queryString.addSelectList( dbSatRelation::getFields() );
    queryString.addTableList( dbSatRelation::getTable() );
...
// where parameters and orderby have been copied from _sqlString above 
	...
	dbQuery query(conn, queryString, "dbSatRelationQuery::doQuery") ;
	...
	{
		ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "Added a new SatRelation") ;
		ACS_SMARTPTR<dbSatRelation> record(dynamic_cast<dbSatRelation*>( dbPersistentFactoryChain::instance()->newObject("dbSatRelation", conn.getKey()))) ; // PRQA S 3081
...
dbSatRelation *dbSatRelationQuery::operator[](unsigned long i)
{
	return dynamic_cast<dbSatRelation *>(item(i)) ; // PRQA S 3081
}

const dbSatRelation *dbSatRelationQuery::operator[](unsigned long i) const
{
	return dynamic_cast<const dbSatRelation *>(item(i)) ;// PRQA S 3081
}

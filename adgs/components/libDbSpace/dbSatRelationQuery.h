/*
	...
	Revision 1.15  2004/08/30 17:49:35  marpas
...
#ifndef _dbSatRelationQuery_H_
#define _dbSatRelationQuery_H_
...
//
//
// class dbSatRelationQuery
//
//
//
class dbSatRelationQuery : // PRQA S 2109
	public dbGeneralQuery
{
public:
	friend class dbSpaceFactory ; // PRQA S 2107
	typedef dbSatRelation _queriedType ;
		
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbSatRelationQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbSatRelation *operator[](unsigned long i) ; 
	virtual const dbSatRelation *operator[](unsigned long i) const ; 

protected:
	explicit dbSatRelationQuery(const std::string &dbkey);
	dbSatRelationQuery(const dbSatRelationQuery &) ;
	dbSatRelationQuery &operator=(const dbSatRelationQuery &) ;  // operator =

private:
   	dbSatRelationQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbSatRelationQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbSatRelationQuery_H_

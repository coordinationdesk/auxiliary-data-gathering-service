...
#ifndef _dbSensorQuery_H_
#define _dbSensorQuery_H_
...
//
//
class dbSensorQuery : // PRQA S 2109
	public dbGeneralQuery
{
public:
	friend class dbSpaceFactory ; // PRQA S 2107
	typedef dbSensor _queriedType ;
	
	virtual dbGeneralQueryIF *clone() const ; // PRQA S 2502
	
	static  std::string staticClassName() ;
	virtual ~dbSensorQuery() throw() ;

	virtual unsigned long doQuery(dbConnection &, bool incremental=false, bool enabled = true)  ; 

	virtual dbSensor *operator[](unsigned long i) ; 
	virtual const dbSensor *operator[](unsigned long i) const ; 

protected:
	explicit dbSensorQuery(const std::string &dbkey) ;
	dbSensorQuery(const dbSensorQuery &) ;
	dbSensorQuery &operator=(const dbSensorQuery &) ;  // operator =
private:
   	dbSensorQuery() ; // not implemented
private:

	ACS_CLASS_DECLARE_DEBUG_LEVEL(dbSensorQuery)
} ;

_ACS_END_NAMESPACE

#endif // _dbSensorQuery_H_

...
#ifndef _AlwaysDistributorCreator_H_
#define _AlwaysDistributorCreator_H_ 
...
class AlwaysDistributorCreator: public DistributorCreator // PRQA S 2109
{		
	public:
		explicit AlwaysDistributorCreator(const std::string &algo);
		virtual ~AlwaysDistributorCreator() throw() ;
		virtual void createDistribution(dbConnectionPool &,ConfigurationSingleton::StopPredicate &) const ;

	private:
		// declared but not implemented 
		AlwaysDistributorCreator();
		AlwaysDistributorCreator(const AlwaysDistributorCreator&);
		AlwaysDistributorCreator & operator=(const AlwaysDistributorCreator&);
	private:
		ACS_CLASS_DECLARE_DEBUG_LEVEL(AlwaysDistributorCreator);
};

_ACS_END_NAMESPACE

#endif /* _AlwaysDistributorCreator_H_ */

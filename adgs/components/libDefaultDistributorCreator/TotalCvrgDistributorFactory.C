...
ACS_CLASS_DEFINE_DEBUG_LEVEL(TotalCvrgDistributorFactory) ;

TotalCvrgDistributorFactory::TotalCvrgDistributorFactory() :
    DistributorCreatorFactory()
{
}

TotalCvrgDistributorFactory::~TotalCvrgDistributorFactory() throw()
{
}

DistributorCreator *TotalCvrgDistributorFactory::newCreator(const string &algo)  // PRQA S 4020
{

	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "algo:\"" << algo << '\"') ;
	string TotalCvrg = "TOTALCVRG" ;

	if (!StringUtils::equalsNoCase(algo,TotalCvrg)) {
		return 0 ;
    }
	return new CvrgDistributorCreator(TotalCvrg, true);
}

void TotalCvrgDistributorFactory::capabilities(vector <string> &cap) const
{
	string algo = ' ' + string("TOTALCVRG") + '\n' ; // PRQA S 3081
	cap.push_back(algo);
	return ;
}

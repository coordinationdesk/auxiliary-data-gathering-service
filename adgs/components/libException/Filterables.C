...
exFMessage::~exFMessage() ACS_NOEXCEPT 
{
    delete _os ; 
}
...
std::ostream &exFMessage::stream() 
{
	...
	if (!_os) { 
		_os = new std::ostringstream ; // PRQA S 2707 # see clear method and dtor
...
bool exFMessage::notifyException(const std::exception &x) ACS_NOEXCEPT
{
	exostream::notifyException(x) ; // notify to the base class
	const exException *e = dynamic_cast<const exException *>(&x) ; // PRQA S 3081
	if (e && not e->stack().empty()) { // add details if any
		detail(e->stack()) ;
    }
	return true ;
}
...
exFPrivateInfo::~exFPrivateInfo() ACS_NOEXCEPT {}
...
exFStatus::~exFStatus() ACS_NOEXCEPT {}
...
exFDebug::~exFDebug() ACS_NOEXCEPT
{
}
...
exFWarning::~exFWarning() ACS_NOEXCEPT
{
}
...
exFWStatus::~exFWStatus() ACS_NOEXCEPT {}
...
exFError::~exFError() ACS_NOEXCEPT
{
}
...
exFEStatus::~exFEStatus() ACS_NOEXCEPT {}
...
exFEvent::~exFEvent() ACS_NOEXCEPT
{
}
...
exFOpAlert::~exFOpAlert() ACS_NOEXCEPT
{
}
...
exFOpEventAlert::~exFOpEventAlert() ACS_NOEXCEPT
{
}
...
exFProgress::~exFProgress() ACS_NOEXCEPT
{
}

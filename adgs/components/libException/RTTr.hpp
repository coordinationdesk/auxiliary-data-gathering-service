...
public:
	...
	operator const char *() const ACS_NOEXCEPT ; // PRQA S 2181
	/*! 
	 ...
	 * 
	 */
	static void envget_(const std::string &, int &) ;
private:
	/*! this is character functor translator method (translates ':' '<' '>' into '_' */
	static bool tr_(char c) ;
	/*! this is character functor translator method (removes ' ' and ',' ) */
	static bool rm_(char c) ;
private:
#if __cplusplus >= 201103L	
    RTTr() = delete ; // PRQA S 2038 5
...
#else
    RTTr() ;  /*! prohibited */
    RTTr(const RTTr &) ;  /*! prohibited */
    RTTr & operator = (const RTTr &) ;  /*! prohibited */
#endif
private:
...
inline
void RTTr::envget_(const std::string &s_, int &pi_) // PRQA S 2133, 2134
{

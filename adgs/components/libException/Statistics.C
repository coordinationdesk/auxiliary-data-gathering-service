// PRQA S 1050 EOF
/*
	Copyright 2009-2018, Advanced Computer Systems , Inc.
...
AccuBase::~AccuBase() ACS_NOEXCEPT {} ; 

ACS_CLASS_DEFINE_DEBUG_LEVEL(StatNode) 

StatNode::~StatNode() ACS_NOEXCEPT
{
	delete _value ;
}
...
StatNode::StatNode(const StatNode &rhs) :
	_value(rhs._value ? rhs._value->clone() : 0), // PRQA S 3380
...
StatNode &StatNode::operator[](const std::string &k) // PRQA S 4020
{
	...
	// find the key
	std::map<std::string,StatNode>::iterator f = _subtree.find(k) ;
	if (f == _subtree.end()) {
...
void StatNode::zeroTree() 
{
	Lock const l(*this) ;// PRQA S 3050
    zero() ;
	for (std::map<std::string,StatNode>::iterator i = _subtree.begin() ; // PRQA S 4238
		 i != _subtree.end();
		 ++i) { // each subtree
        i->second.zeroTree() ;
	}
...
void StatNode::clearTree() 
{
    // do not lock this as it is going to be removed
    StatNode *p = (*Statistics::instance())().findParent(*this) ;
    if (p) {
        Lock const l(*p) ;// PRQA S 3050
...
StatNode *StatNode::findParent(StatNode const &n)   // PRQA S 4020
{
	Lock const l(*this) ;// PRQA S 3050
	for (std::map<std::string,StatNode>::const_iterator i = _subtree.begin() ; // PRQA S 4238
		 i != _subtree.end();
		 ++i) { // each subtree
        if (&(i->second) == &n) {
            return this ;
...
void StatNode::clearEntry(StatNode const &e)
{
	Lock const l(*this) ;// PRQA S 3050
	for (std::map<std::string,StatNode>::iterator i = _subtree.begin() ; // PRQA S 4238
		 i != _subtree.end();
...
namespace {
    ...
    {
        int l = int(k.length()) ;  // PRQA S 3081
...
void StatNode::writeTo(std::ostream &s, int level) const // PRQA S 4214
{
    ...
    s << "\n" ;
	for (std::map<std::string,StatNode>::const_iterator i = _subtree.begin() ;
		 i != _subtree.end();
		 ++i) { // each subtree
       i->second.writeTo(s,level+1) ;
	}
}
...
unsigned long long StatNode::sum_n() const 
{
	...
	for (std::map<std::string,StatNode>::const_iterator i = _subtree.begin() ;
		 i != _subtree.end();
		 ++i) { // each subtree
        sumN += i->second.sum_n() ; // PRQA S 3084
	}
...
Statistics::~Statistics() ACS_NOEXCEPT {}
			
Statistics::Statistics() : 
    pattern::Singleton<Statistics>(),
    statsAreEnabled_(false), 
    root_() 
{}
...
{	
	if ((Statistics::instance()->statsAreEnabled_ == true) or (ACS_CLASS_GET_DEBUG() > 0)) {

...
public:
	...
	public:
        ...
        /**
         ...
         \returns a void * but actually is not a real pointer: it cannot be dereferenced, 
         only to be used in boolean expression
         

		 \see ThreadSafe::Lock::locked() const
		 \see ThreadSafe::Lock::operator()() const
		 */
		operator void * () const ACS_NOEXCEPT ; // PRQA S 2181
		...
		/**
		 ...
		 \see ThreadSafe::Lock::locked() const
		 */
		bool operator()() const ACS_NOEXCEPT ;

		/**
		 \brief ask if the lock was sucessfully acquired.

		 \see ThreadSafe::Lock::operator()() const
		 \see ThreadSafe::Lock::operator void * () const
		 */
		bool locked() const ACS_NOEXCEPT ;
...
public:
	...
	virtual ~ThreadSafe() ACS_NOEXCEPT ; 
...
*/
typedef ThreadSafe Mutex ; // PRQA S 2401 # alias wanted
typedef ThreadSafe::Lock Lock ;
typedef ThreadSafe::TryLock TryLock ;
...
public:
	...
	*/
	...
	virtual ~Condition() ACS_NOEXCEPT ;
	...
	*/
	const ThreadSafe& getMutex() const { return _condMutex; }
	/**
		\brief Returns the embedded Mutex object
	*/
	operator const ThreadSafe& () const { return _condMutex; } // PRQA S 2181
	...
	*/
	...
	static struct timespec& normalize(struct timespec&) ACS_NOEXCEPT;
	static struct timespec normalize(const struct timespec&) ACS_NOEXCEPT;
...
private:
	...
	*/
	...
	// event counter
	mutable unsigned int _eventCounter;
...
public:
    ...
    public:
        ...
        *
        */
        virtual ~BaseRWLock() ACS_NOEXCEPT ;

    private:
        BaseRWLock &operator=(const BaseRWLock &) ; // not implemented
        BaseRWLock() ; // not implemented
...
public:
    ...
    public:
        ...
        *
        */
        virtual ~ReadLock() ACS_NOEXCEPT ;

    private:
        // declared but not defined ...
        ReadLock &operator=(const ReadLock &) ; // not implemented
        ReadLock() ; // not implemented
    ...
    public:
        ...
        *
        */
        virtual ~WriteLock() ACS_NOEXCEPT ;
    ...
    public:
	     ...
	     *
	     */
	     virtual ~TryLockBase() ACS_NOEXCEPT;
	     ...
	     */
	    operator void * () const ; // PRQA S 2181
    ...
    private:
	    TryLockBase(const TryLockBase &) ; // not implemented
	    TryLockBase &operator=(const TryLockBase &) ; // not implemented
	    TryLockBase() ; // not implemented
    ...
    public:
	    ...
	    virtual ~TryLockRead() ACS_NOEXCEPT;

    private:
	    TryLockRead(const TryLockRead &) ; // not implemented
	    TryLockRead &operator=(const TryLockRead &) ; // not implemented
	    TryLockRead() ; // not implemented
    ...
    public:
	    ...
	    virtual ~TryLockWrite() ACS_NOEXCEPT;

    private:
 	    TryLockWrite(const TryLockWrite &) ; // not implemented
	    TryLockWrite &operator=(const TryLockWrite &) ; // not implemented
	    TryLockWrite() ; // not implemented
    };

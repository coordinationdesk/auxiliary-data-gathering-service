...
exAdaptiveFilter::exAdaptiveFilter(exostream *s) : 
	...
	_allowEvent(true),
	_allowMessage(true),
	_allowPrivateInfo(true),
	_allowWarning(true),
	_allowOpAlert(true),
	_allowAnyFilterable(false),
	_allowMCFilterable(true)
{
}

exAdaptiveFilter::~exAdaptiveFilter() ACS_NOEXCEPT
{
}

exAdaptiveFilter::exAdaptiveFilter(const exAdaptiveFilter &f) :
	exFilter(f),
	_allowDebug(f._allowDebug),
	_allowError(f._allowError),
	_allowEvent(f._allowEvent),
	_allowMessage(f._allowMessage),
	_allowPrivateInfo(f._allowPrivateInfo),
	_allowWarning(f._allowWarning),
	_allowOpAlert(f._allowOpAlert),
	_allowAnyFilterable(f._allowAnyFilterable),
	_allowMCFilterable(f._allowMCFilterable)
{
}

exAdaptiveFilter &exAdaptiveFilter::operator=(const exAdaptiveFilter &f) 
{
	if (this != &f) { // avoid auto-assignation 
		// deep copy
		exFilter::operator=(f) ;
		_allowDebug = f._allowDebug ;
		_allowError = f._allowError ;

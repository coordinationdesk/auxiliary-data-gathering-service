...
/*
    ...
    introducing ACS_NOEXCEPT that will be translated into throw() or noexecpt depending on the compiler version
...
// PRQA S 2134 3
template <typename T>
inline void exceptionThrower(T const& t, std::string const& file, std::string const& function,int line)
...
#define ACS_THROW(x)  { exceptionThrower((x),__FILE__,__PRETTY_FUNCTION__,__LINE__) ; throw std::runtime_error("") ; } 

#else // PRQA_ANALYZER

// PR QA states that the last throw is not reachable (and it is indeed true), the throw above is used to fool gcc
...
*/
#define throwException(x) ACS_THROW(x) 
...
*/
#define throwConditionedException(c,x)  ACS_COND_THROW(c,x) 
#endif
// PRQA L:L3
 ...
 */
 /*! \class exWrapUnexpected
    ...
    \brief unexpected handler convenience class

    This class can be used to set a new unexpected() handler as in
    ...
    void f()
    {
        // sets the unexpected handler to be the rethrow() function
        // exiting the function (regularly or not) will restore the 
        // previous unexpected handler
        exWrapUnexpected wu(retrow) ;

        g() ; // if g throws an unexpected exception it will be rethrown
    }
    
    \endcode
*/
class exWrapUnexpected { // PRQA S 2109
public:
        /*! \brief convenience typedef of pointer to function 
        */
        typedef void (*Unexpected)() ;
        /*! \brief ctor
        
            Change "unexpected()" handler ( calling set_unexpected() ) for the object life. 
            Then restore the old handler.
        
            \param f a pointer to a function returning void and accepting no parameters
        */
        explicit exWrapUnexpected(Unexpected f) ;
        
        /*! \brief dtor
        
            Restores the previous "unexpected()" handler
        */
        ~exWrapUnexpected() ACS_NOEXCEPT ;

protected:       
private:
        /*! copy ctor
            declared but not implemented. Deny its use.
        */
        exWrapUnexpected(const exWrapUnexpected &) ;

        /*! assignment operator
            declared but not implemented. Deny its use.
        */
        exWrapUnexpected& operator=(const exWrapUnexpected &) ;
...
/*! \class exWrapTerminate
    ...
    \brief terminate handler convenience class

    This class can be used to set a new terminate() handler.
    \see exWrapUnexpected for details
*/
    
class exWrapTerminate { // PRQA S 2109
public:
        /*! \brief convenience typedef of pointer to function 
        */
        ...
        /*! \brief ctor
            ...
            Change "terminate()" handler ( calling set_terminate() ) for the object life. 
            Then restore the old handler.
        
            \param f a pointer to a function returning void and accepting no parameters
        */
        explicit exWrapTerminate(Terminate f) ;

        /*! \brief dtor
        
            Restores the previous "terminate()" handler
        */
        ~exWrapTerminate() ACS_NOEXCEPT ;
...
private:
        ...
        /*! copy ctor
            declared but not implemented. Deny its use.
        */
        exWrapTerminate(const exWrapTerminate &) ;

        /*! assignment operator
            declared but not implemented. Deny its use.
        */
        exWrapTerminate& operator=(const exWrapTerminate &) ;
        /*! default ctor
            declared but not implemented. Deny its use.
        */
        exWrapTerminate() ;
...
public:
    ...
    */
    exException(error_t eno=0) ACS_NOEXCEPT ; // PRQA S 4634
    ...
    /*! \brief constructor
        ...
        as in strerror(3C) manual page.
    */
    exException(const std::exception &, error_t eno=0) ACS_NOEXCEPT ;
    ...
    /*! \brief constructor
        ...
        as in strerror(3C) manual page.
    */
    explicit exException(const std::string &description, error_t eno=0) ACS_NOEXCEPT ; // PRQA S 4634
    ...
    /*! \brief constructor
        ...
        as in strerror(3C) manual page.
    */
    exException(const std::exception &,const std::string &, error_t eno=0) ACS_NOEXCEPT ;

    /*! \brief destructor

        The destructor is virtual as this class is designed to be inherited
    */
    virtual ~exException() ACS_NOEXCEPT ;
    
    /*! \brief copy ctor
    
        Copies the exception
    */
    exException(const exException&) ACS_NOEXCEPT ;
    ...
    */
    const std::string description() const ACS_NOEXCEPT { return computeDescr() ; }
    ...
    */
    const std::string &file() const ACS_NOEXCEPT { return _file ; }
    ...
    /*! \brief get function name
     ...
     \returns the file where the exception was thrown\
    */
    std::string const& function() const ACS_NOEXCEPT { return _function ; } 
    ...
    */
    const std::string &thread() const ACS_NOEXCEPT { return _thread ; } 
    ...
    */
    pthread_t threadId() const ACS_NOEXCEPT { return _threadId ; }  
    ...
    */
    int line() const ACS_NOEXCEPT { return _line ; }
    ...
    */
    virtual void debugInfo(const std::string &file, const std::string &function, int line) const ACS_NOEXCEPT ;
    ...
    */
    virtual const char* what() const ACS_NOEXCEPT ;
    
    virtual const std::string &stack() const ACS_NOEXCEPT { return _stack ; } // PRQA S 2131
    
    virtual error_t errorNumber() const ACS_NOEXCEPT { return _errno ; } // PRQA S 2131
...
private:
    exException &operator=(const exException &) ACS_NOEXCEPT = delete ;
    exException &operator=(exException &&) ACS_NOEXCEPT = delete ; // PRQA S 2038 2

    std::string computeDescr() const ACS_NOEXCEPT;
...
{
    const exException *ex = dynamic_cast<const exException *>(e) ; // PRQA S 3081
...
/*!	\brief exception class declaration convenience macro
    ...
    \code
        ...
        { 
            public: 
                explicit MyClassException(const std::string &description=std::string(""), error_t eno=0) ACS_NOEXCEPT ;
                explicit MyClassException(const std::exception &,const std::string &, error_t eno=0) ACS_NOEXCEPT ;
                MyClassException(const MyClassException &) ACS_NOEXCEPT 
                MyClassException(MyClassException &&) ACS_NOEXCEPT 
                virtual ~MyClassException() ACS_NOEXCEPT ;
                virtual std::exception *clone() const ;
            private: 
                explicit Derived() ACS_NOEXCEPT ; 
                Derived &operator=(const Derived &) ACS_NOEXCEPT ;
        }
...
#define exDECLARE_EXCEPTION(Derived,Base) \
        ...
        { \
            public: \
\
                explicit Derived(error_t eno=0) ACS_NOEXCEPT : /* PRQA S 4634 */ \
                    Base(eno) \
                    { } \
\
                explicit Derived(const std::exception &e, error_t eno=0) ACS_NOEXCEPT : \
                    Base(e,eno) \
                    { } \
\
                explicit Derived(const std::string &d, error_t eno=0) ACS_NOEXCEPT : /* PRQA S 4634 */ \
                    Base(d, eno) \
                    { } \
\
                Derived(const std::exception &e, const std::string &d, error_t eno=0) ACS_NOEXCEPT : \
                    Base(e,d,eno) \
                    { } \
\
                Derived(const Derived &e) ACS_NOEXCEPT : \
                    Base(e) \
                    { } \
\
                Derived(Derived &&e) ACS_NOEXCEPT : /* not implemented */ \
                    Base(std::move(e)) \
                    { } \
\
                virtual ~Derived() ACS_NOEXCEPT \
                    { } \
\
                virtual std::exception *clone() const \
                    { return new Derived(*this) ; } /* PRQA S 2706 */  \
\
...
\
            ...
            private: /* copy ctor and assignment both declared but not implemented to prevent their use */ \
                Derived &operator=(const Derived &) ACS_NOEXCEPT = delete ; /* not implemented */ \
                Derived &operator=(Derived &&) ACS_NOEXCEPT = delete ; /* not implemented */ \
        } \

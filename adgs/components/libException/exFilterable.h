...
public:
	exFilterable() ;
	virtual ~exFilterable() ACS_NOEXCEPT ;
    ...
    void registerTraversal(StreamGraphTraversal *t) ACS_NOEXCEPT 
    {
        if (0 == traversalRegister_) { traversalRegister_ = t; }
    }
    
    // this unregister the registered StreamGraphTraversal
    void unregisterTraversal(StreamGraphTraversal const *s) ACS_NOEXCEPT 
    {
        if (traversalRegister_ == s) { traversalRegister_ = 0 ; }
    }
    
    //- returns true if this filterable already traversed the given stream
    bool traversed(exostream const * const) ACS_NOEXCEPT ;

    StreamGraphTraversal *traversaleRegister() ACS_NOEXCEPT { return traversalRegister_ ; } // PRQA S 4211, 4627

protected:
	exFilterable &operator=(const exFilterable &) ; // not implemented 
	exFilterable(const exFilterable &) ; // not implemented 
private:
...
public:
    explicit StreamGraphTraversal(exFilterable &f) ACS_NOEXCEPT : f_(f), traversedStreams_() // PRQA S 2528
    { f_.registerTraversal(this) ; }
    
   	~StreamGraphTraversal() ACS_NOEXCEPT { f_.unregisterTraversal(this) ; }
    ...
    */ 
    bool traversed(exostream const * const s) ACS_NOEXCEPT ;

private:
    StreamGraphTraversal() ; // not implemented 
    StreamGraphTraversal(const StreamGraphTraversal &) ; // not implemented 
    StreamGraphTraversal& operator= (const StreamGraphTraversal &) ; // not implemented 

...
namespace {
    ...
    {
        static std::vector<exMsgFmt *>  *_v = 0 ;
        if (0 == _v) {
            _v = new std::vector<exMsgFmt *> ;
        }
        return _v ;
    } 
}
...
{
	if (_instances()->empty()) {
		new exMsgFmtFactory ; // PRQA S 2707, 3802 # is a singleton !
	}
...
exMsgFmtFactory::~exMsgFmtFactory() ACS_NOEXCEPT
{
	try { // no exception out of this method 
		Lock const lock(exostream::globalMutex()) ; // PRQA S 4631 4
		std::vector<exMsgFmt *>::iterator tbd = find(_instances()->begin(),_instances()->end(),this) ;
		...
		std::cerr << explain(x) ; // PRQA S 4631
	}
}
...
exMsgFmt::~exMsgFmt() ACS_NOEXCEPT
{
}
...
std::string exMsgFmt::time() const
{
	...
	struct tm * tm_coarse = localtime(&tm_fine.tv_sec) ; 
    // PRQA S 4400 L1

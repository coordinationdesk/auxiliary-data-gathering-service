...
private:
	...
	exMsgFmt(const exMsgFmt &) ;
	exMsgFmt &operator=(const exMsgFmt &) ;
private:
	std::string _appName ;
...
public:
	...
	exMsgFmtFactory() ;
	virtual ~exMsgFmtFactory() ACS_NOEXCEPT ;
...
private:
	// declared but not implemented
	exMsgFmtFactory(const exMsgFmtFactory &) ;
	exMsgFmtFactory &operator=(const exMsgFmtFactory &) ;
} ;

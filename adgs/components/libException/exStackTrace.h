...
/*! \class exException
	...
	note the use of virtual base class is safe 
*/
class exStackTrace // PRQA S 2109
{
public:
	
	static void allow(bool) ACS_NOEXCEPT ;
	static bool allow() ACS_NOEXCEPT ;
	static std::string getStack(bool force = false) ACS_NOEXCEPT ;
	 ...
	 */
	static std::string demangle(const char* name) ACS_NOEXCEPT ;
	 ...
	 */
	static std::string stackLineBeauty(const char* cline) ACS_NOEXCEPT ;

	/**
	 *\brief  Adds a thread_id at the list of the live threads
     *\param id a thread id
	 */
	static void addLiveThreadId (pthread_t id, const std::string&) ACS_NOEXCEPT;

	/**
	 *\brief  Deletes a thread_id at the list of the live threads
     *\param  id a thread id
	 */
	static void removeLiveThreadId (pthread_t id) ACS_NOEXCEPT;

	/**
	 *\brief  Gets a copy of the live threads list
	 */
	static std::map<pthread_t, std::string> getLiveThreadIdSet () ACS_NOEXCEPT;

private:
	exStackTrace() ;                    // not implemented
	exStackTrace(const exStackTrace&) ;  // not implemented
	virtual ~exStackTrace() ACS_NOEXCEPT ;// not implemented
	exStackTrace &operator=(const exStackTrace &) ; // not implemented
...
private:
	...
	static ThreadSafe _threadListMutex;

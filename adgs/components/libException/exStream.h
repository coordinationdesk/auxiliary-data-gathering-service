// PRQA S 1050 EOF # old comment come from CVS
/*
...
public:
	enum Mode { FormattedOnly, Verbose, Silent } ; // PRQA S 2436, 2438 2
    enum StreamFdEquiv {Cmcf = 0, StdCout = 1, StdCerr = 2, NullStream = -1 } ; 
...
public:
	...
	*/
	virtual ~exostream() ACS_NOEXCEPT ;
	
	/*! \brief inserter
	
		inserts a filterable object into the stream
		The installed filters will regulate its insertion
	*/
	exostream & operator<<(exFilterable &) ;        // PRQA S 2070, 2072

	/*! \brief inserter
	
		inserts an excpetion object into the stream
	*/
	exostream & operator<<(const std::exception &) ACS_NOEXCEPT ; // PRQA S 2070, 2072

	/*! \brief inserter
	
		inserts a string into the stream
	*/
	exostream & operator<<(const std::string &) ;   // PRQA S 2070, 2072
	
	//! \overload
	exostream & operator<<(const char *) ;          // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(char) ;                  // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(bool) ;                  // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(short) ;                 // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(int) ;                   // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(long) ;                  // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(float) ;                 // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(double) ;                // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(long double) ;           // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(unsigned char) ;          // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(unsigned short) ;         // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(unsigned int) ;           // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(unsigned long) ;          // PRQA S 2070, 2072

#if defined __GLIBC_HAVE_LONG_LONG
	//! \overload
	exostream & operator<<(signed long long int ) ;  // PRQA S 2070, 2072
	//! \overload
	exostream & operator<<(unsigned long long int ) ;// PRQA S 2070, 2072
#endif
	//! \overload
	exostream & operator<<(void *) ;                // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(const void *) ;           // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(std::streambuf *) ;      // PRQA S 2070, 2072

	//! \overload
	exostream & operator<<(std::ostream & (*func)(std::ostream &)) ;    // PRQA S 2070, 2072, 2426

#ifdef __linux__
  #if (__GNUC__ < 3)
	  exostream & operator<<(ios & (*func)(ios &)) ; // PRQA S 2426
      exostream & operator<<(const smanip<int> &) ; // covers setprecision(w), setw(w) ...
      exostream & operator<<(const smanip<__fmtflags> &) ; // covers setprecision(w), setw(w) ...
  #else // (__GNUC__ < 3)
	  exostream & operator<<(std::ios_base &(*func)(std::ios_base &)) ; // PRQA S 2070, 2072, 2426
	  // linux manipulators
	  exostream & operator<<(std::_Resetiosflags) ;                 // PRQA S 2070, 2072
	  exostream & operator<<(std::_Setiosflags) ;                   // PRQA S 2070, 2072
	  exostream & operator<<(std::_Setbase) ;                       // PRQA S 2070, 2072
	  exostream & operator<<(std::_Setfill<char>) ;                 // PRQA S 2070, 2072
	  exostream & operator<<(std::_Setprecision) ;                  // PRQA S 2070, 2072
	  exostream & operator<<(std::_Setw) ;                          // PRQA S 2070, 2072
  #endif // (__GNUC__ < 3)
#elif defined _AIX
	  exostream &operator<<(std::ios_base & (*func)(std::ios_base &)) ;
	  exostream &operator<<(_Smanip<int>) ;
	  exostream &operator<<(_Fillobj<char>) ;
#else      
	//! \overload
	exostream & operator<<(ios & (*func)(ios &)) ;                  // PRQA S 2070, 2072
#endif
	...
	exostream &notify(const std::exception &) ACS_NOEXCEPT ;
	...
	*/
    ...
    const std::string &getName() const ACS_NOEXCEPT { return name_ ; }
    
    bool isFilterable() const ACS_NOEXCEPT { return isFilterable_ ; }
    
protected:
    void setIsFilterable(bool v) ACS_NOEXCEPT { isFilterable_ = v ; }
	...
	*/
	void set_stream(std::ostream *) ACS_NOEXCEPT ;
	...
	*/
	virtual bool notifyException(const std::exception &) ACS_NOEXCEPT ;
	...
	*/
	...
	/*! The method applies filters (eventually installed on the stream)
		...
		The method returns true if every filter returns true, else it will return 
		false as soon as a filter will return it.
	*/
	...
	void destroying() ACS_NOEXCEPT ;

	void removeFromParents() ;

	bool isDestroying() const ACS_NOEXCEPT ;

private:
	// declared but not defined
	exostream() ;
	exostream(const exostream&) ;
	exostream &operator=(const exostream&) ;
	
	exostream *operator[](size_t) ;

    //* \note this is not implemented
	const exostream *operator[](size_t) const ;
	void appendToParents() ;
	void checkFd() const ;
	void setDestroying() ACS_NOEXCEPT ;
...
public:
	...
	static Mutex &globalMutex() ACS_NOEXCEPT ;  
     ...
     */
    static exostream *setDefaultErrLogger(exostream *) ACS_NOEXCEPT ;
     ...
     * The method returns the default logger if set, else returns excerr.
     */
    static exostream &getDefaultOutLogger() ACS_NOEXCEPT ;
    /*!
     ...
     * It is responsibility of the caller do not destroy the given logger before setting a new one.
     */
    static exostream *setDefaultOutLogger(exostream *) ACS_NOEXCEPT ;
     ...
     */
    static exostream &getDefaultCoutLogger() ACS_NOEXCEPT ;
    /*!
     ...
     * It is responsibility of the caller do not destroy the given logger before setting a new one.
     */
    static exostream *setDefaultCoutLogger(exostream *) ACS_NOEXCEPT ;
...
// PRQA S 2072 L1
std::ostream & operator <<(std::ostream &, exostream::Mode) ACS_NOEXCEPT ;
exostream & operator <<(exostream &, exostream::Mode) ACS_NOEXCEPT ;

std::ostream & operator <<(std::ostream &, exostream::StreamFdEquiv) ACS_NOEXCEPT ;
exostream & operator <<(exostream &, exostream::StreamFdEquiv) ACS_NOEXCEPT ;
// PRQA L:L1
...
public:
	...
	*/
	...
	/*! Destructor.
	*/
	virtual ~exLogStream() ACS_NOEXCEPT ;
...
private: 
	// declared but not defined
	exLogStream() ;
	exLogStream(const exLogStream&) ;
	exLogStream &operator=(const exLogStream&) ;

    // left open() private non-virtual
	void open() ;           // PRQA S 2502
	void close() ACS_NOEXCEPT ;  // PRQA S 2502
private:
...
public:
	...
	*/
	...
	/*! Destructor.
	*/
	virtual ~exBaseStream() ACS_NOEXCEPT ;

private:
	// declared but not defined
	exBaseStream() ;
	exBaseStream(const exBaseStream&) ;
	exBaseStream &operator=(const exBaseStream&) ;
} ;
...
public:
	...
	*/
	...
	/*! Destructor.
	*/
	virtual ~exSyslog() ACS_NOEXCEPT ;
    void setFacility(int f) ACS_NOEXCEPT { facility_ = f ; }
protected:
	virtual void notifyFilterable(exFilterable &) ;
private:
    static int get_prio(const exFilterable &) ACS_NOEXCEPT ;
private:
	// declared but not defined
	exSyslog() ;
	exSyslog(const exSyslog &) ;
	exSyslog & operator=(const exSyslog &) ;
private:
    int facility_ ; 
...
public:
	...
	*/
	...
	/*! Destructor.
	*/
	virtual ~exLogStringStream() ACS_NOEXCEPT ;
...
private:	
	// declared but not defined
	exLogStringStream() ;
	exLogStringStream(const exLogStringStream&) ;
	exLogStringStream &operator=(const exLogStringStream&) ;
private:
...
public:
	...
	*/
	...
	/*! Destructor.
	*/
	virtual ~exHandler() ACS_NOEXCEPT ;
protected:
...
private:
	// declared but not defined
	exHandler() ;
	exHandler(const exHandler&) ;
	exHandler &operator=(const exHandler&) ;
} ;
...
*/
extern exostream excmcf ;

/*! \brief helper class to dump buffers in hex */
class HexDumper // PRQA S 2109
{
public:
    HexDumper(void const * const buffer, size_t length) : buffer_(buffer), length_(length) 
    {
        ACS_COND_THROW((length_ != 0) && (0 == buffer_), // PRQA S 3081
                                  exIllegalValueException("Invalid input buffer pointer")) ;
    }
    ~HexDumper() ACS_NOEXCEPT {}

    template <typename Stream_>
    void dump(Stream_ &s) const 
    {
        unsigned char const * const b = reinterpret_cast<unsigned char const *>(buffer_) ; // PRQA S 3081
        s << std::hex ;
        for (size_t i=0; i<length_; ++i)
	    {
		    s << std::setw(2) << std::setfill('0') << int(b[i]) ; // PRQA S 3081, 4400
	    }
        s << std::dec ;
    }
    HexDumper(const HexDumper &rhs) : buffer_(rhs.buffer_), length_(rhs.length_) {}
	
private:
    HexDumper() ; // not implemented 
    HexDumper& operator= (const HexDumper &) ; // not implemented 
private:
...
#ifndef __linux__
#ifndef _ABIO32
/* woff the declared but never used warning for exStreamInit. */
...
*/
static class exostream_Init // PRQA S 2109
{
public:
	/*! stream initialization, excerr, excout, excmcf will be initialized after the
		first constructor call
	*/
	exostream_Init() ACS_NOEXCEPT ;
	
	/*! class dtor */
	~exostream_Init() ACS_NOEXCEPT;

private:
	exostream_Init(const exostream_Init &) ;
	exostream_Init &operator=(const exostream_Init &) ;
	
private:
	static int _initcount ;
	ACS_CLASS_DECLARE_DEBUG_LEVEL(exostream_Init) 
	
} exostreamInit ; // PRQA S 1068, 1101, 2313 

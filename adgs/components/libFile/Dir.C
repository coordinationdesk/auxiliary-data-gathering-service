...
struct compare_string_no_case 
{
	bool operator() (const string &x, const string &y) { return StringUtils::compareNoCase(x,y) <= 0 ; } // PRQA S 4211
};

struct compare_string 
{
	bool operator() (const string &x, const string &y) { return strcmp(x.c_str(), y.c_str()) <= 0 ; } // PRQA S 4211
};

struct compare_dirtree_files_no_case
{
	bool operator() (const pair<std::string, signed short> &x, const pair<std::string, signed short> &y) { return StringUtils::compareNoCase(x.first,y.first) <= 0 ; } // PRQA S 4211
};

struct compare_dirtree_files
{
	bool operator() (const pair<std::string, signed short> &x, const pair<std::string, signed short> &y) { return strcmp(x.first.c_str(), y.first.c_str()) <= 0 ; } // PRQA S 4211
};
...
Dir::Dir(const Dir &o) :
	...
	_path(o._path),
	_hidden(o._hidden),
	_reportBrokenLinks (o._reportBrokenLinks)
{
}

Dir &Dir::operator=(const Dir &o) 
{
	// = operator
	if (this != &o) {
		_path = o._path ;
...
bool Dir::showHidden() const throw() // PRQA S 4120
{
	return _hidden ;
}


bool Dir::reportBrokenLinks() const throw() // PRQA S 4120
{
...
const string &Dir::operator[](size_t i) const 
{
	if (i >= size()) {
		ostringstream os ;
		os << "Dir::entry " << i << " greater than " << size()-1 ; // PRQA S 3084
...
bool Dir::empty(const string& path, const bool includeHidden)
{
	...
	ACS_COND_THROW(!dir,exIOException("Cannot read directory \"" + path +"\"", errno)) ; // PRQA S 3081
...
//            err = errno ;
	...
	ACS_COND_THROW(0 != readdirval, exIOException("Cannot read directory \"" + path+"\"", readdirval)) ; // PRQA S 3081

	return returnValue;
}

const string Dir::path() const // PRQA S 4120
{
	return _path ;
}
...
signed long long int Dir::bytes() const 
{
	...
	{
		const string fullPath = ( _path + "/" + _files[i] ) ;
		...
		signed long long s = ( S_ISDIR(stat_struct.st_mode)? Dir::bytes(fullPath) : stat_struct.st_size ) ; // PRQA S 3384, 3385
...
DirTree::DirTree(const string &path, bool hidden) : 
	...
	_newPath(""),
	_regExpPattern(""),
	_regExpRejectPattern(""),
	_reverse(false),
	_regExp(false),
	_regExpReject(false),
	_sortOrder(SORT_DEFAULT),
	_showFiles(true),
	_updated(true) 	
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "path: !" << _path << "!  showHidden: " << boolalpha << _hidden) ;
...
DirTree::DirTree(const string &path, const string &dirEntry, bool hidden) : 
	...
	_updated(true) 	
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "path: !" << _path << "!  dirEntry: !" << _dirEntry << "!  showHidden: " << boolalpha << _hidden) ;
	...
	_updated(true)
{
	...
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "path: !" << _path << "!  dirEntry: !" << _dirEntry << "!  showHidden: " << boolalpha << _hidden
			<< "\t_updated: " << boolalpha << _updated << " size :" << size()) ;
}
...
DirTree &DirTree::operator=(const DirTree &o) 
{
	...
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "path: !" << _path 
			<< "!  dirEntry: !" << _dirEntry << "!  showHidden: " << boolalpha << _hidden 
			<< "\t_updated: " << boolalpha << _updated << " size :" << size()) ;
...
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called:  returning: " << boolalpha << _hidden) ;
...
const string &DirTree::operator[](size_t i)  
{
	...
	if (i >= size()) {
		ostringstream os ;
		os << "DirTree::entry " << i << " greater than " << size()-1 ; // PRQA S 3084
...
//  path
//
const string DirTree::path() const 
{
...
//
void DirTree::generateTree(vector<std::pair<string, signed short> >& entries, const DirStruct& dirStruct, signed short int depth ) // PRQA S 5500
{	
// EC:: Method is recursive in order to increase efficency
    ...
    if (not absPath.empty() && (lastChar(absPath)!=sep)) { absPath += sep ; }
	...
	{
		for (size_t i=0; i<dirStruct.dirEntries.size(); i++) {
			generateTree(entries, dirStruct.dirEntries[i], depth+1 ) ; // PRQA S 3010 
...

	{
		...
		if ((!_reverse) && _dumpDirEntries && ((_minDepth < 0) || (depth >= _minDepth)) && ((_maxDepth < 0) || (depth <= _maxDepth)) )
		{
			...
			if ( (!_regExp || regExp.match(e)) && (!_regExpReject || !regExpReject.match(e)) ) {
                ...
                entries.push_back(std::make_pair(e, depth)) ;	// Add current directory entry
            }
		}
		...
		{ 
			string s = ( (not path.empty() && (lastChar(path)!=sep) )? sep : "" ) ; // PRQA S 3384, 3385
			for (size_t i=0; i<dirStruct.fileEntries.size(); i++)
			{
				string e = (path + s +  dirStruct.fileEntries[i]) ;
				string a = absPath + dirStruct.fileEntries[i] ;
				if ( (!_regExp || regExp.match(e)) && (!_regExpReject || !regExpReject.match(e)) ) 
				{
					entries.push_back(std::make_pair(e, depth + 1)) ;	// Add current directory entry
					_bytes += fileSize(a) ;
				}
			}
		}
...
//
void DirTree::load(DirStruct& dirStruct, const vector<string>& dirPath, size_t depth, bool hidden) // PRQA S 4020, 5500, 5510
{
...
#pragma GCC diagnostic pop
			...
			if (isDir)
			{
   				ACS_CLASS_BGN_DEBUG(ACS_MID_VERB)
					string s; 
                    for (size_t j=0; j<(depth+1); j++) { s += "*---" ; } // PRQA S 3084
				...
				load(newDir, newPath, (depth+1), hidden) ;	// Recursively load dirEntry // PRQA S 3084
						
				// Push dirEntry
				dirStruct.dirEntries.push_back(newDir) ;
			}
			else
			{
    			ACS_CLASS_BGN_DEBUG(ACS_MID_VERB)
					string s; for (size_t j=0; j<(depth+1); j++) { s += "*---" ; } // PRQA S 3084
					ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, s << fullPath) ;
    			ACS_CLASS_END_DEBUG

				// Push filename
				dirStruct.fileEntries.push_back(item) ;
			}
		}	// while
	    if (0 != readdirval) { // an error interrupted cycle above
		    ::closedir(dir) ;
		    ACS_THROW(exIOException("Cannot read directory \"" + path+"\"", readdirval)) ; // PRQA S 3081
	    }
	}
	catch(...) {
		::closedir(dir) ;
...
void DirTree::dumpDirStruct(const DirStruct& dirStruct)
{
	...
	for (size_t i=0; i<dirStruct.dirPath.size(); i++) {
		ACS_LOG_DEBUG(SimpleDebugSignature << "dirPath: \"" << dirStruct.dirPath[i] << "\"") ;	//	Dump pathname
	}
    
	for (size_t i=0; i<dirStruct.fileEntries.size(); i++) {
		ACS_LOG_DEBUG(SimpleDebugSignature << "fileEntries: \"" << dirStruct.fileEntries[i] << "\"") ;	// Dump all file entries
    }

	for (size_t i=0; i<dirStruct.dirEntries.size(); i++) {
		dumpDirStruct(dirStruct.dirEntries[i]) ;	// Dump all directory entries
    }
}
...
//
string DirTree::pathVectorToString(const vector<string>& pathVector, bool relative, const string &newPath)  // PRQA S 4020
{
...
//  STATIC lastChar(...)
//
string DirTree::lastChar(const string& s) // PRQA S 4020
{
// EC:: this method has multiple returns to keep its complexity as low as possible
	...
	return string(s.c_str() + (sl - 1)) ; // PRQA S 3081, 3084
}

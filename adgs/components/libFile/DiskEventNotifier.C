...
DiskEventNotifier::~DiskEventNotifier() throw()
{
    // PRQA S 4631 L1
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_LOW_VERB) ;
	...
	while( !requestedCancel() ) 
	{
		...
		if( 0 != NotifySingleton::instance()->get_events_vector(_notifyUniqueKey, watchEventsV) ) 
		{
			...
			for( size_t i = 0 ; i < wesize ; ++i) 
			{
				...
				_eventsList.push( EventElement(watchEventsV[i]) ); // PRQA S 3081
				evenCnt++;
			}
...
void DiskEventNotifier::addPathToWatch(std::string const & path, bool insertIntoVector) // PRQA S 4020
{
	...
	if (!running())
	{
		// Should it throw the execption here?
		ostringstream os;
		os << "Will not watch to path \"" << path << "\" since DiskEventNotifier thread is not running";
		ACS_LOG_TRYCATCHNOTIFY_EX ( DiskEventNotifierException (os.str()) ); // PRQA S 3081
		return;
	}
	...
	try {
		...
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Path to watch \"" << path << "\" with key \"" << _notifyUniqueKey << "\" (" << (firstWatch ? "never" : "already") // PRQA S 3380 
				<< " watched [id=" << wd << "], process unique: " << boolalpha << _processUnique << ")" ) ;

		if( insertIntoVector ) {
			_pathsToWatch.push_back(path);
        }
	}
	catch( Notify::NoSuchFileOrDirectoryException &e ) {
...
DiskEventNotifier::EventElement::EventElement():
								src_absolute_path(),
								dst_absolute_path(), 
								event(),
								dst_is_set(false)
...
DiskEventNotifier::EventElement::EventElement(EventElement const & rhs):
...
DiskEventNotifier::EventElement & DiskEventNotifier::EventElement::operator=(EventElement const &rhs)
{
	if ( this != &rhs)
	{
		src_absolute_path = rhs.src_absolute_path;
...
void DiskEventNotifier::EventElement::dump() const
{
	...
	if ( e.empty() ) {
		ostringstream o ; o << "0x" << hex << uint32_t(event) << dec ; // PRQA S 3081
...
DiskEventNotifier::EventElementsList::EventElementsList():
										_list(),
										_mutex(),
										_listMaxSize(_listDefaultMaxSize),
										_externalCondition(0)
{
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_LOW_VERB) ;
}
...
DiskEventNotifier::EventElementsList::EventElementsList(DiskEventNotifier::EventElementsList const &el):
																	_list(el._list),
																	_mutex(el._mutex),
																	_listMaxSize(el._listMaxSize),
																	_externalCondition( el._externalCondition)
{}

DiskEventNotifier::EventElementsList & DiskEventNotifier::EventElementsList::operator = (DiskEventNotifier::EventElementsList const &el)
{
	if( this != &el ) {
		_list = el._list;
		_mutex = el._mutex;
...
{
	if( 0 == maxSize ) {
		ACS_THROW( exIllegalValueException("Maximum list size must be >0") ); // PRQA S 3081
    }
...
void DiskEventNotifier::EventElementsList::push(EventElement const & ee)
{
	...
	_list.push_back(ee);
	if (_externalCondition != 0)
	{
		_externalCondition->broadcast();
	}
...
void DiskEventNotifier::EventElementsList::pop(EventElement & ee)
{
	...
	if( _list.begin() == _list.end() ) {
		ACS_THROW( NoDataFound("Events list is empty") ); // PRQA S 3081
	}
	
	list<EventElement>::iterator i = _list.begin();
	ee = *i;
// remove the first element
	_list.pop_front();
...
{
	for( list<EventElement>::const_iterator it = wl.begin(); it != wl.end(); ++it ) {
		it->dump();
    }

...
/*
	...
	Copyright 1995-2019, Exprivia SPA - DADF
...
#define ACS_LIBFILE_TAG_GE_0570 1 /* When set it means version is Greater-Equal than tag Rev_5_70 */

#include <acs_c++config.hpp>

#ifndef CODECHECK
// this is for 32 bit systems with 64bits file offsets 
...
#ifdef __linux__
  #if (__GNUC__ >= 3)
    #include <ext/stdio_filebuf.h>  // PRQA S 1013
  #endif
#endif
#endif
#include <stdint.h>
...
*/
class File                                  // PRQA S 2109
{
public:

	// the class will be closed when destroyed
	class ifdstream : public std::ifstream { // PRQA S 2109, 2153
	public:
		ifdstream(int fd, bool autoclose=true) ;                // PRQA S 2180 2
		ifdstream(FILE *file) ;
		virtual ~ifdstream() throw() ;

	private:
		// declared but not implemented to avoid their use
		ifdstream() ;
        ifdstream(const ifdstream &) ;
		ifdstream &operator=(const ifdstream &) ;
  ...
  #if (__GNUC__ >= 3)
		FILE *_file ;	
		std::basic_filebuf<char> *_bfilebuf ;
        bool _autoclose;
  #endif
#endif
#endif
	...
	// the class will be closed when destroyed
	class ofdstream : public std::ofstream { // PRQA S 2109, 2153
	public:
		ofdstream(int fd, bool autoclose=true) ;                // PRQA S 2180 2
		ofdstream(FILE *file) ;
		virtual ~ofdstream() throw() ;
...
public:
	class Lock { // PRQA S 2109, 2502
	public:
		/*! class LockException declaration */
		exDECLARE_EXCEPTION(LockException,exIOException) ; // PRQA S 2131, 2153, 2502
	...
	public:
        ...
        */
		bool lock(unsigned long timeout_ms=1000, bool shared=false) ; // PRQA S 4400
        ...
        */
        void autoRemoveLockFile(bool v) throw() { _autoRemoveLockFile = v ; }
		...
		*/
        static void tryLock(Lock &, int attempts=10, size_t attemptTimeout_ms=1000, bool shared=false) ; // PRQA S 4400
	...
	private:
		...
		bool _addExt ;
		int _lockedStreamBase ;
        std::string _pfx ;
        bool _autoRemoveLockFile ;
...
public:
    ...
    struct PerfStatistics { // PRQA S 2173, 2175 
        // PRQA S 2100 L3
        ...
        std::string dpath ;
        // PRQA L:L3
        PerfStatistics(): speed(), total_time(), size(), spath(), dpath() {}
    } ;
	...
	*/
	...
	static void copy(const std::string &src, const std::string &dest, std::vector<PerfStatistics> *stats = 0, bool preserveMode = true) ;
	static void link(const std::string &src, const std::string &dest, bool hard=true) ;
	static void hardlink(const std::string &src, const std::string &dest, bool copyOnError=false, bool *copiedOnError = 0, std::vector<PerfStatistics> *stats = 0) ;
	/**
	 ...
	 */
	static short depthLimitedHardlink(const std::string &src, const std::string &dest, signed short maxdepth = -1, bool copyOnError=false, bool *copiedOnError = 0, std::vector<PerfStatistics> *stats = 0);
    ...
    */
	...
	static time_t getModificationTime (const std::string& path, bool* success=0) ;
	...
	*/    
    static unsigned long long int getFSUID (const std::string&, bool followSymLinks = false) ; 
 	...
 	static bool checkFileSystemsMatching(const std::string&, const std::string&, bool followSymLinks = false, unsigned long long int * FSUID1 = 0, unsigned long long int * FSUID2 = 0) ;
...
exostream & operator<< (exostream &, File::CompressionMode) ; // PRQA S 2072

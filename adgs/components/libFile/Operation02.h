...
#define ACS_Operation02_AddPackage(a, siz_a) { memcpy( (package+pkgpointer), (a), (siz_a) ); pkgpointer += (siz_a); } 
#define ACS_Operation02_AddPackageEND(a, siz_a, isBend) { memcpy( (package+pkgpointer), a, siz_a ); \
    ACS_ByteSwap_SWAP((package+pkgpointer), siz_a, isBend); pkgpointer += (siz_a); }
#define ACS_Operation02_AddLData(a, siz_a) { memcpy( (ldata+datapointer), (a), (siz_a) ); datapointer += (siz_a); } 

#define ACS_Operation02_ReadLData(siz_a) { memcpy(package, (ldata+datapointer), (siz_a)); datapointer += (siz_a); }
#define ACS_Operation02_ReadPackage(a, siz_a) { memcpy((a), (package+pkgpointer), (siz_a)); pkgpointer += (siz_a); }
...
public:  
    ...
    struct HINDataParam 
    {
        ...
        signed int              param ;
        unsigned int            buflen ;
        unsigned char           buffer[52] ;        // PRQA S 4403
    ...
    {
        unsigned char           data[63] ;          // PRQA S 4403
        unsigned char           __unused ;
    } ;
    ...
    struct LDataParam
    {
        ...
        unsigned char           appName[48] ;       // PRQA S 4403
        unsigned int            creationDate ;
        unsigned char           type ;
        unsigned short          expDays ;            
        unsigned char           userData[320] ;     // PRQA S 4403
...
#warning "ACS::Operation02.h :: FORCED TO COSMO backward compatibility" ;
    ...
    {
        unsigned char           hwRescan ;                    
        unsigned char           appName[24] ;       // PRQA S 4403
        unsigned int            creationDate ;
        unsigned char           type ;
        unsigned short          expDays ;            
        unsigned char           userData[320] ;     // PRQA S 4403
    ...
    {
    public:
        explicit MemoryGuard(void*) throw() ;
        ~MemoryGuard() throw()  ;
    private:
        void* _address ;	

		MemoryGuard() ;
		MemoryGuard(const MemoryGuard&) ;
		MemoryGuard &operator=(const MemoryGuard&) ;
    } ;

    class ResetAndGuardLDataParam
    {
    public:
        explicit ResetAndGuardLDataParam(struct LDataParam* a) throw() ;
        ~ResetAndGuardLDataParam() throw() ;
    private:
...
static void TEMPORARY_fill_LDataParam(struct LDataParam* ldata) ;    
    ...
    static unsigned char* createLData(unsigned int* outdatalen, char const* name, \
        RSA* privKey, ACS_SSL::CipherStyle cipherRSA, struct LDataParam const*) ;
    static void readLData(struct LDataParam*, unsigned char const*, \
        RSA* pubKey, ACS_SSL::CipherStyle cipherRSA, unsigned int ldatasize);
    ...
    static unsigned char* unpackLData(unsigned int* outlen, \
        ACS_SSL::CipherStyle* cipheredRSA, unsigned char* blobdata, unsigned int bloblen, unsigned short ab);

static bool checkLData(unsigned char *, size_t, const std::string &, bool dumpData=false) ;
static bool checkHWParams(struct LDataParam const*) ;


private: 

	Operation02(const Operation02&) ;
	Operation02 &operator=(const Operation02&) ;

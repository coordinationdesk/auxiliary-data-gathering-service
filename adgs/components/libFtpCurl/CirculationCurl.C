...
CirculationCurl::CirculationCurl(const StopController& stopController) : 
    ...
    _statistics(), 
    _mutex(), 
    _stopController(stopController), // PRQA S 2528
    _simulationMode(conf::CurlConfigurerSingleton::instance()->getSimulateCirculation()),
    _earlyFinalRemoval(true),  // default remove final file BEFORE the transfer
	_streamerId(0),
	_localTransferMode(COPY),
	_source_http_response_code(0),
	_target_http_response_code(0)
{
}
...
CirculationCurl::~CirculationCurl() throw () {}
//
//Public methods.
//
...
void CirculationCurl::localTransfer() {
    ...
    if (_sourceSite.getIsFileSystem() && _targetSite.getIsFileSystem()) {
        ...
        if( temporaryTargetNameDefined ) {
            ...
            if( not StringUtils::isBlank(temporaryTargetDir) && not File::exists(temporaryTargetDir) ) {
        ...
        try {
            ...
            switch (_localTransferMode) {
                ...
                {
                    ostringstream exMsg;
                    exMsg << "Unrecognized local transfer mode: " << int(_localTransferMode); // PRQA S 3081
    ...
    else 
	{
        ...
        else if (_targetSite.isLocalhost()) 
		{
		    ...
		    if ( 0 != _streamerId ) {
                ...
                ss << "Streamer id set to " << _streamerId << ". On-the-fly mode not supported for remote to local transfers.";
                ACS_THROW(exCriticalException(ss.str()));
            }
            ...
            conf::CurlConfigurer* configurer =  conf::CurlConfigurerSingleton::instance();
...
void CirculationCurl::transferFile() {
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB,  "PipedStream created: " << _sourceSite.getPath() << endl << "\t" << tempTargetSite.getPath());
    Buffer buffer;
#if __cplusplus > 199711L
    unique_ptr<PipedStream> handle(new PipedStream(buffer));
#else
    auto_ptr<PipedStream> handle(new PipedStream(buffer));
#endif
...
// if the source curl result corresponds to FtpCurlRecoverableErrorException try target curl one.
	{
		...
		catch( FtpCurl::FtpCurlRecoverableErrorException & )
		{
    ...
    if (conf::CurlConfigurerSingleton::instance()->getEnableStatistics()) {
        if (Application::exists() && (Application::instance()->getDefaultConnectionPool() != 0)) {
...
void CirculationCurl::visitRemoteFile(const string &remoteFile, const string &relativeRemoteFile, void* observer) {
    CirculationCurl* thisObj = reinterpret_cast<CirculationCurl *>(observer); // PRQA S 3081
...
void CirculationCurl::visitRemoteDir(const string &remoteFile, const string & relativeRemoteFile, void* observer) 
{
    //does nothing.
}
...
const RemoteSite &CirculationCurl::getSourceSite() const throw() // PRQA S 4120
{
    return _sourceSite;
}

const RemoteSite &CirculationCurl::getTargetSite() const throw() // PRQA S 4120
{
    return _targetSite;
}
...
std::ostream& operator<<(std::ostream& stream, CirculationCurl& obj) 
{
    stream << obj.str();
...
int CirculationCurl::chown(string const& userId, string const& groupId, string const& filename) // PRQA S 4020
{
    ...
    char *endptr = 0 ;
    uid_t uid = uid_t(strtol(userId.c_str(), &endptr, 10)); /* Allow a numeric string */ // PRQA S 3000, 3081, 4400
    if (*endptr != '\0') { /* Was not pure numeric string */
        try {
    ...
    gid_t gid = gid_t(strtol(groupId.c_str(), &endptr, 10)); /* Allow a numeric string */ // PRQA S 3000, 3081, 4400
    if (*endptr != '\0') { /* Was not pure numeric string */
        try {
...
void CirculationCurl::setSimulationMode(bool v) throw() // PRQA S 4121
{
    _simulationMode = v ;
}

bool CirculationCurl::isSimulated() const throw() // PRQA S 4120
{

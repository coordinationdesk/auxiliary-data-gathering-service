...
public:
	...
	enum LocalTransferMode // PRQA S 2502
    {
        COPY,
        HARDLINK,
        SYMLINK
...
public:
    ...
    const RemoteSite &getSourceSite() const throw() ;
    const RemoteSite &getTargetSite() const throw() ;
     ...
     */
    void setSimulationMode(bool) throw() ;
    bool isSimulated() const throw() ;
	/**
		ENRCAR:: 
	...
	*/
    void setEarlyFinalRemoval(bool v) throw() { _earlyFinalRemoval = v ; } 
    bool getEarlyFinalRemoval() const throw() { return _earlyFinalRemoval ; } 
	...
	*/
	...
	void setStreamerId(dbPersistent::IdType id) throw() { _streamerId = id; }
	dbPersistent::IdType getStreamerId() const throw() { return _streamerId; }
	
	
	void setLocalTransferMode(LocalTransferMode v) { _localTransferMode = v; }
	LocalTransferMode getLocalTransferMode() const throw() { return _localTransferMode ; }

    long getSourceHttpResponseCode() const throw() { return _source_http_response_code ; }
    long getTargetHttpResponseCode() const throw() { return _target_http_response_code ; }
...
private:
    CirculationCurl(); // not implemented
    CirculationCurl(const CirculationCurl &); // not implemented
    CirculationCurl& operator=(const CirculationCurl &); // not implemented
#ifdef ACS_TEST
public:
#else
private:
    ...
    bool _earlyFinalRemoval ;
    dbPersistent::IdType _streamerId;
	LocalTransferMode _localTransferMode;
	long _source_http_response_code ;
	long _target_http_response_code ;

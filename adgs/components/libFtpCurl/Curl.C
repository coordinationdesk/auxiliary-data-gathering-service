...
namespace {
    ...
    protected:
	    CurlInitializer() ;
    private:
	    CurlInitializer(const CurlInitializer &);
	    CurlInitializer & operator= (const CurlInitializer &);
        ...
        void curl_share_housekeeping() throw() ;
    ...
    private:
        char _ftpProxy[1024] ; // PRQA S 4403
    ...
    void CurlInitializer::curl_share_initialization()
    {
        ...
        ACS_COND_THROW ( 0 == curl_share_ , Curl::CurlException("Unable to create CURL shared object")); // PRQA S 3081
    ...
    void CurlInitializer::curl_share_housekeeping() throw() 
    {
        if (0 != curl_share_)
        {
    ...
    void CurlInitializer::curl_global_initialization()
    {
	// any init on SSL has been performed by the CryptoEngineInitializer singleton and Base Soap library linking
        ...
        // if some crash will occour on ssl_init call define the following in the makefile
...
#warning SSL will not be initialized here because FTPCURL_DO_NOT_INIT_SSL is defined
	...
	CurlInitializer::~CurlInitializer() throw() 
    {
#ifndef DO_NOT_USE_CURL_SHARES    
        try {
...
Curl::Curl() :
    _transaction(0), 
    _curl(0), 
    _errorMessage(new char[_errorMessageSize]),
    _site(RemoteSite::ftp), 
    _progressCallback(0), 
    _listCallback(listCallback), 
    _getCallback(getCallback), 
    _putCallback(putCallback), 
    _debugCallback(debugCallback), 
    _userPassword(), 
    _ftpDefaultPort("-"), 
    _createMissingDirs(false), 
    _progressCallbackData(0), 
    _listCallbackData(0), 
    _ptrToHeaders(), 
    _isCleaning(false), 
    // for the following parameters see: http://curl.haxx.se/libcurl/c/curl_easy_setopt.html
    _connectionTimeout_ms(conf::CurlConfigurerSingleton::instance()->getConnectionTimeout_ms()), 
    _connectionTimeout_s(max<size_t>(2,_connectionTimeout_ms/1000)), // PRQA S 3084, 4400
    _response_timeout_ms(min(_connectionTimeout_ms,conf::CurlConfigurerSingleton::instance()->getConnectionResponseTimeout_ms())),
    _response_timeout_s(max<size_t>(1, _response_timeout_ms/1000)),  // PRQA S 3084, 4400
	_ftp_command_response_timeout_s( min(_connectionTimeout_s, conf::CurlConfigurerSingleton::instance()->getFtpCommandResponseTimeout_s()) ),  // PRQA S 3084, 4400
    _lastLoggedErrorMessage(), 
    _operationCode(), 
    _stats(),
    tmp_proxy_(),
    tmp_proxyauth_(),
...
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "use progress callback: " << boolalpha << use_progress_callback);
	
    //enable progress callback call
    if ((0 != _progressCallback) || not use_progress_callback) {
	...
	if ( _site.getIsHttp() ) {
      ...
      if (httpContentType_ != "" ) {
        ...
        for(std::vector<std::string>::const_iterator it = customHeaders.begin(); it != customHeaders.end(); ++it) {
            slist_ = curl_slist_append(slist_, (*it).c_str());
        }
...
void Curl::setProxy() // PRQA S 4020
{
    ...
    Proxy proxy ; 
    bool proxyFound = conf::CurlConfigurerSingleton::instance()->findProxy(_site.getHost(), proxy) ;
    if (not proxyFound) {
        ...
        ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "ignoring proxy setting for host: " << _site.getHost());
        return ;
    }

    ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "proxy: " << proxy << " for host: " << _site.getHost());
    ...
    if (proxy == no_proxy()) {
        ...
        // to not use any proxy for the specific host _site.getHost()
        curl_easy_setopt(_curl, CURLOPT_NOPROXY, _site.getHost().c_str()) ;
        return ;
    }
    
#warning "Set of CURLOPT_PROXYUSERPWD CURLOPT_PROXYUSERNAME CURLOPT_PROXYPASSWORD still to test"
    // case 3: set a configured PROXY for the host _site.getHost()
    if (not proxy.user().empty()) {
        if (not proxy.password().empty()) {
    ...
    os << proxy.host() ;
    ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Using proxy: " << proxy.host() << " to connect with: " << _site.getHost()) ;
...
#warning Proxy settings not supported on this version of Curl
	ACS_THROW(exNotImplemented(string("CURL proxy support not implemeted on this version: ") + LIBCURL_VERSION)) ;
#endif    
}

CURL* Curl::openConnection() // PRQA S 4020
{
	...
	*/
			...
			if( _site.getIsSecureHttp() )
			{
				...
				else
				{
					...
					if ( (_site.getVerifyCertificateParameter(Confkey_verifycert_cacert, param_value)) && (! param_value.empty()) ) 
					{
						...
						ostringstream o ; o << "Site: \"" << _site.getHost() << "\" setting certificate verification parameter: " \
    	...
    	if (_site.getIsFtpes() || _site.getIsFtps()) {
			...
			ACS_LOG_INFO( "FTPS/FTPES Host: " << _site.getHost() << " Certificate verification: " \
        	...
        	if (!_site.getIsSecure()) {
            	ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "Curl connection is passive: " << boolalpha << _site.getIsPassive());
        	}
        	...
        	if (!_site.getIsSecure() && !_site.getIsPassive()) {
    ...
    // This function has been added in libcurl 7.25.00
        ...
        {
            ACS_CLASS_WRITE_DEBUG_TH (ACS_VLO_VERB, "Unable to set the CURLOPT_TCP_KEEPALIVE (" << curl_easy_strerror(res) << "): " << _errorMessage);
        }
    #else
	...
	{
		if( dynamic_cast<const RemoteSiteUtility::CryptException*> (&ex) != 0 ) { // PRQA S 3081
...
CURL* Curl::getCurlHandle() { // PRQA S 4120, 4211
    return _curl;
}

void Curl::ldel(const string &localFile) {
...
void Curl::setHTTPOpts(const CurlHttpOpt& opt)
{
	...
	if (timeoutSet)
	{
		...
		ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "Curl whole transfer time out overwriten [ms] " << connectionTimeout_ms);
#else
		curl_easy_setopt(_curl, CURLOPT_TIMEOUT, connectionTimeout_s);
		ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "Curl whole transfer time out overwriten [s]" << connectionTimeout_s);
#endif

#ifdef CURLOPT_CONNECTTIMEOUT_MS
		curl_easy_setopt(_curl, CURLOPT_CONNECTTIMEOUT_MS, responseTimeout_ms);
		ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "Curl response time out overwriten [ms] " << responseTimeout_ms);
#else
		curl_easy_setopt(_curl, CURLOPT_CONNECTTIMEOUT, responseTimeout_s);
		ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "Curl response time out overwriten [s]" << responseTimeout_s);
#endif
	}
...
CURLcode Curl::httpPropFind (string const& path, int depth, string & output, const string & requestBody, const std::string & userpwd)
{
	...
	ACS_COND_THROW( not _site.getIsHttp(), CurlUnrecoverableException("Protocol is \""+_site.getProtocol()+"\": http(s) expected") );
	
	if( depth > 1 ) {
		ostringstream exMsg;
		exMsg << "Depth request set to " << depth << ": expected values are: 0, 1, or negative values (infinity)";
		ACS_THROW( CurlUnrecoverableException(exMsg.str()) );
	}

	string uri = _site.getProtocol() + "://" + _site.getHost() + "/" + StringUtils::trimBegin(path, '/');
...
CURLcode Curl::put(const string &localFile, const string &remoteFile, mode_t permissions) {
   	...
   	void* default_data_pointer (0) ; 

    return putWithCallback(localFile, remoteFile, permissions, default_data_pointer, _putCallback) ;
}


CURLcode Curl::putWithCallback(const string &localFile, const string &remoteFile, mode_t permissions, void* data_pointer, PutCallback put_func_pointer) {
    //put the local file to the remote _site using the remoteFile name.
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Put file " << localFile << " into " << remoteFile);
    CurlStatistics::log(_site, "put");
    ACS_COND_THROW(not File::exists(localFile), CurlException("Put error: local entry" + localFile + ": does not exist.")) ; // PRQA S 3081 2
    ACS_COND_THROW(not File::isReadable(localFile), CurlException("Put error: local entry" + localFile + ": it is not readable.")) ;
    ACS_COND_THROW( (File::isDir(localFile) && (not File::isExecutable(localFile)) ), CurlException("Put error: local directory" + localFile + ": it is not accessible.")) ;
    CURLcode res = CURLE_OK;
...
CURLcode Curl::getWithCallback(const string &localFile, const string &remoteFile, CurlFile& dataHandler, GetCallback get_func_pointer) // PRQA S 4020
{
    //get the /remote/file/name and write to the /local/file/name.
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "file " << remoteFile << " into " << localFile);
    ...
    BEGIN_TRANSACTION(TrGet, true) // PRQA S 4236
		...
		void* data_handler_forced_zero (0) ; 
		...
		if (CURLE_OK == res)
		{
			...
			if (false == process_success)
			{
				...
				ostringstream o ; o << "Error running the callback method during get. Exit code: " << exit_code << "  exit signal: " << exit_signal ;
				strcpy(_errorMessage, o.str().c_str()) ; 
...
CURLcode Curl::ls(string const &dir, void* output) {
    string remoteDir = dir;
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "dir " << _site.getHost() << "/" << remoteDir);
	...
	try {
    	BEGIN_TRANSACTION(TrList, false) // PRQA S 4236
        	...
        	res = performTransfer(true, remoteDir, "", 0, 0);
...
size_t Curl::dontWriteToConsole(void *buffer, size_t size, size_t nmemb, void *userp)
{
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "file: " << _site.getHost() << "/" << remoteFile);
    CurlStatistics::log(_site, "del");
    CURLcode res = CURLE_OK ;
    BEGIN_TRANSACTION(TrRemove, false) // PRQA S 4236
        boost::shared_ptr < CurlHeaderMaker > maker (new CurlHeaderMaker(_site, _curl)) ;
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "file: " << _site.getHost() << "/" << remoteDir);
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "file: " << _site.getHost() << "/" << remoteFilename << " to: " << newRemoteFilename);
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "file/dir: " << _site.getHost() << "/" << remoteFilename << " to: " << oct << permissions);
    ...
    BEGIN_TRANSACTION(TrChmod, false) // PRQA S 4236
        ...
        ostringstream os;
        if (!_site.getIsSecure()) {
            os << "SITE CHMOD 0" << oct << permissions << " " << remoteFilename;
        } 
        else {
            os << "chmod 0" << oct << permissions << " " << remoteFilename;
        }
    ...
    void* readData)  
{
    ...
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Perform transfer to " << targetFilename 
                            << " and rename to target file name: " << finalTargetFName << " removeAlso: " << boolalpha << removeAlso); // PRQA S 3380
    _ptrToHeaders.push_back(maker); // PRQA S 3081
...
#warning FranAva: Although this lib can be used with any version of libCurl, Curl::performTransferRenameChmod() might not work on version prior 7.24 . Consider installing at least libcurl v7.24
    ...
    if (!_site.getIsSecure())
    {
        ...
        if (0 != permissions) {
            ostringstream os;
            if (!_site.getIsSecure()) {
                os << "SITE CHMOD 0" << oct << permissions << " " << targetFilename;
            } 
            else {
                os << "chmod 0" << oct << permissions << " " << targetFilename;
            }
            maker->addHeader(os.str());
        }
    ...
    if (res == CURLE_OK) {
        ...
        if (res != CURLE_OK) {
            message << "Curl error url: \"" << fileToTransfer << "\" curl returned: " << int(res) << " (" // PRQA S 3081
                    << curl_easy_strerror(res) << ")" << " - ErrMsg = " << _errorMessage;
...
void Curl::fillInCirculationStatistics(bool source, CirculationStatistics* statistics) // PRQA S 4020, 4211
{ 
...
//
int Curl::debugCallback(void *curl, int infotype, char *dbg, size_t dbgsize, void *dbgdata) // PRQA S 4020
{
    if ((0 == dbg) || (0 == dbgsize)) { // no info, just return
        return 0;
    }

    Curl* thisObj = reinterpret_cast<Curl*>(dbgdata); // PRQA S 3081
    string currentSite;
    ...
    char str[dbgsize + 1]; // PRQA S 3084
    memset(str, 0, sizeof(str));
    strncpy(str, dbg, dbgsize);
    map<char, char > tr ; 
    tr['\r'] = ' ' ;
    tr['\n'] = ' ' ;
...
size_t Curl::listCallback(void *buffer, size_t size, size_t nmemb, void *stream) // PRQA S 4020
{
    if (stream) {
        ostream *out = reinterpret_cast<ostream *>(stream); // PRQA S 3081
        if (!out) {
...
size_t Curl::probeCallBack(void *buffer, size_t size, size_t nmemb, void *curl) // PRQA S 4020
{
    ...
    Curl *cUrl = reinterpret_cast<Curl *>(curl); // PRQA S 3081
    if(cUrl->setProbeToken())
    {
        return size * nmemb ; // PRQA S 3084
    }
    return 0; 
...
size_t Curl::getCallback(void *buffer, size_t size, size_t nmemb, void *stream) {

	// The stream argument is assumed to be a pointer to CurlFile* (in this case, a wrapper to FILE*) -- this is the default

    size_t result = 0;
    CurlFile *out = reinterpret_cast<CurlFile*>(stream); // PRQA S 3081
...
size_t Curl::ostreamGetCallback(void *ptr_buffer_in, size_t size, size_t nmemb, void *ptr_ostream_out) {
    ...
    CurlFile *out = reinterpret_cast<CurlFile*>(ptr_ostream_out); // PRQA S 3081
    if (!out || !out->getStream()) {
        ACS_LOG_ERROR(SimpleDebugSignature << "cannot write: null stream") ;
    } 
    else 
	{
		...
		ostream &os(static_cast<ofuntarstream*>(out->getStream())->operator()()) ; 	
		...
		if ( (os.good()) && (to_write > 0) )
		{
			...
			os.write(reinterpret_cast <char*>(ptr_buffer_in), to_write) ;
...
size_t Curl::putCallback(void *ptr, size_t size, size_t nmemb, void *stream) {
...
size_t Curl::istreamPutCallback(void *ptr_buffer_out, size_t size, size_t nmemb, void *ptr_istream_in) {
	...
	if (to_read > 0) 
	{
		...
		(static_cast<istream*>(ptr_istream_in))->read(reinterpret_cast <char*>(ptr_buffer_out), to_read);
		retcode = (static_cast<istream*>(ptr_istream_in))->gcount() ;
	}

    return retcode;
}

void Curl::setProgressCallback(ProgressCallback progressCallback, void* progressCallbackData) {
    _progressCallback = progressCallback;
    _progressCallbackData = progressCallbackData;
}

void* Curl::getProgressCallbackData() const { // PRQA S 4120
    return _progressCallbackData; // PRQA S 4628
}

void Curl::setListCallback(ListCallback listCallback, void* listCallbackData) {
    _listCallback = listCallback;
...
CURLcode Curl::probeElement (bool isFile, string const& element)
{
	...
	BEGIN_TRANSACTION(TrList, false) // PRQA S 4236
		...
		try
		{
			res = performTransfer(true, remoteElement, "", 0, 0);
		}
		catch (exception& ex)
		{
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "dir " << _site.getHost() << "/" << dir);
...
bool Curl::probe(string const &remotePath, bool emitExc, CURLcode *curlCode) 
{
    ...
    try {
        ...
        ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "success: " << boolalpha << success)
    } 
    catch (std::exception const& ex) {
		if( emitExc ) {
			throw;
...
const RemoteSite &Curl::getSite() const throw()  // PRQA S 4120
{
    return _site;
}

void Curl::setTransaction(CurlTransaction* transaction) throw()  // PRQA S 4121
{
    _transaction = transaction;
}

void Curl::setCreateMissingDirs(bool createMissingDirs) throw()  // PRQA S 4121
{
    _createMissingDirs = createMissingDirs;
}

bool Curl::getCreateMissingDirs() const throw()  // PRQA S 4120
{
    return _createMissingDirs;
}

size_t Curl::getConnectionTimeout_ms() const throw()  // PRQA S 4120
{
    return _connectionTimeout_ms;
}

size_t Curl::setConnectionTimeout_ms(size_t const ms, bool adjustResponseTimeout) throw() // PRQA S 4121
{
...
size_t Curl::computeTimeouts(const size_t ms, const bool adjustResponseTimeout, size_t& connectionTimeout_ms, size_t& connectionTimeout_s, size_t& response_timeout_ms, size_t& response_timeout_s) throw() // PRQA S 4121
{
...
const string &Curl::getLastErrorMessage() const throw() // PRQA S 4120
{
    return _lastLoggedErrorMessage;
}


TransactionType Curl::getOperationCode() const throw() { return _operationCode ; } // PRQA S 4120

void Curl::setOperationCode(TransactionType op) throw() { _operationCode = op ; } // PRQA S 4121

CurlTransaction* Curl::getTransaction() const throw() { return _transaction ; } // PRQA S 4120, 4628

void Curl::setCurl(CURL * c) throw() { _curl = c ; } // PRQA S 4121

CURL * Curl::getCurl() const throw() { return _curl ; } // PRQA S 4120, 4628

void Curl::checkFtpCommandResponseTimeout() throw()
{
...
std::string Curl::encodeRFC398(const std::string & inString)
{
	...
	if (0 == encoded_s.length())
	{
		ostringstream os;
		os << " Encoding of " << inString << " failed.";
		ACS_THROW(CurlCodingException(os.str()));
	}
...
std::string Curl::decodeRFC398(const std::string & inString)
{
	...
	if (0 == decoded_s.length())
	{
		ostringstream os;
		os << " Decoding of " << inString << " failed.";
		ACS_THROW(CurlCodingException(os.str()));
	}

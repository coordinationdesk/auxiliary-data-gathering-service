...
public:	
    CurlConfigurer();
    virtual ~CurlConfigurer() throw ();
private:
    CurlConfigurer(CurlConfigurer const& other);
    CurlConfigurer& operator=(CurlConfigurer const& other);
public:
    /**
    ...
    static void setOnceForRun(bool onceForRun) throw() { _onceForRun = onceForRun; }
    ...
    public:
        ...
        ChangeLocalPermissions(ChangeLocalPermissions const& other);
        virtual ~ChangeLocalPermissions() throw ();
        ChangeLocalPermissions& operator=(ChangeLocalPermissions const& other);
    public:
        bool isActivated() const throw ();
        bool isActivatedChangeOwner() const throw ();
        bool isActivatedChangeMode() const throw ();
        const std::string &getUserId() const throw ();
        const std::string &getGroupId() const throw ();
    ...
    private:
#endif
        bool activatedChangeMode;
        bool activatedChangeOwner;
        std::string userId;
        std::string groupId;
        mode_t mode;
    };
...
private:
#endif
    bool checkStatistics() const throw ();
    static bool _onceForRun;

public:
    //Getters/Setters
    size_t getMaxFilesToTransfer() const throw ();
    void setMaxFilesToTransfer(size_t) throw ();
    size_t getConnectionResponseTimeout_ms() const throw ();
    size_t getConnectionTimeout_ms() const throw ();
    size_t getHttpConnectionResponseTimeout_ms() const throw ();
    size_t getHttpConnectionTimeout_ms() const throw ();
    void setConnectionResponseTimeout_ms(size_t) throw ();
    void setConnectionTimeout_ms(size_t) throw ();
    void setHttpConnectionResponseTimeout_ms(size_t) throw ();
    void setHttpConnectionTimeout_ms(size_t) throw ();
    size_t getFtpCommandResponseTimeout_s() const throw ();
    void setFtpCommandResponseTimeout_s(size_t) throw ();
    void setProbeConnectionTimeout_ms(size_t) throw ();
    size_t getProbeConnectionTimeout_ms() const throw ();
    void setLsConnectionTimeout_ms(size_t) throw ();
    size_t getLsConnectionTimeout_ms() const throw ();
    const std::string &getRemoteTempPath() const throw ();
    void setRemoteTempPath(const std::string &);
    void setIsWrapDirTransfer(bool) throw ();
    const std::string &getTempNameStrategy() const throw ();
    void setTempNameStrategy(std::string const&);
#ifdef ACS_CURL_CCC_SUPPORTED
    curl_ftpccc getFtpsCCC() const throw ();
    void setFtpsCCC(curl_ftpccc) throw ();
#endif
    bool getIsFtpsProtectData() const throw ();
    void setIsFtpsProtectData(bool) throw ();
    bool getEnableStatistics() const throw ();
    void setEnableStatistics(bool) throw (); // MP: no table verification done
    std::string getSshPublicKey() const throw ();
    void setSshPublicKet(std::string const& ) throw ();
    std::string getSshPrivateKey() const throw ();
    void setSshPrivateKet(std::string const& ) throw ();
    size_t getLowSpeedLimit() const throw ();
    void setLowSpeedLimit(size_t const ) throw ();
    size_t getLowSpeedTime() const throw ();
    void setLowSpeedTime(size_t const ) throw ();
    bool changePermissions() const throw ();
    bool isActivatedChangeMode() const throw();
    bool isActivatedChangeOwner() const throw();
    std::string getUserId() const throw ();
    std::string getGroupId() const throw ();
    std::string getMode() const throw ();
    const CurlConfigurer::ChangeLocalPermissions& getPermissions() const throw();
    void setPermissions(const CurlConfigurer::ChangeLocalPermissions&) ;
    bool getSimulateCirculation() const throw() ;
    void setSimulateCirculation(bool) throw() ;
    const std::string &getLocalTempPath() const throw ();
    void setLocalTempPath(const std::string &);
    const std::string &getHttpListStylesheetName() const throw ();
    void setHttpListStylesheetName(const std::string &);
    const std::string &getHttpListStylesheetVersion() const throw ();
	...
	bool getFastProbeModeEnabled() const throw();
	void setFastProbeModeEnabled(bool) throw();
#ifdef EXP_UPLOAD_BUFFER_CONFIGURABLE
    size_t getUploadBufferSize() const throw ();
    void setUploadBufferSize(size_t const ) throw ();
#endif  
private:
...
public:
    ...
    static const size_t DEFAULT_LOW_SPEED_LIMIT;
    static const size_t DEFAULT_LOW_SPEED_TIME;
    ...
    // PRQA L:L1
private:
    size_t _maxFilesToTransfer;
    size_t _connectionTimeout_ms ;
    size_t _httpConnectionTimeout_ms ;
    size_t _probeConnectionTimeout_ms;
    size_t _lsConnectionTimeout_ms;
    size_t _connectionResponseTimeout_ms;
    size_t _httpConnectionResponseTimeout_ms;
	size_t _ftpCommandResponseTimeout_s;
    std::string _remoteTempPath;
    std::string _tempNameStrategy;
#ifdef ACS_CURL_CCC_SUPPORTED
    curl_ftpccc _ftpsCCC; // Clear Command Channel
#endif
    bool _ftpsProtectData;
    mutable bool _enableStatistics;
    std::string _sshPublicKey;
    std::string _sshPrivateKey;
    size_t _lowSpeedLimit;
    size_t _lowSpeedTime;
    ChangeLocalPermissions _permissions;
    mutable bool _statisticsCheckGood ;
    bool _simulateCirculation ;
    std::map<std::string, curl::Proxy > _proxies ;
	std::string _localTempPath;
	std::string _httpListStylesheetName;
	std::string _httpListStylesheetVersion;
	bool _fastProbeModeEnabled;
#ifdef EXP_UPLOAD_BUFFER_CONFIGURABLE
    size_t _uploadBufferSize ; // 16K - 2M 
#endif
};

typedef pattern::Singleton<CurlConfigurer> CurlConfigurerSingleton;

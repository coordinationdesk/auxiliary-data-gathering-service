...
namespace {
	CurlErrorBehaviourSingleton::CurlErrors c_err[] = {
		...
		{ 19,"CURLE_FTP_COULDNT_RETR_FILE", false, CurlErrorBehaviourSingleton::UNRECOVERABLE},
	...
	CurlErrorBehaviourSingleton::CurlAliases c_aliases[] = {
	...
	default:
		{
			...
			ss << "Unrecognized CURL error behaviour having value " << static_cast<unsigned int>(beh);	// PRQA S 3081
			ACS_THROW(exIllegalValueException(ss.str())); // PRQA S 3081
		}
	}
...
CurlErrorBehaviourSingleton::CurlErrorBehaviourSingleton() ACS_NOEXCEPT :
	Singleton<CurlErrorBehaviourSingleton>(),
	_mutex(),
	_curlErrors(),
	_curlErrorsBehaviours()
{
	initErrorMaps();
}
...
CurlErrorBehaviourSingleton::CurlErrorBehaviour CurlErrorBehaviourSingleton::getBehaviour(int error) const {
	...
	if (error < 0) {
		...
		std::map<int, CurlErrors>::const_iterator it = _curlErrorsBehaviours.find(error);
...
bool CurlErrorBehaviourSingleton::setBehaviour(int error, CurlErrorBehaviourSingleton::CurlErrorBehaviour beh) {
	...
	std::map<int, CurlErrors>::iterator it = _curlErrorsBehaviours.find(error);
	if (it != _curlErrorsBehaviours.end()) {
		...
		std::ostringstream ss;
		ss << "Curl error having code " << error << " not found.";
...
bool CurlErrorBehaviourSingleton::getErrorCode(const std::string& errname, int& errcode) const {
	...
	bool res = false;
	std::map<std::string, int>::const_iterator it = _curlErrors.find(errname);

...
private:
    CurlFile(); // not implemented
    CurlFile(CurlFile const& rhs);  // not implemented
    CurlFile& operator=(CurlFile const& rhs); // not implemented
public:
    ~CurlFile() throw() ;
    void fopen();
	...
	inline const std::string &getFilename() const throw() { return _filename; }

	inline void* getStream() const throw() { return _stream ; } // PRQA S 4211
	inline void setStream(void* p) throw() { _stream = p ; } 
	inline off_t getWrote() const throw() { return _wrote; }
	inline void setWrote(off_t const off) throw() { _wrote = off; }
	inline void addWrote(off_t const off) throw() { _wrote += off; }

...
CurlHeaderMaker::CurlHeaderMaker(RemoteSite const& site, CURL* curl) :
    _headerList(0),
    _preCommandList(0),
    _fileHandler(0), 
    _res(), 
...
CurlHeaderMaker::~CurlHeaderMaker() throw(){
    close();
}

void CurlHeaderMaker::close() throw() {
...
const string &CurlHeaderMaker::getURL() const throw() { // PRQA S 4120
    return _url;
}
...
void CurlHeaderMaker::setCURL(CURL* curl)  // PRQA S 4121
{
    _curl = curl;
}
...
void CurlHeaderMaker::makeGet(string const& fileToTransfer, void* data, bool quoteOnly) {
...
string CurlHeaderMaker::makeProtocol() const {
    ...
    if (_site.getIsFtp() || _site.getIsFtpes()) {
        url += "ftp://"; //the same protocol for ftpes and ftp
    } 
    if (_site.getIsFtps()) {
        url += "ftps://"; //the same protocol for ftps and ftp
    } 
    else if (_site.getIsSecure()) {
        url += "sftp://";
    }
	else if( _site.getIsSecureHttp() ) {
		url += "https://";
	}
...
string CurlHeaderMaker::makeURL(string const& remoteFilename) const {
	...
	if (_site.getIsHttp() || _site.getIsSecureHttp()) {
		src = StringUtils::transform(_site.getHost() + "/" + remoteFilename, "//", "/");
	} else {
		src = StringUtils::transform(_site.getHost() + "/" + remoteFilename, "/", "//");
	}
...
void* CurlHeaderMaker::makeReadData(string const& fileToTransfer, string const& targetFilename) {
...
void CurlHeaderMaker::makePut(string const& fileToTransfer, string const& targetFilename, void* handler) {
    if (!handler) {

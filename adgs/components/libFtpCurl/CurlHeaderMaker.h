...
public:
     ...
     */
    void makeGet(std::string const& fileToTransfer, void* data = 0, bool quoteOnly = false);
    /**
     ...
     */
    void makePut(std::string const& fileToTransfer, std::string const& targetFilename, void* handler = 0);
...
private:
    CurlHeaderMaker();
    CurlHeaderMaker(CurlHeaderMaker const& rhs);
    CurlHeaderMaker& operator=(CurlHeaderMaker const& rhs);
private:
    curl_slist* _headerList;
    curl_slist* _preCommandList;
    FILE* _fileHandler;
    CURLcode _res;
    std::string _url;
    RemoteSite _site;

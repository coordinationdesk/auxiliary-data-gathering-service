...
public:
    ~CurlStatistics() throw() {}

    static acs::math::StatNode & findNode(const std::string &path);

    template <class T>  
    static void accumulate(std::string path, T t);

    template <class T>  
    static void totalAvrg(std::string path, T value);
...
private:
    CurlStatistics(); // not implemented
    CurlStatistics(CurlStatistics const& ); // not implemented
    CurlStatistics& operator=(CurlStatistics const& ); // not implemented
} ;

template < class T >
inline
void CurlStatistics::accumulate(std::string path, T t) // PRQA S 2010
{
    findNode(path) += t ;
}
...
template < class T >
inline
void CurlStatistics::totalAvrg(std::string path, T value) // PRQA S 2010
{
    accumulate(path + "/total", value) ;
}

...
class Stream {                                      // PRQA S 2109
public:
    Stream() {}
    virtual ~Stream() throw() {}                    // PRQA S 2131
private:
    Stream(Stream const&) ; // not implemented
    Stream& operator=(Stream  const&) ; // not implemented
public:
    virtual bool closed() const = 0 ;
    virtual void close() = 0 ; // PRQA S 2502
};

class InputStream: virtual public Stream {                   // PRQA S 2109
public:
    InputStream() : Stream(), _bytesRead() {}
    virtual ~InputStream() throw() {}               // PRQA S 2131

    virtual size_t read(char* b, size_t len) = 0; // PRQA S 2502

    long long bytesRead() const throw() { return _bytesRead ; }                    
    void bytesRead(long long r) throw() { _bytesRead = r ; }
    void addBytesRead(long long r) throw() { _bytesRead += r ; }

private:
    InputStream(InputStream const&); // not implemented
    InputStream& operator=(InputStream const &); // not implemented

private:
    long long _bytesRead; 
};

class OutputStream: virtual public Stream { // PRQA S 2109
public:
    OutputStream() : Stream(), _bytesWrote() {}
    virtual ~OutputStream() throw() {}              // PRQA S 2131

public:
    virtual void flush() {}   // PRQA S 2131, 2502
    virtual ssize_t write(char* buffer, size_t length) = 0; // PRQA S 2502

    long long bytesWrote() const throw() { return _bytesWrote; } 
    void bytesWrote(long long w) throw() { _bytesWrote = w ; }
    void addBytesWrote(long long w) throw() { _bytesWrote += w ; }
private:
    OutputStream(OutputStream const& rhs);
    OutputStream& operator=(OutputStream const& rhs);
private:
    long long _bytesWrote; 
};

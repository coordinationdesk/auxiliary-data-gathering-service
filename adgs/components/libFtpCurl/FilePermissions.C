...
FilePermissions::FilePermissions(FilePermissions const& r) :
    _type(), 
    _user(),
    _group(),
    _other(),
...
FilePermissions& FilePermissions::operator=(FilePermissions const& rhs) {
    if (this != &rhs) {
        _type = rhs._type;
        memcpy(_user, rhs._user, sizeof(_user)) ;
        memcpy(_group, rhs._group, sizeof(_group)) ;
        memcpy(_other, rhs._other, sizeof(_other)) ;
...
FilePermissions::~FilePermissions() throw(){
    // empty
}

const string &FilePermissions::str() const throw()  // PRQA S 4120
{
    return _permissions;
}

bool FilePermissions::userCanRead()    const throw() { return _user[read]; } // PRQA S 4120 12
bool FilePermissions::userCanWrite()   const throw() { return _user[write]; }
bool FilePermissions::userCanExecute() const throw() { return _user[execute]; }
bool FilePermissions::groupCanRead()    const throw() { return _group[read]; }
bool FilePermissions::groupCanWrite()   const throw() { return _group[write]; }
bool FilePermissions::groupCanExecute() const throw() { return _group[execute]; }
bool FilePermissions::otherCanRead()    const throw() { return _other[read]; }
bool FilePermissions::otherCanWrite()   const throw() { return _other[write]; }
bool FilePermissions::otherCanExecute() const throw() { return _other[execute]; }
bool FilePermissions::isFile() const  throw() { return _type == file; }
bool FilePermissions::isDirectory() const throw() { return _type == directory; }
bool FilePermissions::isLink() const throw() { return _type == symbolicLink; }

bool FilePermissions::isDevice() const throw() { 
    return (_type == characterDevice) || (_type == blockDevice) ; 
}

FilePermissions::Type FilePermissions::getType() const throw() { return _type; } // PRQA S 4120

const string &FilePermissions::getPermissions() const throw() // PRQA S 4120
{
    return _permissions;
}

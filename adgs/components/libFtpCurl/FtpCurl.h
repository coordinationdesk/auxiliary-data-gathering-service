...
public:
    ...
    // PRQA L:L1
    typedef void (*NextLineCallback)(const std::string &line, void* data);
    typedef bool (*FileFilter)(std::string const& filename);
...
public:
    ...
    virtual ~FtpCurl() throw () ;
    
    struct FtpCurlPermissions {
        FtpCurlPermissions() :
            permissions(0),
            userId(-1), // If -1 is used in chown, the owner is not changed
            groupId(-1) // If -1 is used in chown, the group is not changed
        { }
	...
	public:
		PropFindStatItem();
		virtual ~PropFindStatItem() ACS_NOEXCEPT {}                 // PRQA S 2131
		PropFindStatItem (PropFindStatItem const &) ;
		PropFindStatItem &operator=(PropFindStatItem const &) ;
...
public:
   ...
   void putAsTar(const File::CompressionMode& cm, const bool encapsulate_dir, const mode_t& permissions, StopController const& stopController = StopController());
     ...
     * No file filter.
     */
    static bool noFileFilter(std::string const& filename) {
        return true;
    }
     ...
     */
    const std::string &getRempath() const ACS_NOEXCEPT ;
    /**
     * Retrieves the full path and file name (remote resource).
     */
    const std::string &getFullFilename() const ACS_NOEXCEPT ;
    /**
     ...
     */
    const std::string &getLocalPath() const ACS_NOEXCEPT ;
     ...
     */
    const std::string &getLocalFile() const ACS_NOEXCEPT ;
    ...
    void setListFileNamesOnly(bool) ACS_NOEXCEPT;

    /*! \brief A get method to know if only file names from list. (default is false)*/
    bool getListFileNamesOnly() const ACS_NOEXCEPT ;

    /*! \brief A set method for secure Ftp.  */
    void setSecureFtp(bool);

    /*! \brief A get method for secure Ftp.  */
    bool getSecureFtp() const;
    ...
    bool getFtps() const ACS_NOEXCEPT ;

    /*! \brief A get method for ftpes  */
    bool getFtpes() const ACS_NOEXCEPT;

    /*! \brief A get method for ftp  */
    bool getFtp() const ACS_NOEXCEPT;

    const curl::RemoteSite &getSite() const ACS_NOEXCEPT ;
...
private:
	 ...
	 **/
	static void handleCurlErrorBehaviour(CurlErrorBehaviourSingleton::CurlErrorBehaviour beh, const std::string& msg, const std::string& file, const std::string& func, int line);
    ...
    void throwConditionedError(CURLcode res, const std::string &url, char const* function, int line);
     ...
     */
	int spawn(const std::string & exeName, const WaitPid::ProcArgs &, int timeout, bool & goodExit, std::string & spawnArgs);
	...
	std::string buildHttpPropFindBody( const std::vector<std::string> & properties ) ;
	...
	std::vector<std::string> turnRequestsIntoProperties(const PropFindRequests &);
...
private:
    ...
    bool mustOverrideFTPMode (std::string const& thehost, bool& isPassive );
    ...
    bool mustVerifyHostCertificate (const std::string& thehost, std::map<std::string, std::string>& parameters_M);
...
private:
     ...
     */
    bool _listFileNamesOnly;

    NextLineCallback _nextLineCallback;
    ...
    curl::CurlTransaction* _transaction;

    boost::shared_ptr<curl::FtpProgressData> _handleProgressData;
    boost::shared_ptr<curl::HttpProgressData> _handleHttpProgressData;
    std::vector<curl::CirculationStatistics> _statistics;
    static std::map<std::string, acs::curl::RemoteSite> _sites;
    static std::vector<std::string> _inDiscover;
    static Condition _discoverCondition;
    
    // FTP mode override configuration container
    static std::map<std::string, bool> _passiveFtpSiteOverrides;
    ...
    static std::map<std::string, std::map<std::string, std::string> > _verifyHostCertificate ;

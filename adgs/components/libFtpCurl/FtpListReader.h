...
public:
...
#if MARCO
    /**
     * Custom ctor: the mask parameter is used to change the "ls -l" output format.
     * @param mask the custom mask.
     */
    explicit FtpListReader(FtpListItem::Tokens mask);
#endif
    /**
     * Empty dtor.
     */
     ...
     */
    static void createLocalFtpListItems(std::string const& sourcePath , std::vector<FtpListItem> & sourceFiles, bool filterByDate, const time_t& lastScan, StopController const& stopController = StopController(), 
...
private:
    FtpListReader() ;
    FtpListReader(const FtpListReader &) ;
    FtpListReader & operator= (const FtpListReader &) ;

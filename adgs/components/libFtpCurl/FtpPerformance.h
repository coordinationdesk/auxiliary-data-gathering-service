...
public:
    FtpPerformance();
    explicit FtpPerformance(curl::RemoteSite& site);
    ~FtpPerformance() throw ();

    void start();
    void stop();
    void progress(double transf, double total);
    int percentage() const throw() ;
    std::string dump() const ;

    void setRemoteSite(const curl::RemoteSite& site) ;
    const curl::RemoteSite &getRemoteSite() const throw() ;
    double getTotalBytes() const throw() ;
    void setTotalBytes(double) throw() ;
    double getTransfBytes() const throw() ;
    void setTransfBytes(double) throw() ;
    double getSpeedKbSec() const throw() ;
    void setSpeedKbSec(double speed) throw() ;
    double getRemainingSec() const throw() ;
    void setRemainingSec(double sec) throw() ;
    const std::string &getOperation() const throw() ;
    void setOperation(std::string const &operation) ;

private:
    FtpPerformance(const FtpPerformance &rhs);
    FtpPerformance& operator=(FtpPerformance const & rhs);

private:
    double _totalBytes;
    double _transfBytes;
    double _speedKbSec ; // KB/sec
    double _remainingSec ; // sec
    acs::Timer _timer;
...
_ACS_END_NAMESPACE // curl namespace 
    ...
    typedef void (*ProgressCallback)(curl::FtpPerformance const& p);

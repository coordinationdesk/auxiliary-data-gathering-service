...
FtpProgressData::FtpProgressData(StopController const& sc, const BandPriority &prio) :
    ...
    band_priority_(prio),
    memory_(),
    windowLenght_(500000000), // 0.5 s // PRQA S 4400 2
    del_(),
    lastRun_(),
    isRunning_(false),
    perfo_monitor_(),
...
FtpProgressData::~FtpProgressData() throw () { }
...
public:
    ...
    remove_older(const remove_older &r) : 
        now_(r.now_), 
        gap_(r.gap_) 
    {}
...
private:
   remove_older() ; // no    t implemented
   remove_older& operator= (const remove_older&) ; // not implemented
...
FtpProgressData::Time_ns FtpProgressData::origin() throw()
{
...
FtpProgressData::Time_ns FtpProgressData::now() throw()
{
    struct timespec now ; // PRQA S 4102
    ::clock_gettime (CLOCK_REALTIME, &now);
...
void FtpProgressData::computeAndDelay(double partial) // PRQA S 4020
{
    ...
    Time_ns _now = now() ; 
    ++counter_ ;
    if(not isRunning_)// if algo is not running fills memory_ with offset time 
    {
        ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB,"Not Running" );
        memory_.push_front( Event (_now, 0,  0, counter_));
        isRunning_ = true;
        return ;
    }
        ...
        if (isOlder(_now, memory_.front().current_, 30000000)) { // 30ms // PRQA S 4400
            ...
            */
            ...
            memory_.push_front( Event (_now, prev.del_partial_sum_ + machine,  partial, counter_));
        }
    }

    // keeps at least 2 elements
    EventContainer::iterator it_b = memory_.begin() ; advance(it_b,2) ; // PRQA S 4400
    EventContainer::iterator it = find_if(it_b, memory_.end(), remove_older(_now, windowLenght_)) ;
    ...
    if( isOlder(_now , lastRun_, 500000000  )) // 0.5 s // PRQA S 4400
    {
        ...
        if (&last != &first) {
            ...
            BandPriority::Speed_bps avgSpeed = 8E9 * (last.bytes_ - first.bytes_ )/tot_ns ; // bps //PRQA S 3011 

            double ratio = avgSpeed/double(givenBand) ; // PRQA S 3011, 3081, 3084
            ...
            if((ratio > 1.02 )|| (ratio < 0.98)) // PRQA S 4400
            {
                ...
                del_ = (ratio*tot_ns - tot_del)/deltac ; //PRQA S 3011
                // rearrange sums
...
int FtpProgressData::progress_callback(void* ptrtodata, // PRQA S 4020
                                       double downloadTotal, 
                                       double downloadNow, 
                                       double uploadTotal,
                                       double uploadNow) 
{
    FtpProgressData* progressData = reinterpret_cast<FtpProgressData *>(ptrtodata); // PRQA S 3081
    
    double partial = downloadNow + uploadNow ; // any - only one is valorized
    if (0 != progressData->perfo_monitor_) {
    ...
    if ( (not progressData->isEnabled()) || ( 0 == progressData->getBandPriority() )) {
        return 0 ;
    }

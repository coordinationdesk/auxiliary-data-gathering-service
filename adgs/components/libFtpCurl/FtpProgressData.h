...
public:
    ...
    ~FtpProgressData() throw () ;


    void setPerfMonitor(FtpPerformance *v) throw() { perfo_monitor_ = v ;  }
    ...
    typedef long long Interval_ns ;

    struct Event { //PRQA S 2173, 2175, 2185, 2614, 2617
        Event(Time_ns c, Interval_ns p , double b, long long i) :current_(c), del_partial_sum_(p), bytes_(b), counter_(i) {}
        ~Event()throw(){}   
        Time_ns      current_ ; // the time when the event occurred                     //PRQA S 2100
        Interval_ns  del_partial_sum_ ; // the partial sum of delay up to the Event     //PRQA S 2100
        double       bytes_ ; // the byte transferred in this event                     //PRQA S 2100
        long long    counter_ ; // progressive id of the event                          //PRQA S 2100
    } ;
...
private:
    ...
    static Time_ns origin() throw() ;
    static Time_ns now() throw() ;
private:
    // declared but not implemented
    FtpProgressData( ) ;
    FtpProgressData(const FtpProgressData & ) ;
    FtpProgressData & operator= (const FtpProgressData & ) ;
private:

    /**
     * Reference to a Worker object.
     *
     * The default progress callback checks the current state of this referenced object, if a stop request
    ...
    Interval_ns windowLenght_;   //interval in which average speed is calculated 
    Interval_ns del_;       //Applied delay
    Time_ns lastRun_;       //keeps track of last delay() call
    bool isRunning_;           //keeps track whether the data-transfert is running or not

    FtpPerformance *perfo_monitor_ ;
    long long counter_ ;
private:

...
RemoteDirVisitor::RemoteDirVisitor() :
    _fileCallback(),
    _dirCallback(),
    _visitor()
{
...
RemoteDirVisitor::~RemoteDirVisitor() throw(){
    // empty.
}

RemoteDirVisitor::RemoteDirVisitor(VisitFileCallback fileCallback, VisitDirCallback dirCallback, void* visitor) :
...
void RemoteDirVisitor::setVisitor(VisitFileCallback fileCallback, VisitDirCallback dirCallback, void* visitor) {
    _fileCallback = fileCallback;
    _dirCallback = dirCallback;
    _visitor = visitor;
}

void RemoteDirVisitor::nextLineCallback(const std::string &line, void* data) {
    ...
    try {
        while (!subDirs.empty()) {
            ...
            for (size_t counter = 0; counter < l_size; ++counter) {
                ...
                string relativeRemoteName = remotePath.substr(remoteDirectory.length(), remotePath.npos - remoteDirectory.length()); // PRQA S 3084

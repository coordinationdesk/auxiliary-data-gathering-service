...
RemoteSite::RemoteSite(const std::string &protocol, TempRenameStrategy strategy, const std::string& ciphers) :
    _host(), 
    _user(), 
    _password(), 
    _port(-1), 
...
RemoteSite::RemoteSite(const std::string & protocol, 
                       ...
                       const std::string & password, TempRenameStrategy strategy ,  const std::string& ciphers) :
    _host(host), 
    _user(user), 
    _password(password), 
    _port(-1), 
...
void RemoteSite::initRenameStrategy() {
    if (_tempStrategy == unknownStrategy) {
        _tempStrategy = acs::curl::temp::TemporaryFileService::makeStrategy(conf::CurlConfigurerSingleton::instance()->getTempNameStrategy());
        if (_tempStrategy == unknownStrategy) {
            _tempStrategy = acs::curl::temp::TemporaryFileService::makeDefaultStrategy();
        }
		_tempnamevalue = "";
    }
    _tempService = TemporaryFileService(_tempStrategy, _tempnamevalue);
}

RemoteSite::RemoteSite(RemoteSite const& other) :
    _host(other._host), 
    _user(other._user), 
    ...
    _verifyCertParams(other._verifyCertParams), 
    _isDir(other._isDir), 
    _isPasswordEncrypted(other._isPasswordEncrypted), 
    _customHeaders(other._customHeaders),
    _tempService(other._tempService), 
    _tempStrategy(other._tempStrategy), 
    _protocol(other._protocol), 
    _tempnamevalue(other._tempnamevalue),
    _ciphers(other._ciphers)
{
}

RemoteSite& RemoteSite::operator=(RemoteSite const& other) {
    if (this != &other) {
        _host = other._host;
        _user = other._user;
        _password = other._password;
...
string RemoteSite::str() const {
	...
	for (map<string,string>::const_iterator it=_verifyCertParams.begin(); it != _verifyCertParams.end(); it++)
...
string RemoteSite::getPassword() const  // PRQA S 4020
{
...
const string &RemoteSite::getHost() const ACS_NOEXCEPT // PRQA S 4120
{
    return _host;
}

bool RemoteSite::getIsMedia() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, media);
}

bool RemoteSite::getIsSecure() const 
{
    return StringUtils::equalsNoCase(_protocol, sftp);
}

const string & RemoteSite::getUser() const ACS_NOEXCEPT // PRQA S 4120
{
    return _user;
}

void RemoteSite::setHost(const string & host)  // PRQA S 4121
{
    _host = host;
}

void RemoteSite::setIsSecure(bool _isSecure) ACS_NOEXCEPT 
{
    if (_isSecure) {
        _protocol = sftp;
    }
...
void RemoteSite::setPassword(const string &password)  // PRQA S 4121
{
    _password = password;
...
void RemoteSite::setUser(const string & user)   // PRQA S 4121
{
    _user = user;
}

bool RemoteSite::getIsPassive() const ACS_NOEXCEPT   // PRQA S 4120
{
    return _isPassive;
}

void RemoteSite::setIsPassive(bool isPassive) ACS_NOEXCEPT  // PRQA S 4121
{
    _isPassive = isPassive;
}

bool RemoteSite::getVerifyCertificate() const ACS_NOEXCEPT   // PRQA S 4120
{
	string par_value ;
	const bool to_verify = \
		( (true == getVerifyCertificateParameter(Curl::Confkey_verifycert_verifyhost, par_value)) \
		&& (Curl::Confkey_verifycert_verifyhost == par_value) ) ;
		
    return to_verify;
}

void RemoteSite::setVerifyCertificateParameters(const std::map<std::string, std::string>& m) ACS_NOEXCEPT  // PRQA S 4121
{
    _verifyCertParams = m;
}

std::map<std::string, std::string>  RemoteSite::getVerifyCertificateParameters() const ACS_NOEXCEPT   // PRQA S 4120
{
	// Getter for the verify certificate parameters (the whole map of parameters)
    return _verifyCertParams;
}

bool RemoteSite::getVerifyCertificateParameter(const std::string& par_name, std::string& par_value) const ACS_NOEXCEPT   // PRQA S 4120
{
...
bool RemoteSite::getIsFtpes() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, ftpes) ;
}

bool RemoteSite::getIsFtps() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, ftps) ;
}

bool RemoteSite::getIsFtp() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, ftp);
}

bool RemoteSite::getIsAnyFtp() const ACS_NOEXCEPT 
{
    return getIsFtp() || getIsFtps() || getIsFtpes() ;
}

bool RemoteSite::getIsFileSystem() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, fileSystem) || StringUtils::equalsNoCase(_protocol, file);
}

bool RemoteSite::getIsHttp() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, http) || StringUtils::equalsNoCase(_protocol, https);
}

bool RemoteSite::getIsSecureHttp() const ACS_NOEXCEPT
{
    return StringUtils::equalsNoCase(_protocol, https);
}

void RemoteSite::setIsFtpes() ACS_NOEXCEPT 
{
    _protocol = ftpes;
}


void RemoteSite::setIsFtps() ACS_NOEXCEPT 
{
    _protocol = ftps;
}

void RemoteSite::setIsFtp() ACS_NOEXCEPT 
{
    _protocol = ftp;
}

void RemoteSite::setPort(int port) ACS_NOEXCEPT // PRQA S 4121
{
    _port = port;
}

int RemoteSite::getPort() const ACS_NOEXCEPT // PRQA S 4120
{
    return _port;
}
...
const string &RemoteSite::getPath() const  ACS_NOEXCEPT // PRQA S 4120
{
    return _path;
}

void RemoteSite::setIsDir(bool isDir) ACS_NOEXCEPT // PRQA S 4121
{
    _isDir = isDir;
}

bool RemoteSite::getIsDir() const ACS_NOEXCEPT // PRQA S 4120
{
    return _isDir;
}
...
bool RemoteSite::isPasswordEncrypted() const ACS_NOEXCEPT // PRQA S 4120
{
    return _isPasswordEncrypted;
}

void RemoteSite::setIsPasswordEncrypted(bool isPasswordEncrypted) // PRQA S 4121
{
...
const string &RemoteSite::getEncryptedPassword() const ACS_NOEXCEPT // PRQA S 4120 
{
    return _password;
}

const string &RemoteSite::getTempnamevalue() const ACS_NOEXCEPT // PRQA S 4120
{
	return _tempnamevalue;
}

TempRenameStrategy RemoteSite::getRenameStrategy() const ACS_NOEXCEPT // PRQA S 4120
{
    return _tempStrategy;
}

void RemoteSite::setRenameStrategy(TempRenameStrategy s, string const & tempnamevalue) {
    _tempStrategy = s;
	_tempnamevalue = tempnamevalue;
    initRenameStrategy();
}
void RemoteSite::setCiphers(const std::string &c) //PRQA S 4121
{
    _ciphers = c;
}
const std::string & RemoteSite::getCipher()const ACS_NOEXCEPT //PRQA S 4120
{
    return _ciphers;
}
...
void RemoteSite::setProtocol(string const& protocol) {
    ...
    ACS_COND_THROW( // PRQA S 3081
        not ( StringUtils::equals(lp, fileSystem) || 
...
const string &RemoteSite::getProtocol() const ACS_NOEXCEPT // PRQA S 4120
{
    return _protocol;
}

bool RemoteSite::isFtpSetProtocol() const ACS_NOEXCEPT
{
	return getIsAnyFtp() || getIsSecure();
}
...
acs::exostream& operator<<(acs::exostream& stream, const RemoteSite &obj)  // PRQA S 4222
{
    stream << obj.str(); return stream ; // PRQA S 4028
}

bool RemoteSite::protocolSupportsActivePassive(const std::string &protocol) ACS_NOEXCEPT 
{
    const string lp = StringUtils::lowercase(protocol) ;
...
bool RemoteSite::protocolSupportsDirectory(const std::string &protocol) ACS_NOEXCEPT 
{
    const string lp = StringUtils::lowercase(protocol) ;
    // http & https do not support directories
    return not StringUtils::equals(lp, http) && not StringUtils::equals(lp, https) ;           
 
}

bool RemoteSite::protocolSupportsRemove(const std::string &protocol) ACS_NOEXCEPT
{
    const string lp = StringUtils::lowercase(protocol) ;
...
bool RemoteSite::protocolSupportsSource(const std::string &protocol) ACS_NOEXCEPT
{
    const string lp = StringUtils::lowercase(protocol) ;
...
bool RemoteSite::protocolSupportsDestination(const std::string &protocol) ACS_NOEXCEPT
{
    const string lp = StringUtils::lowercase(protocol) ;
    // http & https do not support erasure
    return not StringUtils::equals(lp, http) && not StringUtils::equals(lp, https) ;           
...
Proxy& Proxy::operator=(const Proxy &r)
{
    if (this != &r) {
        host_= r.host_ ;
        protocol_= r.protocol_ ;
...
bool operator==(const Proxy &a, const Proxy &b) ACS_NOEXCEPT // PRQA S 4020
{
    if (a.host() == b.host()) {
        if (a.host().empty()) { return true ; } // empty host on both: no_proxy
...
{
    if (obj == curl::no_proxy()) { os << "[NO-PROXY]" ; }
    else {
        os << "[Host: " << obj.host() << " Protocol: " << obj.protocol() << " User: " << obj.user() << " Password: " << obj.password() <<  " Port: " << obj.port() <<  ']' ; 
    }
    
    return os ; // PRQA S 4028
}

acs::exostream& operator<<(acs::exostream& os, const curl::Proxy &obj)  // PRQA S 4222
{
    if (obj == curl::no_proxy()) { os << "[NO-PROXY]" ; }
    else { 
        os << "[Host: " << obj.host() << " Protocol: " << obj.protocol() << " User: " << obj.user() << " Password: " << obj.password() <<  " Port: " << obj.port() <<  ']' ; 

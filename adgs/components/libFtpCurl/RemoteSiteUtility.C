								...
								StopController const& stopController) 
{
    ...
    if (not site.supportsDirectory()) { // the protocols does not support directories
        return false ; 
    }
...
string RemoteSiteUtility::encrypt(string const& password) 
{
    try {
		...
		unsigned char encrypted_password[ encrypted_password_size ] ; memset( encrypted_password, 0x00, sizeof(encrypted_password) ) ; 
		...
		ACS_COND_THROW ( (password.length() >= encrypted_password_size), CryptException ("Internal error, input buffer larger than expected") ) ;
		...
		ACS_COND_THROW ( (false == success), CryptException ("Error while encrypting password *******") ) ;
		ACS_COND_THROW ( (encrypted_password_size > sizeof(encrypted_password) ), CryptException ("Internal error, output buffer overwflow") ) ;

        vector<char> base64_encrypt ;
        encodeBASE64(encrypted_password, encrypted_password_size, base64_encrypt);
...
string RemoteSiteUtility::decrypt(string const& encrypted_password) 
{
    try {
		...
		unsigned char decrypted_password[ decrypted_password_size ] ; memset( decrypted_password, 0x00, sizeof(decrypted_password) ) ; 
		...
		const bool success ( 
			ACS_SSL::ACS_RSA_decode_password( reinterpret_cast<unsigned char const*>(&base64_encrypt[0]), base64_encrypt.size(), pubkey, decrypted_password, decrypted_password_size) \
		) ; 

		ACS_COND_THROW ( (false == success), CryptException ("Error while decrypting password *******") ) ;
		ACS_COND_THROW ( (decrypted_password_size > sizeof(decrypted_password) ), CryptException ("Internal error, output buffer overwflow") ) ;

...
StreamCurl::StreamCurl(const StopController& stopController, CURLcode& res, const BandPriority &bp) :
    Curl(), 
    _streamGetCallback(streamGetCallback), 
    _streamPutCallback(streamPutCallback), 
    _bytesWrote(0), 
    _bytesRead(0), 
    _inStream(0), 
    _outStream(0), 
    ...
    _res(res),
    band_prio_(bp),
	_removeTargetBeforeTransfer(false) 
{
...
void StreamCurl::printProgress(bool isGet, long long bytes, long long totalBytes) {
    ...
    if (totalBytes % 1000 == 0) { // PRQA S 4400
        ...
        progressStream << humanSize(bytes) << " total: "
                << humanSize(totalBytes);
        if (isGet) {
            ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Stream Get :" << progressStream.str());
        } 
        else {
            ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Stream Put :" << progressStream.str());
        }
    }
    ACS_CLASS_END_DEBUG;
}

size_t StreamCurl::streamGetCallback(void *buffer, size_t size, size_t nmemb, void *stream)  // PRQA S 4020
{
    StreamCurl* thisObj = reinterpret_cast<StreamCurl*>(stream); // PRQA S 3081
    if (thisObj->_res != CURLE_OK) {
        return 0;
    }
    acs::io::OutputStream* outStream = thisObj->_outStream;

    PipedStream* pipedStream = reinterpret_cast<PipedStream*>(thisObj->_outStream); // PRQA S 3030, 3081
    if (pipedStream) {
    ...
    ssize_t length = ssize_t ( size * nmemb ) ; // PRQA S 3081, 3084
    ssize_t w = outStream->write(reinterpret_cast<char*>( buffer), length); // PRQA S 3000, 3081
    while ((w < length) && (thisObj->_res == CURLE_OK)) {
        ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Get waiting....");
        Timer::delay(10); // PRQA S 4400
        w += outStream->write(reinterpret_cast<char*>( buffer ) + w, length - w); // PRQA S 3000, 3081
    }
    if (thisObj->_res != CURLE_OK) {
...
size_t StreamCurl::streamPutCallback(void *buffer, size_t size, size_t nmemb, void *stream) // PRQA S 4020 
{
    ...
    while (!inStream->closed() && (thisObj->_res == CURLE_OK)) {
        read = inStream->read(reinterpret_cast<char*>( buffer ), size * nmemb); // PRQA S 3081, 3084
        ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "read " << read);
        if (0 != read) { // if read anything go on.
            break;
        }
        //give it a chance.
        if (!inStream->closed()) {
...
void StreamCurl::setOutStream(acs::io::OutputStream* outStream) throw()  // PRQA S 4121
{
    _outStream = outStream;
}

void StreamCurl::setInStream(acs::io::InputStream* inStream) throw()  // PRQA S 4121
{
    _inStream = inStream;
}
...
StreamCurlThread::StreamCurlThread(RemoteSite const& site, 
    ...
    Thread("StreamCurlThread-" + site.getUser() + "@" + site.getHost()), 
    _remoteFilename(), 
    _finalRemoteFilename(),
    _operationRequested(), 
    _result()
...
StreamCurlThread::~StreamCurlThread() throw () {
    //empty.
}
...
CURLcode StreamCurlThread::completeAndGetResult() {
    ...
    this->runException(theException);

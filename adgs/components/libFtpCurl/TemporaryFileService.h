...
enum TempRenameStrategy {
    noTemp = 0, prefix, postfix, tempDir, tstamp, unknownStrategy
};

class TemporaryFileService { // PRQA S 2109
...
// PRQA L:L1
    ...
    explicit TemporaryFileService(const std::string &strategyName = PREFIX_NAME_STRATEGY, const std::string & = "");
    ~TemporaryFileService() throw();
...
#else
    std::auto_ptr<AbstractTempStrategy> handle_;
#endif

    TempRenameStrategy strategy_;
...
acs::exostream &operator<< (acs::exostream &, TempRenameStrategy) ; // PRQA S 2072

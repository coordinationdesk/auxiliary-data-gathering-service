...
/*
	...
	Revision 1.4  2013/03/28 14:30:53  marpas
	coding best practices applied
...
#ifndef _TgzStrategy_H_
#define _TgzStrategy_H_ 
 ...
 */
class TgzStrategy : public SMPackingStrategy
{
public:
	/*! class TgzStrategyException declaration */
	exDECLARE_EXCEPTION(TgzStrategyException,exException) ; // PRQA S 2131, 2502

	
	explicit TgzStrategy(dbPersistent &obj, dbConnectionPool &) ;
	virtual ~TgzStrategy() throw() ;
...
private:
	...
	TgzStrategy() ;
	TgzStrategy(const TgzStrategy &) ;
	TgzStrategy &operator=(const TgzStrategy &) ;
	ACS_CLASS_DECLARE_DEBUG_LEVEL(TgzStrategy) ;
} ; 

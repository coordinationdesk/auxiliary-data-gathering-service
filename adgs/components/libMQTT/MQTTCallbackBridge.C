...
// CTOR and DTOR
MQTTCallbackBridge::MQTTCallbackBridge() ACS_NOEXCEPT
{
...
void MQTTCallbackBridge::registerWorker (struct mosquitto* mqtt, MQTTWorker* worker)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_LOW_VERB);
#ifndef DISABLE_MQTT
...
void MQTTCallbackBridge::unregisterWorker (struct mosquitto* mqtt)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_LOW_VERB);
	ThreadSafe::Lock lock (_mutex);
...
void MQTTCallbackBridge::connectCallback (struct mosquitto *mosq, void *obj, int rc) 
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::disconnectCallback (struct mosquitto *mosq, void *obj, int rc)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::publishCallback (struct mosquitto *mosq, void *obj, int mid)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::messageCallback (struct mosquitto *mosq, void *obj, const struct mosquitto_message *msg)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::subscribeCallback (struct mosquitto *mosq, void *obj, int mid, int qos_count, const int * granted_qos)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::unsubscribeCallback (struct mosquitto *mosq, void *obj, int mid)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);
	try
	{
...
void MQTTCallbackBridge::logCallback (struct mosquitto *mosq, void *obj, int level, const char *str)
{
	ACS_CLASS_ANNOUNCE_DEBUG (ACS_MID_VERB);

	ACS_CLASS_WRITE_DEBUG (ACS_LOW_VERB, "Log callback (level " << level <<"): " << string(str));

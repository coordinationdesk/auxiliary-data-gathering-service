...
void Postgres8SQLDialect::setFastConnect() const 
{
	...
	putenv(const_cast<char *>(_minimumConTime_)) ; // PRQA S 3081
}
...
/*
	int defSSWS = dbConnection::getDefaultSingleStepWaitSeconds() ;
	try {
		rset.getValue("DefaultSingleStepSecondsToWait", defSSWS) ;
	}
	catch (rsResourceSet::NotFoundException &) {
		ACS_LOG_WARNING("db.conf missing parameter - setting DefaultSingleStepSecondsToWait to " << defSSWS) ;
	}
	dbConnection::setDefaultSingleStepWaitSeconds(defSSWS) ;
	int defLWS = dbConnection::getDefaultLockWaitSeconds() ;
	try {
		rset.getValue("DefaultSecondsToWait", defSSWS) ;
	}
	catch (rsResourceSet::NotFoundException &) {
		ACS_LOG_WARNING("db.conf missing parameter - setting DefaultSecondsToWait to " << defLWS) ;
	}
	dbConnection::setDefaultLockWaitSeconds(defLWS) ;
*/
}

...
#ifndef _CryosatProcessorTracker_H
#define _CryosatProcessorTracker_H

#include <ProcessorTracker.h>
#include <exException.h>

#ifdef UNIT_TEST
	class testProcessorTrackerCore;
#endif //UNIT_TEST
...
class CryosatProcessorTracker : public ProcessorTracker // PRQA S 2153
{
public:
	explicit CryosatProcessorTracker(const scOrder& wo);
	CryosatProcessorTracker(int orderId, const std::string &rootPath);
	virtual ~CryosatProcessorTracker() throw() ;

private:
    CryosatProcessorTracker() ; // not implemented 
    CryosatProcessorTracker(const CryosatProcessorTracker &) ; // not implemented 
    CryosatProcessorTracker& operator= (const CryosatProcessorTracker &) ; // not implemented 
...
#endif //_CryosatProcessorTracker_H

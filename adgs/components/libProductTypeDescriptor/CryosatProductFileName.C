...
/*
	...
	Revision 2.2  2012/03/12 10:19:18  giucas
	...
	Revision 1.9  2004/12/03 09:50:40  giucas
	Header fixed
	
	Revision 1.8  2004/09/01 10:30:42  giucas
	Added nameWVersionForQuery method
	
	Revision 1.7  2004/08/05 16:03:18  giucas
	Changed to handle a 0-sized list of configured File extensions
	
	Revision 1.6  2004/02/03 14:05:58  marfav
	Added support for Level2 products
	
	Revision 1.5  2004/01/09 10:27:55  marfav
	Added skipNameSizeCheck parameter to checkNameValidity method
	
	Revision 1.4  2003/10/23 08:04:21  giucas
	Added isValidName and isValidRoot methods for L0 and L1 products
	
	Revision 1.3  2003/10/13 15:36:32  giucas
	Added method getFileClassFromRoot
	
	Revision 1.2  2003/10/03 09:28:41  giucas
	Indroduced ProductFileName conf space
	
	Revision 1.1  2003/09/09 13:11:14  giucas
	Created from CryosatL0FileNames class
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(CryosatProductFileName) ;

//Class constructor
CryosatProductFileName::CryosatProductFileName() :
    ProductFileName(),
...
//Class destructor
CryosatProductFileName::~CryosatProductFileName() throw()
{
}
...
// Public methods 
/////////////////////////////////////////////////////////////////////////////////////////
void CryosatProductFileName::getNamesById(const string& prodId, 
										   vector<string>& names)
{
...
void CryosatProductFileName::getNamesByMission(const string& missionId, const string& prodId, vector<string>& names)
{
...
void CryosatProductFileName::getNamesByMission(const string& missionId,
										   const string& prodId, 
									       const string& startTime,
								  	   	   const string& stopTime,
									   	   vector<string>& names)
...
void CryosatProductFileName::getNamesByExtension(const string& ext,const string& prodId,vector<string>& names)
{
...
void CryosatProductFileName::getNamesByExtension(const string& ext,
											 const string& prodId, 
										 	 const string& startTime,
								  	     	 const string& stopTime,
										 	 vector<string>& names)
...
bool CryosatProductFileName::isValidExtension(const string& ext) 
{
	return find(_permittedExtensions.begin(),_permittedExtensions.end(),ext) != _permittedExtensions.end() ;
}


bool CryosatProductFileName::isValidName(const string& fileName)
{
	return checkNameValidity(fileName,_validFileNames);
}

bool CryosatProductFileName::isValidRoot(const string& fileName)
{
	return checkRootValidity(fileName,_validFileNames);
}

bool CryosatProductFileName::isValidL0Name(const string& fileName)
{
	return checkNameValidity(fileName,_validL0FileNames);
}

bool CryosatProductFileName::isValidL0Root(const string& fileName)
{
	return checkRootValidity(fileName,_validL0FileNames);
}

bool CryosatProductFileName::isValidL1Name(const string& fileName)
{
	return checkNameValidity(fileName,_validL1FileNames, true);
}

bool CryosatProductFileName::isValidL1Root(const string& fileName)
{
	return checkRootValidity(fileName,_validL1FileNames);
}

bool CryosatProductFileName::isValidL2Name(const string& fileName)
{
	return checkNameValidity(fileName,_validL2FileNames, true);
}

bool CryosatProductFileName::isValidL2Root(const string& fileName)
{
...
void CryosatProductFileName::getName(const string& root, 
...
void CryosatProductFileName::getName(const string& root, 
								 ...
								 const string& ext,
			 					 string& result)
{
	CryosatProductFileName::getName(root,startDate,stopDate,ext,result);
}
...
string CryosatProductFileName::getProductIDFromRoot(const string& fileName)
{
...
string CryosatProductFileName::getProductIDFromName(const string& fileName)
{
...
string CryosatProductFileName::getVersion(const string& fileName)
{
	if(!isValidName(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name " + fileName)); // PRQA S 3081
...
string CryosatProductFileName::getFileClass(const string& fileName)
{
	if(!isValidName(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name " + fileName)); // PRQA S 3081
...
string CryosatProductFileName::getFileClassFromRoot(const string& fileName)
{
	if(!isValidRoot(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name Root" + fileName)); // PRQA S 3081
...
long double CryosatProductFileName::getStartTime(const string& fileName)
{
	if(!isValidName(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name " + fileName)); // PRQA S 3081
...
long double CryosatProductFileName::getStopTime(const string& fileName)
{
	if(!isValidName(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name " + fileName)); // PRQA S 3081
...
string CryosatProductFileName::nameWVersion(const string& fileName,const string& version)
{
	if(!isValidName(fileName))
	{
		ACS_THROW(ProductFileNameException("Error: invalid Product File Name " + fileName)); // PRQA S 3081
 ...
 */ 
void CryosatProductFileName::init()
{
	//storing MissionId
	_rsConf.getValue("CryosatProductFileName.MissionID", _missionId);
	
	//storing extensions
	unsigned int numberOfExtensions=0;
	_rsConf.getValue("CryosatProductFileName.List_of_File_Ext.count.value", numberOfExtensions);
	for (unsigned int i=0; i < numberOfExtensions; i++)
	{
		...
		string key = "CryosatProductFileName.List_of_File_Ext.File_Ext["+temp+"]";
	...
	_rsConf.getValue("CryosatProductFileName.List_of_File_Classes.count.value", numberOfClasses);
	for (unsigned int i=0; i < numberOfClasses; i++)
	{
		...
		string key = "CryosatProductFileName.List_of_File_Classes.File_Class["+temp+"]";
	...
	_rsConf.getValue("CryosatProductFileName.List_of_Level0_Product_Id.count.value", numberOfFiles);
	string validName="";
	for (unsigned int i=0; i < numberOfFiles; i++)
	{
		...
		num << i;
		string temp(num.str());
		string key = "CryosatProductFileName.List_of_Level0_Product_Id.Id["+temp+"]";
	...
	_rsConf.getValue("CryosatProductFileName.List_of_Level1_Product_Id.count.value", numberOfFiles);
	validName="";
	for (unsigned int i=0; i < numberOfFiles; i++)
	{
		string productID="";
		...
		num << i;
		string temp(num.str());
		string key = "CryosatProductFileName.List_of_Level1_Product_Id.Id["+temp+"]";
		_rsConf.getValue(key,productID);
		//build valid roots	for this productId
        size_t pc_size = _permittedClasses.size() ; 
		for(unsigned int j=0; j<pc_size; ++j)
	...
	_rsConf.getValue("CryosatProductFileName.List_of_Level2_Product_Id.count.value", numberOfFiles);
	validName="";
	for (unsigned int i=0; i < numberOfFiles; i++)
	{
		string productID="";
		...
		num << i;
		string temp(num.str());
		string key = "CryosatProductFileName.List_of_Level2_Product_Id.Id["+temp+"]";
		_rsConf.getValue(key,productID);
		//build valid roots	for this productId
        size_t pc_size = _permittedClasses.size() ; 
		for(unsigned int j=0; j<pc_size; ++j)
	...
	_rsConf.getValue("CryosatProductFileName.FileNameVersion", _fileNameVersion);
	_rsConf.getValue("CryosatProductFileName.FileRootSize", _rootSize);
	_rsConf.getValue("CryosatProductFileName.FileNameSize", _fileSize);
} 
...
bool CryosatProductFileName::checkNameValidity(const string& fileName, vector<string>& validRoots, bool skipNameSizeCheck)
{
	bool result=false;
...
bool CryosatProductFileName::checkRootValidity(const string& fileName, vector<string>& validRoots)
{
	bool result=false;

...
*/
	/**modi by micepi: 
	 * aligned with L0ProductTypeDescriptor.C
	 * removed method definitions not requested for L1 processing
	 */
...
ACS_CLASS_DEFINE_DEBUG_LEVEL(L1ProductTypeDescriptor)

/*
 * Class constructors
 */
L1ProductTypeDescriptor::L1ProductTypeDescriptor() :
 ...
 * Class destructor
 */
L1ProductTypeDescriptor::~L1ProductTypeDescriptor() throw()
{
    // PRQA S 4631 L1
	//free pointers
	map<string, L1ProductInfo*>::iterator it = _productInfo.begin();
...
void L1ProductTypeDescriptor::getNamesById(const string& prodId,
										   vector<string>& names)
{
	...
	ACS_COND_THROW( _productInfo.find(prodId) == _productInfo.end() , L1ProductTypeDescriptorException("Incorrect ProductId " + prodId) ); // PRQA S 3081
...
void L1ProductTypeDescriptor::getNamesByMission(const string& missionId,
												vector<string>& names)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
...
void L1ProductTypeDescriptor::getNamesByExtension(const string& ext,
												  vector<string>& names)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
...
/////////////////////////////////////////
//LeveL1 Specific Methods implementation
/////////////////////////////////////////

string L1ProductTypeDescriptor::setSatelliteAcsName() const
{
    return StringUtils::capitalize(_satelliteName) ;
}


void L1ProductTypeDescriptor::setSatellite(const string& satName)
{
	if(_satFlag)
	{
		delete _fileNamesHandler;
	...
	{
		ACS_THROW(L1ProductTypeDescriptorException("Error in instantiating ProductFileName for satellite "+satName));  // PRQA S 3081
	}
	_satFlag = true;
}

string L1ProductTypeDescriptor::getSensorName(const string& prodId)
{
	//check if prodId is a valid Product Id
	map<string, L1ProductInfo*>::iterator it = _productInfo.find(prodId);
	if( it == _productInfo.end() ){
		ACS_THROW(L1ProductTypeDescriptorException("Incorrect ProductId " + prodId)) ; // PRQA S 3081
	}
	return (*it).second->getSensorName();
}

string L1ProductTypeDescriptor::getSensorMode(const string& prodId)
{
	//check if prodId is a valid Product Id
	map<string, L1ProductInfo*>::iterator it = _productInfo.find(prodId);
	if( it == _productInfo.end() ){
 ...
 */
void L1ProductTypeDescriptor::init()

{
	...
	_rsConf.getValue("L1ProductDescriptor.NumberOfProducts", numberOfFiles);
	...
	for (unsigned int i=1; i <= numberOfFiles; i++)
	{
		...
		num << i;
		string temp(num.str());
		string key = "L1ProductDescriptor.Product" + temp + ".Id";
		_rsConf.getValue(key,productID);
		_l1ProductIds.push_back(productID);
		_productInfo[productID] = new L1ProductInfo("L1ProductDescriptor.Product" + temp);
	}

	//storing other general informations
	_rsConf.getValue("L1ProductDescriptor.RefDocName", _refDocName);
	_rsConf.getValue("L1ProductDescriptor.SatelliteName", _satelliteName);
}

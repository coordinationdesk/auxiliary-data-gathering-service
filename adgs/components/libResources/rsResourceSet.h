...
#warning __GLIBC_HAVE_LONG_LONG not defined: long long integral support removed
#endif
...
public:
	...
	{ // valore
        friend bool operator==(const rsResourceSet::rsValue &, const rsResourceSet::rsValue &) throw() ; // PRQA S 2107

	public:
		//! Ctor
		explicit rsValue(const std::string &) ;
		//! Copy ctor
		rsValue(const rsValue &) ;
		//! Dtor
		~rsValue() throw() ;
		//! Assignement operator
		rsValue &operator=(const rsValue &) ;
...
public:
	...
	rsResourceSet() ;									// costruttore

	rsResourceSet(const rsResourceSet &) ;				// costruttore di copia

	~rsResourceSet() throw() ;							// distruttore
	...
	*/
    operator Mutex &() const throw() { return _mutex ; } // PRQA S 2181

	bool isEqual(const rsResourceSet &) const throw() ;
	...
	size_t getArraySize(const std::string &, bool *isSingle = 0 ) const ;
	...
	size_t getArray(const std::string &res, std::vector<T> &v, size_t n=0, size_t offset = 0) const // PRQA S 4020
    {
        ...
        if (not isSingle) { // a standard Array or a Single value with "count" attribute coming from XML
	...
	size_t keys(std::string * = 0, std::string *v=0, std::vector<std::string> *c=0) const ;
...
private:
	...
	std::map<pthread_t,std::stack<std::string> > _nSpacesMap ; // map of namespace's stack

	bool _writeCmts ;				// flag di scrittura dei commenti
	MergeMode _mergeMode ;			// merge mode 
	mutable std::map<pthread_t,std::string> _nameSpacesMap ; // current namespace
	char _wSeparator ;				// separatore tra chiave e valore usato in scrittura

	std::map<std::string,rsValue *>	_resources ;
...
bool operator==(const rsResourceSet::rsValue &,   const rsResourceSet::rsValue &)   throw() ;

// PRQA S 2134 L1
inline
bool operator!=(const rsResourceSet::rsValue &v1, const rsResourceSet::rsValue &v2) throw() { return not operator==(v1, v2) ; }

inline
bool operator==(const rsResourceSet &r1, const rsResourceSet &r2) throw() { return r1.isEqual(r2) ; }

inline
bool operator!=(const rsResourceSet &r1, const rsResourceSet &r2) throw() { return not r1.isEqual(r2) ; }

...
ACS_CLASS_DEFINE_DEBUG_LEVEL(OnDistributionSelector)

OnDistributionSelector::OnDistributionSelector(const InvPolicy& policy) :
...
OnDistributionSelector::~OnDistributionSelector() ACS_NOEXCEPT {
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB); // PRQA S 4631
}

void OnDistributionSelector::selectInit(dbConnection &conn) {
	...
	try {
		...
		_sqlSelections.push_back( this->getTimeThresholdQuery("exported", hoursThreshold.str(), conn) );

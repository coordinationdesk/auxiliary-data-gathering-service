...
public:
	...
	virtual ~SMInterface() ACS_NOEXCEPT ;

    std::string getInvSmTableName() ACS_NOEXCEPT ; 
	/**
	 * \brief ask the storage manager to download a file
    ...
    /**
	 * \brief ask the storage manager to upload a file 
    ...
    /**
	 * \brief ask the storage manager to erase a file 
	 * \param sminvid is the primary key into the t_inventorysm and into the t_storagemanager.invrelationtable
     * \param opif the adapter to deal with any dbPersistent

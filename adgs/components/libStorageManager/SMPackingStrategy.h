...
public:
	...
	virtual ~SMPackingStrategy() throw() ;
...
protected:
	dbConnectionPool &getPool() throw() ;
    	
private:
	// declared but not implemented to prevent their use
	SMPackingStrategy() ;
	SMPackingStrategy(const SMPackingStrategy &) ;
	SMPackingStrategy &operator=(const SMPackingStrategy &) ;

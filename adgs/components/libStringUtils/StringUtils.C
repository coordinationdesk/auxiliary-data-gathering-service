...
int StringUtils::compareNoCase(const std::string& s1, const std::string& s2) // PRQA S 4020
{
    ...
    return (s2.size() == s1.size()) ? 0 : ((s1.size() < s2.size()) ? -1 : 1); // PRQA S 3382, 3383
}
...
struct compare_no_case_char {
    bool operator()( char c1,  char c2) const throw()  { return toupper(c1) == toupper(c2) ; } ;
} ;
...
struct equal_no_case_char { // PRQA S 2171, 2173, 2175, 2185
    explicit equal_no_case_char(char c) throw()  : c_ (c) {}
    bool operator()( char c) const throw()  { return toupper(c) == toupper(c_) ; } ;
private:
    char c_ ;
} ;
...
string::size_type StringUtils::findNoCase(const std::string&str, char c, std::string::size_type pos)  // PRQA S 4020
{
    ...
    string::const_iterator p = std::find_if(str.begin()+pos, str.end(), equal_no_case_char(c)) ; // PRQA S 3000, 3081
    if (p == str.end()) { 
        return string::npos ;
    }
    else {
    ...
    while (path.length() > 0) {
        ...
        if (!((v_out.size() >= 1) && s == ".")) {
            ...
            if (!s.empty()) {
                ...
                last = ((last.empty() && not root) ? s : (last + delim + s)); // PRQA S 3380, 3385
...
// PRQA S 2173, 2175 L2
template<class T> struct uf_toupper {
    uf_toupper() {}
    T operator()(T x) { return toupper(x); }
};
// PRQA L:L2

string &StringUtils::uppercase(std::string &s1) 
{
    std::transform(s1.begin(), s1.end(), s1.begin(), uf_toupper<char>());
...
// PRQA S 2173, 2175 L3
template<class T> struct uf_tolower {
    uf_tolower() {}
...
{
    std::transform(s1.begin(), s1.end(), s1.begin(), uf_tolower<char>());
    return s1; // PRQA S 4028
}
...
std::string StringUtils::pathJoin(const std::string& path1, const std::string& path2, char s) // PRQA S 4020
{
    ...
    return trimEnd(path1,'/') + '/' + trimBegin(path2,'/') ;
}
...
std::string StringUtils::pathAssemble(const std::vector<std::string>&v, char s) {
    ...
    for (size_t i = 0; i < v.size(); i++) {
        if (0 == i) {
            ret = ((not v[0].empty()) ? v[0] : slash); // PRQA S 3380
        }
        else if ((v[i] != "") && (v[i] != ".")) {
...
{
    unsigned char subnet_address_bytes[16] ; // PRQA S 4403 5
    unsigned char subnet_mask_bytes[16] ;
    ...
    //////////////////////////////////////////
    ACS_COND_THROW(wsubnet.empty(), exIllegalValueException("StringUtils::ipBelongsToSubnet:  Subnet is empty")); // PRQA S 3081 2
    ACS_COND_THROW(wip.empty(), exIllegalValueException("StringUtils::ipBelongsToSubnet:  IP is empty"));
    ...
    if (pos == std::string::npos) {
        ...
        ACS_COND_THROW((pos == std::string::npos), // PRQA S 3081
                exIllegalValueException("StringUtils::ipBelongsToSubnet:  Invalid Subnet format: \"" + wsubnet + "\""));
    }
    ...
    if ((subnet_address.find(".") == std::string::npos) && (subnet_address.find(":") != std::string::npos)) {
        use_ipv6 = true;
    }
    else if ((subnet_address.find(".") != std::string::npos) && (subnet_address.find(":") == std::string::npos)) {
        use_ipv6 = false;
    }
    else {
        ACS_THROW(     // PRQA S 3081
                exIllegalValueException("StringUtils::ipBelongsToSubnet:  Invalid Subnet address: \"" + subnet_address + "\""));
    }
    ...
    if ((use_ipv6 && (wip.find(".") != std::string::npos)) || (!use_ipv6 && (wip.find(":") != std::string::npos))) {
        ACS_THROW( // PRQA S 3081
                exIllegalValueException(
                        "StringUtils::ipBelongsToSubnet:  IP Address \"" + wip
    ...
    if (has_cidr) {
        ...
        ACS_COND_THROW((status != 1), // PRQA S 3081
                exIllegalValueException("StringUtils::ipBelongsToSubnet:  Invalid CIDR: \"" + subnet_mask + "\""));

        ACS_COND_THROW((( not use_ipv6 && (cidr > 32)) || (use_ipv6 && (cidr > 128))), // PRQA S 3081
                exIllegalValueException("StringUtils::ipBelongsToSubnet:  CIDR too large: " + subnet_mask));
        ...
        if (!use_ipv6) {
            int status = sscanf(subnet_mask.c_str(), "%u.%u.%u.%u", &s[0], &s[1], &s[2], &s[3]); // PRQA S 4412
            ACS_COND_THROW(  // PRQA S 3081
                    (status != 4),
                    exIllegalValueException(
                            "StringUtils::ipBelongsToSubnet:  Invalid IPv4 Subnet mask: \"" + subnet_mask + "\""));
            for (size_t i = 0; i < 4; i++) { // PRQA S 4400
                ACS_COND_THROW(  // PRQA S 3081
                        s[i] > 0xff,
                        exIllegalValueException(
                                "StringUtils::ipBelongsToSubnet:  Invalid IPv4 Subnet mask (overflow): \"" + subnet_mask
                                        + "\""));
                subnet_mask_bytes[i] = static_cast<unsigned char>(s[i]); // PRQA S 3081
            }
        } else {
            int status = sscanf(ipv6Normalize(subnet_mask).c_str(), "%hx:%hx:%hx:%hx:%hx:%hx:%hx:%hx", &h[0], &h[1], // PRQA S 4412
                    &h[2], &h[3], &h[4], &h[5], &h[6], &h[7]);
            ACS_COND_THROW( // PRQA S 3081
                    (status != 8),
                    exIllegalValueException(
                            "StringUtils::ipBelongsToSubnet:  Invalid IPv6 Subnet mask: \"" + subnet_mask + "\""));
            for (size_t i = 0; i < 16; i++) { // PRQA S 4400
                subnet_mask_bytes[i] = ((i & 0x1) ? ((h[i / 2]) & 0x00ff) : ((h[i / 2] >> 8) & 0x00ff)); // PRQA S 3000, 3007, 3010, 3054, 3084, 3380, 4400
            }
        }
    }
    ...
    if (!use_ipv6) {
        int status = sscanf(subnet_address.c_str(), "%u.%u.%u.%u", &s[0], &s[1], &s[2], &s[3]); // PRQA S 4412
        ACS_COND_THROW( // PRQA S 3081
                (status != 4),
                exIllegalValueException(
                        "StringUtils::ipBelongsToSubnet:  Invalid IPv4 Subnet address: \"" + subnet_address + "\""));
        for (size_t i = 0; i < 4; i++) { // PRQA S 4400
            ACS_COND_THROW( // PRQA S 3081
                    s[i] > 0xff,
                    exIllegalValueException(
                            "StringUtils::ipBelongsToSubnet:  Invalid IPv4 Subnet address (overflow): \""
                                    + subnet_address + "\""));
            subnet_address_bytes[i] = static_cast<unsigned char>(s[i]); // PRQA S 3081
        }
    } else {
        int status = sscanf(ipv6Normalize(subnet_address).c_str(), "%hx:%hx:%hx:%hx:%hx:%hx:%hx:%hx", &h[0], &h[1], // PRQA S 4412
                &h[2], &h[3], &h[4], &h[5], &h[6], &h[7]);
        ACS_COND_THROW( // PRQA S 3081
                (status != 8),
                exIllegalValueException(
                        "StringUtils::ipBelongsToSubnet:  Invalid IPv6 Subnet address: \"" + subnet_address + "\""));
        for (size_t i = 0; i < 16; i++) { // PRQA S 4400
            subnet_address_bytes[i] = ((i & 0x1) ? ((h[i / 2]) & 0x00ff) : ((h[i / 2] >> 8) & 0x00ff)); // PRQA S 3000, 3007, 3010, 3054, 3084, 3380, 4400
        }
    }
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    if (!use_ipv6) {
        int status = sscanf(wip.c_str(), "%u.%u.%u.%u", &s[0], &s[1], &s[2], &s[3]); // PRQA S 4412
        ACS_COND_THROW((status != 4), // PRQA S 3081
                exIOException("StringUtils::ipBelongsToSubnet:  Invalid IPv4 IP address: \"" + wip + "\""));
        for (size_t i = 0; i < 4; i++) { // PRQA S 4400
            ACS_COND_THROW( // PRQA S 3081
                    s[i] > 0xff,
                    exIllegalValueException(
                            "StringUtils::ipBelongsToSubnet:  Invalid IPv4 IP address (overflow): \"" + wip + "\""));
            ip_bytes[i] = static_cast<unsigned char>(s[i]); // PRQA S 3081
        }
    } else {
        int status = sscanf(ipv6Normalize(wip).c_str(), "%hx:%hx:%hx:%hx:%hx:%hx:%hx:%hx", &h[0], &h[1], &h[2], &h[3], // PRQA S 4412
                &h[4], &h[5], &h[6], &h[7]);
        ACS_COND_THROW((status != 8), // PRQA S 3081
                exIllegalValueException("StringUtils::ipBelongsToSubnet:  Invalid IPv6 Subnet address: \"" + wip + "\""));
        for (size_t i = 0; i < 16; i++) { // PRQA S 4400
            ip_bytes[i] = ((i & 0x1) ? ((h[i / 2]) & 0x00ff) : ((h[i / 2] >> 8) & 0x00ff)); // PRQA S 3000, 3007, 3010, 3054, 3084, 3380, 4400
        }
    }
    ...
    bool mismatch = false;
    size_t n = (use_ipv6 ? 16 : 4); // PRQA S 3000, 3384
    for (size_t i = 0; (!mismatch && (i < n)); i++) {

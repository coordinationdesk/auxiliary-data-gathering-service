...
/*
	...
	after a pthread_join (succesfull) the pthread_is is reset in order to avoid a crash if the thread_id is an hidden pointer to some structures (as happen in some implementations)
...
Thread::Thread(const string &threadName) : 
	...
	_condition(),
	_extCondition(0),
	_svcStatus(SvcNotStarted),
	_fireRegisteredInitialization(true) 
{
...
Thread::~Thread() ACS_NOEXCEPT
{
    try { // PRQA S 4631 L1
	    ...
	    try {
		    ...
		    setRunException(0) ; // delete any runException left
	    }
	    catch (exception &x) {
			ACS_THROW(ThreadException(x,"Thread exception in dtor: logic error")) ;
	    }
    }
    catch(exception &x) {
        ACS_LOG_NOTIFY_EX(x) ;
...
void Thread::start()
{
	...
	ACS_CLASS_BGN_DEBUG(ACS_VLO_VERB) 
		setitimer(ITIMER_PROF, &_itimer, 0);
	ACS_CLASS_END_DEBUG
	...
	if (0 != _threadHd) { // a previous thread ended
		internalJoin() ;
	}
	...
	if (_fireRegisteredInitialization)
	{
		...
		ACS_COND_THROW(!isInit, ThreadException("Initialization singleton not created!")); // PRQA S 3081
	}
	
	
	ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "starting thread") ;

	int sterr = ::pthread_create(&_threadHd,0,svc,this) ;
...
void Thread::exit()
{
	...
	else if (0 != _threadHd) { // a previous thread ended
		internalJoin() ;
	}
    else {
        ; // nothing to do
...
bool Thread::running() const // PRQA S 4020, 4214
{
	if (0 == _threadId) { // not yet started
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "_threadId: " << _threadId) ;
		return false ;
	}

	if (::pthread_self() == _threadId) { // myself ... hence running (cogito ergo sum !)
		return true ;
    }

    // check if the call to thiw method has been done for a cond signal 
    // in this case the _svcStatus pvt member has 'Exiting' value.
    if (_svcStatus == SvcExiting) {
        yield() ;
        if (0 != ::pthread_kill(_threadId,0)) { // no longer running
            _svcStatus = SvcNotStarted ;
        }
    ...
    if (0 == err) { // father: try to send a fake signal to see if the thread exists
	    return true ;
    }
	if (err == ESRCH) { // no longer exists
...
int Thread::getPrio() const // PRQA S 4214
{
	ACS_COND_THROW(!running(),ThreadException("Thread is not running.")) ; // PRQA S 3081

 	struct sched_param param; // PRQA S 4102
  	int policy = 0;
...
void Thread::setPrio(int prio) // PRQA S 4211
{
	ACS_COND_THROW(!running(),ThreadException("Thread is not running.")) ; // PRQA S 3081
...
void Thread::setCpuAffinity(const cpu_set_t& cpuset) // PRQA S 4211
{
	ACS_COND_THROW(!running(),ThreadException("Thread is not running.")) ; // PRQA S 3081
...
#ifndef PRQA_ANALYZER // PRQA does not handle CPU_ZERO
	CPU_ZERO(&cpuset);
#endif
...
std::exception * &Thread::runException(std::exception * &e) const ACS_NOEXCEPT // PRQA S 4214, 4020
{
	try {
        Lock lock(_mutex) ; // PRQA S 3050
	    if (_runException) {
            {
                exException *x = dynamic_cast<exException *>(_runException) ; // PRQA S 3081
                if (x) { return e = x->clone() ; }
            }
...
pthread_t Thread::threadId() const // PRQA S 4214
{
	ACS_COND_THROW(!running(),ThreadException("Thread is not running.")) ; // PRQA S 3081
	return _threadId;
}

bool Thread::join(unsigned long msectimeout) // PRQA S 4020
{
	if (0 == _threadHd) { // thread never run or already joined
		return false ;
	}
    	
	// the test is done on _threadId whose value is set into the children thread
	// this is a stronger constraint that checking _threadHd whose meaning the thread ran and finished, but was still not joined
	// the start method synchronize on _threadId ensuring it will not return until the thread started
	ACS_COND_THROW(!running() && (0 == _threadId),ThreadException("Thread is not running.")) ;  // PRQA S 3081
	
	if (0 != msectimeout) {
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "waiting " << msectimeout << " msec") ;
		unsigned long wait = 0 ;
		// each cycle lasts 250 msec, do not lower this time to avoid the cicle is much longer than the nanosleep
		for (wait=0; wait <= msectimeout ; wait+=_joinTimeoutSlicingMsec) { // PRQA S 3084, 4230
			ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "waited " << wait << " msec" ) ;
			if (!running()) {
                break ;
            }
			nanosleep(250000000) ; // PRQA S 4400
		}
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "out of the wait loop after " << wait << " msec") ;
		if (running()) {
			ostringstream os ;
			os << "Thread: " << _threadId << " timeout expired joining." ;
			ACS_THROW(TimeoutException(os.str())) ; // PRQA S 3081
		}
	}	
...
void Thread::internalJoin() // PRQA S 4020
{
	if (0 == _threadId) {
		_threadHd = 0 ;
		return ; // useless join	
	...
	void * th = 0 ;
	int ret = ::pthread_join(_threadId, &th);
	switch(ret) {
	case 0:
        {
		    _threadHd = 0 ;
	...
	case ESRCH:
		{
			ostringstream os ;
			os << "Thread id: " << _threadId << " was not found." ;
			ACS_THROW(ThreadException(os.str(),ret)) ;
		}
        break ;
	case EINVAL:
		{
			ostringstream os ;
			os << "Thread id: " << _threadId << " was detached or another thread is waiting its termination." ;
			ACS_THROW(ThreadException(os.str(),ret)) ;
		}
        break ;
	case EDEADLK:
...
// A sort of privileges should be ensured to do this.
// So, any code having a non const Thread can set its priority as it can set other properties
void Thread::kill(int sigNum) // PRQA S 4211
{
	ACS_COND_THROW(!running(),ThreadException("Thread is not running.")) ; // PRQA S 3081
...
pthread_t Thread::threadIdUnguarded() const ACS_NOEXCEPT // PRQA S 4120
{
	return _threadId;
}


pthread_t  Thread::parentThreadId() const ACS_NOEXCEPT // PRQA S 4120
{
...
class ConditionScopeManager // PRQA S 2109
{
public:
    explicit ConditionScopeManager (Condition& cond) : _cond (&cond) {} // PRQA S 2528
    explicit ConditionScopeManager (Condition* cond) : _cond (cond) {} // PRQA S 2528
    
    ~ConditionScopeManager() ACS_NOEXCEPT
    {
        // PRQA S 4631 L2
        try
        {
            if (_cond) { _cond->broadcast(); }
        }
        catch (exception& ex)
        {
...
private:
    ConditionScopeManager() ;
    ConditionScopeManager(const ConditionScopeManager &) ;
    ConditionScopeManager &operator= (const ConditionScopeManager &) ;
...
namespace {
    ...
    public:
        explicit SvcStatusGuard(Thread::SvcStatus &s) : status_(s) { status_ = Thread::SvcStarted ; } // PRQA S 2528
 	    ~SvcStatusGuard() ACS_NOEXCEPT { status_ = Thread::SvcExiting ; }
    private:
        SvcStatusGuard() ; // not implemented
        SvcStatusGuard(const SvcStatusGuard &) ; // not implemented
        SvcStatusGuard& operator=(const SvcStatusGuard &) ; // not implemented
    
        Thread::SvcStatus &status_ ;
    } ;


class LiveThreadIdScopeGuard // PRQA S 2109
{
public:
    explicit LiveThreadIdScopeGuard (pthread_t id, const string& name) : _id (id) 
	{
		exStackTrace::addLiveThreadId (_id, name);
	}
    
    ~LiveThreadIdScopeGuard() ACS_NOEXCEPT
    {
		exStackTrace::removeLiveThreadId (_id);
    }
private:
    LiveThreadIdScopeGuard() ;
    LiveThreadIdScopeGuard(const LiveThreadIdScopeGuard &) ;
    LiveThreadIdScopeGuard &operator= (const LiveThreadIdScopeGuard &) ;
...
void* Thread::svc(void *m)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;

	Thread* me = static_cast<Thread *>(m) ; // PRQA S 3081
	...
	try
	{
		...
		int err = ::pthread_setcancelstate(PTHREAD_CANCEL_ENABLE,0) ;
		ACS_COND_THROW(0 != err,ThreadException("Error in thread set cancellation state.", err)) ; 

		err = ::pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS,0) ;
	...
	{
		me->setRunException(dynamic_cast<exException *>(e.clone(me->_threadName))) ; // PRQA S 3081
	}
	catch(exception &e)
	{
		me->setRunException(new ThreadException(e,"std::exception caught")) ;
	}
// MP: The following code has been commented out because the new behaviour of the thread cancellation with stack
// unwinding. If the catch(...) is left, a pthread_cancel would finish here, causing a crash.
//	catch(...)
//	{
//		me->setRunException(new ThreadException("Unknown exception caught.")) ;
//	}
	...
	return me->_runException ; // do not move operator comma !!
}

void Thread::setExternalCondition (Condition* c) // PRQA S 4121
{
    _extCondition = c;
}
...
bool Thread::waitExitOrTimeout (unsigned long msecs) const// PRQA S 4211
{
    ...
    else
    {
        ...
        try
        {
            ...
            { 
                ...
                Lock lock (_condition); // PRQA S 3050 
        ...
        catch (exception& ex)
        {
            ...
            nanosleep (msecs * 1000000); // PRQA S 3084, 4400
            retval = !running();
        }
    }    
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Going to exit. Return value is " << boolalpha << retval) ;
                   
    return retval;
}

void Thread::maskedSignals(sigset_t &mask) ACS_NOEXCEPT 
{
	sigemptyset(&mask) ;

 ...
 */
    ...
    typedef struct {
	unsigned int File_Key;              /* This Key is Used to Recognize the Quick Look RGB Files */
	...
	unsigned short Hddt_Number;         /* High Density Digital Tape Number */
	unsigned short Video_Format;        /* Format of the Video Data (BW/RGB) */
	unsigned short Lines_per_Jpeg_Block;/* Number of Compressed Lines per Jpeg Block */
	unsigned short Line_Size;           /* Length of the Quick Look Video Line for the Given Sensor */
	unsigned short Frame_Size;          /* Length of the Quick Look Frame fof the Given Sensor */
	unsigned short Spectral_Bands[3];   /* Selected Spectral Bands for Red, Green, Blue */
	unsigned short Compression_Flag;    /* Flag for Video Data Compression 0/1 true/false */
	...
	double QL_Line_Time __attribute__ ((aligned(8)));                /* Fly Time for Acquiring a Single Quick Look Line * 10000 */
	double QL_Ifov_X __attribute__ ((aligned(8)));                   /* Pixel Size in the X direction in radiants */
	double QL_Ifov_Y __attribute__ ((aligned(8)));                   /* Pixel Size in the Y direction in radiants */
	unsigned short QL_X_Offset;         /* Pointing offset in X direction of Quick Look */
	...
	unsigned int Loaded_Lines;          /* Number of Lines Loaded on Disk */
	unsigned int Mission_Dependent[100];/* Fields for mission dependent data and contingenty */
...
/* #endif */
    ...
    typedef struct {
         double Satellite_Time __attribute__ ((aligned(8)));
         unsigned int Delta_Time;
         unsigned int Jpeg_Block_Size;
 ...
 */
    ...
    typedef struct  {
	int32_t Frame;      /* frame number         */
	int32_t StartLine;  /* start line of the frame (can be negative !) */
	int32_t EndLine;    /* end   line of the frame (can exceed Lines_Number !) */
...
typedef struct {
    int32_t satid;               /* satellite id number */
    int32_t linesNumber;         /* ql jpeg decopressed number of lines  */
    int32_t linesPerBlock;       /* lines per jpeg decompressed block */	    
    int32_t lineLostPixel;       /* line pixel offset for video */
    int32_t lineSize;            /* decompressed line size */	    
    int32_t videoLineSize;       /* decompressed video line size */	    
    int32_t videoFormat;         /* video format BW/RGB */
    int32_t numberOfBlocks;      /* number of jpeg compressed block */
    int32_t numberOfWrittenFrames;/* number of frames stored onto the jpeg headers */
    int32_t numberOfFrames;      /* real number of frames in the video data */
    int32_t firstFrame;          /* first ql frame number */
    int32_t lastFrame;           /* last ql frame number */
    int32_t startSegmentPadding; /* black lines from the beginning */
    int32_t endSegmentPadding;   /* black lines to the end */

	...
	IF foundOrderId IS NOT NULL THEN
		BEGIN
			IF inUserId IS NOT NULL THEN
				INSERT INTO t_srv_request_x_user (user_id, order_id) VALUES (inUserId, foundOrderId) ; 
			ELSE
				INSERT INTO t_srv_bulkbatch_x_ordersqueue (bulkbatch_id, order_id) VALUES (inBulkbatchId, foundOrderId) ; 
			END IF ;
		...
		EXCEPTION WHEN others THEN
			...
			IF inUserId IS NOT NULL THEN
				INSERT INTO t_srv_request_x_user (user_id, order_id) VALUES (inUserId, returnedOrderId) ; 
			ELSE
				INSERT INTO t_srv_bulkbatch_x_ordersqueue (bulkbatch_id, order_id) VALUES (inBulkbatchId, returnedOrderId) ; 
			END IF ;
...
INSERT INTO t_satellites (satelliteid, satellitename, satelliteacronym) VALUES (34,'Sentinel-1', 'S1') ;
...
INSERT INTO t_satellites (satelliteid, satellitename, satelliteacronym) VALUES (40, 'Sentinel-3', 'S3') ;
...
BEGIN
	IF (TG_OP = 'DELETE') THEN
		INSERT INTO t_srv_pendinginv (
			inv_id, uuid,	name, contenttype, contentlength, origindate, creationdate,	modificationdate, checksum,	validitystart, validitystop, footprint, validity, filetype_id, satellite_id, sensor_id, mission, evictiondate, productiontype, attributes, operation) 
			SELECT OLD.*, 'DELETE' ;
		RETURN OLD;
...
---------------------------------------------------------------------------------
/*
DELETE FROM t_schema WHERE version = 'srv-01.00.00' ;

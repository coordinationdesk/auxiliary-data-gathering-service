...
$option_parser = OptionParser.new do |opts|
  ...
  to Load cblob.
  It is assumed that [.pgpass] File is present in the home of user using the script!!!
...
def read_pgpass
    raise "ERROR: .pgpass file is not present in the admin home" unless File.file?("#{Dir.home}/.pgpass")
    body = File.read("#{Dir.home}/.pgpass")
    parameters = body.split(":")
    hostname = parameters[0]
    dba_user = parameters[3]
    dba_pass = parameters[4].chomp
    return hostname,dba_user,dba_pass
end

def execute_query(schema,query)
  begin
    db = PGconn.open(:host => $hostname, :dbname => schema, :user => $dba_user, :password => $dba_pass )
    res = db.exec(query)
    db.close
  rescue Exception => ex
    puts "[#{schema}] #{query}"
...
if $0 == __FILE__
    ...
    begin
        ...
        $hostname,$dba_user,$dba_pass = read_pgpass

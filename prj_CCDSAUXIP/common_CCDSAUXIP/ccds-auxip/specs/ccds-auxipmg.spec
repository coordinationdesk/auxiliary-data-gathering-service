...
install -m755 bin/LTAInventoryMetricsTracker                             %{buildroot}%{install_path}/bin
install -m755 bin/PRIPRolling                                            %{buildroot}%{install_path}/bin
...
install -m644 systemd/ccds-auxipmanagement.service                       %{buildroot}%{install_path}/systemd
install -m644 systemd/ccds-ltainventorymetricstracker.service            %{buildroot}%{install_path}/systemd
install -m644 systemd/ccds-priprolling.service                           %{buildroot}%{install_path}/systemd
...
if [ ! -d /data/%{_ElementName}/inventory ]; then mkdir -p -m775 /data/%{_ElementName}/inventory; chgrp %{_jgroup} /data/%{_ElementName}/inventory; fi
#Plugins Links
...
sed -i 's|#INVMETWD#|/data/%{_ElementName}/inventory|g' %{install_path}/conf/LTAInventoryMetricsTracker.xml

sed -i 's/#LTAUSER#/%{_juser}/g'                    %{install_path}/systemd/ccds-priprolling.service
sed -i 's/#LTAGROUP#/%{_jgroup}/g'                  %{install_path}/systemd/ccds-priprolling.service
sed -i 's|#VINSTALL_PATH#|%{vinstall_path}|g'       %{install_path}/systemd/ccds-priprolling.service

sed -i 's/#LTAUSER#/%{_juser}/g'                    %{install_path}/systemd/ccds-ltainventorymetricstracker.service
sed -i 's/#LTAGROUP#/%{_jgroup}/g'                  %{install_path}/systemd/ccds-ltainventorymetricstracker.service
sed -i 's|#VINSTALL_PATH#|%{vinstall_path}|g'       %{install_path}/systemd/ccds-ltainventorymetricstracker.service
...
services=( "ccds-priprolling.service" "ccds-garbagecleaner.service" "ccds-garbagecleaner.timer" "ccds-ltainventorymetricstracker.service")
...
services=( "ccds-auxipmanagement.service" )

...
private:
    ...
    // stopManagerThread_ must be volatile in order to prevent de-synchronization of current stop mode.
    // From issue CCDS-72: the problem is probably caused by getCurrentStopMode that return immediate instead of noStop
    //        for a moment due to caching of stopManagerThread_ as nullptr (initial value).
    // Using volatile will prevent caching and force synchronization of stopManagerThread_ value between threads.
    ApplicationStopManager* volatile stopManagerThread_;

...
typedef unsigned char ACS_CRC_Digest_Retbuf[EVP_MAX_MD_SIZE] ; // NOSONAR - openssl/evp defines it
...
public:
    ...
    CRC_Digest_base(const CRC_Digest_base &) = delete ;
    CRC_Digest_base & operator= (const CRC_Digest_base &) = delete ;

   	CRC_Digest_base(size_t blkSize, const EVP_MD* digest, void* engine=nullptr) ;
	virtual ~CRC_Digest_base() noexcept ;

	void addBuffer(const void * buffer, size_t buflen) ;
	std::string getResult(ACS_CRC_Digest_Retbuf retbuf_IN=nullptr, unsigned int* s=nullptr, bool validity=false) ;
	void resetContext() ;
...
private:
	...
	void*           _engine ; /* NOSONAR - to ensure compatibily between older and newer libssl */
	bool 			_valid = false ;  
} ;

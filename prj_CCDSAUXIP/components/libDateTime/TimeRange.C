...
std::ostream &TimeRange::getSqlString(std::ostream &os) const 
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;
...
{ 
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "_start.isFilled(): " << std::boolalpha << _start.isFilled()
			   << " _stop.isFilled(): " << std::boolalpha << _stop.isFilled() ) ;
...
std::istream & operator >> (std::istream &is, TimeRange& r) // PRQA S 4020
{
	...
	if (ell == '(') {
        ...
        ACS_FUNC_WRITE_DEBUG_TH(TimeRange,ACS_LOW_VERB, "r.isFilled(): " << std::boolalpha << r.isFilled()) ;
...
std::ostream & operator << (std::ostream &os , TimeRange::IntervalClosure ic ) 
{
    os << TimeRange::toString(ic) ;
    return os ; // PRQA S 4028
}

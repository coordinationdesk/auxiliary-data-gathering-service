...
string SQLString::SQLCase::toString(const string& key) const 
{ 
...
SQLString::SQLString(const SQLString &e) :
    ...
    _sqlOnConflictDo(e._sqlOnConflictDo ),
...
void SQLString::forceSQLString( string const & s) 
{
    ...
    {
        ACS_THROW(Exception()<< "SQLString::forceSQLString. Cannot force SQL Command if class not proper configured: " << _queryType) ; // PRQA S 3081
    }
...
void SQLString::switchOnConflictOnConstraint(const bool b) 
{
	/** Used to switch between ON CONTSTRAIN or normal */
	_sqlOnConflictDo.onConstraint = b ;
	_sqlOnConflictDo.is_enabled = true ; 
}


void SQLString::setOnConflictDo(string const&column_name)
{
	/** Case: DO NOTHING */
	addToUsedTokenList(SQLDialect::ON_CONFLICT_DO) ;
	_sqlOnConflictDo.index = SQLDialectFactory::sqlDialect(_key).translateColumnName(column_name) ;
	_sqlOnConflictDo.sqlSetList.clear() ;
	_sqlOnConflictDo.is_enabled = true ; 
}


void SQLString::addOnConflictDo(string const&column_name, string const &k, string const &v)
{
	/** A vector of "set" is passed as key/string value */
	addToUsedTokenList(SQLDialect::ON_CONFLICT_DO) ;
	_sqlOnConflictDo.index = SQLDialectFactory::sqlDialect(_key).translateColumnName(column_name) ;
	pair <string, string> p (SQLDialectFactory::sqlDialect(_key).translateColumnName(k), v );
	_sqlOnConflictDo.sqlSetList.push_back(p) ; 
	_sqlOnConflictDo.is_enabled = true ; 
}

void SQLString::addOnConflictDo(string const&column_name, string const &k, SQLString const &v)
{
	/** A vector of "set" is passed as key/SQLString value */
	const bool nested(true) ; 
	addOnConflictDo(column_name, k, v.getSQL(nested) );
}

void SQLString::setOnConflictDoFromUpdate(string const&column_name, SQLString const &sql_update)
{
	/** The vector of "set" is extracted from the provided UPDATE query */
	addToUsedTokenList(SQLDialect::ON_CONFLICT_DO) ;
	ACS_COND_THROW( (QueryType::UPDATE != sql_update.getType()), \
		exIllegalValueException("SQLString::setOnConflictDoFromUpdate: Query type differs from UPDATE") ) ; 
	
	_sqlOnConflictDo.index = SQLDialectFactory::sqlDialect(_key).translateColumnName(column_name) ;
	_sqlOnConflictDo.sqlSetList = sql_update.getSetList() ;
	_sqlOnConflictDo.whereCondition = sql_update.getWhereParameters() ;
	_sqlOnConflictDo.is_enabled = true ; 
}

void SQLString::setOnConflictWhere(string const&w)
{
	_sqlOnConflictDo.whereCondition = w ;
	_sqlOnConflictDo.is_enabled = true ; 
}
...
vector< pair<string,string> > SQLString::getSetList() const
{
	return _sqlSetList ;
}
...
bool SQLString::sqlForcedStringEmpty() const
{
    ...
    {
        ACS_THROW(Exception() << "SQLString::sqlForcedStringEmpty. Invalid query type: " << _queryType << " This method can be used only if type is COMMAND.") ; // PRQA S 3081
    }
...
const vector<string>& SQLString::getJoinParameters() const {
...
void SQLString::checkSQLTokenMap(vector<SQLDialect::SQLToken> const& t) const
{
    for (unsigned int i=0; i<_usedTokens.size(); i++)
    {
        ...
        if (!found)
        {
             ACS_THROW(Exception()<< "Token " 
						 << SQLDialectFactory::sqlDialect(_key).translateToken( _usedTokens[i], false ) 
						 << " not allowed by Template for query type: " << _queryType) ; // PRQA S 3081
        }
...
// complex method
    ...
    default:
        {
            ACS_THROW(Exception() << "Cannot process SQL Query of unknown type: " << int(_queryType)) ;
        }
    }
	...
	{ 
        ...
        case SQLDialect::TEMPLATE_INVALID :
            {
				ACS_THROW(Exception() << "TEMPLATE_INVALID :  This template CANNOT BE USED to get a valid SQL Sentence. ") ; // PRQA S 3081
                break ;
            }
		...
		case SQLDialect::UNION_LIST: // List of SELECT queries to build a UNION
			{
				...
				{
					ACS_THROW(Exception() << "Union List is empty: No SELECT in UNION") ; // PRQA S 3081
				}
        ...
        case SQLDialect::ON_CONFLICT_DO:
			{
				if (_sqlOnConflictDo.is_enabled)
				{
					sqlString+= " " ;
					if (false == _sqlOnConflictDo.onConstraint)
					{
						sqlString += theDialect.translateToken(SQLDialect::ON_CONFLICT_DO) ; 
						if (! _sqlOnConflictDo.index.empty()) { sqlString += " (" + _sqlOnConflictDo.index + ")" ; }
					}
					else
					{ sqlString	+= theDialect.translateToken(SQLDialect::ON_CONFLICT_CNSR_DO) + " " + _sqlOnConflictDo.index ; }
					
					if (_sqlOnConflictDo.sqlSetList.empty())
					{
						sqlString+= " DO NOTHING" ;
					}
					else
					{
						sqlString+= " DO UPDATE" ;
						for (size_t k=0; k<_sqlOnConflictDo.sqlSetList.size(); k++)
						{
							if (!k) { sqlString+= " " +  theDialect.translateToken(SQLDialect::SET) ; }
							else { sqlString+= " ," ; }
							
							sqlString+= " " + (_sqlOnConflictDo.sqlSetList.at(k)).first + " = " + (_sqlOnConflictDo.sqlSetList.at(k)).second ;
						}
					}			
			
					if (! _sqlOnConflictDo.whereCondition.empty())
					{ sqlString+= " " + theDialect.translateToken(SQLDialect::WHERE) + " " + _sqlOnConflictDo.whereCondition ; }
		...
		default: // token not recognized: error
		    {
			    ACS_THROW(Exception() << "Cannot convert token " << SQLTemplate[templ]) ; // PRQA S 3081
		    }
    ...
    if ( theDialect.limitWrapsQuery() && ((_sqlLimit > 0) || (_sqlOffset > 0)) )
    {
	    ...
	    {
	        ACS_THROW(Exception() << "Cannot limit/offset SQL Query of type: " << _queryType << " different from SELECT when limitWrapsQuery set") ; // PRQA S 3081
	    }
...
// << operator
std::ostream &operator<<(std::ostream &os, const SQLString &n) 	// output to stream
{
    os << n.getSQL(true) ;
...
std::ostream & operator << (std::ostream &os, SQLString::QueryType t) 
{

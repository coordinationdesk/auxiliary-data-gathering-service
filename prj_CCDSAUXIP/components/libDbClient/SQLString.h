...
public:	
	...
	public:
        ...
        SQLCase(const SQLCase &) = default ;
        SQLCase& operator=(const SQLCase &) = default ;
        SQLCase(SQLCase &&) = default ;
        SQLCase& operator=(SQLCase &&) = default ;
		~SQLCase() = default ; 
	...
	struct OnConflictDo
	{
		OnConflictDo() : index(""), sqlSetList(), whereCondition(""), onConstraint(false), is_enabled(false) {} 
		std::string		index ; 
	...
	std::vector< std::pair<std::string, std::string> > getSetList() const ;
	...
	void switchOnConflictOnConstraint(const bool b=true) ; 
	
	/** Used to enforce action DO NOTHING */
	void setOnConflictDo(std::string const&c) ; 

	/** A vector of "set" is passed as key/string value */
	void addOnConflictDo(std::string const&c, std::string const &k, std::string const &v) ;
	/** A vector of "set" is passed as key/SQLString */
	void addOnConflictDo(std::string const&c, std::string const &k, SQLString const &v) ;
	/** The vector of "set" is extracted from the provided UPDATE query */
	void setOnConflictDoFromUpdate(std::string const&c, SQLString const &v) ;

	/** Set the WHERE on the ON CONFLICT */
	void setOnConflictWhere(std::string const&w) ;
...
private:
	...
	public:
		...
		~SelectListContainer() = default ;
        SelectListContainer(const SelectListContainer &) = default ;
        SelectListContainer& operator= (const SelectListContainer &) = default ;
        SelectListContainer(SelectListContainer &&) = default ;
        SelectListContainer& operator= (SelectListContainer &&) = default ;

		const std::string &getAlias() const noexcept { return _lbl ; } 
		const std::string &getItem() const noexcept { return _item ; }
		const SQLDialect::SQLCast &getSQLCast() const noexcept { return _sqlCast ; }
	private:

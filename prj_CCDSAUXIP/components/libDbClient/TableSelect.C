...
size_t TableSelect::size() const noexcept 
{ 
	return _values.size(); 
}

bool TableSelect::empty() const noexcept
{ 
...
const std::string &TableSelect::tableName() const // PRQA S 4120
{ 
	return _tableName ; 
}

const std::string &TableSelect::colName(size_t which) const 
{
	if (which > _cols.size()-1) { // check boundary // PRQA S 3084
		ACS_THROW(exIllegalValueException() << "Cannot address colummn #" << which << " in a collection of " << _cols.size() << " columns ") ;   
	} 
	return _cols[which] ; // return it
}


const std::string &TableSelect::value(size_t row, unsigned int col) const 
{
	if (row >= size()) { // row boundaries check
		ACS_THROW(exIllegalValueException() << "Cannot address element #" << row << " in a collection of " << size() << " elements ") ;   
	}
	
	if (col > _cols.size()-1) { // col boundaries check // PRQA S 3084
		ACS_THROW(exIllegalValueException() << "Cannot address colummn # " << col << " in a collection of " << _cols.size() << " columns ") ;   
	} 
...
std::ostream &operator<<(std::ostream &os, const TableSelect &n) 	// output to stream
{
    size_t ns = n.size() ;

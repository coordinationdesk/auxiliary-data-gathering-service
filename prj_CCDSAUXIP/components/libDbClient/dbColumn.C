...
/*
	...
	Added a warning for missing method "string& dbColumn::ValueInst<bool>::getValue"
...
string const dbColumn::_nullString("<null>") ;

string const &dbColumn::nullString() { return _nullString ; }
...
dbColumn::dbColumn(const string &key) :
...
dbColumn::dbColumn(const string &key, dbSpecificColumnType::Type t, bool isNull) :
    _type(t) ,
    _key(key) ,
	_value(0)
{
...
dbColumn::dbColumn(const string &key, void* value, bool isNull) :
...
dbGeoObject* dbColumn::column(dbGeoObject* &v) // PRQA S 2017, 4211
{
    ...
    {
		ACS_THROW(InvalidType() << "dbColumn::column : Attempt to get a geo object value but type is " << dbColumnTypeHelper::getTypeName(_type)) ; // PRQA S 3081
	}
...
string dbColumn::getSqlString() const // PRQA S 4020
{
// W8115_OFF: multiple returns
    if ( !_value )
    {
...
dbColumn::Value * dbColumn::createValue(const string &key, dbSpecificColumnType::Type type) // PRQA S 5500
{
...
// any attemp to lower CC would increase complexity for maintenance
	...
	case dbSpecificColumnType::STRING:					{ res = new ValueInst<string>(key, "") ; break ; }
	...
	default: { ACS_THROW(InvalidType() << "Unknown dbSpecificColumnType: <" << int(type) << ">") ;  }
	}
	return res ;
}       
...
void dbColumn::Value::setValue(string const & v)
{ InvalidType e("Impossible to set this type") ; ACS_THROW(e) ;}
...
dbColumn::BCDValue::BCDValue(string const &key, string const &s) : Value(key), _value(s) {} 

dbColumn::BCDValue::~BCDValue() noexcept {}
...
std::ostream &dbColumn::BCDValue::insertIntoStream(std::ostream &os) const
{
	os<<_value ;
	return os; // PRQA S 4028
}
...
string& dbColumn::BCDValue::getValue(string& v) const
{
	v=_value ;
...
void checkNumeric(const string &s) 
{
	...
	if ((*startConv == '\'') || (*startConv == '\"')) { // can start with double or single quotes (and end with the same char
		if (((startConv[len-1] == '\'') || (startConv[len-1] == '\"')) && (*startConv == startConv[len-1])) { // PRQA S 3084
			...
			ACS_THROW( dbColumn::ErrorInNumericConversion() << "isNumeric : wrong char \""<<s[0]<<"\" in pos 0") ; // PRQA S 3081
		}
	}
	char *realEnd = 0 ;
	...
	if (realEnd != endConv) { // check if consumed all the available chars
		ACS_THROW( dbColumn::ErrorInNumericConversion() << "isNumeric : wrong char \""<<*realEnd<<"\" at pos "<<realEnd-s.c_str() ) ; // PRQA S 3081
	}
}
...
void dbColumn::BCDValue::setValue(string const & v)
{
	checkNumeric(v) ;
	_value = v ;
}

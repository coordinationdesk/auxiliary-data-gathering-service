...
public:
    ...
    dbConnectionParametersI() = delete ; // not implemented
	/*!
	...
	*/
	...
	virtual ~dbConnectionParametersI() = default ;

protected:

	dbConnectionParametersI(const dbConnectionParametersI &) = default ; 	
	dbConnectionParametersI &operator=(const dbConnectionParametersI &) = default ;
...
public:
	...
	const std::string & getDriverFunc() const noexcept ;
	const std::string & driverLibrary() const noexcept ;
	const std::string & vendorLibrary() const noexcept ;

	void roleName(const std::string &) ;
	const std::string & roleName() const noexcept ;

	const std::string & hostname() const noexcept ;
	const std::string & server() const noexcept ;
	const std::string & user() const noexcept ;
	const std::string & password() const noexcept ;	/* Provided "as-is" (i.e. crypted) */
	const ACS_SSL::EncryptionAlgo & pwdEncryptionAlgo() const noexcept ;
	const std::string & db() const noexcept ;
	unsigned short port() const noexcept ;
	const std::string & salt() const noexcept ;
	const size_t & saltSize() const noexcept;
    bool assumeNoPostGIS() const noexcept ;
...
using dbConnectionParameters = std::shared_ptr<dbConnectionParametersI>  ;
...
public:
	dbConnectionParametersBinder() noexcept = default ;
	virtual ~dbConnectionParametersBinder() = default ;
	dbConnectionParametersBinder(const dbConnectionParametersBinder &) = delete ;
	dbConnectionParametersBinder &operator=(const dbConnectionParametersBinder &) = delete ;
} ;
...
inline
MQTTSubIdHelper toMQTTSubId (const keyType__& keyContainer)
{
	return toMQTTSubId(keyContainer.getKey()); // PRQA S 3050
}

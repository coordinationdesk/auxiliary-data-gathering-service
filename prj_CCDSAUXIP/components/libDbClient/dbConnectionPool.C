...
dbConnectionPoolElem::~dbConnectionPoolElem() noexcept 
{
// PRQA S 4631 L3
	try {
		close() ;
...
const string &dbConnectionPool::getKey() const noexcept { return _key ; } // PRQA S 4120
...
size_t dbConnectionPool::getMaxConnection() const noexcept // PRQA S 4120
{
	return _maxConn ;
}
...
void dbConnectionPool::releaseConnection(dbConnection *c) // PRQA S 4020
{
    ...
    bool broken = false ;
    if (c->hasAggregateTransactionState()) {
        // seems the transaction has not closed correctly (commit or rollback)
        // it is a signal that a disconnection occurred so the connection shall be considered 
        // to be not usable any longer
        ACS_LOG_ERROR("Db connection coherency broken - the connection will be closed forcefully") ;  
        broken = true ;
    }
...
void dbConnectionPool::wrongConn() noexcept { _wrongCounter++ ; }
void dbConnectionPool::goodConn() noexcept { _goodCounter++ ; }
void dbConnectionPool::setMaxConnectionAttempts(unsigned short c ) noexcept 
{
	_wrongCounter = 0; 
	_goodCounter =0 ;
...
dbConnectionWrapper::~dbConnectionWrapper() noexcept 
{
// PRQA S 4631 L1
	try
	{
...
const std::string &dbConnectionWrapper::getKey() const noexcept 
{
    return _pool.getKey() ;
}
...
{
    Lock l(*this) ;
    std::map<std::string, std::shared_ptr<dbConnectionPool> >::iterator p = pools_.find(key) ;
    if (p == pools_.end()) { // does not exists
        statN(key)["pools created #"]+=1 ;
        pools_[key] = std::shared_ptr<dbConnectionPool>(new dbConnectionPool(key)) ; // PRQA S 3081
...
namespace {
    struct get_key { // PRQA S 2171, 2173, 2175, 2185
        ...
        void operator()(const pair<string, std::shared_ptr<dbConnectionPool> > &p) { 

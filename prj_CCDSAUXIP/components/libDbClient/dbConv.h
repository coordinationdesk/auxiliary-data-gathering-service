...
public:
	...
	dbConv();
	dbConv& operator=(const dbConv &) = default ;	
	dbConv(const dbConv &) = default ;	
	dbConv& operator=(dbConv &&) = default ;	
	dbConv(dbConv &&) = default ;	
	...
	template <typename T> 
    static std::string nullConv(const std::string &key, const std::string &, bool, const T &) ;

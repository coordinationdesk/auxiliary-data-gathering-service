...
bool dbQueryParameters::has(const string & tableName , const string & fieldName) const // PRQA S 4020
{
...
void dbQueryParameters::deleteParameter(const string & tableName , const string & fieldName) 
{
	ACS_COND_THROW(not _qparams.empty(),InvalidValue("Method not implemented")) ; // PRQA S 3081
...
{ 
	...
	{
		ACS_THROW(exBoundsException() << "Out of bound. Requested position is: " << pos << " allowed " << _params.size()) ; // PRQA S 3081 
	}
	return _params[pos] ; 
}
...
{ 
	...
	{
		ACS_THROW(exBoundsException() << "Out of bound. Requested position is: " << pos << " allowed " << _qparams.size()) ; // PRQA S 3081 
	}
	return _qparams[pos] ; 
}
...
// Output to stream
std::ostream &operator<<(std::ostream &str, const dbQueryParameters &par)
{
...
exostream &operator<<(exostream &str, const dbQueryParameters &par) // PRQA S 4222
{
	ACS_COND_THROW(not par._qparams.empty(),dbQueryParameters::InvalidValue("Method not implemented")) ; // PRQA S 3081
...
// Input from stream
std::istream &operator>>(std::istream &str, dbQueryParameters &par) // PRQA S 4020
{
// W8115_OFF multiple returns

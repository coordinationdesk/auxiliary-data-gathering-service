...
void dbRow::row(dbSQLCursor &itrow, 
                ...
                bool storedRow) 
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "dbSQLCursor empty: " << std::boolalpha << (itrow.get() != 0) 
            << " storedRow: " << std::boolalpha << storedRow ) ;
...
{
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "localRow: " << std::boolalpha << _storedRow) ;
...
void dbRow::storeCurrentRow(const vector<dbSpecificColumnType::Type>& suggestedTypes)
{
    ...
    {
        ACS_THROW(ExecuteException() << "dbRow::storeCurrentRow invalid return from getColumnCount: " << returnedValue) ; // PRQA S 3081
 	}

    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "columnCount: " << columnCount << " _storedRow: " << std::boolalpha << _storedRow ) ;


	if ( ( suggestedTypes.size() > 0) && ( suggestedTypes.size() != columnCount) )
    {
        ACS_THROW(ExecuteException() << "dbRow::storeCurrentRow suggestedTypes size " << suggestedTypes.size() << " differs from ColumnCount: " << columnCount) ; // PRQA S 3081
    }
...
// E IN QT. CASO CHE ECCEZIONE RITORNO????? Direi un'eccezione apposita!
    ...
    for (uint16_t i=0; i<columnCount; i++)
    {
	    ...
	    if ( returnedValue!=SQL_SUCCESS ) 
        {
			ACS_THROW(ExecuteException() << "dbRow::storeCurrentRow invalid return from getColumnType: " << returnedValue) ; // PRQA S 3081
	    }
...
namespace {
    ...
    string getErrMessage(dbSQLCursor &_cursor) 
    {
        UINT16 elen = 0; 
        _cursor->getErrorMessageLen(&elen) ; 
        char es[elen+1] ; // PRQA S 3000
	...
	if (_cursor.get() && (!_storedRow) )
        {
            ...
            if (commandStatus != SQL_SUCCESS )
            {
				/* Unable to obtain value by dbExpress */
 				ACS_THROW(ExecuteException() << "dbRow:columns() Cannot run getColumnCount(). Got status: " << commandStatus << " (" 
                   << getErrMessage(_cursor) << ")") ; // PRQA S 3081
            }         
...
dbColumn const & dbRow::column(uint16_t idx) const
{
	...
	if (idx >= _dbColumnVector.size() ) {
		ACS_THROW(ExecuteException() << "dbRow::column(long) invalid index: " << idx << " shall be less than " << _dbColumnVector.size()) ; // PRQA S 3081
	}
...
unsigned int  dbRow::columnLength(uint16_t idx) const
{
	...
	if ( (queryStatus != SQL_SUCCESS) || (columnCount<=idx) ) {
        ...
        if (queryStatus != SQL_SUCCESS) {
            ACS_THROW(ExecuteException() << "dbRow::columnLength invalid return from getColumnCount: " << queryStatus) ;
        }
        else {
            ACS_THROW(ExecuteException() << "dbRow::columnLength invalid index" << idx << " shall be less than " << columnCount) ;
        }
	}
	...
	if( queryStatus != SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow::columnLength invalid return from getColumnLength: " << queryStatus) ; // PRQA S 3081
    }
...
bool dbRow::getIfColumnIsNullable(uint16_t idx) const
{
	...
	if ( (queryStatus != SQL_SUCCESS) || (columnCount<=idx) ) {
    	// Check column indexes
        if (queryStatus != SQL_SUCCESS) {
            ACS_THROW(ExecuteException() << "dbRow::getIfColumnIsNullable invalid return from getColumnCount: " << queryStatus) ;
        }
        else {
            ACS_THROW(ExecuteException() << "dbRow::getIfColumnIsNullable invalid index" << idx << " shall be less than " << columnCount)  ;
		}
	}
	...
	if( queryStatus != SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow::getIfColumnIsNullable invalid return from getColumnLength: " << queryStatus) ; // PRQA S 3081
    }
...
-------------------------------------------*/
	...
	if ( (queryStatus!=SQL_SUCCESS) || (columnCount<=idx) ) {
        if (queryStatus != SQL_SUCCESS) {
            ACS_THROW(ExecuteException() << "dbRow: invalid return from getColumnCount: " << queryStatus) ;
        }
        else  {
            ACS_THROW(ExecuteException() << "dbRow: invalid index" << idx << " shall be less than " << columnCount) ;
        }
    }
	...
	if( queryStatus != SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow: invalid return from getColumnType: " << queryStatus) ; // PRQA S 3081
    }
    ...
    default:
        {
			ACS_THROW(ExecuteException() << "dbRow::columnType: INTERNAL ERROR Unsupported type: " << logType << ":" << subType) ; // PRQA S 3081
        }
    }   /* switch logType */
...
string dbRow::columnName(uint16_t idx) const
{
	...
	if( queryStatus != SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow: invalid return from getColumnNameLength: " << queryStatus) ; // PRQA S 3081
    }
	...
	if( queryStatus != SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow: invalid return from getColumnName: " << queryStatus) ; // PRQA S 3081
    }
...
dbSpecificColumnType::Type dbRow::getStoredColumn(uint16_t idx, dbColumn& column, bool *isNull) const// PRQA S 4214
{
    ...
    // Check column range
	if (idx >= _dbColumnVector.size() ) {
		ACS_THROW(ExecuteException() << "dbRow::getStoredColumn invalid index: " << idx << " shall be less than " << _dbColumnVector.size()) ; // PRQA S 3081
	}
    ...
    {
        ACS_THROW(NullValueException() << "dbRow::getStoredColumn():  got Null value for column n " <<idx) ; // PRQA S 3081
    }
...
*/
    ...
    // Check row is null
    if (!_cursor.get()) {
        ACS_THROW(ExecuteException() << "dbRow::getAnyColumn Line: " << __LINE__ << " Invalid row state (null pointer) for idx: " << idx) ; // PRQA S 3081
  	}
	...
	if ( (returnedValue!=SQL_SUCCESS) || (columnCount<=idx) ) {
        if (returnedValue != SQL_SUCCESS) {
            ACS_THROW(ExecuteException() << "dbRow::getAnyColumn invalid return from getColumnCount: " << returnedValue) ;
        }
        else {
            ACS_THROW(ExecuteException() << "dbRow::getAnyColumn invalid index " << idx << " shall be less than " << columnCount) ;
		}
	}
	...
	if ( returnedValue!=SQL_SUCCESS ) 
    {
		ACS_THROW(ExecuteException() << "dbRow::getAnyColumn invalid return from getColumnType: " << returnedValue) ; // PRQA S 3081
	}
	...
	if ( returnedValue!=SQL_SUCCESS )
    {
		ACS_THROW(ExecuteException() << "dbRow::getAnyColumn invalid return from getColumnLength: " << returnedValue) ; // PRQA S 3081
	}
    ...
    case fldZSTRING :
        {
            ...
            if (!v)
            {
                 ACS_THROW(ExecuteException() << "dbRow::getAnyColumn - Cannot execute calloc of: " << ulLength+1 << " bytes" ); // PRQA S 3081
            }
            ...
            if( returnedValue != SQL_SUCCESS )
            {
				ACS_THROW(ExecuteException() << "dbRow::getAnyColumn Cannot run getString(). Got status: " << returnedValue << " (" 
                   << getErrMessage(_cursor) << ")") ; // PRQA S 3081
            }
            ...
            case fldstBINARY:
                {
                    ...
                    returnedValue=SQLDialect::SQLReturn(_key,  _cursor->getBlobSize (rel_col, &blobLen, &returnedNull)) ;
  					if( returnedValue != SQL_SUCCESS )
					{
						ACS_THROW(ExecuteException() << "dbRow::getAnyColumn Cannot run getBlobSize(). Got status: " << returnedValue << " (" 
						    << getErrMessage(_cursor) << ")") ; // PRQA S 3081
					}
            ...
            case fldstFMTMEMO:
                {
                    ...
                    if (0 == returnedNull)
                    {
                        ...
                        tmp = StringUtils::trimEnd(tmp, StringUtils::is_not_space);  /* Informix Reverse-engineered */
                        ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Returned value after strip: !" << tmp << "!") ;
                        std::unique_ptr<dbGeoObject> geo(dbGeoObject::newDbGeo(tmp)) ;
            ...
            default:
                {
                    ...
                    // IF HERE - Unknown value required
 					ACS_THROW(ExecuteException() << "dbRow::getAnyColumn unsupported type: " << uLogType << " subType: " << uSubType) ; // PRQA S 3081
                 }
            }   // switch uSubType
            break ;
        }
            ...
            case fldstFMTMEMO:
                {                    
                    ...
                    if (0 == returnedNull)
                    {
                        ...
                        if (!v)
                        {
							ACS_THROW(ExecuteException() << "dbRow::getAnyColumn - Cannot execute calloc of: " << blobLen << " bytes") ; // PRQA S 3081
                        }
    ...
    case fldINT16 :
        ...
        {    
            ...
            {
				ACS_THROW(ExecuteException() << "dbRow::getAnyColumn INT16 Internal Error. Allocated size: " << sizeof(v) << " less than column size: " << ulLength) ; // PRQA S 3081
            }
    ...
    case fldDATETIME:
        {
            ...
            if( returnedValue != SQL_SUCCESS )
            {
				ACS_THROW(ExecuteException()<< "dbRow::getAnyColumn Cannot run getTime(). Got status: " << returnedValue << " (" 
                   << getErrMessage(_cursor) << ")") ; // PRQA S 3081
            }
            ...
            if (0 == returnedNull)
            {
                ...
                {
                    char debug_string[1024]; // NOSONAR 
                    sprintf(debug_string,"YYYY-MM-DD hh:mm:ss.(frac)= %04d-%02d-%02d %02d:%02d:%02d.(%d)", // NOSONAR 
    ...
    case fldINT32 :
        {
            ...
            if ( sizeof(v) < ulLength)
            {
				ACS_THROW(ExecuteException() << "dbRow::getAnyColumn INT32 Internal Error. Allocated size: " << sizeof(v) << " less than column size: " << ulLength) ; // PRQA S 3081
    ...
    case fldFLOAT :
        {
            ...
            if ( sizeof(v) < ulLength)
            {
				ACS_THROW(ExecuteException() << "dbRow::getAnyColumn DFLOAT Internal Error. Allocated size: " << sizeof(v) << " less than column size: " << ulLength) ; // PRQA S 3081
    ...
    case fldINT64 :
        {
            ...
            if ( dialect == "Informix9" )
            {
                ...
                if( returnedValue != SQL_SUCCESS )
                {
 					ACS_THROW(ExecuteException()<< "dbRow::getAnyColumn Cannot execute getBcd(). Got status: " << returnedValue << " (" 
                       << getErrMessage(_cursor) << ")") ; // PRQA S 3081
                }
            }
            else if ( dialect == "Oracle11" )
            {
                ...
                {
  					ACS_THROW(ExecuteException() << "dbRow::getAnyColumn v_ora[128] Internal Error. BCD Allocated size: " << sizeof(v_ora) << " less than column size: " << ulLength) ; // PRQA S 3081

                }
                returnedValue=SQLDialect::SQLReturn(_key, _cursor->getBlob (rel_col, &v_ora, &returnedNull, ulLength)) ;
				if( returnedValue != SQL_SUCCESS )
                {
					ACS_THROW(ExecuteException() << "dbRow::getAnyColumn  Handling type BCD Cannot execute getBlob(). Got status: " << returnedValue << " (" 
                       << getErrMessage(_cursor) << ")") ; // PRQA S 3081
                }
                ...
                if ( sizeof(v) < ulLength)
                {
					ACS_THROW(ExecuteException() << "dbRow::getAnyColumn FMTBcd Internal Error. Allocated size: " << sizeof(v) << " less than column size: " << ulLength) ; // PRQA S 3081
    ...
    {
        ACS_THROW(NullValueException("dbRow::getAnyColumn:  got Null value")) ; // PRQA S 3081
    }
    ...
    ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB) 
        if (0 == isNull) {
            ...
            ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB,"isNull: " << std::boolalpha << *isNull) ;
        }
...
// as low as possible
    ...
    strcpy(buffer, "") ; // reset the output string - NOSONAR safe

    if ( 0 == bcd.iPrecision ) 
    {
    	strcpy(buffer, "0") ; // NOSONAR - safe (buffer dim is 64) 
    	return buffer ;
    }
    ...
    if ( (bcd.iSignSpecialPlaces & 0x80) != 0) { // PRQA S 4400
        strcpy(buffer, "-") ;	/* Negative number */ // NOSONAR - safe (buffer dim is 64) 
    }
	else {
        strcpy(buffer, "") ;	/* Positive number */ // NOSONAR - safe (buffer dim is 64) 
	}
	...
	if (intPartDigits<=0)
	{
        ...
        strcat (buffer, "0.") ;	written ++ ;	 // NOSONAR - safe (buffer dim is 64) 
		for (ssize_t i=0; i<(-intPartDigits); i++)
        {
        	strcat (buffer, "0") ;		 // NOSONAR - safe (buffer dim is 64) 
            written ++ ;
        }    
    }
	...
	for (ssize_t i=0; i<bcd.iPrecision; i++)
	{
		...
		if ( (intPartDigits>0) && (i == intPartDigits) )  { strcat(buffer, ".") ; } // NOSONAR - safe (buffer dim is 64) 

		c[0] += '0' ;	/* e.g. From 5 to '5' */
		strcat(buffer, reinterpret_cast<char*>(c));  // NOSONAR - safe (buffer dim is 64) 
		written ++ ;

	}

	if ( 0 == written) { strcpy (buffer, "0") ; }  // NOSONAR - safe (buffer dim is 64) 

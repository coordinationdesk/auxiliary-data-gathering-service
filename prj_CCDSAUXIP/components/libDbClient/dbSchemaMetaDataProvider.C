...
void dbSchemaMetaDataProvider::assumeNoPostGIS() noexcept 
{
    _postGisVersion = -1 ;
}
...
{
    if (0 == _postGisVersion) { // not yet got
        ...
        try {
            ...
            _postGisVersion = atoi(version.c_str()) ;
        }
...
{
	Mutex::Lock lock(_mutex) ;
	return 	( ! getTable(tableName, conn).primaryKeys.empty() ) ;
}
...
{
	Mutex::Lock lock(_mutex) ;
	return 	( ! getTable(tableName, conn).foreignKeys.empty() ) ;
}
...
std::vector<std::string> dbSchemaMetaDataProvider::getFieldNames(std::string const & tableName, dbConnection &conn)
{
	...
	for ( auto const &f :  getTable(tableName, conn).fields ) { // each field
		res.push_back ( f.fieldName ) ; // push its name
	}
...
{
	Mutex::Lock lock(_mutex) ;
	return ( ! getField( tableName ,  fieldPos, conn).foreignTable.empty() );
}
...
std::string dbSchemaMetaDataProvider::getForeignTable(std::string const & tableName, size_t fieldPos, dbConnection &conn)
{
	...
	{
		ACS_THROW(MetadataError() << "Table \""<< tableName << "\" has not a foreign key at position " << fieldPos) ;// PRQA S 3081
	}
...
std::string dbSchemaMetaDataProvider::getForeignFieldName(std::string const & tableName, size_t fieldPos, dbConnection &conn)
{
	Mutex::Lock lock(_mutex) ;
	if( ! isForeignKey ( tableName, fieldPos, conn )) // not a foreign key
	{
	...
	return getField( tableName ,  fieldPos, conn).foreignFieldName ;
}
...
std::string dbSchemaMetaDataProvider::getForeignFieldName (std::string const & tableName, std::string const & fieldName, dbConnection &conn) 
{
	return getForeignFieldName (tableName , getFieldFromName(tableName , fieldName, conn), conn ) ;
}
...
map<string , vector< pair < string , string> > > dbSchemaMetaDataProvider::getDependantTables(std::string const & tableName, dbConnection &conn) 
{
	...
	map<string , vector< pair < string , string> > > res ; // NOSONAR 
	try
	{
 		// the query class
		...
		ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "dep query for \""<<tableName<<"\" returned " << tableMetadataSet.size() << " fields.") ;
		...
		while( tableMetadataSet.getNextRow(row))
		{
			...
			ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "table \""<<tableName<<"\" is parent of table \"" << childTable << "\".") ;
			
			vector<pair<string , string > > assoc ;
			vector<size_t> childFields = getForeignKeys(childTable, conn) ;
            size_t cfsize = childFields.size() ;
			ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "getForeignKeys \""<<childTable<<"\" returned " << cfsize << ".") ;
			for(size_t cc = 0 ; cc < cfsize ; ++cc) // fill the association
			{
				if (StringUtils::equalsNoCase( getForeignTable(childTable , childFields[cc], conn ) , tableName) )
				{
			        ...
			        ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "childTable \""<<childTable<<"\" childFields " << childFields[cc]) ;
					string parentField = getForeignFieldName(childTable , childFields[cc], conn ) ;

			        ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "tableName \""<<tableName<<"\" parentField " << parentField) ;
					assoc.emplace_back (getFieldName(childTable , childFields[cc], conn), parentField) ;
				}
			}
...
bool dbSchemaMetaDataProvider::existIndex (const string &tableName, const vector<string> &fieldList, dbConnection & conn)
{
	...
	std::set<string> indexSet; // NOSONAR
...
void dbSchemaMetaDataProvider::retrieveTableIndexes (string const & tableName, dbConnection & conn) // PRQA S 4020
{
	...
	try
	{
		...
		std::map< string, std::set<string> > reverseIndexMap; // NOSONAR 
		...
		for (const auto &idx : reverseIndexMap)
		{
			theIndexSet [ idx.second ] = idx.first;
		}
...
void dbSchemaMetaDataProvider::checkTable(std::string const & tableName, dbConnection &conn) // PRQA S 4020
{
	// Note assume case convertion already performed.
	Mutex::Lock lock(_mutex) ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "checking table \""<<tableName) ;
...
#warning Tables Label still to be implemented.
		...
		{
			ACS_THROW(TableNotPresent()  << "Table \""<< tableName << "\" is not present in db or it seems to have 0 fields.") ;
		}
		...
		for(size_t i=0 ; i<rcount ; ++i) // each record
		{
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", error in obtaining row n " << i) ; // PRQA S 3081
			}

			
			if( fieldMetadataRow.columns() < 14 ) // expected row size is too short // PRQA S 3000, 4400
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", TableMetadataQuery result has not enough columns: " << fieldMetadataRow.columns()) ; // PRQA S 3081
			}
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", Record n " << i <<" seems to correspond to field in position "<< position) ; // PRQA S 3081
			}
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", is PK field is \"" << isPk << "\".") ; // PRQA S 3081
			}
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", is Not Null field is \"" << isNotNull << "\"." ) ; // PRQA S 3081
			}
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", serial field is \"" << isSerialString << "\".") ; // PRQA S 3081
			}
			...
			{
				ACS_THROW(MetadataError() << "Table \""<< tableName << "\", default field is \"" << hasDefaultString << "\".") ; // PRQA S 3081
			}
			...
			fieldMetadataRow.column(12  , newField.foreignFieldName , &nullValue) ; 
			...
			if( fieldMetadataRow.columns() < 15 ) // it does not contain uniqueness
			{
			    ...
			    if(isUnique == "T") { // primary key ?
			     ...
			     {
				     ACS_THROW(MetadataError() << "Table \""<< tableName << "\", isUnique field is \"" << isUnique << "\".") ; // PRQA S 3081
			     }
            }
        ...
        for(size_t i=0 ; i< rfcols ; ++i)
		{
			...
			{
				// wrong date format
				ACS_THROW(MetadataError() << "Unresolved datetime mode in table \"" <<tableName<< "\" field " << i) ; // PRQA S 3081
			}
		}		
...
#warning Field Labels still to be implemented.
		...
		for(size_t i=0 ; i< ntfsize ; ++i )
		{
			if( ! newTable.fields[i].foreignTable.empty())
			{
...
//			       || (newTable.fields[i].type == dbSpecificColumnType::BLOB) )) {
		...
		if(newTable.primaryKeys.size() == 1 )
		{
			...
			if (t == dbGenericColumnType::INTEGER) {
		...
		for(size_t i=0 ; i< ntfsize ; ++i )
		{
			if(newTable.fields[i].isSerial)
			{
				...
				{
					ACS_THROW(MetadataError()<< "Table has two fields of type serial: field n " <<  newTable.serialField << " and field n. " << i) ;
			    }
	...
	catch(const TableNotPresent &) { // NOSONAR - rethrow this exception as is 
		throw ;
	}
	catch(const std::exception &e) // NOSONAR - any exception 
	{
...
dbSchemaMetaDataProvider::FieldMetaData const & dbSchemaMetaDataProvider::getField(std::string const & tableName, size_t fieldPos, dbConnection &conn)
{
	...
	{
		ACS_THROW(FieldNotPresent() <<"Field n " << fieldPos << " exceed array dimension in table \"" << tableName << "\" (" << table.fields.size() << ")") ; // PRQA S 3081
	}

...
public:
	...
	dbSet() ;
	dbSet(const dbSet &) = default ;
	dbSet &operator=(const dbSet &) = default ;
	virtual	~dbSet() = default ;
	...
	*/
	void rewind() noexcept { _actualIndex = 0 ; } // PRQA S 2502

	/*!
		Used to get the a specified row in the set. \note index is 1-relative
	...
	/*!
		access to internal vector of rows
		\internal
	  */
    const std::vector<dbRow> &set() const noexcept ;

	/*!
		Used to assign a vector of rows
		\internal
	*/
	...
	/*!
		to assign column types no values
		\internal
	*/

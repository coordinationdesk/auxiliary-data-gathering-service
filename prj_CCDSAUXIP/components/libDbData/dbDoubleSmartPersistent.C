...
dbDoubleSmartPersistent::~dbDoubleSmartPersistent() noexcept 
{
    destroy() ;
}
...
std::string dbDoubleSmartPersistent::getUserIdentifier() const noexcept //  PRQA S 4020, 4214
{
    return getUserIdentifier(dbConnectionWrapper(db::ConnPools::instance()->getPool(_key))) ; // PRQA S 3050, 3081
}

std::string dbDoubleSmartPersistent::getUserIdentifier(dbConnection &conn) const noexcept //  PRQA S 4020
{
	try
	{
	    ThreadSafe::Lock guard(*this); // PRQA S 3050
...
void dbDoubleSmartPersistent::destroy() noexcept 
{
	ThreadSafe::Lock guard(*this); // PRQA S 3050

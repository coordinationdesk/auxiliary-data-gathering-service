...
dbFileAction::~dbFileAction() noexcept
{
}
...
void dbFileAction::setSimulationMode(bool v) noexcept { _simulationMode = v ; } // PRQA S 4121
bool dbFileAction::isSimulated() const noexcept { return _simulationMode ; } // PRQA S 4120

void dbFileAction::setClientName(const std::string &c ) { clientName_ = c; }
const std::string &dbFileAction::getClientName() const noexcept { return clientName_ ; }
    
std::ostream &operator<<(std::ostream & s, dbFileAction::CacheStatus c) 
{

	switch (c)  {
    	case dbFileAction::NOT_AVAILABLE: 
			{ 
				s << "NOT_AVAILABLE" ; 
				break;
			}
    	case dbFileAction::OFF_CACHE: 
			{ 
				s <<  "OFF_CACHE"; 
				break;
			}
    	case dbFileAction::IN_PROGRESS: 
			{ 
				s << "IN_PROGRESS"; 
				break;
			}
    	case dbFileAction::ON_CACHE: 
			{ 
				s << "ON_CACHE"; 
				break;
			}
...
acs::exostream &operator<<(acs::exostream &s, dbFileAction::CacheStatus c) 
{

...
public:
    ...
    /*- the follwing enum is in order of priority, so ON_CACHE has priority higher than OFF_CACHE 
        and so on.
    */
    enum CacheStatus {
        NOT_AVAILABLE,
	...
	/*! class OnCacheException declaration */
	exDECLARE_EXCEPTION(OnCacheException,dbException) ; // PRQA S 2131, 2153, 2502
    ...
    // to be called only after isComplete returned true on those action dealing
    // with caches - for any other file action the return value will be NOT_AVAILABLE
    virtual CacheStatus getCacheStatus() { return NOT_AVAILABLE ; }
    void setSimulationMode(bool) noexcept ;
    bool isSimulated() const noexcept ;

    void setClientName(const std::string &) ;
    const std::string &getClientName() const noexcept ;
...
std::ostream &operator<<(std::ostream &, dbFileAction::CacheStatus) ;
acs::exostream &operator<<(acs::exostream &, dbFileAction::CacheStatus) ;

 ...
 */
extern SQLResult __stdcall (*getSQLDriver)(char * VendorLib, char * SResourceFile, ppSQLDriver); // NOSONAR - non const: hook to enter the plugins
 ...
 */
typedef SQLResult __stdcall (*getSQLDriverPrototype)(char * , char * , ppSQLDriver); // NOSONAR - old style
...
public:
	 ...
	 */
	void operator()(T *p) { if (p) p->Release() ; } // NOSONAR
} ;
...
public:
	 ...
	 * \param p : The pointer to be disposed (it is checked for null avoiding segmentation faults
	 */
	void operator()(SQLDriver *) {  /* driver can't be deleted */ } // NOSONAR - shall be as coded
} ;
...
template<class T>
inline
std::shared_ptr<T> dbSQLCreator(T *p) 
{
	return std::shared_ptr<T>(p, dbSQLDeleter<T>()) ;
}
...
std::string dbSQLGetError(T &t) 
{
	...
	unsigned char s[ std::max( (128*1024), elen+1) ] ; // NOSONAR - need long buffer
	memset(s, 0, sizeof(s)) ; 
	t->getErrorMessage(s) ; 
	const std::string es ( reinterpret_cast<const char*>(s) ) ;  // NOSONAR 
 ...
 */
inline
long dbSQLDriverCreator(getSQLDriverPrototype f,  // NOSONAR - shall be a function pointer
						const char *vlib, 
						const char *resf, 
						dbSQLDriver &d) 
...
{
	SQLDriver *p = nullptr ;
	int status = f(const_cast<char *>(vlib), const_cast<char *>(resf), &p) ; // NOSONAR
	d = dbSQLCreator(p);
	return status ;
}
 ...
 */
inline 
long dbSQLConnectionCreator(const dbSQLDriver &d, dbSQLConnection &c) 
{
	SQLConnection *p = nullptr ;
 ...
 */
inline 
long dbSQLMetaDataCreator(const dbSQLConnection &c, dbSQLMetaData &m) 
{
	SQLMetaData *p = nullptr ;
 ...
 */
inline 
long dbSQLCommandCreator(const dbSQLConnection &c, dbSQLCommand &cmd)   
{
	SQLCommand *p = nullptr ;
 ...
 * \return the completion code (SQL_SUCCESS means success) 
 */
inline long dbSQLCommandExecute(const dbSQLCommand &comm, dbSQLCursor &cur) 
{
	SQLCursor *p = nullptr ;
 ...
 * \return the completion code (SQL_SUCCESS means success) 
 */
inline long dbSQLCommandExecuteImmediate(const dbSQLCommand &comm, const char *sql, dbSQLCursor &cur) 
{
	SQLCursor *p = nullptr ;
	int status = comm->executeImmediate(const_cast<char *>(sql),&p) ;    // NOSONAR   
	cur = dbSQLCreator(p) ;
	return status ;
}
 ...
 * \return the completion code (SQL_SUCCESS means success) 
 */
inline long dbSQLMetaDataGetTables(const dbSQLMetaData &m, const char *pszTableName, UINT32 uTableType, dbSQLCursor &cur)  
{
	SQLCursor *p = nullptr ;
	int status = m->getTables ( const_cast<char *>(pszTableName),uTableType,&p) ; // NOSONAR   
	cur = dbSQLCreator(p) ;
	return status ;
}

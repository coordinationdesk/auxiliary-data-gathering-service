...
#include "dbScenePnt.h"


_ACS_BEGIN_NAMESPACE(acs)


dbScenePnt::dbScenePnt(const dbScenePnt &p) throw():
	_pixel(p._pixel),
	_line(p._line)
{
}

dbScenePnt::~dbScenePnt() throw() { }

dbScenePnt &dbScenePnt::operator=(const dbScenePnt &p) throw() 
{
	if (this != &p) {
		_pixel = p._pixel ;
		_line = p._line ;
...
bool operator==(const dbScenePnt &a, const dbScenePnt &b) throw() // PRQA S 4020
{
    if ((a.pixel() > b.pixel()) || (a.pixel() < b.pixel())) { return false ; }
    if ((a.line() > b.line()) || (a.line() < b.line())) { return false ; }
    return true ;
}

bool operator!=(const dbScenePnt &a, const dbScenePnt &b) throw()
{	
	return  not (a == b) ;
}

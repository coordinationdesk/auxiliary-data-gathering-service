...
public:
	...
	exFMessage(const exFMessage &) = delete ;
	exFMessage &operator=(const exFMessage &) = delete ;
	~exFMessage() override = default ;
	...
	// sets the details
	void detail(const std::string &) override ;

	// inherited from ostream 
	std::ostream &stream() override ;
	
	// it put the exception stack if any into the details
	bool __attribute__((deprecated("do not use this"))) notifyException(const std::exception &) noexcept override ;
	
	
	/*! Inherited from exFilterable, prevents appending of more messages after the class has been inserted into a stream.
	 */
	void insertionLock() override ;
	
	bool defaultNotify() const override ;

private:	
	std::unique_ptr<std::ostringstream>_os = nullptr ;
	bool _osLocked = false ;	
	std::string _details = "" ;
} ;

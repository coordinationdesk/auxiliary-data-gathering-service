...
public:
	...
	explicit RAIIGuard(R_ &r) : r_(r) { acquire() ; }
     ...
     */
	virtual ~RAIIGuard() noexcept(false) { // PRQA S 2045 # it is invoked expecting the exception // NOSONAR

#if __cplusplus >= 201703L
    int st_unwid_ = std::uncaught_exceptions() ;
#else
    bool st_unwid_ = std::uncaught_exception() ;
        ...
        try {
            ...
            if (! st_unwid_) { throw ; } // NOSONAR 
            std::cerr << exDebugFullSignature << explain(x) ; // NOSONAR (last chance to know)
        }
    } ;
     ...
     */
    void acquire() { if (! acquired_) { (r_.*Acq_)() ; acquired_ = true ; } } ;
...
private:
    ...
    bool acquired_ = false ; ///< the flag to remember the <i>acquired</i> status
};
...
public:
	...
	virtual ~ScopeGuard() noexcept(false) { // PRQA S 2045 # it is invoked expecting the exception // NOSONAR

#if __cplusplus >= 201703L
            // the st_unwind_ is > 0 if the stack is unwinding because 
...
#endif
            try {
                if (addr_) { Release_(addr_); } // PRQA S 4631 # can throw !!! // NOSONAR
            }
            catch(const std::exception &x) { // NOSONAR - any exception
                if (! st_unwid_) { throw ; } // no exception handling in progres it is safe to throw this one // NOSONAR
                std::cerr << exDebugFullSignature << explain(x) ; // NOSONAR - last chance to know
            }
        } 
    ...
    void detach() const noexcept { addr_ = 0 ; } 
private:
    mutable T_ addr_ ;
} ;
 ...
 *
 */
inline void free_mem_wrapper__(void *m) { std::free(m) ; } // NOSONAR - avoiding to use std::free directly 
using CMemoryScopeGuard = pattern::ScopeGuard<void*, &free_mem_wrapper__> ;
/*! 
 ...
 *  it is defined as 
 ...
 \endcode
*/
using Release_w_string = int (*)(std::string const &); /*! \brief convenience pointer to func typedef */
...
struct PathTraits {
	...
	static void releaseResource(std::string const &s) { T(s) ; } // PRQA S 2010
} ;
 ...
 *  it is defined as 
 ...
 \endcode
*/
// typedef int (*Release_w_int)(int ) ; 
using Release_w_int = int (*)(int); /*! \brief convenience pointer to func typedef */
...
public:    
    explicit AutoGuard(const T_ &v = Traits_::emptyValue() ) : guarded_(v) {}
    ~AutoGuard() noexcept { release() ; }
    AutoGuard(const AutoGuard &rhs) : guarded_(rhs.guarded_) { rhs.detach() ; }
    AutoGuard& operator=(const AutoGuard &rhs) {
        if (this != &rhs) {
            if ((guarded_ !=  Traits_::emptyValue()) && (guarded_ != rhs.guarded_)) { 
                release() ; 
            } 
    ...
    */
    void detach() const noexcept { guarded_ = Traits_::emptyValue() ; }

    /*! releases the resource if different from emptyValue()
    */
    void release() const noexcept { // PRQA S 4214
        if (guarded_ != Traits_::emptyValue()) { 
            try { Traits_::releaseResource(guarded_) ; } 
            catch(const std::exception &x) { ACS_LOG_NOTIFY_EX(x) ;}  // NOSONAR - any exception
            detach() ; 
        } 
    }
     ...
     */
    operator const T_ &() const { return guarded_ ; } // NOSONAR - implicit cast 
 ...
 * the above example shows how to automatically close an opened file when file_guard goes out of scope.
 */
using FDGuard = AutoGuard<int, FDTraits< &::close > >  ; // PRQA S 0228, 2502

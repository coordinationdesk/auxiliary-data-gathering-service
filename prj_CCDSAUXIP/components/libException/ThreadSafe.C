...
ThreadSafe::~ThreadSafe() noexcept 
{
    // cerr << SimpleDebugSignature << hex << this << dec << '\n' ;
	// attention, the exception handling is tricky
	int ret = ::pthread_mutex_destroy(&_mutex) ; // NOSONAR - false positive 
	bool oldTrace = exStackTrace::allow() ; // store previous trace mode 
	try { // PRQA S 4631 L1
		if (!OnExitDetector::instance()->exiting() && ret != 0) { // not exiting && was not able to destroy
	...
	try { // PRQA S 4631 L2
		if (!OnExitDetector::instance()->exiting() && ret != 0) {
            exStackTrace::allow(true) ; // trace mode true
...
void ThreadSafe::lock() const                           // PRQA S 4020, 4214
{
	...
	int ret = ::pthread_mutex_lock(&_mutex) ; // NOSONAR - recursive mutexes
...
bool ThreadSafe::unlock(bool noExIfNotOwner) const                         // PRQA S 4020, 4214
{
	...
	int ret = ::pthread_mutex_unlock(&_mutex) ; // NOSONAR - recursive mutexes need multiple unlock (one for each lock)
	switch(ret) {
	case 0:                                             // PRQA S 4066
...
{
    try {
	    _mutex.lock() ; // NOSONAR - recursive mutexes 
	    // if here the lock executes correctly
	    _locked = true ;
    }
    catch(const DeadLock &) { // NOSONAR 
...
ThreadSafe::Lock::operator bool() const noexcept // NOSONAR -- as for the standard streams
{
	return _locked ; 
}


bool ThreadSafe::Lock::operator()() const noexcept
{
	return _locked ;
}


bool ThreadSafe::Lock::locked() const noexcept // PRQA S 4120
{
	return _locked ;
}
...
ThreadSafe::TryLock::~TryLock() noexcept
{
	try {
		unlock() ; // PRQA S 4631
	}
...
ThreadSafe::TryLock::operator bool() const noexcept // NOSONAR -- as for the standard streams
{
	return _locked ; // NOSONAR - used only for bool expressions
}


bool ThreadSafe::TryLock::operator()() const noexcept
{
	return _locked ;
}


bool ThreadSafe::TryLock::locked() const noexcept // PRQA S 4120
{
	return _locked ;
}

void ThreadSafe::TryLock::unlock() const // PRQA S 4211
{
	if (_locked) {
		_mutex.unlock() ;
    }
...
Condition::~Condition() noexcept 
{
	...
	catch(const exception& e) // NOSONAR - no exception from here
	{
		ACS_LOG_NOTIFY_EX(e) ;
	} // PRQA L:L4
}
...
struct timespec& Condition::normalize(struct timespec& ts) noexcept
{
...
struct timespec Condition::normalize(const struct timespec& ts) noexcept
{
...
void Condition::broadcast(const bool& increase_counter, const bool& reset_counter, const bool& reset_timer) const // PRQA S 4214
{
	ThreadSafe::Lock lock (*this); // PRQA S 3050

	ACS_COND_THROW( (increase_counter && reset_counter), \
		ThreadSafe::Exception("Misleading parameters: both \"increase_counter\" and \"reset_counter\" are set !")) ; 

	if (increase_counter)	{ incrementEventCounter(); }
	if (reset_counter)		{ resetEventCounter(); }
...
uint64_t Condition::getTimer_ms () const
{
	ThreadSafe::Lock lock (*this); 

	struct timeval lapsed, stop ; 
	{
    	// Get the current time
    	struct timespec tp; 
    	const int callret ( ::clock_gettime (CLOCK_REALTIME, &tp) ) ;

    	// In case of issues gathering the system time throw an exception
    	ACS_COND_THROW(callret != 0, ThreadSafe::Exception("Unable to get system time", errno)) ; // PRQA S 3081

		stop.tv_sec  = tp.tv_sec ;
		stop.tv_usec = tp.tv_nsec /1000 ;
	}

	if (_timerFromLastSignal.tv_usec > stop.tv_usec) {
	    stop.tv_usec += 1000000 ;  // 1 sec more (1e06 usecs)         
	    stop.tv_sec-- ;
	}
	
	// deal with secs and fraction separately
	lapsed.tv_usec = stop.tv_usec - _timerFromLastSignal.tv_usec ;
	lapsed.tv_sec  = stop.tv_sec  - _timerFromLastSignal.tv_sec ;

	// remember the return value is in msec
	return (static_cast<uint64_t>(lapsed.tv_sec)*1000 + (lapsed.tv_usec/1000)) ;          
}


bool Condition::timerIsOlder_ms (const int64_t& value_ms) const
{
	const int64_t timer(getTimer_ms()) ; 
...
void Condition::resetTimer() const // PRQA S 4214
{
	ThreadSafe::Lock lock (*this); // PRQA S 3050
	
	{
		...
		_timerFromLastSignal.tv_sec = tp.tv_sec ;
		_timerFromLastSignal.tv_usec = tp.tv_nsec /1000 ;
	}
}
    ...
    _read(read)
{
    ...
    catch(const DeadLock &x) { // NOSONAR 
...
RWMutex::BaseRWLock::~BaseRWLock() noexcept
{
    // PRQA S 4631 L3
	try { // no exception out of this method
		if (_locked) {
...
RWMutex::TryLockBase::~TryLockBase() noexcept
{
	try {
		unlock() ; // PRQA S 4631
	}
...
RWMutex::TryLockBase::operator bool() const  // NOSONAR -- as for the standard streams
{
	return _locked ;  
}
...
void RWMutex::TryLockBase::unlock() const // PRQA S 4211
{
	if (_locked) {
		_mutex.unlock() ;
    }

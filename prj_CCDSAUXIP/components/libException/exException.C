...
namespace {
    std::string str_error_r(error_t eno) noexcept // PRQA S 4020
    {
        std::array<char, 1024> errs; // NOSONAR - buffer is needed
        errs[0] = 0 ;
# if defined __USE_XOPEN2K && !defined __USE_GNU
        int ret = strerror_r(eno, errs.begin(), errs.size()) ;
        if (0 == ret) {
            return errs ;
        }
# else
        char const *ret = strerror_r(eno, errs.begin(), errs.size()) ;
        if (ret) {
            return ret ; 
        }
# endif
...
exException::exException(error_t eno) noexcept :
    exception(),
    _threadId(pthread_self()), // NOSONAR - dynamic for each thread
    _errno(eno)
{
    _stack = exStackTrace::getStack() ;
}

exException::exException(const std::exception &x, error_t eno) noexcept :
    exception(),
    _threadId(pthread_self()), // NOSONAR - dynamic for each thread
    _errno(eno),
    _causedBy(x.what())
{
    try {
        // get the stack trace 
        _stack = exStackTrace::getStack() ;
        auto xx_ = dynamic_cast<const exException *>(&x) ; // PRQA S 3081
        if (xx_) { 
            _stack += "\n\tthrown by:\n\t" + xx_->_stack ;
        }
    }
    catch(const std::exception &azz_) { // NOSONAR - any exception 
        std::cerr << explain(azz_) ; // NOSONAR - can't use streams here
    }
}

exException::exException(const std::string &description, error_t eno) noexcept :
    exception(),
    _threadId(pthread_self()), // NOSONAR - dynamic for each thread
    _errno(eno)
{
...
exException::exException(const std::exception &x, const std::string &description, error_t eno) noexcept :
    exception(),
    _threadId(pthread_self()), // NOSONAR - dynamic for each thread
    _errno(eno),
    _causedBy(x.what())
...
exException::exException(const exException &x) noexcept :
    exception(x),
    _file(x._file),
    _line(x._line),
...
exException::exException(exException &&x) noexcept :
    exception(x), // 
    _descr(std::move(x._descr)),
    _file(std::move(x._file)),
    _line(x._line), // no move on builtin
    _function(std::move(x._function)),
    _thread(std::move(x._thread)),
    _threadId(x._threadId), // no move on builtin
    _text(std::move(x._text)),
    _stack(std::move(x._stack)),
    _errno(x._errno), // no move on builtin
    _causedBy(std::move(x._causedBy))
{
}


void exException::debugInfo(std::string const& file, std::string const& function, int line) const noexcept {
    try { _file = file;} catch (const std::exception &e) { std::cerr << explain(e) ; } // NOSONAR - can use only cerr
    try { _function = function;} catch (const std::exception &e) { std::cerr << explain(e) ; } // NOSONAR - can use only cerr
    try { _line = line;} catch (const std::exception &e) { std::cerr << explain(e) ; } // NOSONAR - can use only cerr
}
...
{
    auto cloned = clone() ; // clone it
    
    auto x = dynamic_cast<exException *>(cloned) ; // PRQA S 3081
...
const char* exException::what() const noexcept
{
    try {
    ...
    catch(const std::exception &azz_) {
        std::cerr << explain(azz_) ; // NOSONAR - cant use streams
    }
...
std::string exException::computeDescr() const noexcept
{
    try
    {
        std::ostringstream os;
...
namespace {
_ExplainEx explain_internal(const std::exception &e_, bool s_) noexcept
{ 
  _ExplainEx __x; 
  __x._M_s.reserve(256) ; // PRQA S 4400
  __x._M_s = e_.what() ; // \n added at the end

  const exException *x_ = nullptr ;
...
_ExplainEx explain(const std::exception &e_) noexcept
{ 
    return explain_internal(e_, true) ;
}

_ExplainEx explain(const std::exception &e_, bool stack_) noexcept
{ 
    return explain_internal(e_, stack_) ;
}

std::ostream &operator<<(std::ostream &s_, const _ExplainEx &e_) noexcept
{

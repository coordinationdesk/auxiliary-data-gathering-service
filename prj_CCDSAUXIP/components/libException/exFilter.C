...
exFilter::exFilter(exostream *s) :
	_filteredStream(s)
{
	if (nullptr != _filteredStream) {
		_filteredStream->installFilter(this) ;
    }
}

exFilter::~exFilter() noexcept
{
	...
	catch(const std::exception &x) { // PRQA S 4631 3
		std::cerr << explain(x) ; // NOSONAR - cannot use streams
	}
}

exFilter::exFilter(const exFilter &) : _filteredStream(nullptr)
{
}
...
void exFilter::streamDestroying(exostream const *s) noexcept 
{
	if (s == _filteredStream) {
		_filteredStream=nullptr ;
    }
}

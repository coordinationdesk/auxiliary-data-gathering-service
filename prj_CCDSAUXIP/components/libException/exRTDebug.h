...
public:
	using _v_ptr = int *  ;
	using _m_type = std::unordered_multimap<std::string,  _v_ptr >  ;
	
	void registerDebug(const std::string &, _v_ptr, const std::string &src) noexcept ;
	void registerDebug(const std::string &, _v_ptr) noexcept ;
    ...
    */
    ...
    /*! just a persistence for a string (get) */
	const std::string& mode() const noexcept { return _mode ; }
    /*! just a persistence for a string (set) */
...
private:
    ...
    */
    ...
    int get_max(const std::string &) ;

private: // NOSONAR - for clarity	
	_m_type map_ = {} ;
	std::string _mode = "" ;
	std::unordered_map<std::string, std::string> classlinks_ = {} ;

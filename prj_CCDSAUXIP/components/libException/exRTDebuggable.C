...
void registerDebug(const std::string &ref_, int *_debugLevel) noexcept
{
    exRTDebug::instance()->registerDebug(ref_, _debugLevel) ;
}

void registerDebug(const std::string &ref_, int *_debugLevel, const std::string &myself_ref_) noexcept
{
...
exRTDebuggable::~exRTDebuggable() noexcept 
{
	// _debugRefString was obtained by strdup --> malloc hence free and not delete
	free(_debugRefString) ; // NOSONAR
	_debugRefString = nullptr ;
}
// PRQA L:L1


const char *exRTDebuggable::getDebugRef() const noexcept // PRQA S 4120
{
	return _debugRefString ;
}


int exRTDebuggable::getDebug() const noexcept // PRQA S 4120
{
	return _debugLevel ;
}

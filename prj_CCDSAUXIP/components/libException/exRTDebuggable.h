...
public:
	...
	explicit  __attribute__((deprecated("Use the newer RTDebuggable class"))) exRTDebuggable(const std::string &, int startValue=0) ; 
	~exRTDebuggable() noexcept ;
	
	void setDebug(int v ) noexcept { _debugLevel = v ; } 

	int getDebug() const noexcept ; // inlined for perfo
	const char * getDebugRef() const noexcept ;
	int *get_ref() noexcept { return &_debugLevel ; } // PRQA S 4024    
private:	
	int _debugLevel ;
	char *_debugRefString = nullptr ;
} ;


// all the inline template functions to deal with debug (int or exRTDebuggable)
// PRQA S 2134 L1
template <typename T> inline int asInt(const T &d) noexcept { return d.getDebug() ; }
template <> inline int asInt(const int &d) noexcept { return d ; } // PRQA S 2620

template <typename T> inline void setInt(T &d, int v) noexcept { d.setDebug(v) ; }  
template <> inline void setInt(int &d, int v) noexcept { d = v ; }  
// PRQA L:L1
...
public:
    ...
    /*! this ctor is used to link debug symbols. after linking, 
        the debug value is set to the maximum value assigned to */
	explicit RTDebuggable(const std::string &dbg_string, int start=0) noexcept ;	 
	explicit RTDebuggable(int start=0) noexcept ;	 
	~RTDebuggable() = default ;
	 ...
	 */
	operator const int &() const noexcept { return _debugLevel ; } // NOSONAR - implicit conversion

	/*! \brief set the debug level */
	void setDebug(int v) noexcept { _debugLevel = v ; }

	/*! \brief returns the debug level */
	int getDebug() const noexcept { return _debugLevel ; }
...
void registerDebug(const std::string &, int *) noexcept ;
void registerDebug(const std::string &, int *, const std::string &) noexcept ;
		

template<typename T_D_>
RTDebuggable<T_D_>::RTDebuggable(const std::string &dbg_string, int start) noexcept :
	_debugLevel(start)
{
	auto myself_ref_ = std::string(strings::RTTr(exStackTrace::demangle(typeid(T_D_).name()))) ; // PRQA S 3050, 3081
	if (0 == _debugLevel) { strings::RTTr::envget_(dbg_string, _debugLevel) ; }
	registerDebug(dbg_string, &_debugLevel, myself_ref_) ;
}

template<typename T_D_>
RTDebuggable<T_D_>::RTDebuggable(int start) noexcept :
	_debugLevel(start)
{
	auto ref_ = std::string(strings::RTTr(exStackTrace::demangle(typeid(T_D_).name()))) ; // PRQA S 2839, 3050, 3081
...
#include <exException.h> // helper to users // NOSONAR - must be here

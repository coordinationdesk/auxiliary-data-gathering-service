...
// Static attributes
Mutex exStackTrace::_threadListMutex;
map<pthread_t, string> exStackTrace::_threadList;

//Thread list management
void exStackTrace::addLiveThreadId (pthread_t id, const string& name) noexcept
{
...
void exStackTrace::removeLiveThreadId (pthread_t id) noexcept
{
	Lock lock (_threadListMutex);
	// this will never fail
...
map<pthread_t, string> exStackTrace::getLiveThreadIdSet () noexcept
{
	Lock lock (_threadListMutex);
	return _threadList;
}
...
void exStackTrace::allow(bool v)  noexcept 
{
    _allow = v ;
}

bool exStackTrace::allow() noexcept 
{
	return _allow ;
}

string exStackTrace::demangle(const char* name) noexcept // PRQA S 4020
{
	...
	realname = abi::__cxa_demangle(name, realname, &len, &stat); // realname is reallocated
    pattern::CMemoryScopeGuard rn_scope(realname) ; // and freed when no longer needed
...
string exStackTrace::stackLineBeauty(const char* cline) noexcept // PRQA S 4020
{
	try {
		if ((nullptr == cline) || ('\0' == *cline)) { // PRQA S 3051
	...
	} catch(const std::exception &x_) { // NOSONAR - no exceptions from here 
		std::cerr << SimpleDebugSignature << explain(x_) ; // NOSONAR - cannot use other streams
	}
	
	return cline ;
}

string exStackTrace::getStack(bool force) noexcept // PRQA S 4020
{
	if (_allow || force) { // shoud I notify the stack ?
		try {
			// allocate memory and get the backtrace 
			std::array<void *,backtraceElements> array; 
			int size = ::backtrace(&array[0], array.size()); // PRQA S 3000,3084
			char ** strings = ::backtrace_symbols(&array[0], size); // PRQA S 2425 # no way to avoid two levels
			...
			// mostly ignored write just to know
			std::cerr << explain(x) ; // NOSONAR
		}
	}
	return "";

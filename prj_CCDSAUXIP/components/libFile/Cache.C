/*
	...
	Copyright (C) 2010-2022, Exprivia SpA - DFDA-AS
	...
	EC:: added noexcept to prevent dtor launching exceptions
...
//Use this for timestamping: #define CACHE_CLASS_WRITE_DEBUG_TH(level, message) ACS_CLASS_WRITE_DEBUG_TH(level, to_iso_extended_string(boost::posix_time::microsec_clock::universal_time()) << " " << message) 
...
Cache::AutoRelease::AutoRelease(Cache* pcache, const std::string &filename) : _pcache(pcache), _filename(filename) 
{
	...
	ACS_COND_THROW( !_pcache , exIOException( "NULL cache ptr!" ) ) ; 
	ACS_COND_THROW( _filename.empty() , exIOException( "NULL filename!" ) ) ;

	ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "cache ptr: " << static_cast<void*>(_pcache) <<  "  cache path: \"" <<  _pcache->getPath() << "\""   
		  << "  filename: \"" << _filename << "\"") ;
}
...
//
Cache::AutoRelease::~AutoRelease() noexcept  
{
...
//
void Cache::AutoRelease::release() const 
{	
...
namespace {
    ...
    {
	    auto output_facet = new boost::posix_time::time_facet("%Y-%m-%d %T.%f"); // NOSONAR - the locale will get rid of this

	    std::ostringstream os ;
	    os.imbue(std::locale(std::locale::classic(), output_facet));
...
//
Cache::ReserveSpace::ReserveSpace(const string &path, off_t bytes) : 
    _path(path), 
    _bytes(bytes)
{
...
//
Cache::ReserveSpace::~ReserveSpace() noexcept
{
...
void Cache::ReserveSpace::stillReserve(off_t bytes)
{
	...
	ACS_COND_THROW( _absFileName.empty(), exIOException( "Empty filename: \"" +_absFileName+"\"" ) ) ; 
...
//
void Cache::ReserveSpace::release() const 
{	
...
void Cache::ReserveSpace::readReservation(
	...
	bool& bexpdate) 
{
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called for file: \"" << fileName << "\"  useRelFileName: " << std::boolalpha << useRelFileName) ;
	...
	string absFileName = (useRelFileName ? 
	...
	if (!fileName.empty() && File::exists(absFileName))	/* Not a typo: check if filename!="" AND check the existence of ABSFileName */
	{
		int fd = open(absFileName.c_str(), O_RDONLY) ; if (fd >= 0) {
			...
			if (success)
			{
                ...
                success = (r == sizeof(bufr)) ; 
				if (success) { 
                    ACS_ByteSwap_SWAP(&bufr,sizeof(bufr),storedBigEnd) ; 
                    bytes = off_t(bufr) ; 
                } 
                bbytes = success ; 
			}
		...
		{	
			CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Couldn't open file: \"" << fileName << "\"  useRelFileName: " << std::boolalpha << useRelFileName) ;
		}	// END if (fd >= 0)
	
	}	
	else
	{
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Couldn't file file: \"" << fileName << "\"  useRelFileName: " << std::boolalpha << useRelFileName) ;
	}	// END if (!fileName.empty() && File::exist(absFileName))


	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		std::ostringstream o ; 
		o << "Method finished for file: \"" << fileName << "\"  useRelFileName: " << std::boolalpha << useRelFileName ;
...
{	
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Method finished for file: \"" << fileName << "\"  useRelFileName: "  << std::boolalpha << useRelFileName 
		  << "  returning: " << status) ;
	...
	bool useRelFileName) 
{
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Returning: " << std::boolalpha << status) ;
...
{
/*	
	Unique filename random pattern is: .r_sssssssssssssssss.reserve
		sssssssssssssssss:  17 chars [a-z] random string
		Total length: 3+17+8 = 28 bytes
*/

	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called for cachePath: \"" << cachePath << "\"  useRelFileName: " 
			<< std::boolalpha << useRelFileName) ;
    ...
    for (size_t i=0; ((!success) && (i<4096)); ++i) 
	{
		std::array<char, 32> rnd_buf ; 
        rnd_buf.fill(0) ;
		
		// Generate a random buffer of 17 chars, in range 'a' to 'z'. Example: "xjrkgqqywcqfgajll"
		// Entropy: ~ 5*17 = 85 bits. Collisions: 1 over 2^85 
		ACS_SSL::ACS_RAND_bytes(reinterpret_cast<unsigned char*>(rnd_buf.data()), 17, pair <unsigned char, unsigned char>('a','z')) ;	

		relFileName = ".r_" + string(rnd_buf.data()) + ".reserve" ; // Example: ".r_ouuslmebpxugcknfq.reserve" (28 chars).

		fileName = StringUtils::pathJoin(cachePath, Cache::_reserveSpaceRelDir, relFileName) ;
		int fd = open (fileName.c_str(), O_CREAT|O_EXCL, 0666) ; // NOSONAR - EXCLUSIVE CREATION MODE: will fail if file already exists !
		success = (fd>=0) ; 
        if (success) { 
            if  ( close(fd) != 0 ) 
	...
	ACS_COND_THROW( !success, exIllegalValueException( "Error creating file") ) ; 

	string ret = useRelFileName ? relFileName : fileName ;
										...
										time_t expdate)	/* payload valid only if (expdate>0). o/w expiration is omitted */
{
	...
	ACS_COND_THROW( absFileName.empty(), exIOException( "Empty filename: \"" +absFileName+"\"" ) ) ; 

	int fd = open (absFileName.c_str(), O_WRONLY|O_CREAT|O_TRUNC, 0666) ;  // NOSONAR - internal cache no sec violation
	ACS_COND_THROW( (fd<0), exIOException( "Error opening file: \"" +absFileName+"\"" ) ) ; 
	FDGuard g(fd) ;	// self close(fd) 

	const int storeBigEnd = 1 ;

	{ // NOSONAR - Write payload: bytes
		unsigned long long int bufr = bytes ; 
		ACS_ByteSwap_SWAP(&bufr, sizeof(bufr), storeBigEnd) ;  
		ssize_t w = write(fd, &bufr, sizeof(bufr)) ;
		ACS_COND_THROW( (w!=sizeof(bufr)) , exIOException( "Error writing file: \"" +absFileName+"\"" ) ) ; 
	}
	
	if (expdate > 0)	// Write payload: expdate
	{
		unsigned long long int bufr = expdate ; 
		ACS_ByteSwap_SWAP(&bufr, sizeof(bufr), storeBigEnd) ;   
		ssize_t w = write(fd, &bufr, sizeof(bufr)) ;
		ACS_COND_THROW( (w!=sizeof(bufr)) , exIOException( "Error writing file: \"" +absFileName+"\"" ) ) ; 
	}
...
string Cache::ReserveSpace::createReservation(const string& cachePath, bool useRelFileName, off_t bytes, time_t expdate)
{
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
        ...
        ptime d = from_time_t(expdate) ;
        ptime n = from_time_t(now) ;
		std::ostringstream o ; o << "Method called for cachePath: \"" << cachePath << "\"  bytes: " << bytes
			<< "  expiration: " << expdate << "  [" << ptime2s(d) << "]   ( now: " << now << "  [" << ptime2s(n) << "] )" 
			<< "  useRelFileName: " << std::boolalpha << useRelFileName ;  
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, o.str()) ;
	ACS_CLASS_END_DEBUG

	ACS_COND_THROW( (bytes<0) , exIOException( "Occupation cannot be NEGATIVE" ) ) ; 

	string absFileName ;
	string fileName = ReserveSpace::createUniqueFileName( cachePath, useRelFileName ) ;	/* Get unique filename */

	absFileName = (useRelFileName ? 
		(StringUtils::pathJoin(cachePath, Cache::_reserveSpaceRelDir, fileName)) : 
		fileName ) ;
...
void Cache::ReserveSpace::checkAndCreateReservationDir(const string& cachePath)
{
	...
	ACS_COND_THROW( !(File::exists(cachePath) && File::isDir(cachePath)),  
	...
	ACS_COND_THROW( (File::exists(reserveSpaceAbsDir) && File::isRegularFile(reserveSpaceAbsDir)),  
	...
	{
		::mkdir(reserveSpaceAbsDir.c_str(), 0777) ;	// NOSONAR - Ignore errors (directory might exist, etc. -- A test will be run below) 
		FileGuard tmpName = File::tempname( StringUtils::pathJoin(reserveSpaceAbsDir, "tempfile."), "") ;/* Will be self-removed */
		File::touch(tmpName) ;	// Try to create a test entry. O/W: EXCEPTION ! 
	}
...
Cache::Entry::Entry() : 
    Lock("", false), 
    _mtime(0), 
    _tryLockTimeout(Cache::_tryLockDefaultTimeout)
{
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_VLO_VERB) ;
}
	...
	_tryLockTimeout(tryLockTimeout)
{
	...
	ACS_COND_THROW( !regexp.match(lockFileAbsPath), exIOException("Invalid lockfile: " + filename()) ) ; 
    ...
    _tryLockTimeout(rhs._tryLockTimeout)	
{
	...
	ACS_COND_THROW(rhs.isLocked(), 
		exIllegalValueException(rhs.filename() + " is alredy locked")) ;
	ACS_COND_THROW(rhs._reserveSpace, 
...
//
Cache::Entry& Cache::Entry::operator=(const Entry &rhs) 
{
...
std::string Cache::Entry::debugString() const 
{
	...
	s << "Entry: \"" << filename() << "\"  useCounter: " << _useCount 
	  ...
	  << "  reservedSpace[Ptr]: " << _reserveSpace.get() << "  tryLockTimeout[s]: " << _tryLockTimeout ;
	return s.str() ;
}
...
void Cache::Entry::updateStat() 
{
	...
	try {
		...
		else
		{
			...
			CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Entry: \"" << entry << "\"  is a " << (File::isDir(entry)? "direct." : "file") 
					   << "  size: " << _bytes << " bytes") ;
		}
	} 
	catch(const std::exception &e)  // NOSONAR - any exception
...
//
size_t Cache::Entry::useCount() const noexcept
{
	return _useCount ;
}
...
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "lock type: " << (shared? "SH" : "EX" ) << "  [" << Cache::lockTypeToString(type) << "]"  
		  << "  Filename: \"" << filename() << "\"  isLocked: " << std::boolalpha << isLocked() <<"  tryLockTimeout: " << _tryLockTimeout ) ;

	if (! isLocked())	
	{
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "lock type: " << (shared? "SH" : "EX" ) << "  [" << Cache::lockTypeToString(type)   
				<< "]  Trying to lock... (isLocked: " << std::boolalpha << isLocked() ) ;
		...
		try {
			...
			ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Trying to aquire lock. Type: " << (shared? "SH" : "EX" ) << "  Attempts: " << attempts << "  Timeout per each [ms]: " << attemptTimeout ) ; 
			tryLock(*this, attempts, attemptTimeout, shared) ;	// flock the lockfile 
		}
		catch(const exIOException &e)	/* Since exceptions due to lock-denial belong to this type */
		{
			std::ostringstream o ;
			o << "Couldn't lock entry (" << __LINE__ << ")  Absolute Path: \"" << filename() << "\"" ; 
			throwExceptionFromFile (&e, o.str()) ; 	/* Forge a specific lock-denial exception */	 
		}
		catch(const std::exception &e)  // NOSONAR - any exception
		{
	...
	else if ( (!shared) || (!_shared) )
	{
		...
		throwExceptionFromFile (nullptr, o.str()) ; 		
	}
    else { /* nothing */ }
	...
	Mutex::Lock lock(_mutex) ;	// since counter is about to be modified
	_shared = shared ;
	_useCount ++ ;
...
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Bytes to reserve: " << bytes << "  Filename: \"" << filename() << "\"  isLocked: " << std::boolalpha << isLocked() 
			<< "  lock type: " << (shared? "SHARED": "EXCLUSIVE")) ;

	ACS_COND_THROW(_reserveSpace, 
	...
	Mutex::Lock lock(_mutex) ;	// since shared variable is about to be modified

	if (bytes>0) {
		_reserveSpace = std::make_unique<Cache::ReserveSpace>(absPathOfCache, bytes) ;
    }
	else {
		_reserveSpace.reset() ;
    }
...
//
void Cache::Entry::reservedSpace(string& reserveFileAbsPath, off_t& bytes) const
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "isLocked: " << std::boolalpha << isLocked()) ;

	Mutex::Lock lock(_mutex) ;	// since shared variable is about to be modified

	ACS_COND_THROW( !isLocked(),  
...
//
void Cache::Entry::updateReservedSpace(const off_t& new_bytes) 
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "isLocked: " << std::boolalpha << isLocked() << "  new_bytes: " << new_bytes) ;
...
//  Cache::Entry::release
//
size_t Cache::Entry::release(bool force) 
{	
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called.  Filename: \"" << filename() << "\"  Force: " << std::boolalpha << force) ;
	...
	if (!force)
	{
		...
		{ // NOSONAR - lock scope shorter as possible
			Mutex::Lock lock(_mutex) ;	// since counter is about to be modified
			if (_useCount >= 0)
			{
...
//  Cache::Entry::touch
//
void Cache::Entry::touch() // NOSONAR - non const: touches a file
{	
...
*/
bool Cache::Entry::isErasable() const 
{
	...
	{	
		...
		if (s.op == Cache::PREV_AUTOERASE)
		{
			...
			ACS_CLASS_BGN_DEBUG(ACS_HIG_VERB)
                auto expdate = time_t(s.expdate) ; 
			    ptime n=from_time_t(now) ;
                ptime d=from_time_t(expdate) ; 
                string r = ((expdate<0)? "NEGATIVE": ptime2s(d)) ; 
				ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB, "Found operation: " << std::hex << int(s.op) << std::dec << "  expdate: " << expdate << " (" << r 
				  << ")  now: " << now << " (" << ptime2s(n) << ")  isErasable: " << std::boolalpha << e) ;
			ACS_CLASS_END_DEBUG
		}
		else
		{
			ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB, "Skipped operation: " << std::hex << int(s.op)) ; 
		}
	}	// for


	ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Method finished.  Filename: \"" << filename() << "\"  Returning: " << std::boolalpha << isErasable) ;
...
struct same_oper { 
    ...
    bool operator() (const Cache::LockOperDateStruct &i) const { return i.op == op_ ; } 
private:
    Cache::LockOper op_ ;
} ;
...
struct match_remove_vector 
{
	explicit match_remove_vector(const std::vector<Cache::LockOperDateStruct> &v) : _v(v) {} ; 

	bool operator() (const Cache::LockOperDateStruct& s) const { 
        return find_if(_v.begin(), _v.end(), same_oper(s.op)) != _v.end() ; 
    }
...
//
void Cache::Entry::setLockfilePayload(Cache::LockType t, 
                                      ...
                                      bool reset) const
{
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		...
		if (!reset) { 
			...
			if (! v_add.empty()) { Cache::dumpLockOperDateStructVector(v_add) ; }
			if (! v_remove.empty()) { Cache::dumpLockOperDateStructVector(v_remove) ; }
		} else {
			o << "  reset: " << std::boolalpha << reset ; 
		}
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, o.str()) ;
	ACS_CLASS_END_DEBUG
    ...
    std::array<unsigned char, 64*1024> buf_in ;
    ssize_t bufsize = 0 ;
    ...
    while (retries-- > 0)
	{
		...
		if (!reset)
		{
			...
			auto discarded = remove_if(v_current.begin(), v_current.end(), match_remove_vector(tmp_remove)) ; 
		...
		write_payload = ! v_current.empty() ;	// Payload is written only if needed
		...
		try
		{
			...
			if (create_file)
			{
                ...
                if (write_payload)
				{
					auto c = static_cast<unsigned char>(lockTypeToInt(t) & int(0xff)) ; 
					memcpy(buffer.data(), &c, sizeof(c)) ;	// Set locktype
                    ...
                    for (size_t i=0; i<v_c_size; ++i)
					{
						...
						ACS_ByteSwap_SWAP(&oper.op, sizeof(oper.op), storeBigEnd) ; 
						ACS_ByteSwap_SWAP(&oper.expdate, sizeof(oper.expdate), storeBigEnd) ; 
						memcpy(buffer.data()+bufsize, &oper, sizeof(oper)) ;	// Set lockoperdate
						bufsize += sizeof(oper)	; 
					}
				...
				if (fd>=0)
				{
					...
					if (bufsize>0)
					{
						w = write(fd, buffer.data(), bufsize) ; 
						success = (w == bufsize) ;		
					}
					else {
						success = true ;
				...
				ACS_COND_THROW(!success, CacheLockPayloadException() <<"Couldn't set payload for file: \"" + filename() + "\"  fd: " << fd << "  wrote: " << w << " / " << bufsize ) ; 
				...
				if (fd>=0)
				{
					...
					r = read(fd, buf_in.data(), buf_in.size()) ;
					success = (r == bufsize) ;	

					if (success && (bufsize>0) ) {
						success = ( memcmp( buffer.data(), buf_in.data(), bufsize) == 0 ) ; 
                    }

				}	/* if (fd>=0) */
				
				ACS_COND_THROW(!success, CacheLockPayloadException() <<"Couldn't verify payload for file: \"" + filename() + "\"  fd: " << fd << "  read: " << r << " / " << bufsize ) ; 

			}	/* if (create_file) */
			
			break ; // quit loop on retries
		}
		catch(const CacheLockPayloadException&)
		{
			if (retries <= 0) {
				throw ;
            }
		...
		// retrhow any other exception
        
	}	// while (retries-- > 0) ...
	
	
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		std::ostringstream o ; o << "Method finished.  Filename: \"" << filename() << "\"" ;
...
void Cache::Entry::getLockfilePayload(Cache::LockType& t, vector<Cache::LockOperDateStruct>& v) const
{
	...
	try
	{
		ACS_COND_THROW(! File::exists(filename()), CacheLockPayloadException("Lockfile not found: \"" + filename() + "\"")) ; 
        ...
        while (retries-- > 0)
		{
			...
			if (success)
			{
				if (bufsize>0)
				{
					...
					t = intToLockType(int(buffer[0]), emitException) ; 
					bufread = 1 ;

					while ( (bufread+ssize_t(sizeof(LockOperDateStruct))) <= bufsize) 
					{
						LockOperDateStruct oper ; 
						memcpy(&oper, buffer.data()+bufread, sizeof(LockOperDateStruct)) ;
						bufread += sizeof(LockOperDateStruct) ; 
			...
			{
				ACS_COND_THROW(0 == retries, CacheLockPayloadException("Retries finished")) ; 
				/* ELSE: still retries to do */
                std::this_thread::sleep_for (std::chrono::milliseconds(100)) ;
	...
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		...
		if (! v.empty()) { Cache::dumpLockOperDateStructVector(v) ; }
	ACS_CLASS_END_DEBUG
}
...
//
[[noreturn]] void Cache::Entry::throwExceptionFromFile( const std::exception *x, const string& msg_in) const
{	
	Cache::LockType l ; 
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Filename: \"" << filename() 
			   	<< "\"  Lock Type: " << std::hex << Cache::lockTypeToInt(l) << std::dec << "  [" << lockTypeToString(l) << "]")  ;

	switch(l)
	{
    case EX_WRITE:
        if (x) { ACS_THROW( CacheExWriteLockException( *x, msg_in ) ) ; }
        else { ACS_THROW( CacheExWriteLockException( msg_in ) ) ; }
    case EX_ERASE:
        if (x) { ACS_THROW( CacheExEraseLockException( *x, msg_in ) ) ; }
        else { ACS_THROW( CacheExEraseLockException( msg_in ) ) ; }
    default:
        if (x) { ACS_THROW( CacheLockException( *x, msg_in ) ) ; }
        else { ACS_THROW( CacheLockException( msg_in ) ) ; }
	}	// switch(l)
}
...
Cache::Cache(const string &path, off_t maxSize, double eraseQuotaPercentage, bool updateCacheEveryLock) : 
	...
	_maxSize(maxSize)
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "path: \"" << _path << "\"  maxSize: " << maxSize << "  updateCacheEveryLock: " << std::boolalpha << _updateCacheEveryLock) ;
		
	memset(&_lastUpdate, 0, sizeof(_lastUpdate)) ;

	// Check if valid cache was provided :
	ACS_COND_THROW( (!(File::exists(_path) && File::isDir(_path) && File::isExecutable(_path) && File::isReadable(_path))),  
	...
	if ((_eraseQuotaPercentage<-0.001) || (_eraseQuotaPercentage>1.001)) { 
		ACS_THROW( exIllegalValueException() << "Invalid eraseQuotaPcg: \"" << _eraseQuotaPercentage << "\"  Out of valid range: [ 0.00 - 1.00 ]") ; 
	}
	...
	const off_t freeDisk = File::getFreeSpaceInMB(_path) * megabyte_ ; 
	const off_t bytesInCache = bytes() ;
	const double perc = (_maxSize>0)? ( (bytesInCache * 100.0) / (_maxSize * 1.0) ) : 100.0 ; // NOSONAR - precision is not important
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Cache successfully instantiated on: \"" << _path << "\" Cache total size: " << _maxSize << " bytes  -  containing " 
			<< size() << " entries = " << bytesInCache << " bytes  (full: " << std::setprecision(3) << perc << "\045)   logical free space: " 
			<< (_maxSize-bytesInCache) << "  free space on filesystem: " << freeDisk << "  Irregular entries fixed: " 
			<< std::boolalpha << updated) ;


	// Check for available space (a warning might be issued)
	if ( (_maxSize-bytesInCache) > freeDisk) 
	{
		ptime d(boost::posix_time::microsec_clock::universal_time()) ;
		ACS_LOG_WARNING("WARNING  [" << ptime2s(d) << 
                        "]   Possible cache misconfiguration:  Cache quota (total size): " << _maxSize 
                        << "  -  bytes in cache: " <<  bytesInCache 
                        << "  --  Logical free space: " << (_maxSize-bytesInCache) 
                        << " LOWER THAN free space on filesystem: " << freeDisk) ; 
	}
 	...
 	for( auto &entry : _entries) { 
		try {
			if (entry.second.isLocked())
			{
				...
				entry.second.release(force) ; // forced release 
			}
		}
		catch(const std::exception &) { // NOSONAR - no exception from here
            ; // nothing
		}
	}	// for
...
bool Cache::setInitHighAccuracy()  
{
	...
	// HIGH ACCURACY : Manage irregular entries (i.e. without a matching .lock file) 
 	...
 	catch (std::exception& ex) // NOSONAR - any exception
 	{
...
void Cache::evaluateLowAccuracyBytes()  
{
	...
	ACS_COND_THROW( (_lowAccuracy == false), exIllegalValueException( "INTERNAL ERROR: DO NOT CALL evaluateLowAccuracyBytes" ) ) ; 

	// Compute the ACTUAL value of the Cache occupation, in bytes
	_lowAccuracyBytes = (1048576.0 * File::getDirectoryOccupationInMB(_path)) ; // NOSONAR - low accuracy 
	...
	{ // NOSONAR - lock 
		...
		for (const auto &entry : _entries ) { 
			tot+= entry.second.getBytes() ; // wrong qa warning off_t is 64 bit 
    	}
		
		_lowAccuracyBytes = ( (_lowAccuracyBytes>tot)? (_lowAccuracyBytes-tot) : 0) ; 
	}
...
//
//////////////////////////////////////////////////////////////////////////////////////////
bool Cache::manageCacheIrregularEntries()  
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called." ) ;
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Cache updated: " << std::boolalpha << updated) ;
...
//
//////////////////////////////////////////////////////////////////////////////////////////
bool Cache::addLockExtensionToEntries(const string& entry, bool isMainEntry) 
{
	ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Method called.  entry: \"" << entry << "\"  isMainEntry: " << std::boolalpha << isMainEntry) ;
	...
	else if ( File::isDir(entry) )
	{
		if ( isMainEntry || !File::exists(entry_lock) )
		{
			...
			for (size_t i=0; i<d.size(); i++)
			{
				...
				bool newIsMainEntryFlag = (isMainEntry && !File::isDir(fullEntry))? true : false ;  
				
				ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Adding entry: \"" << fullEntry << "\"  Type: " << (File::isDir(fullEntry)?"DIR":"FILE")  
						<< "  newIsMainEntryFlag: " << std::boolalpha << newIsMainEntryFlag) ;
...
//
//////////////////////////////////////////////////////////////////////////////////////////
bool Cache::removeOrphanLockOlderThanDays(const string& relPath, const ssize_t& days) 
{
	...
	if ( isEntryDir )
	{
		...
		if (regexp.match(relPath)) 
		{
			...
			if (!File::exists(abs_entry_no_lock))
			{
				...
				bool success = false ;
				time_t now = time(nullptr) ;	// Unit is s ;
				time_t mtime = File::getModificationTime(absPath, &success );	
				if ( success && (mtime<=now) && ((now-mtime)>=(86400*days)) ) 
				{
...
//
//////////////////////////////////////////////////////////////////////////////////////////
bool Cache::removeNestedLocks(const string& path) 
{
...
*/
signed long int Cache::cmpTimeval(const struct timeval& a, const struct timeval& b) noexcept 
{
...
//
//////////////////////////////////////////////////////////////////////////////////////////
time_t Cache::get_time_t(const struct timeval& t) noexcept 
{
	return t.tv_sec ;
}
...
{
	char msg[64] ; // NOSONAR 
    sprintf(msg, "%ld.%06d", a.tv_sec, static_cast<signed int>(a.tv_usec)) ; // NOSONAR - no buf overflow possible
	return msg ;
}
...
void Cache::debugDumpCache() const
{
	...
	{
		o << "\t" << j << "  key: \"" << entry.first << "\"  " << entry.second.debugString() << "\n" ;
        j += 1 ;
	}
	o << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n" ;

	j = 0 ; 
    for (auto const &entry : _entriesMTime )
	{
		ptime tmp = from_time_t(entry.first) ; 
        string d = "(" + ptime2s(tmp) + ")" ;
		o << "\t" << j << "  key: " << entry.first << "  " + d + "  \"" << entry.second << "\"\n" ;
        j += 1 ;
	}
	o << "===========================================================[ End ]==\n" ;
...
{		
	...
	for (size_t j=0; j<le_size; ++j)
	{
		...
		ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
			const size_t scanStep = std::max(size_t(1), lockEntries.size()/10)  ; 
			if (0 == (j%scanStep)) 
			{
				ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Scanning: " << (j+1) << "/" << lockEntries.size()) ; 
			}
		ACS_CLASS_END_DEBUG
	...
	for ( auto j = _entries.begin();  ( (_entries.begin()!=_entries.end()) && (j!=_entries.end()) ); )  
	{
	    auto i=j ; ++j; // Prepare next loop
...
{	
	...
	*/
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called.  cacheModified: " << std::boolalpha << cacheModified << "  rescanForced: " << std::boolalpha << rescanForced) ;

	const string cacheUpdateFileName = StringUtils::pathJoin(_path, _cacheUpdateFileNameRelPath) ;
	string mtimeStr ;
	struct timeval mtimev; 
	...
	ACS_COND_THROW( 0 != gettimeofday(&curtime, 0), exIllegalValueException( "Error calling gettimeofday", errno) ) ; 
	...
	if (cacheModified || !cacheUpdateFileRead) {
		ACS_COND_THROW( 0 != gettimeofday(&mtimev, 0), exIllegalValueException( "Error calling gettimeofday", errno) ) ; 
    }
	...
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		std::ostringstream o ; o << "mutex acquired.  mtime: " 
	...
	if (executeReadEntries)
	{
		...
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "_lastUpdate updated to: " << dumpTimeval(mtimev) << "(" << mtimeStr << ")") ; 
	}	
	
	
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "touch executed? " << (cacheModified? "T ("+mtimeStr+")":"F") << "  readEntries executed? " << std::boolalpha << executeReadEntries) ; 
								...
								bool forceUpdate)					// TRUE to force update
{
	...
	if (forceUpdate || !success) { 
        lockMTime = time(nullptr); 
    }

	{ // NOSONAR - lock scope
		Lock lock(*this) ;

		auto it = _entries.find(lockFileRelPath) ;
		if (it == _entries.end()) {
			...
			_entries.emplace(lockFileRelPath, e ) ;
		...
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, (toAdd? "added": "updated") << " file: \"" << lockFileRelPath << "\"  Returning: " << ct ) ; 
...
off_t Cache::bytesUsed() const
{
	...
	off_t tot = 0 ;
	for (auto const &entry : _entries ) {
		if (entry.second.isLocked()) { tot+= entry.second.getBytes() ; }
    }
								...
								size_t& count)
{
	...
	{	ACS_THROW(CacheInvalidFilenameException("Empty or invalid filename: \"" + relPath + "\"") );	} 
	else if ( regexp.match(relPath) )
	{	ACS_THROW(CacheInvalidFilenameException("Invalid filename (LOCK EXTENSION NOT ALLOWED) \"" + relPath + "\"") );	} 
    else { /* nothing */ }
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. inputpath (relative): \"" << relPath << "\"\tfile exists? " << std::boolalpha << exists)  ;
...
{
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called. inputpath (relative): \"" << relPath << "\"  lock? " << std::boolalpha << lock) ;
...
{
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		std::ostringstream o	; o << "Method called. inputpath (relative): \"" << relPath << "\"\trequested bytes: " << size 
			<< "  Lock: " << (shared? "SHARED" : "EXCLUSIVE") ; 
	...
	*/
	...
	if (!relReservedFileName.empty())
	{
		...
		if (res_bytes_success && (res_bytes>0))
		{
			...
			to_reserve = ( (res_bytes >= reserved)? 0 : (reserved-res_bytes) ) ; 
		}
	
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Read file: \"" << relReservedFileName << "\"  success: " << std::boolalpha << res_bytes_success << "  res_bytes: " << res_bytes
				<< "  bytes to be initially reserved: " << reserved << "  remainder: " << to_reserve) ;
	}

	{ // NOSONAR temporaryReservedSpace scope
		...
		const off_t freeDisk = File::getFreeSpaceInMB(_path) * megabyte_ ;	/* Physical space */ 
		...
		ACS_COND_THROW( freeDisk < reserved, CacheFullException("Device Full. Cannot store file: " + relPath) ); 

		/* Check if there is enought logical space in cache (available) to store the logical file */
		ACS_COND_THROW( ((available+reserved) <= size), CacheFullException("Cache Full. Cannot store file: " + relPath) ); 
									...
									vector <string>& filenamesRemoved)	/* List of files removed */
{
	...
	*/
	...
	{ // NOSONAR ReserveSpace scope
		...
		/* Check if there is enought phyisical space (freeDisk) to store the logical file */
		ACS_COND_THROW( freeDisk < reserved, CacheFullException("Device Full. Cannot reserve space") ); 

		/* Check if there is enought logical space in cache (available) to store the logical file */
		ACS_COND_THROW( ((available+reserved) <= bytes), CacheFullException("Cache Full. Cannot reserve space") ); 
									...
									vector <string>& filenamesRemoved)	/* List of files removed */
{
	...
	ACS_COND_THROW(!foundInList, CacheInvalidFilenameException("File not currently locked: \"" + relPath + "\"") );	 
	...
	try
	{
		if (bytes>=0)
		{
			off_t reserved = (bytes + (bytes/5)) ;	// Increased by 20% -- It might be zero 
			ReserveSpace temporaryReservedSpace(_path, reserved) ;	/* Reserve space. It will be released when (this) scope is lost */
			...
			/* Check if there is enought logical space in cache (available) to store the logical file */
			ACS_COND_THROW( ((available+reserved) <= bytes), CacheFullException("Cache Full. Cannot store file: " + relPath) ); 
	...
	catch(const CacheFullException&)
	{
									...
									time_t expdate)				/* Prevent AutoErase till this date */
{
	...
	bool foundInList = false ;
	string lockFileRelPath = searchLockFilePath(relPath, foundInList) ;
	ACS_COND_THROW( !foundInList,  
		CacheInvalidFilenameException("File unknown: \"" + relPath + "\"") );	
	ACS_COND_THROW( !_entries[lockFileRelPath].isLocked(),  
		CacheInvalidFilenameException("File not locked: \"" + relPath + "\"") );	

	try
	{
		// Prepare the new payload for the lockfile :
		const Cache::LockType t = ( _entries[lockFileRelPath].getShared()? Cache::SH_GENERIC : Cache::EX_WRITE ) ; 
		...
		oper.expdate = static_cast<signed long long int>(expdate) ; 
							...
							bool shared)	/* If a lock is required, it can be SHARED (true) or EXCLUSIVE (false) */
{
	ACS_CLASS_BGN_DEBUG(ACS_MID_VERB)
		...
		o << "Method called. inputpath (relative): \"" << relPath << "\"  bytes: " 
		  << bytes << " lock? " << std::boolalpha << lk ;
		if (lk) { o << "  type: " << ( shared? "SHARED": "EXCLUSIVE" ) ; } 
	...
	// Check inputs
	ACS_COND_THROW( ((bytes>=0) && !lk), CacheInvalidFilenameException("Internal Error: can't reserve space without locking the entry") ) ; 

	// Check physical space in cache 
	const off_t freeDisk = File::getFreeSpaceInMB(_path) * megabyte_ ;	/* Physical space */ 
	ACS_COND_THROW( ((bytes>=0) && (freeDisk<bytes)), CacheFullException("Disk Full") ) ; 
	...
	if (foundInList)
	{
		...
		CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB,"Inputpath (relative): \"" << relPath << "\"  to_lock: " << std::boolalpha << lk  
	...
	if (lk) 
	{
		if (bytes<0)
		{
			Cache::LockType type = (shared ? Cache::SH_GENERIC : Cache::EX_WRITE) ; 
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB,"Method finished. Inputpath (relative): \"" << relPath << "\"\tlockFile: \""
		  << lockFileRelPath << "\"\tto_lock: " << std::boolalpha << lk << " bytes: " << bytes 
		  << "  Rescan was needed: "  << std::boolalpha << rescan << "  Returning: " << uc) ;
						...
						bool relock_shared)
{
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Method called." 
			<< " inputpath (relative): \"" << relPath << "\"  relock_shared: " << std::boolalpha << relock_shared) ;
	...
	ACS_COND_THROW(!foundInList, CacheInvalidFilenameException("Cache::release: Empty or invalid filename: \"" + relPath + "\"") );	 
	...
	if (!relock_shared)
	{
		...
		try {
			...
			for (auto const & lfrp : v) 
			{
				if (lfrp.op == DELETE_ENTRY_ASAP)
				{
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Inputpath (relative): \"" << relPath 
			<< "\"\trelock_shared: " << std::boolalpha << relock_shared  << "  Rescan was needed: "  << std::boolalpha << rescan << "  Returning: " << uc) ;
...
off_t Cache::freeSpace() const
{
	...
	off_t left = (_maxSize > usedBytes) ? _maxSize - usedBytes : 0 ; // how much space left (for maxSize) 
	...
	off_t ret = std::min(left, freeDisk) ;
...
bool Cache::deleteEntry(const multimap<time_t, string>::iterator &i,	/* IN: Iterator to _entriesMTime */
						const map<string, Entry>::iterator &e, 		/* IN: Iterator to _entry */ // NOSONAR - no transparent
						...
						off_t& freedBytes)							/* OUT: bytes freed */
{
	...
	if ( forceErase || (!e->second.isLocked() && e->second.isErasable()) ) 
	{
...
{
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called. lockFileAbsPath: \"" << lockFileAbsPath << "\"  abortOnError: " << std::boolalpha << abortOnError) ;
	...
	if ( File::exists (absPath) && File::isDir(absPath) )
	{
		...
		try
		{
			...
			{
				try { // NOSONAR - scope to count errors
					CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Removing entry: \"" << dir[i] << "\"") ;

					ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
						const size_t scanStep = std::max(size_t(1), dir.size()/10)  ; 
						if (0 ==  (i%scanStep)) 
						{
							ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Removing: " << (i+1) << "/" << dir.size()) ; 
						}
					ACS_CLASS_END_DEBUG
...
/** tries to free an amount of bytes in order to allow the requester to write new data.
	 ...
	 When triggered, the cleaning will remove the oldest files, up to  (wished_space +  std::max(_quota_min_threshold, erase_quota))
...
off_t Cache::emptySpace(off_t wished, off_t& left, vector <string>& filenamesRemoved, size_t *filesFreed)  
{
	Lock lock(*this) ;
	...
	off_t required = ((wished+getQuotaMinThreshold()) > left) ? ((wished+getQuotaMinThreshold()) - left) : 0 ; 
	...
	*/
	...
	{ // NOSONAR - File::Lock scope
		...
		if (_lowAccuracy) {
			...
			left = freeSpaceWithSign() ;	/* Free space in cache (free quota) */
			off_t inner_required = ((wished+getQuotaMinThreshold()) > left) ? ((wished+getQuotaMinThreshold()) - left) : 0 ; 
		...
		const off_t eraseQuota = 1.0 * (_eraseQuotaPercentage * _maxSize) ; // NOSONAR - precision loss
		...
		wished += std::max(eraseQuota, getQuotaMinThreshold()) ; 
		...
		do
		{
			...
			left = freeSpaceWithSign() ;	/* Free space in cache (free quota) */
			required = (wished > left) ? wished - left : 0 ; 
			...
			found = false ; 
			for (auto j=_entriesMTime.begin();  
				 ...
				 )
        	{
				...
				auto i = j ; ++j ;	// Set current value (i) and prepare next loop (j)

				//(unused) const time_t mtime = i->first ; 
				const string lockFileRelPath = i->second ;
				auto e = _entries.find(lockFileRelPath) ;
								...
								size_t *filesFreed) 
{
	...
	ACS_COND_THROW( (availableSpace < wished), CacheFullException("Cache full. Couldn't free enought space") ); 
...
bool Cache::eraseEntryAndForget(const std::string &relPath) 
{
	...
	catch(const CacheLockException&)
	{
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Entry to remove: \"" << relPath << "\"  Returning: " << std::boolalpha << immediate) ;
...
void Cache::eraseEntry(const std::string &relPath) 
{
	...
	for (size_t i=0; (!foundInList && (i<2)); i++) 
	{
	...
	ACS_COND_THROW( (!foundInList), exIllegalValueException("Internal Error: Unable to add entry: \"" + relPath + "\"" )) ; 
	...
	for (auto j=ret.first; ((j!=ret.second) && (ret.first != ret.second)); )
	{
        auto i = j ; ++j ;	// Set current value (i) and prepare next loop (j)
		...
		{  
			...
			auto e = _entries.find(lockFileRelPath2) ;
	...
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		if (found) {
            ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished.  FREED: " << tot 
                << " bytes (" << counter << " entries).") ;
        }
		else {
            ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished.  NOTHING TO DO.") ;
        }
	ACS_CLASS_END_DEBUG
...
size_t Cache::sizeUsed() const
{
	...
	ptrdiff_t size = count_if(_entries.begin(), _entries.end(), [](auto const &item) { return item.second.isLocked() ; }) ;
...
void Cache::setMaxSize(off_t maxSize)
{
	...
	if (byt > maxSize) {
		ACS_LOG_WARNING("Cache::setMaxSize  [" << Cache::getTID() << "]  WARNING: used bytes (" << byt << ") are more than maxSize (" 
...
string 																/* OUT: relative pathname of lockfile (e.g. dir1/dir2/file.lock or dir1/dir2.lock) */
		Cache::searchLockFilePath(	const std::string &relPathIN,	/* IN: file/dir relative path (e.g. dir1/dir2/file ) */ 
									bool &found) const				/* OUT: TRUE if a lockfile was found */
{
	...
	if (! v_rel.empty()) {
		...
		for (ssize_t j=v_rel.size(); (!found && (j>0)) ; j--) 
		{
			...
			for (size_t i=0; ssize_t(i)<j; i++) { v_subpath.push_back(v_rel[i]) ; } 
			...
			CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "checking lockfile: \"" << subpath << "\"" << "  found: " << std::boolalpha << found) ;
		}	// for (ssize_t j=v_rel.size(); (!found && j>0); j--)
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Inputpath (relative): \"" << relPath << "\"\tfound: " 
...
string Cache::lockTypeToString(Cache::LockType t) 
{
    ...
    case Cache::ER_ERROR :
		...
		break ;
    default :
		ACS_THROW( exIllegalValueException() << "Cache::lockTypeToString  [" << Cache::getTID() << "]  " 
                  << __LINE__ << "  INTERNAL ERROR. Invalid type: 0x" << std::hex << int(t) ) ;
	}
...
Cache::LockType Cache::intToLockType(int t, bool emitException) 
{
	...
	if ( t == int(Cache::UN_GENERIC) ) { 
		lt = Cache::UN_GENERIC ;
    }
	else if ( t == int(Cache::SH_GENERIC) ) { 
		lt = Cache::SH_GENERIC ;
    }
	else if ( t == int(Cache::EX_GENERIC) ) { 
		lt = Cache::EX_GENERIC ;
    }
	else if ( t == int(Cache::EX_WRITE) ) { 
		lt = Cache::EX_WRITE ;
    }
	else if ( t == int(Cache::EX_ERASE) ) { 
		lt = Cache::EX_ERASE ;
    }
	else if ( t == int(Cache::ER_ERROR) ) { 
		lt = Cache::ER_ERROR ;
    }
	else
	{
		if (emitException)
		{
			ACS_THROW( exIllegalValueException() << "[" << Cache::getTID() << "] INTERNAL ERROR. Invalid type: 0x" << std::hex << t) ; 
		}
		else {
			lt = Cache::ER_ERROR ;
        }
	...
	CACHE_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, ": " << std::hex << t << std::dec << "  Returning: 0x" << std::hex << int(lt)) ; 
...
void Cache::dumpLockOperDateStructVector(const vector<Cache::LockOperDateStruct>& v) 
{
	...
	{
		Cache::LockOperDateStruct s = v[i] ;
		auto t = time_t(s.expdate) ; 
		o << "[" << i << "]:  oper: " << std::hex << int(s.op) << std::dec << "   expdate: " << t << "  (" 

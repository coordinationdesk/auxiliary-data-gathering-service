...
public:
    ...
    operator Mutex &() const noexcept { return _internal_mutex ; } // NOSONAR - allow Lock(*this)
    
public: // NOSONAR - clarity

	/* Types of lock (to be stored on lockfile payload) */
	enum LockType {  UN_GENERIC	= 0x00,		/* UNLOCKED */  // NOSONAR - old enum
	...
	enum LockOper { NOP					= 0x00,		/* UNDEFINED / N/A */ // NOSONAR - old enum
	...
	struct LockOperDateStruct {
		...
		unsigned char 	__pad[4] ;		/* In order to force alignment every 8 bytes (longest type in structure) NOSONAR */ 
...
private:
	...
	class Entry : public File::Lock {   
	public:
		Entry() ;
		explicit Entry(const std::string &lockFileAbsPath, const unsigned short int tryLockTimeout, time_t mtime=time(nullptr)) ; 
		...
		void reservedSpace(std::string& reserveFileAbsPath, off_t& bytes) const ;
		void touch() ;
	...
	private: // NOSONAR - for clarity
		...
		[[noreturn]] void throwExceptionFromFile(const std::exception *e, const std::string&) const ;

		static const std::string _lockFilePattern;
        static const std::string _lockFileExt ;
		static ThreadSafe  _mutex ;
		size_t _useCount = 0;
		signed long long int _bytes = 0 ;
		time_t _mtime ;
		bool _marker = false ;
		bool _shared = true ;	
		std::unique_ptr<Cache::ReserveSpace> _reserveSpace = {} ;
...
public: // NOSONAR - for clarity
	...
	*/
	off_t emptySpace(off_t wished, off_t& left, size_t *filesFreed = nullptr ) ;
	off_t emptySpace(off_t wished, off_t& left, std::vector <std::string>& filenamesRemoved,  size_t *filesFreed = nullptr ) ;

	/** As above, but emits an exception if it couldn't free requested space
	*/
	off_t emptySpaceAndCheck(off_t wished, size_t *filesFreed = nullptr ) ;
	off_t emptySpaceAndCheck(off_t wished, std::vector <std::string>& filenamesRemoved, size_t *filesFreed = nullptr ) ;
	...
	*/
	...
	/** Create (and reserve) space, using the reservatiom ticket if provided. 
		...
		If not enough free space is available a CacheFullException will be released.
	*/
	off_t makeSpaceReserveAndUse(const std::string &relPath, off_t size, const std::string & relReservedFileName="") ; 
	off_t makeSpaceReserveAndUse(const std::string &relPath, off_t size, std::vector <std::string>& filenamesRemoved, const std::string & relReservedFileName="") ; 
	...
	*/
	...
	void setLockTimeout(unsigned short timeout) noexcept { _tryLockTimeout = timeout ; /* Unit is [s]. Use 0 for NO TIMEOUT */ ; }
	
	/** Get the configured timeout while trying to acquire a lock 
	*/
	unsigned short getLockTimeout() const noexcept { return _tryLockTimeout ; }
...
private:
	...
	*/
	...
	static signed long int cmpTimeval(const struct timeval& a, const struct timeval& b) noexcept ;
	static std::string dumpTimeval(const struct timeval& a) ;
	static time_t get_time_t(const struct timeval& t) noexcept ;
	static struct timeval getFileModificationTime (const std::string& path, bool* success=nullptr) ;
    ...
    mutable Mutex _internal_mutex = {} ; 
	std::string _path ;
	bool _updateCacheEveryLock ;
	std::map<std::string, Entry, std::less<> > _entries = {} ;
	std::multimap<time_t, std::string> _entriesMTime = {} ;
	struct timeval _lastUpdate = {} ;
	bool _fullScanDone = false ;	/*	TRUE if an actual disk scan was performed at least once */
	bool _lowAccuracy = true ;		/*	TRUE if disk scan should be avoided until a free space is required. 
								        FALSE if a disk scan can be performed every time is needed to keep the accuracy high */ 
	off_t _lowAccuracyBytes = 0 ;
	...
	off_t _maxSize ;
	unsigned short int _tryLockTimeout =  _tryLockDefaultTimeout ;	/* Unit is [s] */
	bool _updateListRemovedFiles = false  ; 			/** Set to TRUE to keep a list of removed files */
	std::vector <std::string> _userRemovedFiles_V = {} ; /**  List of files removed as per user request */
	double _quotaMinThresholdPercentage = 0.0 ; 	/** Used to trigger the automatic cleaning. Range [0.0 - 1.0]. So 0.1 means that when the free space is below 10% 

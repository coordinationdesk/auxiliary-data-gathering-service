...
public:
	...
	__attribute__((deprecated("DO NOT USE: transfers ownership to get back to starting path"))) ChDir(const ChDir &) ;
	__attribute__((deprecated("DO NOT USE: transfers ownership to get back to starting path"))) ChDir &operator=(const ChDir &) ;
	~ChDir() noexcept ;
	static std::string getCwd() ;
private:
	std::string _oldp = "" ;
	mutable bool _autoReturn = false ;
} ;

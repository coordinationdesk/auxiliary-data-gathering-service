...
//
void DigestTree_base::setObject(const std::string& entry_IN, bool check_perm) 	 
{
	// Entry point to set an object and compute the ID
	// Object can be a file or a directory

	ACS_CLASS_WRITE_DEBUG_TH( ACS_LOW_VERB, "Method called. Object: \"" << entry_IN << "\"  check_permissions: " << std::boolalpha << check_perm ) ;

	ACS_COND_THROW(!(File::exists(entry_IN)), ACS_CRC_DigestTreeError("Object not found: \"" + entry_IN + "\"")) ; // PRQA S 3081
	
	ACS_COND_THROW( (!_digestP), ACS_CRC_DigestTreeError("FATAL: DIGEST IS UNDEFINED (null pointer)") ) ;  // PRQA S 3081

	const std::string entry = File::readlink(entry_IN) ; // Resolve symlink(s)
...
std::string DigestTree_base::getObjectID(ACS_CRC_Digest_Retbuf retbuf, unsigned int* s, bool validity) // PRQA S 4211
{
	ACS_COND_THROW( (!_digestP), ACS_CRC_DigestTreeError("FATAL: DIGEST IS UNDEFINED (null pointer)") ) ; // PRQA S 3081
...
std::string DigestTree_base::getDigestName() // PRQA S 4211
{ 
	ACS_COND_THROW( (!_digestP), ACS_CRC_DigestTreeError("FATAL: DIGEST IS UNDEFINED (null pointer)") ) ; // PRQA S 3081
...
void DigestTree_base::hashFile(const std::string& entry, bool resetContext) // PRQA S 4211
{
	// Just compute the hash of a given file (using the engine in CRC_Digest_base; e.g. MD5, SHA1...)

	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method called. File: \"" << entry << "\"  reset: " << std::boolalpha << resetContext ) ;
	...
	unsigned char buffer[ bufSize ] ; // NOSONAR - size computed
	ssize_t r = 0 ; 
	...
	{ // NOSONAR - guard on fd
		...
		do {
			...
			{
				ACS_THROW(ACS_CRC_DigestTreeError(err) << "DigestTree_base::hashFile. Could not read file: \"" << entry <<  "\"  r: " << r) ; // PRQA S 3081
			}
            else { /* nothing */ }
...
void DigestTree_base::getBlobID(const std::string& entry)
{
	...
	if ( ! header.str().empty() )
	{
		...
		_digestP->addBuffer( header.str().c_str(), header.str().size() ) ;
	}
...
void DigestTree_base::getTreeID(const std::string& dir_entry, bool check_perm)
{
	...
	strcpy(tmpl.data(), "/tmp/hash.XXXXXX") ; // NOSONAR - safe

	{ // NOSONAR - guard on fd
		const int fd ( mkstemp(tmpl.data()) ) ;	// Also, set a valid name inside the templaye tmpl
		...
		{
			const std::string entryFullPath = File::getAbsolutePath(  StringUtils::pathJoin(dir_entry, dir[i] ) ); 
	...
	for (size_t i=0; i<e_size; ++i)
	{
		...
		std::array<unsigned char, EVP_MAX_MD_SIZE> object_id ;
        object_id.fill(0x00) ;

		const std::string entryFullPath( File::readlink(entries[i], true) );
		...
		_digestP->getResult( object_id.data() ) ; // Compute the object_id

		std::array<unsigned char, 64*1024> buffer_tree ; // PRQA S 4403 2	
		std::array<char, 64*1024> hdr ;			
		buffer_tree.fill(0) ;	// Do not move, do not change: it must contain all zeros.
		hdr.fill(0) ;	// Do not move, do not change: it must contain all zeros.

		const std::string entryFilename = File::getFileName(entries[i]) ; // entries[i] is correct ! do not replace with entryFullPath
		snprintf(hdr.data(), hdr.size()-1, "%06o %s", mode_num, entryFilename.c_str()) ;  // PRQA S 3084 2
		const size_t hdr_size = mode_width +1 /*SPACE*/ +entryFilename.size() +1 /* BYTE "0" */ ; // One byte more (set to zero) is copied from the header 

		size_t 	tree_size(0) ;
		memcpy((buffer_tree.data()+tree_size), hdr.data(), hdr_size) ; 	// Copy the header, plus one byte (set to zero) // PRQA S 3084 2
		tree_size += hdr_size ; 

		memcpy((buffer_tree.data()+tree_size), object_id.data(), getHashSize()) ;	// Add the payload // PRQA S 3084 2
		tree_size += getHashSize() ;

		const int fd ( open(tmpl.data(), O_WRONLY|O_APPEND) ) ;
		ACS_COND_THROW((fd<0), ACS_CRC_DigestTreeError("Could not update temp file.")) ; // PRQA S 3081

		FDGuard g(fd) ; // Note: SELF-close(fd)
		const ssize_t w (write(fd, buffer_tree.data(), tree_size)) ; 
		ACS_COND_THROW((w!=static_cast<ssize_t>(tree_size)), ACS_CRC_DigestTreeError("Error writing structure")) ; // PRQA S 3081


	    ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Entry: \"" << entryFullPath << "\"  Hash: { " << bufferDump(object_id.data(), EVP_MAX_MD_SIZE) << " }") ;
	}

	// Compute the hash for the whole tree
	
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Computing the hash for temporary file: \"" << tmpl.data() << "\"" ) ;
	
	const bool resetContext(true) ; // In order to take the header into account when computing the digest
	hashFile(tmpl.data(), resetContext) ; 
	...
	ACS_CLASS_BGN_DEBUG(ACS_INS_VERB)
		...
		ACS_CLASS_WRITE_DEBUG(ACS_INS_VERB, "INSANE VERBOSITY: TROUBLESHOOTING MODE. To improve the debug, the temporary file: \"" 
				<< tmpl.data() << "\" won't be removed.") ;
	ACS_CLASS_END_DEBUG

	if (removeTempFile)
	{
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Removing temporary file: \"" << tmpl.data() << "\"" ) ;
		::unlink(tmpl.data()) ; 
	} else {
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "TROUBLESHOOTING MODE. Keeping temporary file: \"" << tmpl.data() << "\"" ) ;
	}
...
std::string DigestTree_base::bufferDump(const unsigned char* p, const size_t s) 
{
	std::ostringstream ret ; ret << "size: " << s << " ; dump:";
	std::array<char, 8> z; 
    z.fill(0) ;
	for (size_t i=0; i<s; ++i) { snprintf(z.data(), 3, "%02x", p[i]) ; ret << " " << z.data() ; } 
	ret << " ;"  ;
	return ret.str() ;
}

...
public:
    ...
    DigestTree_base(const DigestTree_base &) = delete ;
    DigestTree_base & operator= (const DigestTree_base &) = delete ;
   	DigestTree_base() = default ;
   	explicit DigestTree_base(CRC_Digest_base*) ;
	virtual ~DigestTree_base() = default ;

	void setObject(const std::string&, bool ) ;	
	
	void setDigest(CRC_Digest_base* digestP) { _digestP = digestP ; } 
	
	std::string getObjectID(ACS_CRC_Digest_Retbuf retbuf=nullptr, unsigned int* s=nullptr, bool validity=false) ;

	std::string getDigestName() ;
	
	static std::string bufferDump(const unsigned char* p, const size_t s) ;
...
// private data member
	CRC_Digest_base *_digestP = nullptr ;

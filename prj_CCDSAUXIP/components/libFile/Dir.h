...
inline bool DirNopPredicate(const std::string &, const std::string &, bool ) { return true ; } 
...
public:
    ...
    inline
    Dir(const std::string &path, Predicate pred, bool hidden = true, bool brokenln = false) :
	    _path(path),
	    _hidden(hidden),
	...
	template<typename Predicate>
    inline
	void load(Predicate pred) 
    {
	    // Scan directory
	    ::DIR *dir = ::opendir(_path.c_str()) ;
	    ACS_COND_THROW(!dir,exIOException("Cannot read directory \"" + _path +"\"", errno)) ; 
        ...
        struct dirent* entry = reinterpret_cast<struct dirent*> ( malloc(len) ); // NOSONAR - tricky 
...
#pragma GCC diagnostic pop
            ...
            if (pred( _path, item, S_ISDIR(stat_struct.st_mode))) {
		        _files.push_back(item) ;
            }
	    }
	    if (readdirval) {
		    ::closedir(dir) ;
		    ACS_THROW(exIOException("Cannot read directory \"" + _path+"\"", readdirval)) ; 
	    }
	    ::closedir(dir) ;
    }
	...
	bool reportBrokenLinks() const noexcept ;
...
private: // NOSONAR - for clarity
	...
	bool _hidden ;
	off_t _bytes = 0 ;
	bool _dumpDirEntries = true ;
	std::string _dirEntry = "" ;
	signed short _minDepth = -1 ;
    signed short _maxDepth = -1 ;
	...
	bool _reverse = false ;
	bool _regExp = false ;
    bool _regExpReject = false ;
	enum SortOrder _sortOrder = SORT_DEFAULT ;

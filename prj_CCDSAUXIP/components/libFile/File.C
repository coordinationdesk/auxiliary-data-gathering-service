...
namespace {
    const acs::lfV version ; // NOSONAR - versioning 
}
    ...
    _autoclose (autoclose)
{
	...
	if (!_file) {
		int err = errno;
		ACS_THROW(exIOException(err) << "Cannot fdopen on file descriptor #" << fd) ; 
	}
    // MP note: __gnu_cxx::stdio_filebuf<char>(int, ...) 
	_bfilebuf.reset(new __gnu_cxx::stdio_filebuf<char>(_file,std::ios::in)) ; // NOSONAR - __gnu_cxx
	basic_ios<char>::rdbuf(_bfilebuf.get()) ;
  #endif
#else
	attach(fd) ;
#endif
...
#ifdef __linux__
  #if (__GNUC__ >= 3)
    // MP note: __gnu_cxx::stdio_filebuf<char>(FILE *, ...) 
	_bfilebuf.reset(new __gnu_cxx::stdio_filebuf<char>(file,std::ios::in)) ; // NOSONAR - __gnu_cxx
	basic_ios<char>::rdbuf(_bfilebuf.get()) ;
  #endif
#else
	attach(fileno(file)) ;
...
File::ifdstream::~ifdstream() noexcept 
{
	if (_file && _autoclose && fclose(_file) ) { // from left to right !!
		int err = errno;
    ...
    _autoclose (autoclose)
{
    ...
    // MP note: __gnu_cxx::stdio_filebuf<char>(int, ...) 
	_bfilebuf.reset(new __gnu_cxx::stdio_filebuf<char>(_file,std::ios::out)) ; // NOSONAR - __gnu_cxx
	basic_ios<char>::rdbuf(_bfilebuf.get()) ;
  #endif
#else
	attach(fd) ;
...
#endif
{
  ...
  #if (__GNUC__ >= 3)
    // MP note: __gnu_cxx::stdio_filebuf<char>(FILE *, ...) 
	_bfilebuf.reset(new __gnu_cxx::stdio_filebuf<char>(file,std::ios::out)) ; // NOSONAR - __gnu_cxx
	basic_ios<char>::rdbuf(_bfilebuf.get()) ;
  #endif
#else
	attach(fileno(file)) ;
...
File::ofdstream::~ofdstream() noexcept 
{
	if (_file && _autoclose && fclose(_file) ) { // from left to right
		int err = errno;
		ACS_LOG_TRYCATCHNOTIFY_EX(exIOException ("::fclose syscall failed", err));
...
string File::file(const string &name, const string &options) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "pcmd: \"" << pcmd << "\"") ;
	FILE *filep = nullptr;
	{ // NOSONAR - keep the lock shorter
		// Protect popen
		Mutex::Lock lock (_fileGlobalMutex);
		filep = popen(pcmd.c_str(),"r") ;	// open pipe for command to be executed 
	}
	int err = errno;
	ACS_COND_THROW(!filep,exFileOpenException("Cannot stat file type on \"" + name +"\"",err)) ;  


	size_t bufSize = 0 ;
	char *buffer=nullptr;
	ssize_t retSize = ::getline( &buffer , &bufSize , filep) ; // will allocate memory
    pattern::CMemoryScopeGuard bguard(buffer) ;
	err = errno;
	if( retSize == -1)
	{
		try {
			ACS_THROW(exIOException( "Cannot read from cmd \"" + pcmd +"\"", err )) ; 
		}
	...
	if (buffer && 0 != isspace(buffer[retSize-1])) {
		buffer[retSize-1] = 0 ;
    }
	...
	if (b != string::npos) {
		result = result.substr(b+sep.length()) ; 
	}
...
double File::getDirectoryOccupationInMB(	const string& rootDir,  
													bool raiseException) 
{
	...
	while (!dirList.empty())
	{
		...
		if (dir == nullptr) {	
			if (!raiseException) {
				...
				ACS_THROW(exIOException("Cannot access path: " + dirName,err)) ;	 
			}
		} else {	
...
#pragma GCC diagnostic pop
	    	// for(struct ::dirent* e = 0; errno=0, e=::readdir(dir);) {  
				...
				if ( S_ISDIR(s.st_mode) ) { 							 
					...
					totSize += ( (s.st_size) / unit) ; // NOSONAR - ok for precision
			...
			if (readdirval != 0) {	
				if (!raiseException) {
					...
					ACS_THROW(exIOException("Error while accessing path: " + dirName,readdirval)) ;	 
				}
			} 
...
bool File::entry_is_symlink(const string &s, bool prevent_exception) 
{
	...
	buffer.fill(0);	// from man: readlink() does not append a null byte to buf.

	bool isSymLink = true ; 
    if ( ::readlink( s.c_str(), buffer.data(), buffer.size() ) <0 )
	{
		int err = errno;
		if (err == EINVAL) {
			...
			isSymLink = false ; 
            ACS_COND_THROW( !prevent_exception, exIOException("Couldn't run readlink (non-recursive call): \"" + s + "\"", err ) ) ;  
		}
	}	// END: if ( ::readlink ... )
...
{	
	...
	if (0 == acc_status) { 
        ...
        isBrokenSymlink = false ; 
        ACS_COND_THROW( !prevent_exception, exIOException("Error accessing entry: \"" + s +"\"", err ) ) ;  
    } 
...
void File::rename(const string &old, const string &n)  
{
/*
...
*/
	...
	ACS_COND_THROW( !File::exists(old) && !entry_is_broken_symlink(old),  
		exIOException("Source file not found: \"" + old +"\"" )) ; 
	
	const bool followSymLinks = true ;
	if( entry_is_broken_symlink(old) || checkFileSystemsMatching( getDirPath(old), getDirPath(n), followSymLinks ) )  
	{
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "\"" << old << "\" and \"" << n << "\"  are in the same filesystem") ;

		int status = (::rename(old.c_str(), n.c_str())) ; 
		int err = errno;
		ACS_COND_THROW(0 != status, exIOException("Couldn't rename \"" + old +"\" in \"" + n + "\"",err)) ;	 
	}
	else
	{
		...
		if (File::isDir(old))
		{
			...
			ACS_COND_THROW((File::exists(n) && File::isRegularFile(n)),  
...
string File::readtargetlink(const string &o, bool recursive)  
{
// EC: this method has multiple returns to keep its complexity as low as possible
    ...
    ssize_t status = ::readlink( trimmedPath.c_str(), buffer.data(), buffer.size() );
    int err = errno;
    ...
    if (status == -1)
    {
		...
		ACS_COND_THROW(err != EINVAL,exIOException("::readlink error: "+o , err )) ;  
        return o ; // no exception - returns the input
    }

    buffer[buffer.size()-1] = 0 ; // avoid segfault 
	if (!recursive) {
		return buffer.data() ;
    }
	return File::readtargetlink(buffer.data(), true) ;
}


string File::readlink(const string &o, bool recursive) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	if ( !recursive || trimmedPath.empty() )
	{
    	...
    	if (status == -1)
    	{
			ACS_COND_THROW(err != EINVAL,exIOException("Cannot run readlink: "+o ,err )) ;  
        	return o ; // no exception - returns the input
    	}
	...
	{
		bool ignored = false ;
		const string full_path = ( (trimmedPath[0] == '/' ) ? trimmedPath : StringUtils::pathJoin(getCurrentPath(), trimmedPath) ) ;  
		const string prefix = "" ; 
...
string 													/* OUT: Processed PATH */
		File::readlinkRec(	const string& s_in,			/* IN: PATH to be processed */ 
...
void File::copy( const string &src, const string &dest, vector<PerfStatistics> *stats, bool preserveMode)  
{
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	if ( File::exists(src) && File::isDir(src) )
	{
		...
		ACS_COND_THROW((File::exists(dest) && File::isRegularFile(dest)),  
		...
		for (size_t i=0; i<d_size; ++i)
		{
			...
			if (isDir(s))
			{
				...
				mkdir(d.c_str(), 0777) ;	// Create destination (sub)dir. Ignore error code // NOSONAR - octal: any perm
		...
		for (size_t i=0; i<modessize; ++i)
		{
			...
			{
				ACS_LOG_ERROR("Couldn't set permissions: 0" << std::oct << m << std::dec << " to directory: \"" << d << "\"") ; 
				ACS_LOG_NOTIFY_EX(e) ;
			}
	...
	else
	{
		...
		try {
			...
			if (!in) {  // error cannot open
				...
				ACS_COND_THROW(!File::isReadable(src), exIOException("Cannot access \"" + src + "\"", errno)) ; 
			}
            using std::ios_base ;
			in.exceptions(ios_base::badbit | ios_base::failbit) ;

			ndest = ( (File::exists(dest) && File::isDir(dest))?	
			...
			if ( File::exists(ndest) )
			{
				...
				{
					ACS_THROW ( exIOException("Source and Destination are the same file! Src:\"" + src + "\" - Dst:\"" + ndest + "\"")) ;	
				}
			...
			if (!out) {  // error cannot open
				// note isReadable use access and set errno
				ACS_COND_THROW(!File::isReadable(ndest), exIOException("Cannot access \"" + ndest + "\"", errno)) ; 
			}
			...
			off_t n = 0 ;
			do {
				try { // NOSONAR - intercept and check eof
			...
			// Preserve file permissions
				try { // NOSONAR - an error here is not lethal
            ...
            if (stats) {
                ps.total_time = pt.elapsed() ; // NOSONAR - precision loss - speed stat computation below
                ps.size = File::size(ps.spath) ;  // NOSONAR - precision loss - speed stat computation below
    	...
    	catch (const std::exception& e) // NOSONAR - any exception
    	{
			// forward errno 
			int errnoex = errno ;
            auto ex = dynamic_cast<const exException*>(&e) ; 
			if (ex) {
				errnoex = ex->errorNumber() ;
            }
        	ACS_THROW(exIOException(e,"Cannot copy \"" + src +"\" to \""+ dest + "\"",errnoex)) ;	 
    	}
	}        
...
void File::hardlink(const string &src, 
					const string &dest,
...
short File::depthLimitedHardlink(const string &src, 
					const string &dest,
...
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	if (File::exists(src) && File::isDir(src)) {
		...
		if (exists(dest) && !isDir(dest)) {
			ACS_THROW(exIOException("Cannot link (hard) to "+ dest + ": it is a regular file")) ;	 
		}
		else if (!exists(dest)) { // create the dest directory

			mkdir(dest, 0777) ; // NOSONAR - octal
		...
		for (size_t i = 0; i < dt_size; ++i) {
			...
			if (isDir(source)) 
			{
				try
				{
					...
					modes.emplace_back(destination, m) ;
					gids.push_back( g ) ;
				} 
				catch(const std::exception &e) // NOSONAR - any exception
				{
				...
				mkdir(destination, 0777) ;	// is a directory // NOSONAR - octal 
			}
			else {
				depthLimitedHardlink(source, destination,maxdepth,copyOnError, nullptr, stats) ;	// is a file
			}
	...
	if (0 != ::link(src.c_str(), dest.c_str())) 
	{
		...
		else
		{
			...
			ACS_THROW(exIOException("Cannot link (hard) \"" + src +"\" to \""+ dest + "\"" ,err)) ;	 
		}
	}
	else 
	{
	...
	if (stats) {
		...
		ps.spath=src ;
		ps.dpath=dest ;
		ps.total_time = pt.elapsed() ; // NOSONAR - precision loss - to compute speed
		ps.size = File::size(ps.spath) ;  // NOSONAR - precision loss - to compute speed
		ps.speed = ps.size / ps.total_time ;
		stats->push_back(ps) ;
	}
...
void File::symlink(const string &src, const string &dest ) 
{
	if (0 != ::symlink(src.c_str(), dest.c_str())) {
		ACS_THROW(exIOException("Cannot link (soft) \"" + src +"\" to \""+ dest + "\"" ,errno)) ;	 
	}
}


bool File::unlink(const string &f, bool changePerm, bool raiseException) 
{
	// EC: this method has multiple returns to keep its complexity as low as possible
	...
	else
	{
		...
		if (0 != ::rmdir(f.c_str()) )
		{
			...
			if (raiseException)
			{
				ACS_THROW(exIOException("Cannot remove directory: '" + f + "' ", err)) ;	 
...
bool File::remove(const string &f) 
{
	if ( File::exists(f) && !File::isDir(f) )
	{
		...
		{
			int err = errno;
			ACS_THROW(exIOException("It couldn't remove entry: '" + f + "' ", err)) ;	 
		}
		return true ;
	}
	else
...
{
	if (exists(f)) {
		if (0 != ::utime(f.c_str(),nullptr)) {
			int err = errno;	
			ACS_THROW(exIOException("Cannot touch \"" + f +"\"" ,err)) ;	 
		}
	}
	else {
		FDGuard fdguard(open(f.c_str(), O_CREAT | O_WRONLY, 0666)) ; // NOSONAR - permission ok - file doesn't exist: open
		if (fdguard < 0) { 
			int err = errno;
			ACS_THROW(exIOException("Cannot create \"" + f +"\"" ,err)) ;	 
		}
	}
}
...
void File::touch(const string &f, time_t amtime) 	// Force touch to amtime (secs from EPOCH). 
{
	...
	if (!exists(f))
	{
		FDGuard fdguard(open(f.c_str(), O_CREAT | O_WRONLY, 0666)) ; // NOSONAR - permission ok
		if (fdguard < 0) { 
			int err = errno;	
			ACS_THROW(exIOException("Cannot create \"" + f +"\"" ,err)) ;	 
		}
	...
	if (0 != ::utime(f.c_str(), &utimbuf_struct)) {
		int err = errno;	
		ACS_THROW(exIOException("Cannot touch/set a new time to file: \"" + f +"\"" ,err)) ;	 
	}
}
...
void File::concat(const string &file1, const string &file2, const string & fileOut)
{
	...
	if (file1 == fileOut) { // append requested
		ACS_COND_THROW(file2 == fileOut,exIOException("Cannot append over the same file: " + fileOut)); 
        streamOut.open(file1.c_str(), ios::out | ios::app | ios::ate) ;
		int err = errno;
		ACS_COND_THROW(!streamOut,exIOException("Error while opening file \"" + file1 + "\" .",err)) ;  

		stream2.open(file2.c_str() );
		int err2 = errno;
		ACS_COND_THROW(!stream2,exIOException("Error while opening file \"" + file2 + "\" .",err2)) ;  
	}
	else {	
	
		// Check if target file already exists
		ACS_COND_THROW(File::exists(fileOut),exIOException("File \"" + fileOut + "\" already exists .")) ;  

		ACS_COND_THROW(file2 == fileOut,exIOException("Input file and output file must have different path")); 

		// Open file1
		ifstream stream1(file1.c_str() );
		int err = errno;
		ACS_COND_THROW(!stream1,exIOException("Error while opening file \"" + file1 + "\" .",err)) ;  
		...
		int err3 = errno;
		ACS_COND_THROW(!streamOut,exIOException("Error while creating file \"" + fileOut + "\" .",err3)) ;  
...
void
		File::concatMatchExt(	const string& file1, 
                                const string& file2, 
                                string &fileOut, 
...
// EC: the method can be divided into sections
    ...
    else if ( !isDir(file1) && !isDir(file2) ) 
    {
        ...
        {      
			if (file1 != fileOut)
			{
                ...
                else
                {
                    ...
                    concat(file2, file1, fileOut) ; // NOSONAR - order 
                }
            ...
            {
            	// file1 == fileOut
                string tmpFile = File::tempname( newPath + "/File" ) ;	/* Guard is automatic */ 
    	...
    	ACS_THROW( exIOException("File::concatMatchExt. Cannot merge directories !")) ;  
	;
	...
	// Final check on size:
    ACS_COND_THROW( (File::size(fileOut) != (file1Size+file2Size)), exIOException("File::concatMatchExt. Invalid size of output file.")) ;  
                                    ...
                                    bool splitInput)
{
	...
	if ( handleSingleExt && !extS.empty() && (getExt(fileIn) == extS) ) {
	...
	if (splitInput)
    {
	    ...
	    file1 = (ext1 != "")? (newName + "." + ext1): newName ; 
	    file2 = (ext2 != "")? (newName + "." + ext2): newName ;

        // ACS_COND_THROW( (file1==file2),  
...
void File::splitMatchExtHandleTrailer(	const string &fileInWTrailer, 
                            			string& file1, 
                            			string& file2,                            
                            			off_t file1Size, 
                            			off_t file2Size, 
...
// EC: file is easy to be read, it's divided into sections
	...
	// Check inputs
    ACS_COND_THROW( (file1Size < 0), exIOException("File::splitMatchExtHandleTrailer Invalid (negative) file1 size.")) ;     
    ACS_COND_THROW( (file2Size < 0), exIOException("File::splitMatchExtHandleTrailer Invalid (negative) file2 size.")) ;  
    ACS_COND_THROW( ( fileInWTrailer.empty() || !File::exists(fileInWTrailer) ), 
		exIOException("File::splitMatchExtHandleTrailer  Input File \"" + fileInWTrailer + "\" not Found.")) ; 
    ACS_COND_THROW( checkSize && ( File::size(fileInWTrailer) != (file1Size+file2Size) ),  
	...
	if ( !trailerPattern.empty() )  { trailer = trailerPattern + trailer ;	}/* since trailer doesn't begin with the pattern */
    ...
    if ( (file1Size > 0) && (file2Size > 0) && !isDir(fileInWTrailer) )
	{
	    ...
	    // Create new filenames 
	    file1 = (ext1 != "")? (newName + "." + ext1 + trailer): (newName + trailer); 
	    file2 = (ext2 != "")? (newName + "." + ext2 + trailer): (newName + trailer);

        ACS_COND_THROW( (file1==file2), exIOException("Internal error. Obtained the same names for File1 and File2: \"" + file1 + "\"")) ;  


	    // Create temporary filenames (to easily handle contingencies where file1 or 2 matches fileInWTrailer)
	    string tmpFile1 = File::tempname( newPath + "/File1" ) ;   /* Guard is automatic */ 
        ...
        unlink(file1); if (File::exists(tmpFile1))
        {
            ...
            ACS_COND_THROW( (File::size(file1) != file1Size), exIOException("Internal error. Output file1 has an invalid size.")) ;  
        }
        
        unlink(file2); if (File::exists(tmpFile2)) 
        {
            File::rename(tmpFile2, file2) ;
            ACS_COND_THROW( (File::size(file2) != file2Size), exIOException("Internal error. Output file2 has an invalid size.")) ;  
        }
    }
    else if ( ((0 == file1Size) || (0 == file2Size)) && (handleSingleExt && !extS.empty()) )
	{	
		...
		if (file1 != fileInWTrailer)
        {
			...
			else
            {
                ...
                ACS_COND_THROW( (File::size(file1) != (file1Size+file2Size) ),  
...
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	ACS_COND_THROW(File::exists(file1),exIOException("File \"" + file1 + "\" already exists .")) ;  

	ACS_COND_THROW(File::exists(file2),exIOException("File \"" + file2 + "\" already exists .")) ;  

	// Check if filenames differ
	if( fileIn == file1  || fileIn == file2 || file1 == file2)
	{
		ACS_THROW(exIOException("Files must have different path")); 
	}
	off_t FileInSize= File::size(fileIn) ;
	
	if(fileSize >  FileInSize)
	{
		ACS_THROW(exIOException() << "File \"" << fileIn << "\" size is " << FileInSize 
                                  << " requested split size is " << fileSize);
	}
	...
	int err = errno;
	ACS_COND_THROW( !streamIn,exIOException("Error while opening file \"" + fileIn + "\" .",err)); 

	ofstream stream1(file1.c_str() );	// open first output file
	int err2 = errno;
	ACS_COND_THROW( !stream1,exIOException("Error while creating file \"" + file1 + "\" .",err2)) ; 
	
	ofstream stream2(file2.c_str() );	// open second output file
	int err3 = errno;
	ACS_COND_THROW( !stream2,exIOException("Error while creating file \"" + file2 + "\" .",err3)) ; 
	...
	while(bytesToRead > 0)
	{
		if(bytesToRead > off_t(buffer.size())) {
			n=buffer.size();
        }
		else {
			n=bytesToRead;
        }
		streamIn.read(buffer.data(), n);
		stream1.write(buffer.data() , n);
		bytesToRead-=n;
	}
	stream1.close();
	...
	while(n == buffer.size())
	{
		streamIn.read(buffer.data(), buffer.size());
		n=streamIn.gcount();
		stream2.write(buffer.data() , n);
	}
}
...
File::CompressionMode File::compressionMode(const string &name) 
{
// EC: this method has multiple returns to keep its complexity as low as possible

	string result = file(name)  ;
	ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "result \"" << result << "\"") ;
...
bool File::isSymbolicLink(  const string &path, 
...
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	if (!recursive)
	{
		...
		{
			int err = errno;
			ACS_THROW(exFileOpenException("Cannot lstat  \"" + path +"\"",err)) ; 
		}
...
void File::fileStat(const string &path, struct stat &s, bool resolveLinks) 
{
	if (resolveLinks)
	{
		ACS_COND_THROW( 0 != stat(path.c_str(), &s), exFileOpenException("Cannot stat  \"" + path +"\"",errno)) ; 
	}
	else
	{
		ACS_COND_THROW( 0 != lstat(path.c_str(), &s), exFileOpenException("Cannot lstat  \"" + path +"\"",errno)) ; 
	}
}
...
void File::chown(const std::string &path, uid_t owner, gid_t group) 
{
    ...
    if (r != 0) {
	    int err = errno ;
        ACS_THROW(exIOException (err) << "Cannot chown(" << path << "," << owner << "," << group << ")") ; 
    }
}
...
void File::chown(const std::string &path, const std::string &owner, const std::string &group) 
{
    try {
        ...
        ACS_THROW(exIOException(x, "chown(" + path + "," + owner + "," + group + ")")) ; 
    }
}
...
void File::chown_rec(const std::string &path_in, const std::string &owner, const std::string &group, bool dirs_only) 
{
    try {
        ...
        ACS_THROW(exIOException(x, "chown_rec(" + path_in + "," + owner + "," + group + ")")) ; 
    }
}
...
{	
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB) 
		const string m = (dirs_only ? "only to directories" : "to files and directories") ; 
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called for path \"" << path_in << "\" owner: " << owner << " group: " << group 
				<< "  Applies " << m ) ;
	ACS_CLASS_END_DEBUG
	
	ACS_COND_THROW(!File::exists(path_in),exIOException("Path \"" + path_in + "\" do not exists .")) ;  

	char fsName_resolved[16*1024] ; // NOSONAR - easy
	const string path = ( realpath(path_in.c_str(), fsName_resolved) ? fsName_resolved : path_in ) ; 
...
{	
	...
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB) 
		const string m = (dirs_only ? "only to directories" : "to files and directories") ; 
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called for path \"" << path_in << "\" mode: " << prefix << "0" << std::oct << mode << std::dec 
				<< "  Applies " << m ) ;
	ACS_CLASS_END_DEBUG
	
	ACS_COND_THROW(!File::exists(path_in),exIOException("Path \"" + path_in + "\" do not exists .")) ;  

	char fsName_resolved[PATH_MAX] ; // NOSONAR - easy
	const string path = ( realpath(path_in.c_str(), fsName_resolved) ? fsName_resolved : path_in ) ; 
...
void File::chmod(const string &path, mode_t mode, PermissionSet perm)
{
	...
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called for file \"" << path << "\" mode: " << prefix << "0" << std::oct << mode) ;
	...
	if (skip)
	{
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "file \"" << path << "\" current_mode: 0" << std::oct << current_mode << std::dec << "  matched with the requested one: " 
				<< prefix << "0" << std::oct << mode) ;
	}
	else
	{
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "file \"" << path << "\" current_mode: 0" << std::oct << current_mode << std::dec << "  setting new mode: 0"
				<< std::oct << new_mode) ;
		
		if( ::chmod(path.c_str() , new_mode) == -1 ) {	// set new mode
			int err = errno ;
			ACS_THROW(exIOException (err) << "Cannot chmod  \"" << path << "\" to [octal] mode: 0" << std::oct << mode) ; 
		}
	}	
}
...
{
	ACS_COND_THROW(!File::exists(path),exIOException("File \"" + path + "\" do not exists .")) ;  
	struct stat s; 
    fileStat(path, s) ;

	ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "file \"" << path << "\" mode: " << std::oct << s.st_mode) ;
...
{
	ACS_COND_THROW(!File::exists(path),exIOException("Entry \"" + path + "\" do not exists ."));  
...
gid_t File::getGroupIDFromPath(const string &path, string& group_name)
{
	...
	try {
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "entry \"" << path << "\" gid: " << gid << "  group_name: \"" << group_name << "\"" ) ; 

	    return gid;
    }
    catch(const std::exception &x) { // NOSONAR - any exception
	    ACS_THROW( exIOException(x) << "Unable to get group name for entry \"" << path << "\"  gid: " << gid) ; 
    }
}
...
uid_t File::getUserIDFromPath(const string &path, string& user_name)
{
	...
	try {
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "entry \"" << path << "\" uid: " << uid << "  user_name: \"" << user_name << "\"" ); 

	    return uid;
    }
    catch(const std::exception &x) { // NOSONAR - any exception
	    ACS_THROW( exIOException(x) << "Unable to get user name for entry \"" << path << "\"  uid: " << uid) ; 
    }
}
...
void  File::setgroup(const std::string &path, const gid_t &gid)
{
	...
	const uid_t same_owner = -1 ;	/* DO NOT CHANGE */ 
    try {
	    File::chown(path.c_str(), same_owner, gid);
	    ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "file \"" << path << "\" gid: " <<gid ) ;
    }catch(const std::exception &x) { // NOSONAR - any exception
	    ACS_THROW(exIOException(x) << "Cannot change group to gid: " << gid << "  for entry: \"" << path << "\"") ; 
	}
}
...
void  File::setgroup(const std::string &path, const string& group_name)
{
	...
	try {
	    ...
	    ACS_THROW(exIOException(x, "Unable to get gid for entry \"" + path + "\"  group_name: " + group_name)) ;  
    }
}
...
FileGuard File::tempdir(const std::string& prefix, mode_t mode, PermissionSet perm) {
	...
	std::array<char, 4086> tmpl; 
	
	std::ostringstream tmpnameos;
	tmpnameos << prefix << "_" << ::pthread_self() << "_";
	
	strcpy(tmpl.data(), tmpnameos.str().c_str()); // NOSONAR - safe
	strcat(tmpl.data(), "XXXXXX"); // NOSONAR - safe
	const char *ret = mkdtemp(tmpl.data()); // NOSONAR - safe
	int err = errno;
	ACS_COND_THROW(0 == ret, exIOException(string("Cannot create ") + tmpl.data() + " directory.", err));

	std::string tmpdir = tmpl.data();
...
FileGuard File::tempname(const string &prefix, const string &ext)
{
	...
	std::ostringstream tmpnameos;
	tmpnameos << prefix << "_" << ::pthread_self() << "_";
	std::array<char, 4096> tmpl ; 
	strcpy(tmpl.data(), tmpnameos.str().c_str()) ; // NOSONAR - safe
	strcat(tmpl.data(), "XXXXXX") ; // NOSONAR - safe
	int fd = mkstemp(tmpl.data());
	int err = errno;
	ACS_COND_THROW(fd < 0,exIOException(string("Cannot create ") + tmpl.data() + " file.", err)) ; 
	
	string ret(tmpl.data()) ;
	if ( 0 != ::close(fd) )
	{
		int errC = errno;
	...
	return FileGuard(ret) ; 
}

void File::truncate(const string &path, off_t size) 
{
	File::touch(path) ;
	ACS_COND_THROW(0 != ::truncate(path.c_str(),size), 
...
void File::chdir(const string &path) 
{
	...
	{
		int err = errno;
		ACS_THROW(exIOException("Cannot change to directory  \"" + path +"\"",err)) ; 
	}
}
...
{
	if(::mkdir(path.c_str() , 0777) == -1 )	// Create directory (must not already exist)  // NOSONAR - permission ok
	{
        ...
        raiseException = err == EEXIST ? !allowExistingDir : true ;
        
    	ACS_COND_THROW(raiseException,exIOException("Cannot create directory  \"" + path +"\"",err)) ; 
    }
}
...
void File::rmdir(const string &path) 
{
	...
	{
		int err = errno;
		ACS_THROW(exIOException("Cannot remove directory  \"" + path +"\"",err)) ; 
	}
}

bool File::isOlder(const string &path, size_t seconds) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
	...
	if( stat(path.c_str() , &s) == -1 )
	{
		...
		ACS_THROW(exFileOpenException("Cannot stat  \"" + path +"\"",err)) ; 
	}
	time_t now = time(nullptr) ;
	return s.st_mtime < (static_cast<long long>(now) - static_cast<long long>(seconds)) ; 
}
...
string File::computeLockName(const string &fname, bool addExt) 
{
...
int File::lock(const string &fname, bool addExt, bool shared) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
    ...
    int f = open(lockname.c_str(), O_CREAT|O_RDWR, 0666) ;	// open lockfile NOSONAR - permission ok
	if (f<0)
	{
        	int err = errno;
		...
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "flock " << (shared ? "SH":"EX") << " could not open lockfile " 
	...
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "fd is " << fd) ; 


	// note the shared argument to lock in SHARED or EXCLUSIVE mode (non blocking)
	int fail = ::flock(fd, shared ? (LOCK_SH|LOCK_NB): (LOCK_EX|LOCK_NB)) ;	// lock is non-blocking (either shared or exclusive) 
    
	int err = errno ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "flock " << (shared ? "SH":"EX") << " returned " << fail << " " 
...
{	
    ...
    {
		int err = errno;
		ACS_THROW(exIOException("Cannot stat on volume " + volumeName, err)) ;  
    }
    
	if (struct_statf.f_bsize >= 1024) 
    {
        /* In order to reduce the loss of precision, a 10bit-shift is performed twice */
        long int bsize = (struct_statf.f_bsize >> 10) ; 
        bfree = (struct_statf.f_bavail >> 10) * bsize ; 
	}
    else
    {
        /* A 20bit-shift is performed once */
        bfree = (struct_statf.f_bavail >> 20) ; 
                    					...
                    					long int& blockSize)				/* OUT: Block size */
{
    ...
    return struct_statf.f_bavail ;		// Number of blocks 
}
...
{     
    ...
    return struct_statf.f_blocks ;		// Number of blocks 
}


// add the extension, avoid mistakes (e.g. ".<BLANK>" or "file..ext")
string File::addExt(const string &path, const string &ext)  
{
// EC: this method has multiple returns to keep its complexity as low as possible

	ACS_COND_THROW(path.empty(), exIOException("Empty file passed")) ; 
	ACS_COND_THROW((path.at(path.length()-1)=='/'), exIOException("Invalid file passed")) ; 
	
	if (ext.empty()) { return path ; }
	
	if ( (path.at(path.length()-1)=='.') || (ext.at(0)=='.')) { return (path + ext) ; } 
...
string File::getFExt(const string &path) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
...
string File::removeExt(const string &path, const string& extIN)  
{
// EC: this method has multiple returns to keep its complexity as low as possible
...
bool File::testMatchExt(const string &path, const string& extIN) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
	string ext = extIN ;
	...
	if (path.substr(path.length()-ext.length(), ext.length()) == ext) { 
...
{
	ACS_COND_THROW(fileWPath.empty(), exIOException("Empty file passed")) ; 
	ACS_COND_THROW(path.empty(), exIOException("Empty path passed")) ; 
	return getAbsolutePath(fileWPath).find(getAbsolutePath(path)) == 0 ;
} 

string File::getCurrentPath()
{
	std::array<char, PATH_MAX> p ;
	p.fill(0) ;
	
	const char * ret = ::getcwd(p.data(),p.size());
	int err = errno;
	ACS_COND_THROW(!ret, exIOException("Cannot determine current working dir", err)) ; 
	return p.data() ;
}


string File::getAbsolutePath(const string & path_in) 
{
	string ret_path ; 
	...
	else
	{
		...
		if (false == v.empty())
		{
			...
			for (auto it=lst.begin(); it!=lst.end(); /*No automatic increment, to allow erasing*/)
			{
				if ("." == *it) 
				{ 
...
string File::getDirPath(const string & path_in, bool normalizePath)
{
	...
	string path = (normalizePath? StringUtils::pathNormalize(path_in) : path_in) ; 
	...
	if (!path.empty())
	{
		...
		do
		{
			slash = path.rfind('/', pos-1) ;	// the slash we choose MUST NOT BE the last caracther in the path 

			ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "(WHILE/LOOP) slash: " << slash << " pos: " << pos) ;

		} while ((pos>0) && (slash != string::npos) && (slash == --pos)) ; // NOSONAR - side effect wanted
					...
					time_t & inventoryDate)	/* inventoryDate */
{
	...
	{
		ACS_THROW(exCriticalException("invalid format; expected filename#fileversion#inventorydate")); 
	}
	

	fileName = cacheName.substr(0, cacheName.find("#")) ;
	ACS_COND_THROW (fileName.empty(), exCriticalException("fileName resulted Empty")); 
	

	string tmp = cacheName.substr(cacheName.find("#")+1) ; 
	fileVersion = tmp.substr(0, tmp.find("#")) ;
	ACS_COND_THROW (fileVersion.empty(), exCriticalException("fileVersion resulted Empty")); 


	string invDate = tmp.substr(tmp.find("#")+1) ; 
	ACS_COND_THROW (invDate.empty(), exCriticalException( "invDate resulted Empty")); 
	inventoryDate = time_t(atoll (invDate.c_str())) ; 
	return ;
}
...
{	
	...
	if (inventoryDate>0) { d << std::setw(10) << std::setfill('0') << inventoryDate ; } 
...
string File::getFileName (const string & thepath)
{
	...
	if (slash != string::npos)
	{
		res += path.substr(slash+1,path.length()-slash ); 
	}
	else { res=path; }
...
string File::which(const string &exe)
{
	...
	{ // NOSONAR Thread safe guard around popen
		Mutex::Lock lock (_fileGlobalMutex);
		filep = popen(pcmd.c_str(),"r") ; 
	}
	int err = errno;
	ACS_COND_THROW(!filep,exFileOpenException("Cannot run which on \"" + exe +"\"",err)) ; 

	//******** PAOLO ************
	size_t bufSize = 0 ;
	char *buffer = nullptr;
	ssize_t retSize = ::getline( &buffer , &bufSize , filep) ;
    pattern::CMemoryScopeGuard bg(buffer) ;
    if( retSize == -1 ) {
		err = errno;
		Mutex::Lock lock (_fileGlobalMutex); // Thread safe guard around pclose
		if ( -1 == pclose(filep)  ) {
		
			int errC = errno;
			ACS_LOG_TRYCATCHNOTIFY_EX(exIOException ("::pclose syscall failed", errC));
		}
		ACS_THROW(exFileOpenException("Cannot run which on \"" + exe +"\"", err)) ; 
	}

	
	{ // NOSONAR - Thread safe guard around pclose
		Mutex::Lock lock (_fileGlobalMutex);
		if (-1 == pclose(filep) ) 
		{
			int errC = errno;
	...
	// get rid of ending spaces ff etc ...
	if (0 != isspace(buffer[retSize-1])) { // NOSONAR - buffer has been allocated from getline above
		buffer[retSize-1] = 0 ;
    }
...
string File::computeMd5Sum(const string & path)
{
	...
	ACS_COND_THROW(0 == fp, exFileOpenException("Cannot open file \""+path+'\"') ); 
	CRC_Digest_md5 md5 ;
	
	// Create buffer using digest's expected Size 
	unsigned char buffer[ md5.expectedSize() ] ; // NOSONAR - runtime computed
	...
	if( 0 != ferror(fp) ) {
		int error = fileno(fp) ;
		fclose(fp);
		ACS_THROW( exIOException(error) << "Error occurred reading from \"" << path << "\""); 
	}
	fclose(fp);
...
void File::Lock::unlock()
{
	...
	if (_lockedStreamBase>=0) {
        ...
        if (_autoRemoveLockFile && ! _lockFileName.empty()) {
...
void File::Lock::setLock(const std::string &file, bool addExt) // std added for doxygen
{
	...
	if ((_lockedStreamBase>=0) && (file != _lockedFile)) {
		...
		ACS_THROW(LockException("Cannot change the lock status, the file " + _lockedFile + " is already locked")) ; 
	}
	_lockedFile = file ;
	_addExt = addExt ;
}
...
bool File::Lock::lock(unsigned long timeout_ms, bool shared) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
    ...
    Timer::Delay delay = std::min(_defaultRetryTime_ms,timeout_ms) ; 
	...
	do {
		...
		if ( _lockedStreamBase < 0) {
		    if (-_lockedStreamBase != EWOULDBLOCK) {
			    ACS_THROW(exIOException("Cannot open lockfile \"" + lockFile + "\"", -_lockedStreamBase)) ;  
            }
...
void File::Lock::tryLock(Lock &l, int attempts, size_t attemptTimeout_ms, bool shared)  
{	
	ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Method called.  File: \"" << l._lockedFile 
	...
	if (0 != abs(attempts)) {
		...
		int err = errno;
	    	ACS_THROW(LockException("Cannot lock over " + l.filename(), err)) ; 
	}
	else {
		ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "looping until lock succesful") ;
		while (!l.lock(attemptTimeout_ms, shared)) { /* empty */ } ; 
		return ;
	}
}
...
*/
std::multimap<string, string> File::getMountEntries () // NOSONAR - no trasnparent comparator for this multimap
{
    ...
    for ( struct mount_entry* pMount_entry = pMount_entry_list; pMount_entry; (pMount_entry = pMount_entry->me_next) )
    {
        ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, 
                             "\n\tme_devname:!" << pMount_entry->me_devname
                         << "!\n\tme_mountdir:!"<< pMount_entry->me_mountdir 
                         << "!\n\tme_type:!"    << pMount_entry->me_type 
                         << "!\n\tme_dummy:!"   << pMount_entry->me_dummy
                         << "!\n\tme_remote:!"  << pMount_entry->me_remote 
                         << "!\n\tme_type_malloced:!" << pMount_entry->me_type_malloced << "!") ;
...
unsigned long long int 
						File::getFSUID (const string& fsName, 
...
// EC: this method has multiple returns to keep its complexity as low as possible
    ...
    if (followSymLinks && File::exists(filteredFsName) )
    {
		...
		filteredFsName = ( realpath(fsName.c_str(), fsName_resolved) ? fsName_resolved : fsName ) ; 
	...
	for (auto i=mountEntries.begin(); i != mountEntries.end(); ++i)
    {
	...
	if (found) 
	{
		...
		int c = 0; while ( (c = *str++) != 0) { hash = ((hash << 5) + hash) + c; /* hash * 33 + c */ }  
...
string File::trimAllSlashes(const string& inPath) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
...
string File::trimTrailingSlashes(const string& inPath) 
{
// EC: this method has multiple returns to keep its complexity as low as possible
    ...
    for (i=( static_cast<signed int>(inPath.size())-1); i>=0; i--) 
    {
    	char c = (inPath.c_str())[i] ;
...
*/
void File::splitTrailer(const string& inString,	/* I: Input string: e.g. Test#4#5*/ 
						...
						string& trailer)		/* O: Trailer: e.g. "4#5" */
{
	...
	trailer = inString.substr(ppos+pattern.length()) ; 
}

exostream & operator<< (exostream &os, File::CompressionMode m) 
{
    switch (m) {
    case File::Plain:
        os << "Plain" ;
    ...
    case File::TARred:
        ...
        break ;
    default:
        os << "<unrecognized File::CompressionMode: "<< int(m) << ">" ;
        break ;
    }
    return os ; 
}

std::ostream & operator<< (std::ostream &os, File::CompressionMode m) 
{
    switch (m) {
    case File::Plain:
        os << "Plain" ;

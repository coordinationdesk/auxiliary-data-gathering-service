...
public:  
    enum HWScanCommands	{ // NOSONAR - old enum
        GETMAC=0, 
        GETHDDSERIAL=1,
        GETHOSTNAME=2,
        GETCPUID=3
 ...
 */
static std::map<std::string, HWScanNetworkInterfaceStruct> getNetworkInterfacesMap(bool omit_domain, bool omit_loopback=false) ; // NOSONAR - map w/o transparent container

/**
 * memory dump
 */
static void dump(const unsigned char* buffer, size_t n) ; 

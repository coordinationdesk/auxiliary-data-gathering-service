/*
	...
	Copyright (C) 2011-2022, Exprivia SpA - DFDA-AS
		...
		Notify::Watch_Event::build(const string& userkey, const struct inotify_event *pevent, const std::map <int, Notify::Entry_feat_by_userkey>& entry_map)
{
	...
	ACS_COND_THROW( !pevent, NotifyCriticalException("Null pointer received as input") ) ; 
...
{
	Mutex::Lock lock(inot_entries_mutex_) ;
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_LOW_VERB) ;
	...
	// wd not found in map
	ACS_COND_THROW(i == entry_map.end(), NotifyCriticalException() << "descriptor not found in map: " << wd) ; 
	...
	if (j == (i->second).end()) {
		...
		ACS_THROW( NotifyCriticalException() << "descriptor with userkey: \"" << userkey << "\" not found in map") ; 

	}

	const string entry_name = (ext_inot_event.ionot_event.len>0)? string(ext_inot_event.stored_name.begin()): string("") ;  
					
	event = Notify::EventsMask(ext_inot_event.ionot_event.mask) ; 
		...
		Notify::Watch_Event::merge(const Notify::Watch_Event& src, const Notify::Watch_Event& dst)
{
	...
	out.event = static_cast<EventsMask> (int(src.event) | int(dst.event)) ;  
	out.dst_is_set = true ;
...
//
//////////////////////////////////////////////////////////////////////////////////////////
void Notify::Watch_Event::dump() const
{
	std::ostringstream o ; 
	...
	o << "\tevent: 0x" << std::hex << static_cast<uint32_t>(event) << std::dec << "  (" << eventmask_to_string(event) << ")"  
...
const size_t Notify::buffer_size_ = Notify::inot_max_events_ * (sizeof(inotify_event) + PATH_MAX ) ; 
const size_t Notify::filesystem_delay_usec = 2000000 ; // 2 sec  [unit: microsec = 1e-6 sec]
const size_t Notify::max_stored_events_threshold_default_ ;
const size_t Notify::max_stored_events_lowlimit_default_ ;
...
Notify::Notify() 
{
	...
	if ( (inot_instance_ = inotify_init()) < 0 ) {
		int err = errno;
		ACS_THROW(NotifyException(err) << "inot: " << inot_instance_ << "  Unable to initialize inotify"); 
	}
	else {
	...
	if ( (inot_buffer_ = reinterpret_cast<unsigned char*>(malloc( buffer_size_))) == nullptr ) { // NOSONAR - inotify
		int err = errno;
		ACS_THROW(NotifyException(err) << "inot: " << inot_instance_ << "  Unable to allocate a buffer of " << buffer_size_ << " bytes"); 
	}
	else {
...
//	lock on inot_mutex_ not needed ;
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "userkey: \"" << userkey << "\"  abs_path: \"" << abs_path << "\"  rel_base_path: \"" << rel_base_path << "  mask: " 
			<< int(m) << " ( " << eventmask_to_string(m) << " )") ;

	if (! File::exists(abs_path) ) {
		ACS_THROW(NoSuchFileOrDirectoryException("Fatal Error: Directory not found: \"" + abs_path + "\"" )) ; 
    }
	if (! File::isDir(abs_path) ) {
		ACS_THROW(NotADirectoryException("Fatal Error: Not a directory: \"" + abs_path + "\"" )) ; 
    }
...
{
	Mutex::Lock lock(inot_entries_mutex_) ; // Could be moved after the POLL -- to be checked 
 
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_LOW_VERB) ;
			
	ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "userkey: \"" << userkey << "\"  poll waiting time: " << wait_msec) ;
	
	struct pollfd plfd[1] ; // NOSONAR 
	...
	/*
		...
		This may result in new events (e.g. the creation of nested subdirectories). In order to ensure faster discoveries, 
        the reading loop is exectued more than one time, until:
	...
	*/
	...
	if (wait_msec != 0) {		
		do
		{
			int p = poll(plfd, sizeof(plfd)/sizeof(plfd[0]), wait_msec) ; 
             
			if ( p < 0 ) { 
				int err = errno;
				ACS_THROW(NotifyException(err) << "inot: " << inot_instance_ << "  Error running command poll"); 
			}
			...
			if ( new_event )
			{
				...
				if ( (r=read (plfd[0].fd, inot_buffer_, buffer_size_))  < 0) {
					int err = errno;
					ACS_THROW(NotifyException(err) << "inot: " << inot_instance_ << "  Read error ! (" << r << ")"); 
				}
				...
				if (r > 0)
				{
                    ...
                    while (ptr < r)
					{
						auto pevent = reinterpret_cast<struct inotify_event *> (inot_buffer_ + ptr) ; // NOSONAR - inotify

						if ( ! ((pevent->mask & events_to_be_discarded) == events_to_be_discarded) )
						{
							ACS_COND_THROW( (inot_entries_.find(pevent->wd) == inot_entries_.end()), NotifyCriticalException("Internal Error: wd not in map") ) ; 
							...
							{	
								...
								for (const auto &uk : userkeys)
								{
									...
									ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "About to built a new WatchEvent for this entry: " 
											<< "userkey: " << uk << "  wd: " << pevent->wd << "  mask: 0x" 
											<< std::hex << pevent->mask << std::dec << "  (" << eventmask_to_string(static_cast<EventsMask>(pevent->mask)) 
                                    ...
                                    { // NOSONAR - lock for shortest time possible
										/* Lock the stored_events_ map and add the event to the user's own list */
										Mutex::Lock lock1(inot_entries_mutex_) ;
						...
						}	// END: if ( ! ((pevent->mask & events_to_be_discarded) == events_to_be_discarded) )
		...
		} while (new_event && (--loop >0) ) ; // NOSONAR - if no new_event stop, else keep decrementing loop until it goes to 0
	...
	auto stored_event_it = stored_events_.find(userkey) ;
	
	
	if (stored_event_it != stored_events_.end())
	{	
		ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Moving " << (stored_event_it->second).size() 
				<< " events from the map stored_events_ to the container of userkey: \"" << userkey << "\"") ;

		for (auto const &we : stored_event_it->second) {
			container->insert( stored_event_it->first, we ) ;	// Add to the container
		}
		
		stored_events_.erase(stored_event_it) ;
	...
	ACS_CLASS_BGN_DEBUG(ACS_VLO_VERB)
		if ( ! container->empty() ) { container->dump(userkey) ; } 
	ACS_CLASS_END_DEBUG
...
{
	ACS_COND_THROW( (ll > th), NotifyCriticalException("Invalid pair of threshold/lowlimit") ) ; 
...
bool Notify::check_stored_events_trasholds()
{
	...
	{			
		...
		for (auto i=stored_events_.begin() ; i!=stored_events_.end() ; i++) 
		{	
			if ( (i->second).size() > max_stored_events_threshold_)
			{
				...
				found = true ;
				(i->second).erase( (i->second).begin(), (i->second).end()-max_stored_events_lowlimit_) ; 
			}
		}  
		
		ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Method finished. Returning: " << std::boolalpha << found ) ;
	}
	else
	{	
		...
		Notify::flush_mismatched_cookies() 
{
	...
	if ( ! inot_arch_events_by_date_.empty() )
	{
		...
		const uint64_t threshold_time = curtime - filesystem_delay_usec ; 
...
bool Notify::unregister_userkey(const string& userkey, bool emit_exception) 
{
	...
	for (auto it=inot_entries_.begin(); it!=inot_entries_.end(); ) 
	{
	...
	if (found && stored_events_.erase(userkey) > 0 ) {
        ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Stored events corresponding to key \"" << userkey << "\" successfully removed" ) ; 
	}
	
	ACS_COND_THROW((!found && emit_exception), NotifyException("No path to watch for userkey \"" + userkey + "\"") ); 

	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished with userkey: \"" << userkey << "\"  returning: " << std::boolalpha << found) ; 
...
{
	std::ostringstream o ; o << tid << "_" << get_key_from_curtime() ;
	std::string s = o.str() ; 
...
//
//////////////////////////////////////////////////////////////////////////////////////////
string Notify::eventmask_to_string(EventsMask m) // NOSONAR - complexity can be neglected: is a debug method
{
// ACS_CLASS_WRITE_DEBUG_TH ( ACS_HIG_VERB, "Method started. Input mask: " << std::hex << m << std::dec ) ;
	...
	if ( ret.empty() )
	{
		if (m & NOTIFY_ISDIR) { ret += "IS_DIRECTORY | " ; } 
	
		if (m & NOTIFY_ACCESS) { ret += "ACCESS | " ; }
		if (m & NOTIFY_MODIFY) { ret += "MODIFY | " ; }
		if (m & NOTIFY_ATTRIB) { ret += "ATTRIB | " ; }

		if ( (m & NOTIFY_CLOSE) == NOTIFY_CLOSE) { ret += "CLOSE | " ; }
		else
		{
			if (m & NOTIFY_CLOSE_WRITE)  { ret += "CLOSE_WRITE | " ; }
			if (m & NOTIFY_CLOSE_NOWRITE)  { ret += "CLOSE_NOWRITE | " ; }
		}
		
		if ( (m & NOTIFY_MOVE) == NOTIFY_MOVE) { ret += "MOVE | " ; }
		else
		{
			if (m & NOTIFY_MOVED_TO) { ret += "MOVED_TO | " ; } 
			if (m & NOTIFY_MOVED_FROM) { ret += "MOVED_FROM | " ; } 
		}

		if (m & NOTIFY_OPEN) { ret += "OPEN | " ; }  
		if (m & NOTIFY_CREATE) { ret += "CREATE | " ; }  
		if (m & NOTIFY_DELETE) { ret += "DELETE | " ; }  
		if (m & NOTIFY_DELETE_SELF) { ret += "DELETE_SELF | " ; }  
		if (m & NOTIFY_MOVE_SELF) { ret += "MOVE_SELF | " ; }  
		if (m & NOTIFY_IGNORED) { ret += "IGNORED | " ; }  

		size_t l_ret = ret.length() ;
		if ( (l_ret>=3) && (ret.substr(l_ret-3)==" | ") ) { ret = ret.substr(0, l_ret-3) ; } // Beautify 
				...
				Notify::get_userkeys_from_wd(const int& wd) const
{
	...
	auto wd_it = inot_entries_.find(wd) ;	

	if (wd_it != inot_entries_.end())
	{
		// (wd_it->second) is a map of type Entry_feat_by_userkey. 
        // It will be referenced below with the const_iterator j :

		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "wd: " << wd << "  Adding " << (wd_it->second).size() << " userkeys") ;

		for (auto const &wdp : wd_it->second)
		{
			/* Scan the map <userkey, Entry_features> , i.e. scan all the userkeys which registered the Entry_feature */
			ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "wd: " << wd << "  adding userkey: \"" << wdp.first << "\"") ;

			ret.emplace_back( wdp.first ) ;
...
{	
	...
	ACS_COND_THROW(wd_in < 0, NotifyCriticalException() << "invalid watch-descriptor passed as input: " << wd_in ) ; 
    ...
    do
	{
		...
		if ( i == inot_entries.end() ) {
			...
			ACS_THROW( NotifyCriticalException() << "descriptor not found in map: " << wd << "  (input descriptor: " << wd_in << ")" ) ; 
		}	// if ( i == inot_entries.end() ) 
		...
		if (j == (i->second).end())
		{
			...
			ACS_THROW( NotifyCriticalException() << "descriptor " << wd << " not found in map with userkey: \"" << userkey << "\"  (input descriptor: " << wd_in << ")" ) ; 
		}
										...
										bool emit_exception) const	/* IN: TRUE to emit exception in case of error. If FALSE -1 will be returned */
{
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "input wd: " << wd_in << "  userkey: \"" << userkey << "\"  emit_exception: " << std::boolalpha << emit_exception);
    ...
    do
	{
		...
		if ( i == inot_entries_.end() ) {
			...
			if (emit_exception) {
				...
				ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "descriptor not found in map: " << wd << "  (input descriptor: " << wd_in << ")") ;
				return -1 ; 
			}
		}	// if ( i == inot_entries_.end() ) 
		...
		if (j == (i->second).end())
		{
			...
			if (emit_exception) {
				...
				ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "descriptor " << wd << " not found in map with userkey: \"" << userkey << "\"  (input descriptor: " << wd_in << ")") ;
				return -1 ; 
			}
		}
		...
		Notify::automatically_add_new_entries(bool proc_unique, const string& userkey, const struct inotify_event *pevent)	
{
	...
	{
		char filename [ PATH_MAX + 1024 ] ; // NOSONAR - inotify
		memset (filename, 0, sizeof(filename)) ;
		if (pevent->len > 0) { strncpy (filename, pevent->name, pevent->len) ; } // NOSONAR - safe
		...
		if (recursive)
		{
			...
			ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "wd: " << wd << "  mask_in: " << std::hex << mask_in << std::dec 
					<< "  (" << eventmask_to_string(static_cast<EventsMask>(mask_in)) << ")" 
					<< "  resolved originator_wd: " << originator_wd 
					<< "  adding entry abs_path: \"" << abs_path << "\"  rel_base_path: \"" << rel_base_path
					<< "\"  mask: " << std::hex << mask_out) ;

			int wd_out = 0; 
			bool b = add_directory_tree(proc_unique, userkey, abs_path, rel_base_path, static_cast<EventsMask>(mask_out), originator_wd, wd_out) ; 
		...
		{
			ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "wd: " << wd << "  mask: " << std::hex << mask_in << std::dec  
					<< "  (" << eventmask_to_string(static_cast<EventsMask>(mask_in)) << ")  originator_wd not recursive: SKIPPED") ; 
		}
	}
	else
	{
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "wd: " << wd << "  mask: " << std::hex << mask_in << std::dec  
				<< "  (" << eventmask_to_string(static_cast<EventsMask>(mask_in)) << ")  not the creation of a new directory: SKIPPED") ; 
	}
...
bool 															/* OUT: TRUE if inserted in watch list (FALSE if already monitored) */
		Notify::add_directory_tree(	bool unique,				/* IN: TRUE means that the entry is PROCESS UNIQUE */ 
...
//	lock on inot_mutex_ not needed ; ????
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "abs_path: \"" << abs_path << "\"  rel_base_path_in: \"" << rel_base_path_in << "\"  mask: "  
			<< int(m) << " ( " << eventmask_to_string(m) << " )  originator_wd: " << ((wd_orig_in < 0)? "N/A" : debug_to_string(wd_orig_in))
			<< "  process_unique: " << std::boolalpha << unique)	;
	...
	try
	{
		...
		const string rel_base_path = ( (wd_orig_in > -1) ?  StringUtils::pathJoin(rel_base_path_in, entry_name) : rel_base_path_in) ;  
		...
		const int originator_wd = ( (wd_orig_in<0)? wd_out : wd_orig_in) ; 	
		...
		for (size_t i=0; i<dsize; ++i)
		{
			...
			ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Processing entries of directory: \"" << abs_path << "\"  #" << (i+1) << " of " <<  d.size()  
					<< "  : \"" << entry_abs_path << "\"  (\"" << entry_rel_path << "\"") ;

			bool b = false ;

			try { // NOSONAR - inner try to ignore add_entry problems ?
				
				ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Trying to insert in map entry #" << (i+1) << " of " <<  d.size() << "  : \""  
							...
							int& wd_out)					/* OUT: wd (watch_descriptor) */
{
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "userkey: \"" << userkey << "\"  full_path: \"" << full_path << "  rel_path: \"" << rel_path  
			<< "\"  originator_wd: " << ((wd_orig_in < 0)? "N/A" : debug_to_string(wd_orig_in)) 
			<< "  mask: " << int(m) << " ( " << eventmask_to_string(m) << " )" << "  process_unique: " << std::boolalpha << proc_unique) ;
...
*/
	...
	*/
	...
	if ( (wd_out = inotify_add_watch(inot_instance_, full_path.c_str(), m)) < 0) 
	{
		int err = errno;
		ACS_THROW(NotifyException(err) << "inot: " << inot_instance_ << "  Unable to add a watch to path: \""  << full_path << "\""); 
	}
	...
	auto it = inot_entries_.find(wd_out) ;

	if (it != inot_entries_.end())
	{
		if (!proc_unique) {
			/* Not process unique: it will be inserted if not watched by the very same user */
			to_insert = ((it->second).find(userkey) == (it->second).end()) ;
		} else {
			to_insert = ((it->second).empty()) ; 
		}
	}
	...
	{
		Entry_features ef ;
		ef.originator_wd = ( (wd_orig_in<0)? wd_out : wd_orig_in) ; 
		ef.mask = m ; 
															...
															Extended_inotify_event& matched_event)		/* OUT: matched event, if any */
{
	...
	const auto event_in = Notify::EventsMask(pevent->mask) ; 
		
	if ( 0 == (event_in & maskable_events) ) 
	{
		...
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Rejecting NON-MASKABLE Event: 0x" << std::hex << int(event_in) << std::dec << "  ( "  
	...
	else 
	{
		...
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Processing cookie: " << cookie << "  for event: 0x" << std::hex << int(event_in) << std::dec  
				<< "  ( " << eventmask_to_string(event_in) << " )") ;

		if ( (i == inot_arch_events_by_cookie_.end()) || ( (Notify::EventsMask((i->second).ionot_event.mask) & NOTIFY_MOVE) == (event_in & NOTIFY_MOVE)) )  { 
			/** EITHER: 
		...
		else
		{
			...
			ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Matching cookie: " << (i->second).ionot_event.cookie << "  found in map. Matching event: 0x" << std::hex << int((i->second).ionot_event.mask) << std::dec  
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Current size of cookie map: " << inot_arch_events_by_cookie_.size() 
			...
			<< "  Returning:   to_insert: " << std::boolalpha << to_insert  << "  to_merge: " << std::boolalpha << to_merge) ;
										...
										Watch_Event& ev) const								/* IN/OUT: current event, to be merged */
{
	...
	/* Check the cookies */
	ACS_COND_THROW( (matched_event_in.ionot_event.cookie != ev.cookie), NotifyCriticalException("Internal error: Cookies don't match!") ) ; 

	
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Matching cookie: " << ev.cookie << "  found in map. Matching event: 0x" << std::hex << int(ev.event) << std::dec 
...
void Notify::remove_merged_event(const Extended_inotify_event& matched_event_in)
{
	...
	else
	{
		...
		ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "Cookie " << cookie << " REMOVED from cookie_ map. Current size of map: " 
				<< inot_arch_events_by_cookie_.size() << "  Removed from date_ map ? " << std::boolalpha << r) ;
	}
		...
		Notify::remove_from_inot_arch_events_by_date(const uint64_t& key, const uint32_t& value) 
{
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Returning: " << std::boolalpha << found) ;
...
void	Notify::dump_entry_map() const
{
	...
	for ( auto i=inot_entries_.begin() ; i != inot_entries_.end() ; i++)
	{
...
void	Notify::dump_archived_entries_maps() const
{
	...
	for ( auto i=inot_arch_events_by_cookie_.begin() ; i != inot_arch_events_by_cookie_.end() ; i++)
	{
	...
	for ( auto i=inot_arch_events_by_date_.begin() ; i != inot_arch_events_by_date_.end() ; i++)
	{
		ACS_LOG_DEBUG("\tkey: " << i->first << "n\tvalue: ") ;
	}
			...
			Notify::get_key_from_curtime()
{
	...
	struct timeval t ; 
   	...
   	if (0 != gettimeofday(&t, nullptr) ) {
		ACS_THROW(NotifyException("gettimeofday bad return value", errno)); 
    }
	uint64_t ret = ( (uint64_t(t.tv_sec) * 1000000) + uint64_t(t.tv_usec) ) ; // SSSSSSSSSSUUUUUU (s=second, u=microsecs) 
...
{
	ACS_LOG_DEBUG(SimpleDebugSignature << "\t\toriginator_wd: " << e.originator_wd << "\n\tmask: 0x" << std::hex << e.mask << std::dec << "  ( " << eventmask_to_string( EventsMask(e.mask) )  
		<< " )\n\tabs_path: \"" << e.full_path << "\"\n\trel_path: \"" << e.rel_path << "\"\n\trecursive? " << std::boolalpha << e.rec) ;
...
*/
bool 														
		Notify::check_entry_exists_and_is_dir(const std::string& entry) 
{
	...
	ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "Method finished. Entry: \"" << entry << "\"  Returning: " << std::boolalpha << ret) ;

...
public:
	...
	virtual ~Operation01() = default ;

	// returns how much data have been set
	virtual size_t get(void *, size_t size, std::string &, const std::vector<std::string> &mods) const = 0 ; // NOSONAR - void * because generic
	
	virtual void set(const void *, size_t size, const std::string &) const = 0 ; // NOSONAR - void * because generic
	
	virtual bool remove(const std::string &mod) const = 0 ;             // PRQA S 2502
protected:
	Operation01(const Operation01 &) = default ;

	Operation01 &operator=(const Operation01 &)  = default ; // operator =

	const std::string &getFeature() const noexcept ;
	const std::string &getInstance() const noexcept ;
private:
	std::string _feature ;

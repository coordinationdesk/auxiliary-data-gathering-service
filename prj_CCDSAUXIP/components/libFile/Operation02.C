...
extern "C" {
	...
	void ACS_Operation02_checkData(const char* p, int exceptionType_BOOL) // PRQA S 2502
	{
		try 
		{
			acs::Operation02::checkData( p, (exceptionType_BOOL != 0) ) ;
		}
		catch(const std::exception& )
		{
			...
			if (exceptionType_BOOL != 0) {
				...
				os << " " << p ;
				os << " on " << "hostname " << acs::HWScan::hostname() ;  
				os << " please" << " report" << " to" << " technical" << " support." ;
			}
			else {
				// Emit not-explicit message
				os << "Cannot create temporary file \"" << "/tmp/dbOp" << "feature" << ".tmp" << "\"" ; // NOSONAR - safe & secure
				os << "on hostname " << acs::HWScan::hostname() ;
				os << " please" << " report" << " to" << " technical" << " support." ;
			}

			acs::exostream::getDefaultOutLogger() << os.str() << "\n" << "\n" ;
...
using std::vector ;
using std::map ;
using pattern::CMemoryScopeGuard ;
...
//------------------------------------------------------------------------
Operation02::ResetAndGuardLDataParam::ResetAndGuardLDataParam(struct LDataParam* a) noexcept 
    :_address(a)
{
...
//                            ~ResetAndGuardLDataParam
//------------------------------------------------------------------------
Operation02::ResetAndGuardLDataParam::~ResetAndGuardLDataParam() noexcept // PRQA S 4020
{ 
	// Guard destructor 
    if (!_address) { return ; }

    if (_address->hwInData) { free(_address->hwInData) ; } // NOSONAR - free ;
    
    if (_address->hwOutData) { free(_address->hwOutData) ; } // NOSONAR - free ;
    
    free (_address);  // NOSONAR 
    _address = nullptr;
} 
...
{
    ldata->hwRescan = 1;
    memset(ldata->appName, 0, sizeof(ldata->appName)) ; strcpy(reinterpret_cast<char*>(ldata->appName), "test"); // NOSONAR 
    ldata->creationDate = int(time(nullptr)) ; // PRQA S 3000, 3081

    ldata->type = 0x01 ;
    ldata->expDays = 29 ;   // PRQA S 4400
    memset(ldata->userData, 0, sizeof(ldata->userData) ) ;
    strcpy(reinterpret_cast<char*>(ldata->userData), "COSMO TEST LICENSE $Id$ 2005.11.04 Very preliminary version. No hardware encoded. No private key. Little Endian.") ; // NOSONAR 
    ldata->hwParamNum = 0 ;
    struct HINDataParam* hw_invector = reinterpret_cast<struct HINDataParam*>( calloc(ldata->hwParamNum, sizeof(struct HINDataParam))) ; // NOSONAR - memory to be allocated here
...
// Filling hardware element....
    ...
    strcpy(reinterpret_cast<char*>(hw_invector[tmpcounter].buffer), "test") ; // NOSONAR 
    hw_invector[tmpcounter].buflen = strlen(reinterpret_cast<char*>(hw_invector[tmpcounter].buffer)) ; // NOSONAR 
        ...
        Operation02::dumpLDataParam_DEBUG(struct LDataParam const* ld) 
{
    ...
    try
    {
        ...
        std::array<char, 30> buf ; // should be at least 26 bytes long
		string stmp = ctime_r(&tmp, buf.data()) ;
    ...
    try
    {
        ...
        */
        ...
        { 
            ...
            #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
                ACS_CLASS_WRITE_DEBUG(ACS_INS_VERB,std::hex << " ret = 0x" << static_cast<unsigned int>(ret[i])  // PRQA S 3081
                                        ...
                                        unsigned int l)                     /* input data length */
{
    ...
    try
    {
        ...
        */
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_INS_VERB,std::hex  // PRQA S 3081 
                << " a: 0x" << static_cast<unsigned int>(a) << " b: 0x" << static_cast<unsigned int>(b) << std::dec << " len: " << l);
        #endif

        // Generate TEMPORARY sequence
        unsigned char* seq = reinterpret_cast<unsigned char*> (malloc(l)) ; // NOSONAR - memory to be allocated here
		ACS_COND_THROW(!seq,Operation02_InternalError("Operation02::Internal error IE0x0203")) ; 
        
        CMemoryScopeGuard memoryGuard(seq) ;  // no free() is required
                                ...
                                unsigned int l)             /* input data length */
{
    ...
    try
    {
        ...
        */
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_INS_VERB, std::hex // PRQA S 3081  
                << " ab: 0x" << static_cast<unsigned int>(ab) << std::dec 
        ...
        if (scrambleWithMAC)
        {
            ...
            unsigned char* macAddr = reinterpret_cast<unsigned char*>( HWScan::ScanHW_Scan(&retval, HWScan::GETMAC, param, nullptr, 0)) ;  // NOSONAR 
            ...
            CMemoryScopeGuard m(macAddr) ; // no free is required ;
            ...
            Operation02::fitDataIntoBuffer(reinterpret_cast<unsigned char*>(&mac), sizeof(mac), macAddr, retval) ;  // NOSONAR 
...
void Operation02::checkData(const std::string &, bool ) // PRQA S 4020
{
	// EnrCar
    ...
    #endif
	vector <string> v ;
    map <string, char, std::less<>> m ;	 /* optional overriding map. To override the standard behaviour for a specific task 
                                            (feature), just insert the feature name into the map, with the parameter: C (core), 
                                            E (standard error), L (license error).
...
// 	}
	...
	char override = ( m.find( feature) != m.end() ) ? m[feature]: 0 ; // PRQA S 3010, 3380, 3385
	...
	if (0 != override)
	{
        ...
        ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB) 
            ...
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "feature has been found. Override: !"  << ov << "!  exType:" << std::boolalpha << exType) ;
        ACS_CLASS_END_DEBUG
    #endif
...
#endif // ACS_OPERATION02_CHECKLDATA
}
...
unsigned char*                                                                  /* OUT: LDATA  */
               Operation02::createLData(unsigned int*   outdatalen,             /* OUT_LDATA length  */
                                        ...
                                        const RSA* privKey,                     /* IN: RSA Private Key (or 0) */
                                        ACS_SSL::CipherStyle cipherRSA,			/* IN: cipher with RSA key */
                                        struct LDataParam * indataparam)   /* IN: data param  */
{	
	// EnrCar:
    ...
    try
    {
        if (strlen(name) >= sizeof(indataparam->appName)) // NOSONAR - not a sec issue
        {
        ...
        strncpy(reinterpret_cast<char*>(indataparam->appName), name, sizeof(indataparam->appName)-1);  // NOSONAR 
 
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called for AppName:" << reinterpret_cast<const char*>(indataparam->appName)) ; // NOSONAR 
        #endif

        unsigned int    datapointer = 0 ;
        std::array<unsigned char, 1024> package ;

        std::array<unsigned char, 1024> temp_buffer ;
        // UNUSED. DECOMMENT IF NEEDED:  unsigned int    temp_i ;
        // UNUSED. DECOMMENT IF NEEDED:  unsigned short  temp_s ;
        unsigned char   temp_b = 0 ;

        unsigned char*  ldata = reinterpret_cast<unsigned char*>(malloc(64*1024)) ;  // NOSONAR - memory to be allocated here
        if (! ldata)
        {
        ...
        for (unsigned int i=0; i<2; i++) // PRQA S 4400
        {
            ...
            // RSVD
            memset(temp_buffer.data(), 0, dataLen); ACS_Operation02_AddPackage(temp_buffer.data(), (dataLen-pkgpointer)) ; // PRQA S 3084
            ...
            {
                unsigned int outl = 0 ;
                unsigned char* tmp = Operation03::EncodePkg(cipherRSA, &outl, privKey, package.data(), pkgpointer) ; 
                CMemoryScopeGuard g(tmp);
                ACS_Operation02_AddLData(tmp, outl) ; // PRQA S 3084
            }
            else
                ACS_Operation02_AddLData(package.data(), pkgpointer) ; // PRQA S 3084
        ...
        retdata = reinterpret_cast<unsigned char*>( realloc(ldata, datapointer) ); // NOSONAR - memory to be reallocated here
        if (!retdata)
        {
            // Unable to realloc
            if (ldata) { free(ldata) ;  ldata = 0 ; } // NOSONAR 
...
void 
        Operation02::readLData( struct LDataParam* outdataparam,    /* OUT: struct LDataParam */
                                ...
                                const RSA* pubKey,                  /* IN: RSA Public Key (or 0) */
                                ACS_SSL::CipherStyle cipherRSA,		/* IN: ciphered with RSA key */
                                unsigned int)             /* IN: IN_LDATA_length */
{
	// EnrCar:
    ...
    try
    {
        ...
        for (unsigned int i=0; i<2; i++) // PRQA S 4400
        {
            ...
            if (cipherRSA != ACS_SSL::NOCIPHER)
            {
                ...
                unsigned int outl = 0;
                unsigned char* tmp = Operation03::DecodePkg(cipherRSA, &outl, pubKey, package.data(), packageLen) ;
                CMemoryScopeGuard g(tmp); 
                if (outl != dataLen)
                {
                ...
                package.fill(0) ;
                memcpy(package.data(), tmp, outl) ;    // fill package with unciphered data
            }
            else
            ...
            // RSVD
            ACS_Operation02_ReadPackage(temp_buffer.data(), (dataLen-pkgpointer) ) ; // PRQA S 3084
        }
        ...
        outdataparam->hwInData = reinterpret_cast<HINDataParam*>( calloc(outdataparam->hwParamNum, sizeof(struct HINDataParam))) ; // NOSONAR - memory to be allocated here
        outdataparam->hwOutData = reinterpret_cast<HOUTDataParam*>( calloc(outdataparam->hwParamNum, sizeof(struct HOUTDataParam))) ; // NOSONAR - memory to be allocated here
...
// Due to endianness fast-swapping macros. The code is fully readable
    ...
    unsigned char* retBuffer = nullptr ;
    ...
    try
    {
        ...
        retBuffer = reinterpret_cast<unsigned char*>( malloc( len+7 ) ) ;  // NOSONAR - memory to be allocated here
        ...
        { // NOSONAR - Add data length
            unsigned int temp_i = len ; // Create temp var. for byte swapping
            ACS_ByteSwap_SWAP( &temp_i, sizeof(temp_i), true ); // Force BIGENDIAN
            memcpy(retBuffer+3, &temp_i, sizeof(temp_i) ); 
        }
...
unsigned char*																	/* OUT: LData */
                Operation02::unpackLData(   unsigned int* outlen,				/* LData size (should be const)*/
                                            ...
                                            const unsigned char* blobdata,			/* IN blobdata */
                                            ...
                                            unsigned short ab)					/* ab parameter */
{
    ...
    try
    {
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Method called with ab: 0x" << std::hex << ab << std::dec 
                    << " bloblen: " << bloblen) ;
        #endif
        ...
        retBuffer = reinterpret_cast<unsigned char*>( malloc( len ) );  // NOSONAR - memory to be allocated here
        ...
        // Evaluate checksum
        unsigned char checkSum[2] ;  // NOSONAR - pure low level C 
...
///////////////////////////////////////////////////////////////////////////////////////
bool    // TRUE is good
        Operation02::checkLData(    const unsigned char *buffer,    // buffer // PRQA S 4020
                                    ...
                                    bool dumpData)                  // IN: SET TRUE to dump data (DEBUG ONLY)
{
	...
	unsigned short ab = ACS_SSL::ACS_CRC16_eval( featmod.c_str(), 0, featmod.length() ) ; // PRQA S 3010
        ...
        ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Method called. Feature !" << featmod << "!" 
	    	<< "  length: " << featmod.length() << "  ab: 0x" << std::hex << ab) ;
#endif
    ...
    CMemoryScopeGuard g(ldataout) ; // no free is required
    ...
    struct LDataParam* lDataParam = reinterpret_cast<LDataParam*>( malloc( sizeof(LDataParam) )) ;   // NOSONAR - memory to be allocated here
...
// Hardware should be re-aligned if rescan == true and dumpData==false
   ...
   if (0 != strncmp( reinterpret_cast<char*>(lDataParam->appName), featmod.c_str(), sizeof(lDataParam->appName) )) // NOSONAR 
   {
        // Check on appName failed!
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "LData_Appname: " << reinterpret_cast<char*>(lDataParam->appName) // NOSONAR 
    ...
    if (lDataParam->type == 0xff) // PRQA S 4400
    {
        ...
        found = true ;
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Type 0x" << std::hex << lDataParam->type << std::dec
    ...
    if ((lDataParam->type == 0x00) || (lDataParam->type == 0x01))
    {
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Type 0x" << std::hex << lDataParam->type << std::dec // PRQA S 3380
    ...
    if ((lDataParam->type == 0x00) || (lDataParam->type == 0x10)) // PRQA S 4400
    {
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Type 0x" << std::hex << int(lDataParam->type) << std::dec // PRQA S 3081, 3380
    ...
    {
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Type 0x" << std::hex << int(lDataParam->type) << std::dec // PRQA S 3081
    ...
    #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Returning value: " << std::boolalpha << status) ;
    #endif
        ...
        Operation02::checkHWParams( struct LDataParam const* ld)
{
    ...
    {    
        ...
        unsigned char *data = reinterpret_cast<unsigned char*>( HWScan::ScanHW_Scan(&outlen, ld->hwInData[i].cmd, ld->hwInData[i].param, ld->hwInData[i].buffer, ld->hwInData[i].buflen)) ; // NOSONAR 
        ...
        {
        	CMemoryScopeGuard m(data) ; // no free is required ;
			unsigned char cmpbuffer[sizeof(ld->hwOutData[i].data)]  ; // NOSONAR - dynamic size
            ...
            for (size_t j=0; j<sizeof(ld->hwOutData[i].data); j++) 
            {
            	...
            	if (0 != cmpbuffer[j]) 
                {
                	...
                	if ( ld->hwInData[i].blocking || (++nonBlkCounter > nonBlkThreshold) ) // NOSONAR - side effect wanted (if not blocking )
                    {
                        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
        ...
        #ifdef ACS_OPERATION02_HAS_DEBUG_ENABLED
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "[" << i << "] Status is: " << std::boolalpha << status ) ;
        #endif


	}	/* for (unsigned int i=0; i<ld->hwParamNum; i++) */

    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Status is: " << std::boolalpha << status ) ;
...
{	
	...
	if (sbuflen>(2*dbuflen)) { return nullptr ; } // PRQA S 3084, 4400
	
	d[0] = 0 ;

	for (size_t i=0; i<sbuflen; i++)
	{
		char t[8] ; // NOSONAR - pure low level C 
		sprintf(t, "%02X", int(s[i])) ; // NOSONAR - 3 char used
		strcat(d, t) ; // NOSONAR - 3 char used
	}
                                                								...
                                                								OUT: how many bytes written into buffer */
{
	...
	if ((!s) || (!d) || (!dbuflen) ) { return nullptr ; }
	
    size_t len_s = strlen(s) ; // NOSONAR - no buf overflow
    if ( len_s/2 > (*dbuflen) ) { return nullptr ; } 
    
    if (dbuflen) { *dbuflen = 0; }
	for (size_t i=0; i<len_s; i+=2) // PRQA S 3084, 4400
	{
		char t[3]; // NOSONAR - pure low level C 

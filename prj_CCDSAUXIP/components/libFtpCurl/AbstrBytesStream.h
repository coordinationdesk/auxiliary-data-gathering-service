...
{
public:
   explicit AbstrBytesStream(IoBuffer& buffer) : Stream(), _buffer(buffer) { } // PRQA S 2528

   AbstrBytesStream(const AbstrBytesStream &) = delete ; 
   AbstrBytesStream& operator= (const AbstrBytesStream &) = delete ; 
   
   ~AbstrBytesStream() override = default ;

   IoBuffer& getBuffer() const noexcept { return _buffer; }

   size_t available() const { return _buffer.size(); }

   bool closed() const override { return _closed; }

   void setClosed(bool v) noexcept { _closed = v ; }

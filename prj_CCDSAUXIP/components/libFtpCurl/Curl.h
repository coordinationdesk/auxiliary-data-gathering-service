...
// PRQA S 1020, 1023 L1
  #define BEGIN_TRANSACTION(operationCode, withCallback_)     \
      ...
      std::unique_ptr<CurlTransaction> handle;\
	  ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "BGN_TRN withCallback_=" << std::boolalpha << (withCallback_));\
      if (t == 0) { \
...
public:
	...
	public:
        ...
        CurlHttpOpt(const CurlHttpOpt& ) = default ;
		
		~CurlHttpOpt() = default ;
		
		CurlHttpOpt& operator=(const CurlHttpOpt& ) = default ;
	...
	/** Setters */
        ...
        {
          computeTimeouts(ms, adjustResponseTimeout, _connectionTimeout_ms, _connectionTimeout_s, _responseTimeout_ms, _responseTimeout_s);
          _timeoutSet = true;
        }

    /** Getters */
		inline const std::string& getUri(bool* set = nullptr) const {
			if (set) { *set = _uriSet; }
			return _uri;
		}
		
		inline const std::string& getMessage(bool* set = nullptr) const {
			if (set) { *set = _messageSet; }
			return _message;
		}
		
		inline const std::string& getFileToUpload(bool* set = nullptr) const {
			if (set) { *set = _fileToUploadSet; }
			return _fileToUpload;
		}
		
		inline const std::string& getContentType(bool* set = nullptr) const {
			if (set) { *set = _contenttypeSet; }
			return _contenttype;
		}
		
		inline const std::string& getUser(bool* set = nullptr) const {
			if (set) { *set = _userSet; }
			return _user;
		}
		
		inline const std::string& getPassword(bool* set = nullptr) const {
			if (set) { *set = _passwdSet; }
			return _passwd;
		}
		
		inline const std::string& getUserPassword(bool* set = nullptr) const {
			if (set) { *set = _userpwdSet; }
			return _userpwd;
		}
        ...
        inline void getTimeouts(size_t& connectionTimeout_ms, size_t& connectionTimeout_s, size_t& responseTimeout_ms,
                                size_t& responseTimeout_s, bool* set = nullptr) const
        {
          if (set) { *set = _timeoutSet; }
  ...
  private:
		...
		bool _secure = false ;				// If true, secure authentication will be used
		bool _strictAuth = true ;			// If true, strict authentication is used. (Used only if _secure is true)
        size_t _connectionTimeout_ms = 0 ;
        size_t _connectionTimeout_s = 0 ;
        size_t _responseTimeout_ms = 0 ;
        size_t _responseTimeout_s = 0 ;

        bool _uriSet = false ;
		bool _messageSet = false ;
		bool _fileToUploadSet = false ;
		bool _contenttypeSet = false ;
		bool _userSet = false ;
		bool _passwdSet = false ;
		bool _userpwdSet = false ;
        bool _timeoutSet = false ;
  };

    using ProgressCallback = int (*)(void *curl, double downloadTotal, double downloadNow, double uploadTotal, double uploadNow); // PRQA S 2502
    using DebugCallback = int (*)(void *curl, int infotype, char *dbg, size_t size, void *data);
    using ListCallback = size_t (*)(void *buffer, size_t size, size_t nmemb, void *stream);
    using GetCallback = size_t (*)(void *buffer, size_t size, size_t nmemb, void *stream);
    using PutCallback = size_t (*)(void *buffer, size_t size, size_t nmemb, void *stream);
     ...
     */
    CURL* openConnection(); // NOSONAR - CURL is a type from curl
    /**
     ...
     */
    CURLcode ls(std::string const &, void* output = nullptr);
     /**
    ...
    */
     ...
     */
    CURL* getCurlHandle(); // NOSONAR - CURL is a type from curl
     ...
     */
    bool probe(std::string const &remotePath, bool emitExc = false, CURLcode* curlCode = nullptr);
...
public: // NOSONAR - for clarity
    ...
    const RemoteSite &getSite() const noexcept ;

    void setHttpContentType(const std::string &cT ) { httpContentType_ = cT; }
    const std::string &getHttpContentType( ) const noexcept{ return httpContentType_;}
    void clearSlist();

    void setTransaction(CurlTransaction* transaction) noexcept ;
    void setCreateMissingDirs(bool createMissingDirs) noexcept ;
    bool getCreateMissingDirs() const noexcept ;
    bool setProbeToken();
    size_t getConnectionTimeout_ms() const noexcept ;
    /*- returns old value */
    size_t setConnectionTimeout_ms(size_t ms, bool adjustResponseTimeout = true) noexcept ;
    static size_t computeTimeouts(const size_t ms, const bool adjustResponseTimeout, size_t& connectionTimeout_ms,
                                  ...
                                  size_t& response_timeout_s) noexcept;
...
public:
     ...
     */
    void setProgressCallback(ProgressCallback progressCallback, void* progressCallbackData = nullptr);
    /**
     ...
     */
    void setListCallback(ListCallback listCallback, void* listCallbackData = nullptr);
    /**
     * Reset the default list callback;
     */
    void setDefaultListCallback();

    void fillInCirculationStatistics(bool source, CirculationStatistics* ) const ;
    ...
    */
    long getHttpResponseCode() const noexcept { return _http_response_code ; }
...
protected:
    ...
    TransactionType getOperationCode() const noexcept ;
    void setOperationCode(TransactionType) noexcept ;
    CurlTransaction* getTransaction() const noexcept ; 
    void setCurl(CURL *) noexcept ; 
...
private:
    ...
    void setProxy() ;
	void checkFtpCommandResponseTimeout() noexcept;
    void setHTTPOpts(const CurlHttpOpt& opt);

private: // NOSONAR - for clarity
    CurlTransaction* _transaction = nullptr ;
    CURL* _curl = nullptr ; // NOSONAR - CURL is an external type from curl
        
    std::array<char, 512> _errorMessage = {} ;
    RemoteSite _site;
    
    ProgressCallback _progressCallback = nullptr ;
    ListCallback _listCallback  = &listCallback ;
    GetCallback _getCallback    = &getCallback ;
    PutCallback _putCallback    = &putCallback ;
    DebugCallback _debugCallback= &debugCallback ;

    std::string _userPassword = "" ;
    std::string _ftpDefaultPort = "-" ;
    bool _createMissingDirs = false ;

    void* _progressCallbackData = nullptr ; // NOSONAR - callback data
    void* _listCallbackData  = nullptr  ; // NOSONAR - callback data

    std::vector<std::shared_ptr<CurlHeaderMaker> > _ptrToHeaders = {} ;
    bool _isCleaning = false ;

    // for the following parameters see: http://curl.haxx.se/libcurl/c/curl_easy_setopt.html
    size_t _connectionTimeout_ms = conf::CurlConfigurerSingleton::instance()->getConnectionTimeout_ms() ;
    size_t _connectionTimeout_s = std::max<size_t>(2,_connectionTimeout_ms/1000);
    size_t _response_timeout_ms = std::min(_connectionTimeout_ms,conf::CurlConfigurerSingleton::instance()->getConnectionResponseTimeout_ms());
    size_t _response_timeout_s = std::max<size_t>(1, _response_timeout_ms/1000) ;
    size_t _ftp_command_response_timeout_s = std::min(_connectionTimeout_s, conf::CurlConfigurerSingleton::instance()->getFtpCommandResponseTimeout_s());

    mutable std::string _lastLoggedErrorMessage = "" ;
    TransactionType _operationCode = TrUnset ;
    Statistics _stats = {} ;
    ...
    long _http_response_code = 0 ;
    bool _httpFailOnError = true ;

...
#else
#warning libcurl < 7.61.1 configurable buffer support DISABLED
#endif


namespace {

    const acs::lfcV _module_version_ ;
...
const std::string CurlConfigurer::defaultLocalTempPath = "/tmp"; // NOSONAR - safe
...
void CurlConfigurer::confLoadedCallback(void *cp) // PRQA S 4020
{
    ...
    setOnceForRun(false);
    CurlConfigurer* c = reinterpret_cast<CurlConfigurer *>(cp); // NOSONAR - callback
    c->load();
    c->dump();
}
...
CurlConfigurer::CurlConfigurer() 
{
    ...
    catch(std::exception &x) { // NOSONAR - any exception
        ACS_LOG_NOTIFY_EX(x) ;
    }
    std::string homedir = pw.pw_dir;
    boost::replace_first(_sshPublicKey, "$HOME", homedir );
    boost::replace_first(_sshPrivateKey, "$HOME", homedir );
...
void CurlConfigurer::dump() const 
{
    ...
    ACS_LOG_PRIVINFO("\n"
       ...
       << left << setw(50) << "--------------------------------------------  " << '\n'
       << left << setw(36) << std::right << "version: " << _module_version_.name() << '\n'
...
void CurlConfigurer::load() // PRQA S 5500, 5510
{
    ...
    try {
		const std::string listOfProxiesNode("List_Of_Proxies");
		if( resultSet.hasKey(listOfProxiesNode) ) {
            std::vector<ConfProxy> proxies ;
    ...
    { // NOSONAR - permissionsNS scope
        ...
        try { // NOSONAR - nested
            ...
            if (! getValueIfHasKey(resultSet, "User",_permissions.userId)) {
                ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "User id not found, using the process owner user");
            }

            if (! getValueIfHasKey(resultSet, "Group",_permissions.groupId)) {
    ...
    try {
		...
		if (resultSet.hasKey("CurlError")) {
			...
			for (size_t idx = 0; idx < arraySize; ++idx) {
				...
				} catch (std::exception& e) { // NOSONAR - any exception
					ACS_LOG_NOTIFY_EX(e);
				}
			}
		}
		
	} catch (std::exception const& e) { // NOSONAR - any exception
    ...
    if (! getValueIfHasKey(resultSet, "probeConnectionTimeout",_probeConnectionTimeout_ms)) {
    ...
    if (! getValueIfHasKey(resultSet, "lsConnectionTimeout",_lsConnectionTimeout_ms)) {
    ...
    if (! getValueIfHasKey(resultSet, "remoteTempPath",_remoteTempPath)) {
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "remoteTempPath not found, using default");
    }

	//Reads the local temporary path
    if (! getValueIfHasKey(resultSet, "localTempPath",_localTempPath)) {
    ...
    if (! getValueIfHasKey(resultSet, "httpListStylesheetName",_httpListStylesheetName)) {
    ...
    if (! getValueIfHasKey(resultSet, "httpListStylesheetVersion",_httpListStylesheetVersion)) {
    ...
    if (! getValueIfHasKey(resultSet, "sshPublicKey",_sshPublicKey)) {
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "sshPublicKey not found, using default");
    }

    if (! getValueIfHasKey(resultSet, "sshPrivateKey",_sshPrivateKey)) {
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "sshPrivateKey not found, using default");
    }

    if (! getValueIfHasKey(resultSet, "lowSpeedLimit",_lowSpeedLimit)) {
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "lowSpeedLimit not found, using default");
    }

    if (! getValueIfHasKey(resultSet, "lowSpeedTime",_lowSpeedTime)) {
...
const std::string &CurlConfigurer::getRemoteTempPath() const noexcept // PRQA S 4120
{
...
size_t CurlConfigurer::getMaxFilesToTransfer() const noexcept  // PRQA S 4120
{
    return _maxFilesToTransfer;
}

size_t CurlConfigurer::getConnectionResponseTimeout_ms() const noexcept   // PRQA S 4120
{
    return _connectionResponseTimeout_ms ;
}

size_t CurlConfigurer::getHttpConnectionResponseTimeout_ms() const noexcept   // PRQA S 4120
{
    return _httpConnectionResponseTimeout_ms ;
}

size_t CurlConfigurer::getConnectionTimeout_ms() const noexcept  // PRQA S 4120
{
    return _connectionTimeout_ms;
}

size_t CurlConfigurer::getHttpConnectionTimeout_ms() const noexcept  // PRQA S 4120
{
    return _httpConnectionTimeout_ms;
}

size_t CurlConfigurer::getProbeConnectionTimeout_ms() const noexcept // PRQA S 4120
{
    return _probeConnectionTimeout_ms;
}

size_t CurlConfigurer::getLsConnectionTimeout_ms() const noexcept // PRQA S 4120
{
    return _lsConnectionTimeout_ms;
}

void CurlConfigurer::setConnectionResponseTimeout_ms(size_t to) noexcept  // PRQA S 4121
{
    _connectionResponseTimeout_ms = to;
}

void CurlConfigurer::setConnectionTimeout_ms(size_t to) noexcept  // PRQA S 4121
{
    _connectionTimeout_ms = to;
}

void CurlConfigurer::setHttpConnectionTimeout_ms(size_t to) noexcept  // PRQA S 4121
{
    _httpConnectionTimeout_ms = to;
}

size_t CurlConfigurer::getFtpCommandResponseTimeout_s() const noexcept // PRQA S 4120
{
	return _ftpCommandResponseTimeout_s;
}
void CurlConfigurer::setFtpCommandResponseTimeout_s(size_t to) noexcept // PRQA S 4121
{
	_ftpCommandResponseTimeout_s = to;
}

void CurlConfigurer::setProbeConnectionTimeout_ms(size_t to) noexcept // PRQA S 4121
{
    _probeConnectionTimeout_ms = to;
}

void CurlConfigurer::setLsConnectionTimeout_ms(size_t to) noexcept // PRQA S 4121
{
    _lsConnectionTimeout_ms = to;
}

void CurlConfigurer::setMaxFilesToTransfer(size_t maxFilesToTransfer) noexcept // PRQA S 4121
{
    _maxFilesToTransfer = maxFilesToTransfer;
}

const std::string &CurlConfigurer::getTempNameStrategy() const noexcept  // PRQA S 4120
{
    return _tempNameStrategy;
}

void CurlConfigurer::setTempNameStrategy(std::string const& strategy)  // PRQA S 4121
{
    _tempNameStrategy = strategy;
}

#ifdef ACS_CURL_CCC_SUPPORTED
curl_ftpccc CurlConfigurer::getFtpsCCC() const noexcept  // PRQA S 4120
{
    return _ftpsCCC;
}

void CurlConfigurer::setFtpsCCC(curl_ftpccc useCCC) noexcept   // PRQA S 4121
{
    _ftpsCCC = useCCC;
}
#endif

bool CurlConfigurer::getIsFtpsProtectData() const noexcept   // PRQA S 4120
{
    return _ftpsProtectData;
}

void CurlConfigurer::setIsFtpsProtectData(bool protect) noexcept   // PRQA S 4121
{
    _ftpsProtectData = protect;
}

std::string CurlConfigurer::getSshPublicKey() const noexcept   // PRQA S 4120
{
    return _sshPublicKey;
}

void CurlConfigurer::setSshPublicKet(std::string const& key) noexcept   // PRQA S 4121
{
    _sshPublicKey = key;
}

std::string CurlConfigurer::getSshPrivateKey() const noexcept   // PRQA S 4120
{
    return _sshPrivateKey;
}

void CurlConfigurer::setSshPrivateKet(std::string const& key) noexcept   // PRQA S 4121
{
    _sshPrivateKey = key;
}

size_t CurlConfigurer::getLowSpeedLimit() const noexcept   // PRQA S 4120
{
	return _lowSpeedLimit;
}

void CurlConfigurer::setLowSpeedLimit(size_t const lowSpeedLimit) noexcept // PRQA S 4121
{
	_lowSpeedLimit = lowSpeedLimit;
}

size_t CurlConfigurer::getLowSpeedTime() const noexcept   // PRQA S 4120
{
	return _lowSpeedTime;
}

void CurlConfigurer::setLowSpeedTime(size_t const lowSpeedTime) noexcept   // PRQA S 4121
{
	_lowSpeedTime = lowSpeedTime;
}

const std::string &CurlConfigurer::getLocalTempPath() const noexcept // PRQA S 4120
{
...
const std::string &CurlConfigurer::getHttpListStylesheetName() const noexcept // PRQA S 4120
{
...
const std::string &CurlConfigurer::getHttpListStylesheetVersion() const noexcept  // PRQA S 4120
{
...
bool CurlConfigurer::getEnableStatistics() const noexcept // PRQA S 4020
{
    if (_enableStatistics && _statisticsCheckGood) { return true ; }
    return checkStatistics();
}

void CurlConfigurer::setEnableStatistics(bool enabled) noexcept // PRQA S 4211
{
...
bool CurlConfigurer::checkStatistics() const noexcept 
{
    if (! _enableStatistics) { // statistics are not enabled
        _statisticsCheckGood = true ; // whatever table exists on the db
    }
    
    //verify if the T_Circulationstats exists.
    if (! _statisticsCheckGood) {
        try {
            if (! Application::exists() || ! Application::instance()->getDefaultConnectionPool()) {
	            ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "No default connection pool found, Application::exists(): " << std::boolalpha << Application::exists()) ;
...
bool CurlConfigurer::changePermissions() const noexcept 
{
    return _permissions.isActivated();
}

const CurlConfigurer::ChangeLocalPermissions &  CurlConfigurer::getPermissions() const noexcept // PRQA S 4120
{
    return _permissions;
}
...
bool CurlConfigurer::isActivatedChangeMode() const noexcept // PRQA S 4120
{
    return _permissions.activatedChangeMode;
}

bool CurlConfigurer::isActivatedChangeOwner() const noexcept  // PRQA S 4120
{
    return _permissions.activatedChangeOwner;
}

bool CurlConfigurer::ChangeLocalPermissions::isActivated() const noexcept  // PRQA S 4120
{
    return activatedChangeMode || activatedChangeOwner;
}

bool CurlConfigurer::ChangeLocalPermissions::isActivatedChangeMode() const noexcept  // PRQA S 4120
{
    return activatedChangeMode;
}

bool CurlConfigurer::ChangeLocalPermissions::isActivatedChangeOwner() const noexcept  // PRQA S 4120
{
    return activatedChangeOwner;
}

const std::string &CurlConfigurer::ChangeLocalPermissions::getUserId() const noexcept // PRQA S 4120
{
    return userId;
}

const std::string &CurlConfigurer::ChangeLocalPermissions::getGroupId() const noexcept // PRQA S 4120
{
    return groupId;
}

mode_t CurlConfigurer::ChangeLocalPermissions::getMode() const noexcept // PRQA S 4120
{
    return mode;
}

bool CurlConfigurer::getSimulateCirculation() const noexcept  // PRQA S 4120
{
    return _simulateCirculation ;
}

void CurlConfigurer::setSimulateCirculation(bool v) noexcept  // PRQA S 4121
{
...
std::string CurlConfigurer::ChangeLocalPermissions::str() const noexcept 
{
...
bool CurlConfigurer::getFastProbeModeEnabled() const noexcept // PRQA S 4120
{
	return _fastProbeModeEnabled;
}

void CurlConfigurer::setFastProbeModeEnabled(bool b) noexcept // PRQA S 4121
{
	_fastProbeModeEnabled = b;
}

#ifdef EXP_UPLOAD_BUFFER_CONFIGURABLE
size_t CurlConfigurer::getUploadBufferSize() const noexcept { return _uploadBufferSize ; }
void CurlConfigurer::setUploadBufferSize(size_t const ubs) noexcept 
{
...
namespace { // unnamed
    ...
    const CurlConfigurerLoader _CurlConfigurerLoader ; // just to invoke singleton
}

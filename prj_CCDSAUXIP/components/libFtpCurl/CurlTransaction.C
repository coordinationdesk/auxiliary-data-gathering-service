...
CurlTransaction::~CurlTransaction() noexcept 
{
    try {
        close();
    }
...
CURL* CurlTransaction::begin() { // NOSONAR - external type (CURL)
...
void CurlTransaction::logTransaction(TransactionType operationType, std::vector<std::string>& args, CURLcode responseCode) { // PRQA S 4020
    ...
    case TrGet:
        _partialGets.emplace_back(TrGet, remoteFile);
        break ;
    case TrPut:
        _partialPuts.emplace_back(TrPut, remoteFile);
        break ;
    case TrRename:

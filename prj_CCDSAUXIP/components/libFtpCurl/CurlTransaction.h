...
enum TransactionType { // NOSONAR - old enum
    TrUnset = 0, 
    TrOther, 
    TrGet, 
    TrPut, 
...
public:
    ...
    CurlTransaction(CurlTransaction const& ) = delete ;
    CurlTransaction& operator=(CurlTransaction const& ) = delete  ;

    /**
     * Closes the cURL connection (if it is still opened).
     */
    ~CurlTransaction() noexcept;

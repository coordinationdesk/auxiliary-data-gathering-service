...
map<string, RemoteSite, std::less<> > FtpCurl::_sites;
Condition FtpCurl::_discoverCondition;
vector<string> FtpCurl::_inDiscover;
map<string, bool, std::less<>> FtpCurl::_passiveFtpSiteOverrides;
map<string, map<string, string>, std::less<>> FtpCurl::_verifyHostCertificate; // NOSONAR - inner map not transparent comparator
...
FtpCurl::~FtpCurl() noexcept 
{
    ...
    try {
        ...
        ACS_LOG_NOTIFY_EX(x) ; // NOSONAR - any exception 
    }
    // PRQA L:L1
...
string FtpCurl::toString(Operation op) // PRQA S 4020 
{
    ...
    default:
        ACS_THROW(exIllegalValueException()<< "Unrecognized Operation (" << int(op) << ")" ) ;
    }
}
...
void FtpCurl::initializeCurl(Operation op, StopController const& stopController) // PRQA S 4020
{
    ...
    setOperation(op);
    createCurl();
    ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Object dump, Operation:  " << toString(op) << '\n' << str());
...
void FtpCurl::finalizeCurl(CURLcode res, string const& target) 
{
    ...
    if (_handleProgressData.get()) {
        _handleProgressData->setPerfMonitor(nullptr) ;
    }
...
Curl& FtpCurl::createCurl() 
{
    ...
    if (!_curl) {
        _curl = std::make_unique<acs::curl::Curl>();
    }
    _curl->setSite(_site);
...
void FtpCurl::openConnection(bool skipIfAlreadyCreated) 
{
    ...
    if (!skipIfAlreadyCreated) {
        ACS_COND_THROW(_transaction, FtpCurlException("cURL handler already created.")) ; // PRQA S 3081
    }
    if (!_transaction) {
        _transaction = std::make_unique<CurlTransaction>(createCurl());
        _transaction->begin();
    } 
    else {
...
void FtpCurl::ldel(const string &localFile) // NOSONAR - non const !
{
...
long FtpCurl::httpRequest(const curl::Curl::CurlHttpOpt& opt, const Operation& op, std::string& responsePayload, bool failOnError, StopController const& stopController) {
    ...
    setupVerifyHostCertificate(tmpSite.getHostAndPort());
...
void FtpCurl::list(NextLineCallback nextLineCallback, void* data, StopController const& stopController) // NOSONAR - callback
{
...
{
    FtpListReader* reader = reinterpret_cast<FtpListReader*>(data); // NOSONAR
    FtpListItem item;
    item.setLine(line) ;
    reader->addListItem(item);
}

void httpNextLineCallback(const string &line, void* data)  // NOSONAR - callback
{
    oftmpstream* outfile = reinterpret_cast<oftmpstream*>(data); //NOSONAR
    (*outfile)() << line << '\n';
}
...
{
	const bool loaded (! _config_loaded) ; 
    ...
    if (!_config_loaded)
    {
		...
		{ // NOSONAR - scope
        	...
        	for( size_t i=0; i < elements; i++)
        	{
            	...
            	if ( ! theHost.empty() )
            	{
					map <string, string> parameters_M ; // NOSONAR - no transparent comparator
					...
					_verifyHostCertificate[theHost] = parameters_M;
			 		...
			 		std::ostringstream o ; 
                    o << "Configuring the certificate verification flag to " << (checkCertificate?"TRUE":"FALSE") 
                      << " for the host with certificate parameters: { " ;
					  for (const auto &par : parameters_M ) { 
					      o << "\"" << par.first << "\": \"" << par.second << "\" ; " ; 
                      }
					o << "}" ; 	
		...
		key = "FtpCurl.TarBlockFactor" ;  
        if (true == rs.hasKey(key)) 
		{
		...
		key = "FtpCurl.TarBlockFactorThreshold" ;  
        if (true == rs.hasKey(key)) 
		{
...
bool FtpCurl::mustOverrideFTPMode (bool& isPassive ) const
{
    { // NOSONAR - lock scope
        // Read configuration locking it for the whole time
        ThreadSafe::Lock lock (_overridesMutex);
    ...
    string hostToCheck = _site.getHostAndPort();
    ...
    if ( search == _passiveFtpSiteOverrides.end() )
    {
        ...
        ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Override active/passive configuration for the remote host " << _site.getHostAndPort() << " NOT FOUND");
        return false;
    }
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Override active/passive configuration for the remote host " << _site.getHostAndPort() << " FOUND and set to " << (isPassive ? "PASSIVE" : "ACTIVE"));
...
bool FtpCurl::mustVerifyHostCertificate (
        const string& hostToCheck_in, 
        map<string, string>& parameters_M // NOSONAR - map transparent comparator
    ) const 
{
    ...
    if ( _verifyHostCertificate.find(hostToCheck) == _verifyHostCertificate.end() )
    {
...
bool FtpCurl::setupVerifyHostCertificate(const string& host_name)
{
    ...
    if ( host_name.empty() || !(found = mustVerifyHostCertificate (host_name, parameters_M )) ) // NOSONAR - host_name must be checked before
	{
    ...
    ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		std::ostringstream o ; 
        o << "Setting the " << (found?"configured":"default/fallback") \
		<< " certificate verification with certificate parameters: { " ;
		for (auto it=parameters_M.begin(); it != parameters_M.end(); it++) // NOSONAR 
...
{
  setupVerifyHostCertificate(_site.getHostAndPort());
  if (!_site.supportsActivePassive())
  {  // nop if the protocol does not support the active/passive concept
    return;
  }
  const string phost = _site.getHostAndPort();
  ...
  for (;;)
  {
    ...
    try 
    {
        ...
        bool isPassive;
        if ( mustOverrideFTPMode ( isPassive ) )
        {
          ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Overriding the active/passive configuration for the remote host " << _site.getHostAndPort() << " to " << (isPassive ? "PASSIVE" : "ACTIVE"));
    ...
    { // NOSONAR - lock scope
        ...
        // Change them
        _inDiscover.erase(find(_inDiscover.begin(), _inDiscover.end(), phost)) ;
        _sites[_site.getHostAndPort()] = _site;
        // wake up all other waiting on this condition
        _discoverCondition.broadcast();
    }
...
RemoteResourceType FtpCurl::probeRemoteResource(string &lastErrorMessage, StopController const& stopController, bool emitException, bool logError)  // PRQA S 4020
{
    ...
    try {
        ...
        for (auto const &item : items) { // PRQA S 4238
            ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB,  "Current Item: " << item.str() << "\n\t" << item.getPermissions().str());
            ...
            if (!item.getPermissions().isLink()) {
                ...
                if (item.getName() ==  baseName) {
                    ACS_CLASS_WRITE_DEBUG(ACS_VLO_VERB, "Item found: " << item.str() << "\n\t" << item.getPermissions().str());
                ...
                if ( (link == baseName) && !linkedItem.empty() ) {
                    ...
                    try { // NOSONAR - inner try to ignore probe exceptions
                        string dummy ;
                        result = probeRemoteResource(dummy, stopController);
                    } 
                    catch (std::exception const& ignore) { // NOSONAR - any exception 
        ...
        if(  dynamic_cast<const FtpCurlLoginDeniedException*> (&ex)  ||  // PRQA S 3081 2 
             dynamic_cast<const Curl::CurlUnrecoverableException*> (&ex)  || emitException ) {
            
            try {
                //restore the original path in order to not change this object behaviour.
                _site.setPath(originalPath);
            }
            catch(const std::exception & ee)  // NOSONAR - any exception 
            {
...
bool FtpCurl::existRemoteResource (bool& isDir, StopController const& sc, bool emitException) 
{
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Resource exists: " << std::boolalpha << retval << " isDir = " << std::boolalpha << isDir ); // PRQA S 3081
...
bool FtpCurl::existDestinationFile(bool testFile, StopController const& stopController, bool emitException)
{
    ...
    if (!testFile && !_site.supportsDirectory()) {
        return false ;
    }
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Remote item exists: " << std::boolalpha << retval <<  " (" << (testFile ? "file" : "directory") << ")" ); // PRQA S 3081, 3380
...
void FtpCurl::get(const FtpCurlPermissions& ftpCurlPerm, StopController const& stopController) {
    ...
    if (CURLE_OK == res && _site.hasTemporaryTargetName()) { // got and need rename
        ...
        res = _curl->lrename(targetFile, localFile); // overwrite res
    }
    finalizeCurl(res, _site.getPath());
    fillGetStats();
                        ...
                        StopController const& stopController) 
{
    ...
    { // NOSONAR - scope
        ...
        if (CURLE_OK == res && _site.hasTemporaryTargetName()) { // got and need rename
            ...
            res = _curl->lrename(targetFile, localPath); // overwrite res
        }
        finalizeCurl(res, _site.getPath()); 
...
void FtpCurl::put(bool alsoRemove, mode_t permissions, StopController const& stopController) 
{
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "_site.hasTemporaryTargetName() " << std::boolalpha << _site.hasTemporaryTargetName() << " final " << remoteFile);
    CURLcode res = CURLE_OK ;
    if (_site.hasTemporaryTargetName()) {
...
void FtpCurl::putDir(string const& localDirectory, string const& remoteDirectory, FileFilter fileFilter, // NOSONAR - callback
        StopController const& stopController) 
{
    ...
    try {
        ...
        for (size_t counter = 0; counter < localDir_size; ++counter) {
            ...
            if (!acs::File::isRegularFile(_localFile)) {
                if (acs::File::isDir(_localFile)) {
                    ...
                    try { // NOSONAR - inner try to ignore mkDir errors
                        remoteFile = remotePath + "/" + _localFile.substr(localDirectory.length(), std::string::npos);
            ...
            remoteFile = remotePath + "/" + _localFile.substr(localDirectory.length(), std::string::npos);
            if (fileFilter(_localFile.substr(localDirectory.length(), std::string::npos))) {
                ++filesTransferred;
                ...
                string destRemoteFile = destinationRootPath + "/"
                        + _localFile.substr(localDirectory.length(), std::string::npos);
        ...
        try {
            for (auto const &tempFile :  transferredFiles) {
        ...
        catch (std::exception const& ex2) { // NOSONAR - any exception 
...
void delDirCallback(const string &line, void* data) // NOSONAR - callback 
{
...
void FtpCurl::delDirTree(string const &remoteDirectory, StopController const& stopController, bool logError)  // PRQA S 4020
{
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Object dump:  \n" << str());
...
void FtpCurl::getDir(string const &localTargetDir, const FtpCurlPermissions& ftpCurlPerm, string const &remoteSourceDir, StopController const& stopController) 
{
    ...
    try {
        while (!subDirs.empty()) {
            ...
            for (size_t counter = 0; counter < lines_size; ++counter) {
                ...
                if (!lines[counter].getPermissions().isDirectory()) {
                    ...
                    CURLcode res_inner = _curl->get(localDirectory + "/" + relativeRemoteName + "/" + remoteFile, // PRQA S 4412 2
                            remotePath + "/" + remoteFile);
                    throwConditionedError(res_inner, remotePath + "/" + remoteFile, __PRETTY_FUNCTION__, __LINE__);
                    fillGetStats();
        ...
        catch (std::exception const& ex1) { // NOSONAR - any exception 
...
void FtpCurl::throwConditionedError(CURLcode res, const string &url, char const* function, int line) const 
{
    if (res != CURLE_OK) {
...
long FtpCurl::getHttpResponseCode() const {
    ACS_COND_THROW(!_curl, exIllegalValueException("Curl object not initialized."));
    return _curl->getHttpResponseCode();
}

int FtpCurl::spawn(const string & exeName, const WaitPid::ProcArgs & procArgs, int timeout, bool & goodExit, string & spawnArgs) const 
{
    SpawnProc sp(exeName);
    sp.args(procArgs);
    
    std::ostringstream spawnArgsOs;
    ...
    ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB,  "Going to spawn executable \"" << exeName << "\" with the following arguments: " << spawnArgs) ;
    ...
    if(exitCode != 0) {
        ACS_COND_THROW(wdTimeout,exIOException() << watchdog.elapsed() / 1000 << " [sec] elapsed since " << exeName << " was started. It is bigger than the timeout (" << timeout << " [sec]). The process was killed. ") ; // PRQA S 3081
        int exitSignal = 0 ;
        sp.exitSignal(exitSignal) ;
        ACS_COND_THROW(exitSignal, exIOException() << exeName << " terminated with exitCode: " << exitCode  << " exitSignal: " << exitSignal) ; // PRQA S 3081
    }
...
void FtpCurl::fillFtpListReader(const string & filepath, curl::FtpListReader & reader) const
{
    ...
    for( size_t i=0; i < elements; i++)
    {
        ...
        string hour;
        { // NOSONAR - scope
            std::istringstream is(modificationDate);
            string date;
            is >> date;
            is >> hour;
...
#warning MP - rationalize and speed up
...
void FtpCurl::fillPropFindStatItems( string const& remotePath, int depth, const string & xmlIn, const vector<string> & properties, bool directory, bool pathIsDir, vector<PropFindStatItem> & statItems ) const
{
    ...
    for( size_t i=0; i < elements; i++) {
        ...
        if( addItem && directory ) {
            // turn into error?
            ACS_COND_THROW(!dirSet, FtpCurlUnrecoverableErrorException() << "Directories only requested to PROPFIND, but directory not set for \"" << itemName << "\" item: resourcetype property request returned error" );
...
// extract creation date: format is e.g. Tue, 03 Dec 2019 15:40:15 GMT
                struct tm tm0;
                if( strptime( propertyValue.c_str(), "%a, %d %b %Y %H:%M:%S %z", &tm0 )) {
                    ACS_LOG_ERROR("Cannot extract modification time from \"" << item.getName() << "\" item" );
                }
                else {
                    item.setModificationDate( DateTime( DateTime::UTCD( (1900 + tm0.tm_year), (tm0.tm_mon + 1), tm0.tm_mday, tm0.tm_hour, tm0.tm_min, tm0.tm_sec )) ); // NOSONAR - int to short (signed or unsigned)
                }
                ...
                // get item length
                item.setSize(stoul(propertyValue));
            }
        }
...
string FtpCurl::buildHttpPropFindBody( const vector<string> & properties ) const 
{
...
void FtpCurl::propFind( string const& remotePath, int depth, const vector<string> & properties, bool directory, bool isDir, vector<PropFindStatItem> & statItems, StopController const& sc )
{
    ...
    long int httpres = _curl->getHttpResponseCode() ;
    ACS_COND_THROW(httpres != 207, FtpCurlUnrecoverableErrorException() << "Http response code: " << httpres << "; 207 expected for Multi-Status" );
    
    fillPropFindStatItems( remotePath, depth, output, properties, directory, isDir, statItems );    
}

vector<string> FtpCurl::turnRequestsIntoProperties(const PropFindRequests & request) const 
{
    vector<string> properties;
    
    if( request & PROPFINDREQ_TYPE ) {
        auto it = propFindRequestsMap.find(PROPFINDREQ_TYPE);
        if( it == propFindRequestsMap.end() ) {
            ACS_THROW(FtpCurlUnrecoverableErrorException() << "Internal error: " << int(PROPFINDREQ_TYPE) << " element not found in const static map");
        }
    ...
    if( request & PROPFINDREQ_LENGTH ) {
        ...
        if( it == propFindRequestsMap.end() ) {
            ACS_THROW(FtpCurlUnrecoverableErrorException() << "Internal error: " << int(PROPFINDREQ_LENGTH) << " element not found in const static map" );
        }
        properties.push_back(it->second);
    }
    ...
    if( request & PROPFINDREQ_CREATION ) {
        ...
        if( it == propFindRequestsMap.end() ) {
            ACS_THROW(  FtpCurlUnrecoverableErrorException() << "Internal error: " << int(PROPFINDREQ_CREATION) << " element not found in const static map" );
        }
        properties.push_back(it->second);
    }
    ...
    if( request & PROPFINDREQ_MODIFICATION ) {
        ...
        if( it == propFindRequestsMap.end() ) {
            ACS_THROW(  FtpCurlUnrecoverableErrorException() << "Internal error: " << int(PROPFINDREQ_MODIFICATION) << " element not found in const static map");
        }
        properties.push_back(it->second);
    }
    ...
    ACS_CLASS_BGN_DEBUG(ACS_HIG_VERB)
        ...
        ACS_CLASS_WRITE_DEBUG(ACS_HIG_VERB,msg.str() );
    ACS_CLASS_END_DEBUG
...
FtpCurl::handleCurlErrorBehaviour(CurlErrorBehaviourSingleton::CurlErrorBehaviour beh, const std::string& msg, const std::string& file, const std::string& func, int line) {
    switch (beh) {
    ...
    // case CurlErrorBehaviourSingleton::UNDEFINED: - redundant -> default
    default:
        // Noop
...
size_t FtpCurl::readLine_callback(void* buffer, size_t size, size_t nmemb, void* data)  // NOSONAR - callback
{
    ...
    //if read something...
    if (strSize) {
        FtpCurl* ptrToFtpCurl = reinterpret_cast<FtpCurl *>(data); // NOSONAR - callback
        if (!ptrToFtpCurl) {
            ...
            char* charBuffer = reinterpret_cast<char *>(buffer); // NOSONAR - callback
            char str[strSize + 1]; // NOSONAR - safe
            memset(str, 0, sizeof(str));
            strncpy(str, charBuffer, strSize); // NOSONAR - safe: str size is strSize+1
...
string FtpCurl::getHostAndPort() const
{
  return _site.getHostAndPort();
}
...
const string &FtpCurl::getRempath() const noexcept 
{
...
const string &FtpCurl::getLocalFile() const noexcept // PRQA S 4120
{
    return _localFile;
}
...
const string &FtpCurl::getLocalPath() const noexcept // PRQA S 4120
{
    return _localPath;
}
...
void FtpCurl::setListFileNamesOnly(bool namesOnly) noexcept // PRQA S 4121
{
    _listFileNamesOnly = namesOnly;
}

bool FtpCurl::getListFileNamesOnly() const noexcept // PRQA S 4120
{
    return _listFileNamesOnly;
}

void FtpCurl::setSFTP()
{
  _site.setIsSFTP();
}

bool FtpCurl::isSFTP() const
{
  return _site.getIsSFTP();
}
...
bool FtpCurl::getFtps() const noexcept
{
    return _site.getIsFtps();
}

bool FtpCurl::getFtpes() const noexcept
{
    return _site.getIsFtpes();
}

bool FtpCurl::getFtp() const noexcept
{
    return _site.getIsFtp();
}

void FtpCurl::setNextLineCallback(FtpCurl::NextLineCallback callBack, void* data) // NOSONAR - callback 
{
...
const string &FtpCurl::getFullFilename() const noexcept 
{
    return _site.getPath();
}
...
const RemoteSite &FtpCurl::getSite() const noexcept // PRQA S 4120
{
    return _site;
}
...
string FtpCurl::generate_compression_ext(const File::CompressionMode& cm)
{
    ...
    switch (cm)
    {
        ...
        default:    ACS_THROW ( exIllegalValueException("Invalid File::CompressionMode") ) ;    
    }
...
{
    using std::ios ;
    std::ofstream ofile( filename.c_str(), ios::out|ios::binary|ios::trunc|ios::ate );

    ACS_COND_THROW(ofile.fail(), exIOException(errno) << "Error opening file: \"" << filename << "\"") ;  
    ofile << s ; 
    ACS_COND_THROW(ofile.bad(), exIOException(errno) << "Error writing to file: \"" << filename << "\"") ;  
    ofile.close() ; 
...
string FtpCurl::self_test__read_from_file(const string& filename) 
{
    ...
    ACS_COND_THROW(ifs.fail(), exIOException(errno) << "Error opening file: \"" << filename << "\"") ;  
...
namespace {
    std::minstd_rand0 _random ; // NOSONAR - no need to be stronger 
}
...
int FtpCurl::self_test__tar_untar(const curl::RemoteSite& remote_site_in)
{
    ...
    */
    ...
    { // NOSONAR - scope
        ...
        FileGuard FG__reference_dir( File::tempdir("/tmp/refdir") ) ;   // NOSONAR 
        const string reference_dir = FG__reference_dir ;
        ...
        std::ostream &outs = std::cout ; // NOSONAR - test
            
        for (size_t k=0; k<16; k++)
        {
            FileGuard FG__output_dir( File::tempdir("/tmp/outdir") ) ;  // NOSONAR
            const string output_dir = FG__output_dir ; 
            ...
            { // NOSONAR - scope
                std::ostringstream o ; o << "/tmp/file_" << _random() ; // NOSONAR 
            ...
            outs << "------------------------------------------------------------------------------------------------------------------------\n" ;
            outs << "\nTest " << (k+1) << "/16: " << "Compression mode: " << tar_mode << "  Encapsulate dir: " << (encapsulate_dir?'T':'F') << "  Renaming strategy: " << (rename_strategy?'Y':'N') << "\n\n"; 
            ...
            { // NOSONAR - scope
                ...
                outs << "Setting local input reference directory: " << reference_dir << '\n' ;
                ftpCurl.setLocalFile(reference_dir);
                outs << "Setting remote file: " << remote_file << '\n' ;
            ...
            { // NOSONAR - scope
                ...
                // Set the local entry and the remote directory:
                outs << "Setting local output directory: \"" << output_dir << "\"\n" ;
                ...
                outs << "Test Successfull. After transfer, removing the remote file: \"" << remote_file << "\"\n" ;
                ftpCurl.del() ;
            }
            ...
            { // NOSONAR - scope
                ...
                f = StringUtils::pathJoin(comp_dir, file1) ;  
                outs << "Checking file content for: \"" << f << "\" ...\t" ; 
                ...
                f = StringUtils::pathJoin(comp_dir, file2) ;  
                outs << "Checking file content for: \"" << f << "\" ...\t" ; 
                ...
                f = StringUtils::pathJoin(comp_dir, file3) ;  
                outs << "Checking file content for: \"" << f << "\" ...\t" ; 
    ...
    { // NOSONAR - scope 
        ...
        for (size_t k=0; k<4; k++)
        {
            ...
            { // NOSONAR - scope
                std::ostringstream o ; 
                o << "file_" << _random() << generate_compression_ext(tar_mode) ; 
                file_name_1 = o.str() ; 
            }   
            const string remote_file (StringUtils::pathJoin("/tmp", file_name_1)) ; // NOSONAR 
            ...
            outs << "------------------------------------------------------------------------------------------------------------------------\n" ;
            outs << "\nContingency Test " << (k+1) << "/4: " << "Compression mode: " << tar_mode << "\n\n"; 
            ...
            { // NOSONAR - scope
                ...
                // Set the local entry and the remote directory:
                outs << "Setting local input reference file: " << reference_file << "  [THIS IS INTENTIONALLY A BROKEN COMPRESSED FILE]\n" ;
            ...
            { // NOSONAR - scope
                ...
                outs << "Setting local output directory: \"" << output_dir << "\"\n" ;
                ftpCurl.setLocalPath(output_dir);
                outs << "Setting remote file: \"" << remote_file << "\"  [THIS IS INTENTIONALLY A BROKEN COMPRESSED FILE]\n" ;
                ftpCurl.setRempath(remote_file);
                ...
                catch (acs::curl::Curl::CurlException& e) // NOSONAR - any exception 
                {
                    excerr << "\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n" ;
                    excerr << "Received exception, as expected since compressed file is indeed broken: \n" ;
                    excerr << "\t" << e ; 
                    excerr << "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n\n"  ;
                }

                // Remote the remote file
                outs << "Contingency test successfull. After interruped transfer, removing the remote file: \"" << remote_file << "\"\n" ;
                ftpCurl.del() ;
            }
...
namespace {        
    std::string toString(const FtpCurl::PropFindStatItem & v) 
    {
        std::ostringstream s ;
...
std::ostream &operator<<(std::ostream &s, const FtpCurl::PropFindStatItem & v)
{
    return s << toString(v) ;
}

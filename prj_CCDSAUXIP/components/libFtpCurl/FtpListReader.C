...
namespace {
    ...
    public:
        ...
        bool operator() (const std::string& path, const acs::os::FSTree::Entry& entry) const 
        {
...
void FtpListReader::createLocalFtpListItems(
        string const& sourcePath, 
        std::vector<FtpListItem> & sourceFiles, 
        bool filterByDate, 
        const time_t& lastScan, 
        StopController const& stopController, 
        const set<string> & filepathsToSkip) // NOSONAR - set w/o transparent comp
{
	...
	if( filepathsToSkip.empty() ) {
		...
		set<string> filepaths;  // NOSONAR - set w/o transparent comp
		...
		set<string> filepathsToKeep;  // NOSONAR - set w/o transparent comp
		set_difference(
		...
		for( auto const &tokeep : filepathsToKeep) {
            
			FSTree::EntriesMap::const_iterator itMap = entriesMap.find(tokeep);
			ACS_COND_THROW( itMap == entriesMap.end(), exIllegalValueException( '\"' + tokeep + "\" element does not occur in map!") ); // it should not occur
			FtpListItem item ;

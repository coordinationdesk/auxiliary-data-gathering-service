...
FtpPerformance::FtpPerformance(const curl::RemoteSite& site) : 
    _site(site)
{
    //empty.
}
...
void FtpPerformance::stop() {
    ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "is running " << std::boolalpha << _running);
    if (_running) {
...
{
    ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "is running " << std::boolalpha << _running << '(' << transf << '/' << total << ')');
    if (_running) {
        _transfBytes = transf;
        if (total > 0) {
            _totalBytes = total;
...
int FtpPerformance::percentage() const noexcept // PRQA S 4020
{
    if (0 != _totalBytes) { // PRQA S 3270
        return _transfBytes * 100 / _totalBytes ; // NOSONAR - implicit conversion - is a percentage
    }
    return 0;
}

double FtpPerformance::getSpeedKbSec() const noexcept // PRQA S 4120
{
    return _speedKbSec;
}

void FtpPerformance::setSpeedKbSec(double speed) noexcept // PRQA S 4121
{
    _speedKbSec = speed;
}

double FtpPerformance::getRemainingSec() const noexcept // PRQA S 4120
{
    return _remainingSec ;
}

void FtpPerformance::setRemainingSec(double sec)  noexcept // PRQA S 4121
{
    _remainingSec = sec;
}

double FtpPerformance::getTransfBytes() const noexcept  // PRQA S 4120
{
    return _transfBytes;
}

void FtpPerformance::setTransfBytes(double transferred) noexcept  // PRQA S 4121
{
    _transfBytes = transferred;
}

double FtpPerformance::getTotalBytes() const noexcept // PRQA S 4120
{
    return _totalBytes;
}

void FtpPerformance::setTotalBytes(double totalBytes) noexcept  // PRQA S 4121
{
...
const string &FtpPerformance::getOperation() const noexcept  // PRQA S 4120
{
    return _operation;
}
...
const curl::RemoteSite &FtpPerformance::getRemoteSite() const noexcept // PRQA S 4120
{
    return _site;
}

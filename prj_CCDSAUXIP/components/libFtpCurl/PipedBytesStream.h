...
public:
    ...
    PipedBytesStream(const PipedBytesStream &) = delete ;
    PipedBytesStream& operator= (const PipedBytesStream &) = delete ;

    ~PipedBytesStream() noexcept override 
    {
        _in.close();
        _out.close();
    }
    ...
    void close() override { _outputStreamClosed = true; }

    ssize_t write(char* buffer, size_t length) override 
    {
...
private:
    ...
    IStream _in;
    OStream _out;
    bool _outputStreamClosed = false ;

};

using Buffer = acs::io::CircularBuffer ;

using PipedStream = acs::io::PipedBytesStream<Buffer> ;

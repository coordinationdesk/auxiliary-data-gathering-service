 ...
 * Reset the object
 */
void AbstractTempStrategy::reset() { /* nothing to do */  }
 ...
 */
bool TempDirStrategy::isDirNameChanged() const { return ! tempDir_.empty() ; }
...
AbstractTempStrategy* TempDirStrategy::clone() const {
	return new TempDirStrategy(tempDir_); // NOSONAR - clone
}
 ...
 */
bool PrefixTempStrategy::isFileNameChanged() const { return ! prefix_.empty() ; }
...
AbstractTempStrategy* PrefixTempStrategy::clone() const {
	return new PrefixTempStrategy(prefix_); // NOSONAR - clone
}
 ...
 * Returns always true since this class modify the filename of an input path
 */
bool PostfixTempStrategy::isFileNameChanged() const { return ! postfix_.empty() ; }
...
AbstractTempStrategy* PostfixTempStrategy::clone() const {
	return new PostfixTempStrategy(postfix_); // NOSONAR - clone
}
...
AbstractTempStrategy* TimestampTempStrategy::clone() const {
	Lock lock(mutex_);
	auto cloned = new TimestampTempStrategy(prefix_); // NOSONAR - clone
...
AbstractTempStrategy* NoTempStrategy::clone() const {
	return new NoTempStrategy(); // NOSONAR - clone
}

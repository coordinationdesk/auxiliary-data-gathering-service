...
{
// NB init has not been called 
    operator=(other);
}

TempRenameStrategy TemporaryFileService::getStrategy() const
{
    return strategy_;
}

void TemporaryFileService::init() {
    switch(strategy_) {
    case tempDir: 
    ...
    case noTemp: 
        ...
        return ;
    default: 
        ACS_THROW(exIllegalValueException() << "Temp file startegy: \"" << strategy_ << "\" unknown") ;
    }
}
...
void TemporaryFileService::resetStrategyState() { // NOSONAR - non const
...
exostream & operator<< (exostream &os, TempRenameStrategy s)
{
    switch (s) {
    case noTemp:
        os << "noTemp" ;
	...
	case unknownStrategy:
        ...
        break ;
    default:
        os << "<unrecognized TempRenameStrategy: "<<int(s) << ">" ;
        break ;
...
std::ostream & operator<< (std::ostream &os, TempRenameStrategy s) 
{
    switch (s) {
    case noTemp:
        os << "noTemp" ;

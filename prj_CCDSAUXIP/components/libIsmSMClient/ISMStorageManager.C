...
void ISMStorageManager::download(dbPersistent::IdType sminvid, const string &localpath, const dbPersistentOperatorIF& oper)
{
	...
	try {	
		...
		try {
			...
			getParameters(theConnection, paramfile, rset, table, statN()) ;
		}
		catch(exception &e) {
    ...
    timer.stop() ;
	double timerElapsed = timer.elapsed() ;
    statN()["download"]["elapsed [ms]"]+= timerElapsed ; // PRQA S 3081
	double fileSizeKB = file_size/1024.0 ;
    statN()["download"]["size [Kb]"]+= fileSizeKB ; // PRQA S 3011, 4400
	statN()["download"]["throughput [Kb/s]"]+= ( fileSizeKB / ( timerElapsed * 0.001 ) ) ;
    statN()["download"]["ok"]+= 1 ;
}	

void ISMStorageManager::upload(const string &path, const dbPersistentOperatorIF& oper )
{
	normalUpload(path, oper) ;
}
...
void ISMStorageManager::run()
{
	...
	ACS_COND_THROW(this->getOper() == 0, ISMStorageManagerException("dbPersistentOperatorIF base class pointer attribute is NULL!")) ; // PRQA S 3081
	...
	const string path = getPath(); // from base class
	try {	
		string invreltable ;
		rsResourceSet rset ;
		try {
	        dbConnectionWrapper pw(getPool()) ;
	        dbConnection &theConnection = pw ; // PRQA S 3050
			getParameters(theConnection, paramfile, rset, invreltable, statN()) ;
		}
		catch(exception &e) {
		...
		if(computeSignature)
		{
			...
			SignatureAlgoFactory* factory = SignatureAlgoFactory::instance();
			SignatureAlgo* algo = factory->getAlgo(signatureAlgo,path);
			SignatureAlgoScopeGuard guard(algo);
			if (!algo) {
				ACS_THROW(SignatureAlgoFactory::NotFoundException("No SignatureAlgo got with name " + signatureAlgo  )) ; // PRQA S 3081
			}
		...
		string intermediateIsmPath ;
		{
			...
			intermediateIsmPath = ismpath.str() ;
		}

		ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Intermediate ISM path is \"" <<  intermediateIsmPath << '\"' ) ;

		IsmSoapClient::fileDefinition fdef ;
		
		// if is a directory size will be computed 
		fdef.size = File::size(path) ;
		fdef.user = user;
		fdef.group = group;
		...
		string filegroup;
		{
			...
			if( filegroup.empty() ) {
				...
				filegroup = this->getOper()->getGroup(*getObject(), theConnection);
			}
		}
		...
		try {
			...
			if (!_check.simulate()) {
				...
				ismclient.setConnParameters(rset, SMId()) ;
				unsigned int soapRecvTotalTimeout = ismclient.getSoapRecvTotalTimeout();
				ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Soap receiving message total timeout: " << soapRecvTotalTimeout << " ms" );
				
				Timer upltimer;
				upltimer.start();
				Timer soapTimeoutTimer ;
				for (int counter = 0; ; counter++ ) {
					...
					ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Before put in Ism "
							   ...
							   << " ismpath " << fdef.filePath) ;				

					try {
						ismclient.upload(fdef , path , true, _references ) ;
						...
						ACS_CLASS_WRITE_DEBUG(ACS_HIG_VERB, "job time=" << jobTime << " [s]" );
					...
					catch( IsmSoapClient::SoapReceiveTimeOutException &e )
					{
						if( 0 == counter ) {
							soapTimeoutTimer.start();
						}
						soapTimeoutTimer.stop();
						ostringstream wmsg ;
						wmsg << e.what() << "\n Attempt #" << counter << " time elapsed: " <<  soapTimeoutTimer.elapsed() << " [ms]" ;
						if( soapTimeoutTimer.elapsed() >= soapRecvTotalTimeout ) {
							ostringstream exMsg ;
							exMsg << wmsg.str() << ". Timeout (" << soapRecvTotalTimeout << " [ms]) exceeded" ;
							ACS_THROW(ISMStorageManagerException(e, exMsg.str() ) );
						}
						ACS_LOG_WARNING( wmsg.str() << ". Timeout (" << soapRecvTotalTimeout << " [ms]) not exceeded: retrying to upload \"" << filename << "\" file...") ;
					}
				}

			}
			else {
				ACS_LOG_WARNING("Simulating Ism.upload of file \"" << path << "\"") ;
	...
	catch(exception &be)
	{
		...
		ostringstream os1 ;
		os1 << "Uploading inventory: " << this->getOper()->getFileName(*getObject()) 
		   << " version " << this->getOper()->getFileVersion(*getObject()) 
		   << " on SmId " << SMId() << " failed from path: " << path ;
		ACS_THROW(ISMStorageManagerException (be, os1.str())) ; // PRQA S 3081
	}
    timer.stop() ;
	double timerElapsed = timer.elapsed() ; 
    statN()["upload"]["elapsed [ms]"]+= timerElapsed ; // PRQA S 3081 3
	double fileSizeKB = file_size/1024.0 ;
    statN()["upload"]["size [Kb]"]+= fileSizeKB ; // PRQA S 3011, 4400
	statN()["upload"]["throughput [Kb/s]"]+= ( fileSizeKB / ( timerElapsed * 0.001 ) ) ;
    statN()["upload"]["ok"]+= 1 ;
}

void ISMStorageManager::erase(dbPersistent::IdType sminvid, const dbPersistentOperatorIF& oper,string* p) 
{
	erase(sminvid, oper.getFileName(*getObject()), oper.getFileVersion(*getObject()), p);
}

void ISMStorageManager::erase(dbPersistent::IdType sminvid, const std::string& fname, const std::string& fvers, std::string*) 
{
	...
	string ismpath ;
	doAction(sminvid, fname, fvers, REMOVE_FILE, ismpath)	;
	...
	catch (exception &e)
	{
		...
		// APF-270
		ACS_LOG_MC_ERROR( mc_param("filename", fname) << mc_param("fileversion", fvers) << mc_param("fileid", ismpath) << mc_param("smid", SMId()) // PRQA S 3081
...
void ISMStorageManager::getAvailableStorageInfo(vector<pair<string, long long int> > &v) 
{
	...
	try {	
		...
		try {
			dbConnectionWrapper pw(getPool()) ;
			dbConnection &theConnection = pw ; // PRQA S 3050
			getParameters(theConnection, paramfile, rset, dummy, statN()) ;
		}
		catch(exception &e) {
...
void ISMStorageManager::giveup(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &oper)
{
	...
	string ismpath;
	doAction(sminvid, oper.getFileName(*getObject()), oper.getFileVersion(*getObject()), GIVEUP_REQUEST, ismpath)	;
...
dbFileAction::CacheStatus ISMStorageManager::isOnCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &oper)
{
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_MID_VERB) ;
	...
	string ismpath;
	dbFileAction::CacheStatus cacheStatus = static_cast<dbFileAction::CacheStatus>( 
			doAction(sminvid, oper.getFileName(*getObject()),  oper.getFileVersion(*getObject()), IS_ONCACHE, ismpath) ) ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Check on cache request for \"" << ismpath << "\" file returned " << cacheStatus );
			
    timer.stop() ;
    statN()["isOnCache"]["elapsed [ms]"]+= double(timer.elapsed()) ; // PRQA S 3081
	
	return cacheStatus ;

}

dbFileAction::CacheStatus ISMStorageManager::putOnCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &oper)
{
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_MID_VERB) ;
	...
	string ismpath;
	dbFileAction::CacheStatus cacheStatus = static_cast<dbFileAction::CacheStatus>( 
		doAction(sminvid, oper.getFileName(*getObject()),  oper.getFileVersion(*getObject()), PUT_ONCACHE, ismpath) ) ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Put on cache request for \"" << ismpath << "\" file returned " << cacheStatus );
			
    timer.stop() ;
    statN()["putOnCache"]["elapsed [ms]"]+= double(timer.elapsed()) ; // PRQA S 3081
	
	return cacheStatus ;

}

void ISMStorageManager::releaseCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &oper)
{
    ACS_CLASS_ANNOUNCE_DEBUG(ACS_MID_VERB) ;

    Timer timer ;
    timer.start() ;
    statN()["releaseCache"]+= 1 ;

	string ismpath;
	doAction(sminvid, oper.getFileName(*getObject()),  oper.getFileVersion(*getObject()), RELEASE_CACHE, ismpath) ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Release cache request for \"" << ismpath << "\" file sent");
			
    timer.stop() ;
    statN()["releaseCache"]["elapsed [ms]"]+= double(timer.elapsed()) ; // PRQA S 3081
	
}

bool ISMStorageManager::shallRemoveUnknownLinks()
{
	...
	Timer timer ;
    timer.start() ;
    statN()["shallRemoveUnknownLinks"]+=1 ;
	ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "SMId() " << SMId()) ;
	...
	try {	
		...
		if ( not rset.getValue ("SMConfiguration.RemoveUnknownLinks", removeUnknownLinks, false)) {
            ACS_CLASS_WRITE_DEBUG( ACS_LOW_VERB, "No SMConfiguration.RemoveUnknownLinks key found. Set to default value: " << boolalpha << removeUnknownLinks );
        }
		else { // just for debug
            ACS_CLASS_WRITE_DEBUG( ACS_LOW_VERB, "SMConfiguration.RemoveUnknownLinks key found, value: " << boolalpha << removeUnknownLinks );
		}
...
bool ISMStorageManager::uploading() // PRQA S 4020
{
	...
	shared_ptr<const std::exception> e = runException();
	if( e.get() )
	{
...
int ISMStorageManager::doAction(const dbPersistent::IdType sminvid, const std::string& fname, const std::string& fvers, const Action & action, string & ismpath)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_MID_VERB) ; // PRQA S 4631 2
	int retCode = 0;
	...
	try {	
		...
		string group ;
		rset.getValue("SMConfiguration.group", group);
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "group " << group) ;
		...
		switch (action)  {
    		case REMOVE_FILE: 
			{
				...
				// APF-270
				ACS_LOG_MC_INFO( mc_param("filename", fname) << mc_param("fileversion", fvers) << mc_param("fileid", ismpath) << mc_param("smid", SMId()) // PRQA S 3081
    		...
    		case IS_ONCACHE: 
			{ 
				dbFileAction::CacheStatus cacheStatus ;
				ismclient.isFileOnCache( ismpath, user, group, cacheStatus  ) ;	
				retCode = static_cast<int>(cacheStatus) ;
				break;
			}
    		case PUT_ONCACHE: 
			{ 
				dbFileAction::CacheStatus cacheStatus ;
				ismclient.putFileOnCache( ismpath, user, group, getClientName(), cacheStatus  ) ;	
				retCode = static_cast<int>(cacheStatus) ;
				break;
			}
    		case RELEASE_CACHE: 
			{ 
				ismclient.releaseCacheFile( ismpath, user, group  ) ;	
				break;
			}
			default:
        	{
		...
		switch (action)  {
    		case REMOVE_FILE: 
			{
				...
				if( dynamic_cast<IsmSoapClient::FileToDeleteOnMigratingMediumExc*> (&ex) ) {
					...
					exMsg << "Cannot erase file \"" << fname << "\" version \"" << fvers << "\" since medium migration is in progress; retry later." ;
				}
				else {
					mcErrorMsg = string("Erase error ") + ex.what() ;
					exMsg << "Error erasing file \"" << fname << "\" version \"" << fvers << "\"." ;
				}
				// APF-270
				ACS_LOG_MC_ERROR( mc_param("filename", fname) << mc_param("fileversion", fvers) << mc_param("fileid", ismpath) << mc_param("smid", SMId()) // PRQA S 3081
    		...
    		case GIVEUP_REQUEST: 
			{ 
				exMsg << "Error deleting request for file \"" << fname << "\" version \"" << fvers << "\"." ;
				break;
			}
    		case IS_ONCACHE: 
			{ 
				exMsg << "Error when checking if file \"" <<fname << "\" version \"" << fvers << "\" is on cache." ;
				break;
			}
    		case PUT_ONCACHE: 
			{ 
				exMsg << "Error when putting file \"" << fname << "\" version \"" << fvers << "\" on cache." ;
				break;
			}
    		case RELEASE_CACHE: 
			{ 
				exMsg << "Error when releasing file \"" << fname << "\" version \"" << fvers << "\" from cache." ;
				break;
			}
			default:
        	{
...
{	
	...
	switch (action)  {
		...
		case IS_ONCACHE:
			{
				actionString = "IsOnCache"; 
				break;
			}
		case PUT_ONCACHE:
			{
				actionString = "PutOnCache"; 
				break;
			}
		case RELEASE_CACHE:
			{
				actionString = "ReleaseCache"; 
				break;
			}
		default:
        	{

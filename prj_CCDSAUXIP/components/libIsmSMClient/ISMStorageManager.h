...
public:
	...
	*/
     ...
     * \returns dbFileAction::ON_CACHE if the inventory is actually on cache, dbFileAction::OFF_CACHE if it is not on cache
     * and dbFileAction::NOT_AVAILABLE if either the inventory is not known or corresponds to a file in ISM in not READY status (e.g. REMOVED, etc.)
	 */
	virtual dbFileAction::CacheStatus isOnCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &) ; 
	
    /**
	 * \brief asks the storage manager to put an inventory on cache 
	 * \param sminvid is the primary key into the t_inventorysm and into the t_storagemanager.invrelationtable
     * \param opif the adapter to deal with any dbPersistent
     * \returns dbFileAction::ON_CACHE if the inventory is actually on cache, dbFileAction::IN_PROGRESS if it is moving toward the cache 
     * and dbFileAction::NOT_AVAILABLE if either the inventory is not known or corresponds to a file in ISM in not READY status (e.g. REMOVED, etc.)
	 */
	virtual dbFileAction::CacheStatus putOnCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &) ;

	 /**
	 * \brief send the storage manager the request to delete a given file from cache. The SM accomplishes the request if file is STORED on media and cache is enabled
	 * \param sminvid is the primary key into the t_inventorysm and into the t_storagemanager.invrelationtable
     * \param opif the adapter to deal with any dbPersistent
     *
     * \note no operation neither exception if the file is not known.
	 */
	virtual void releaseCache(dbPersistent::IdType sminvid, const dbPersistentOperatorIF &) ;
	
	/**
	 * \brief If a specific file is UNKNOWN for the storage manager, the link between inventory and the archive handler (t_inventorysm record) can be either removed or not depending on configuration (default do remove).
...
private:
	...
	virtual int doAction(dbPersistent::IdType sminvid, const std::string& fname, 
							const std::string& fvers, const Action &, std::string & ismpath);

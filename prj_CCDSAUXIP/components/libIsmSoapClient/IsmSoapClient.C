...
void fillS3Handler( AwsS3Wrapper::AwsS3WrapperConfiguration & awsS3WrapperConfiguration, ACS_SMARTPTR<AwsS3Wrapper> &s3handler_p, const StopController & sc, IsmSoapClient::Operation *op = NULL) 
{
	...
	s3handler_p = ACS_SMARTPTR<AwsS3Wrapper>( new AwsS3Wrapper(awsS3WrapperConfiguration, sc) ) ;
	...
	for( size_t i = 0; i < sources_size; ++i ) {
		...
		try {
			...
			if( _packagingAlgoOnTheFly.empty() ) {
				...
				PackagingAlgo *packagingAlgo = PackagingAlgoSingleton::instance()->newPackagingAlgo(StringUtils::lowercase(_packagingAlgoOnTheFly), \
						 _parent.getPackageAlgoArchiveOptions(),   _parent.getPackageAlgoExtractOptions(),  _parent.getPackageAlgoFilesChangeFixStrategy() );
...
void convertCacheResponseCode(const Ism__onCacheResponseCodeEnum & retCode, dbFileAction::CacheStatus & onCacheStatus)
{
	switch (retCode)  {
    case onCacheCode_NOT_AVAILABLE: 
		{ 
			onCacheStatus = dbFileAction::NOT_AVAILABLE; 
			break;
		}
    case onCacheCode_OFF: 
		{ 
			onCacheStatus = dbFileAction::OFF_CACHE; 
			break;
		}
    case onCacheCode_IN_PROGRESS: 
		{ 
			onCacheStatus = dbFileAction::IN_PROGRESS; 
			break;
		}
    case onCacheCode_ON: 
		{ 
			onCacheStatus = dbFileAction::ON_CACHE; 
			break;
		}
	default:
        {
			ostringstream exMsg;
			exMsg << "Internal ERROR: Ism__onCacheResponseCodeEnum value not recognized: " << int(retCode); // PRQA S 3081 2
			ACS_THROW( exIllegalValueException( exMsg.str() ) ) ;
			break;
        }				
	}
...
bool IsmSoapClient::_emitWarning4GiveupFileRequest = true;
bool IsmSoapClient::_emitWarning4IsFileOnCache = true;
bool IsmSoapClient::_emitWarning4PutFileOnCache = true;
bool IsmSoapClient::_emitWarning4ReleaseCacheFile = true;
ThreadSafe IsmSoapClient::_endPointsMutex;
const int IsmSoapClient::_defaultSoapRecvTimeout ;
const int IsmSoapClient::_defaultSoapRecvTimeoutAttempts ;
...
IsmSoapClient::IsmSoapClient(StopController const * sc) : 
	...
	_packageAlgoFilesChangeFixStrategy(),
	_soapRecvTimeout( -1000 * _defaultSoapRecvTimeout ),  // negative value to set us -> 30 s
	_soapRecvTotalTimeout( _defaultSoapRecvTimeout * (_defaultSoapRecvTimeoutAttempts - 1) ),
	_serverVersion(-1),
	_clientReference()
{
...
void IsmSoapClient::setConnParameters(rsResourceSet const &rset, long long int smId) 
{
	...
	// fill here options maps
		...
		try {
			rset.getValue("SMConfiguration.OnTheFlyPackage.filesChangeFixStrategy", _packageAlgoFilesChangeFixStrategy);
			ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "SMConfiguration.OnTheFlyPackage.filesChangeFixStrategy:\n" << _packageAlgoFilesChangeFixStrategy) ;
			if( _packageAlgoFilesChangeFixStrategy.retryParameters.maxNumber < 0 ) {
				ACS_LOG_WARNING("SMConfiguration.OnTheFlyPackage.filesChangeFixStrategy.retryParameters.maxNumber value not permitted: " 
							<< _packageAlgoFilesChangeFixStrategy.retryParameters.maxNumber << "; setting to the default value" ) ;
				_packageAlgoFilesChangeFixStrategy.retryParameters.maxNumber = 0 ;
			}
		}
		catch (rsResourceSet::NotFoundException & )
		{}
	...
	int soapRecvTimeout_ms = _defaultSoapRecvTimeout ; // this set needed for next block
	try {
		rset.getValue("SMConfiguration.SoapRecvTimeout", soapRecvTimeout_ms);
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "SMConfiguration.SoapRecvTimeout: " << soapRecvTimeout_ms << " [ms]" ) ;
		if( soapRecvTimeout_ms <= 0 ) {
			ACS_LOG_WARNING("SoapRecvTimeout invalid value: " << soapRecvTimeout_ms << " (>0 expected). Setting default value: " << _defaultSoapRecvTimeout << " [ms]") ;
		}
		else {
			_soapRecvTimeout = soapRecvTimeout_ms * (-1000); // < 0 gives socket recv timeout in usec  
		}
	}
	catch (rsResourceSet::NotFoundException & )
	{}
	
	_soapRecvTotalTimeout = soapRecvTimeout_ms * ( _defaultSoapRecvTimeoutAttempts - 1 ) ;
	try {
		int soapRecvTimeoutAttempts = _defaultSoapRecvTimeoutAttempts ;
		rset.getValue("SMConfiguration.SoapRecvTimeoutAttempts", soapRecvTimeoutAttempts);
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "SMConfiguration.SoapRecvTimeoutAttempts: " << soapRecvTimeoutAttempts ) ;
		if( soapRecvTimeoutAttempts <= 0 ) {
			ACS_LOG_WARNING("SoapRecvTimeoutAttempts invalid value: " << soapRecvTimeoutAttempts << " (>0 expected). Setting default value: " << _defaultSoapRecvTimeoutAttempts ) ;
		}
		else {
			_soapRecvTotalTimeout = soapRecvTimeout_ms * ( soapRecvTimeoutAttempts - 1 ) ;
		}
	}
	catch (rsResourceSet::NotFoundException & )
	{}
	...
	ACS_CLASS_END_DEBUG
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "on the fly package algo files change fix strategy:\n" << _packageAlgoFilesChangeFixStrategy ) ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "soapRecvTimeout " << (-1)*_soapRecvTimeout << " [us]") ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "soapRecvTotalTimeout " << _soapRecvTotalTimeout << " [ms]") ;
...
void IsmSoapClient::internalFileUpload( fileDefinition const & f , string const &  localpath , bool recursive, bool checkPermissions, const long long int realFileSize, vector< pair<string , string> > & smEntryKeys )
{
		...
		for (int counter = 0; ; counter++ ) {
			...
			try {
				...
				if ( (soap_error_code = q.uploadSingleFile(&uploadRequest , &uploadResponse)) != SOAP_OK)
				{
					...
					if( ( SOAP_EOF == q.error ) && ( 0 == q.errnum ) ) { // exit with no retry since uploadSingleFile method is not stateless: it could return DuplicatedFileException
						error << "Soap message transfer TIMEOUT" ;
						ACS_THROW( SoapReceiveTimeOutException(error.str() ) ) ; // PRQA S 3081
					}
		...
		while( worker.running() )
		{
			...
			if( DateTime() > updateTime ) 
			{
				...
				for (int counter = 0; ; counter++ ) {
					...
					catch(SoapReceiveTimeOutException &e) {
						retryConnectionToServer( e.what(), timer, counter+1, endpointToTry, true );
					}
					catch(ConnectionException &e) {
						retryConnectionToServer( e.what(), timer, counter+1, endpointToTry );
					}
	...
	shared_ptr<const std::exception> workerException = worker.runException() ;
	if( workerException.get() != nullptr )
	{
		ostringstream error ;
							...
							bool checkPermissions, bool extendDirPermissions)
{
	...
	if( File::isDir(localpath) && _packageAlgo.empty() ) {
		...
		if( not _packageAlgo.empty() ) {
			...
			PackagingAlgo *packagingAlgo = PackagingAlgoSingleton::instance()->newPackagingAlgo(StringUtils::lowercase(f.packagingAlgo), \
											_packageAlgoArchiveOptions, _packageAlgoExtractOptions, _packageAlgoFilesChangeFixStrategy );
...
void IsmSoapClient::fileDownload( string  const &  filePath , int fileId, string  const & user ,  string  const & localpath , int timeOut, string const & group, bool checkPermissions, string const & clientReference ) // PRQA S 4020
{
	...
	try {
		...
		string extReqMsg;
		abortDownload( request, extReqMsg ) ;
...
bool IsmSoapClient::internalFileDownload( string const & localpath, Ism__DownloadFileRequest & request, int downloadTimeout,  int transferTimeout, string const & clientReference ) // PRQA S 4020
{
	...
	if(workerException.get() != nullptr)
	{
		if( dynamic_cast<const FileToDownloadDisappearedExc*> (workerException.get()) ) { // PRQA S 3081
...
void IsmSoapClient::directoryDownload( string  const &  filePath , int fileId, string  const & user ,  string  const & localpath , int timeOut, string  const & group, bool checkPermissions ) // PRQA S 4020
{
	...
	{
		string extReqMsg; 
		abortMultiDownload( request.filesRequest, localpath, extReqMsg ) ;
		ACS_LOG_ERROR(SimpleDebugSignature << " " << e.what() << "\n" << extReqMsg) ;
		return;
	} 
	catch( const exception &e ) 
		...
		try {
			...
			if( ( soap_error_code = q.giveupFileRequest(&request , &response) ) != SOAP_OK ) {
				...
				if( soap_tcp_error_check(soap_error_code) ) { // PRQA S 3700
					...
					ostringstream os;
					os << "Method not implemented: " << error.str() ;
					if( _emitWarning4GiveupFileRequest ) {
						ACS_LOG_WARNING( os.str() << ". No further notification of this type will be logged.");
						_emitWarning4GiveupFileRequest = false ;
					}
					else {
						ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, os.str() ) ;
					}
					return;
				}
				ACS_THROW( Exception(error.str() ) ) ; // PRQA S 3081
...
void IsmSoapClient::isFileOnCache( string  const &  filePath , string  const & user, string const & group, dbFileAction::CacheStatus & cacheStatus )
{
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "file path is " << filePath << " ; user  is " << user << " ; group is \"" << group << '\"') ;
	...
	for (int counter = 0; ; counter++ ) {
		...
		try {
			...
			Ism__onCacheRequest request;
			request.filePath = filePath ;
			request.user = user ;
			request.group = group ;
			ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB, "OnCacheRequest:\n" << request) ;

			Ism__onCacheResponseCodeEnum retCode ;
			int soap_error_code = -255;
			if( ( soap_error_code = q.isFileOnCache(&request , &retCode) ) != SOAP_OK ) {
				ostringstream error ;
				error << "isFileOnCache request SrvErr[ " << soap_error_code << "]: " ;
				...
				if( soap_tcp_error_check(soap_error_code) ) { // PRQA S 3700
					...
					ostringstream os;
					os << "Method not implemented: " << error.str() ;
					if( _emitWarning4IsFileOnCache ) {
						ACS_LOG_WARNING( os.str() << ". No further notification of this type will be logged.");
						_emitWarning4IsFileOnCache = false ;
					}
					else {
						ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, os.str() ) ;
					}
					cacheStatus = dbFileAction::ON_CACHE ;
					return;
				}
				ACS_THROW( Exception(error.str() ) ) ; // PRQA S 3081
			}
			ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB, "OnCacheResponse:\n" << retCode) ;
			convertCacheResponseCode(retCode, cacheStatus) ;
			ACS_CLASS_WRITE_DEBUG_TH(ACS_HIG_VERB, "Converted cacheStatus: " << cacheStatus ) ;
			mapIt->second.setGoodEndPoint(endpointToTry) ;
...
void IsmSoapClient::putFileOnCache( string  const &  filePath , string  const & user, string const & group, string const & clientReference, dbFileAction::CacheStatus & cacheStatus )
{
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "file path is " << filePath << " ; user  is " << user << " ; group is \"" << group << "\"; client reference is \"" << clientReference << '\"') ;
	...
	for (int counter = 0; ; counter++ ) {
		...
		try {
			...
			Ism__onCacheResponseCodeEnum retCode ;
			int soap_error_code = -255;
			if( ( soap_error_code = q.putFileOnCache(&request , &retCode) ) != SOAP_OK ) {
				ostringstream error ;
				error << "putFileOnCache request SrvErr[ " << soap_error_code << "]: " ;
				...
				if( soap_tcp_error_check(soap_error_code) ) { // PRQA S 3700
					...
					ostringstream os;
					os << "Method not implemented: " << error.str() ;
					if( _emitWarning4PutFileOnCache ) {
						ACS_LOG_WARNING( os.str() << ". No further notification of this type will be logged.");
						_emitWarning4PutFileOnCache = false ;
					}
					else {
						ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, os.str() ) ;
					}
...
void IsmSoapClient::releaseCacheFile( string  const &  filePath , string  const & user, string const & group )
{
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "file path is " << filePath << " ; user  is " << user << " ; group is \"" << group << '\"') ;
	...
	for (int counter = 0; ; counter++ ) {
		...
		try {
			...
			Ism__releaseCacheResponse response ;
			int soap_error_code = -255;
			if( ( soap_error_code = q.releaseCacheFile(&request , &response) ) != SOAP_OK ) {
				ostringstream error ;
				error << "Release cache file request SrvErr[ " << soap_error_code << "]: " ;
				...
				if( soap_tcp_error_check(soap_error_code) ) { // PRQA S 3700
					...
					ostringstream os;
					os << "Method not implemented: " << error.str() ;
					if( _emitWarning4ReleaseCacheFile ) {
						ACS_LOG_WARNING( os.str() << ". No further notification of this type will be logged.");
						_emitWarning4ReleaseCacheFile = false ;
					}
					else {
						ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, os.str() ) ;
					}
...
void IsmSoapClient::getAWS_S3ObjectMetadata( const string & bucket, const string & cachePath, const string & fileUrl, multimap <AwsS3Wrapper::Enum_S3MetadataTag, string> & objectsMap, Ism__cacheConfiguration & cacheConfiguration )
{
	...
	try {
		...
		ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "AwsS3WrapperConfiguration filled by Ism__cacheConfiguration:\n" << awsS3WrapperConfiguration );
...
void IsmSoapClient::retryConnectionToServer( string const & errMsg, Timer & timer, int attempt, unsigned short &endpointToTry, bool soapRecvTimeout) // PRQA S 4211
{
// retry added for soap timeout errors too, since in stress cases requests are accepted without being served, for all configured soap threads are engaged. If IsmManager stops for phoenix,
// All queued requests are thrown and Client could interpret such a behaviour as a timeout error even if configured soap timeout is not expired ( see jira issue LTA-407 )
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_HIG_VERB) ;
	...
	unsigned int timeout_ms = _connectTimeout_ms;
	{ // while in this scope - endpoint are locked
		...
		if( soapRecvTimeout ) {
			endpNo = 1 ;
			allPortsEqualNo = true ;
			timeout_ms = _soapRecvTotalTimeout ;
		}
		else {
		...
		if ( (timer.elapsed() >= timeout_ms)  && allPortsEqualNo ) {
			ostringstream error ;
			error	<< "Timeout (" << timeout_ms << " ms) attempts: " << attempt
	...
	if (allPortsEqualNo && not soapRecvTimeout) {
...
void IsmSoapClient::setSoapRecvTotalTimeout( const int timeout_ms) 
{
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_MID_VERB) ;
	...
	_soapRecvTotalTimeout = timeout_ms ;
	ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "soapRecvTotalTimeout " << _soapRecvTotalTimeout << " [ms]") ;
...
void IsmSoapClient::abortDownload( Ism__DownloadFileRequest & request, std::string & outmsg ) 
{
	...
	if( request.requestId != -1 ) {
		...
		{
			ostringstream error ;
			error 	<< "Even failed to remove ext request:  AbortDownload SrvErr[ " << soap_error_code << "]: " ;
			if( ( SOAP_EOF == q.error ) && ( 0 == q.errnum ) ) {
				error << "Soap message transfer TIMEOUT" ;
			}
			else {
			...
			outmsg = "Ext request delete instruction successfully executed.";
        }

	}
	else { // just for debug
		outmsg = "No external request removed since id not valorized" ; 
		ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "External request id not valorized: no need to abort") ;
	}
...
// Still to test since directory management is not supported in this phase
void IsmSoapClient::abortMultiDownload( Ism__downloadMultiFilesRequest * filesRequest, std::string const & localpath, std::string & outmsg ) 
{
	ACS_CLASS_ANNOUNCE_DEBUG_TH(ACS_MID_VERB) ;
	...
	if( filesRequest->jobId >= 0 ) {
		ACS_CLASS_WRITE_DEBUG_TH(ACS_LOW_VERB, "going to remove " << filesRequest->requestsIds.size() << " external requests (job_id=" 
				   <<  filesRequest->jobId << ")") ;
		...
		Ism__notifyDownloadResponse abortResponse ;
		int soap_error_code = -255;
		if ((soap_error_code = q.abortMultiFilesDownload(filesRequest , &abortResponse)) != SOAP_OK)
		{
			ostringstream error ;
			error 	<< "Even failed to remove ext requests: AbortDownload SrvErr[ " << soap_error_code << "]: " << q.soap_fault_string() ;
			outmsg = error.str() ;
		}
		else {
			outmsg = "Ext requests successfully removed";
        }
	}
	else { // 
		ACS_CLASS_WRITE_DEBUG_TH(ACS_MID_VERB, "job_id not valorized: no need to abort") ;
	}

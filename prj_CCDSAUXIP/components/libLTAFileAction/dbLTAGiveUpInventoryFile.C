...
namespace {
    StatNode &statN() { return lta::stat()["GiveUp"] ; }
}

ACS_CLASS_DEFINE_DEBUG_LEVEL(dbLTAGiveUpInventoryFile)

using acs::dbConnectionWrapper ;

// constructor
dbLTAGiveUpInventoryFile::dbLTAGiveUpInventoryFile(dbConnectionPool &pool, db::LTAInventoryObject &s) :
	dbGiveUpFile(pool, s) ,  
	_inventory(s),
...
void dbLTAGiveUpInventoryFile::start() 
{
    try {
        statN()["started"] += 1 ;
        Timer timer ;
	    ...
	    ACS_LOG_PRIVINFO("Start giving up file \"" <<_inventoryName << "\"") ;
	    ...
	    SQLString sqlString = getSyncSMsForDownloadQuery();
	    dbSet smset;
	    {
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "The file \"" << _inventoryName << "\" can be gived up from " << smsetsize << " synchronous SMs.") ;
...
SQLString dbLTAGiveUpInventoryFile::getSyncSMsForDownloadQuery() const
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB) ;

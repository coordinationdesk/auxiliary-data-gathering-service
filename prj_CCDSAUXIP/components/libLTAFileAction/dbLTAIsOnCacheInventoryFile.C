...
ACS_CLASS_DEFINE_DEBUG_LEVEL(dbLTAIsOnCacheInventoryFile)

using acs::dbConnectionWrapper ;

// constructor
dbLTAIsOnCacheInventoryFile::dbLTAIsOnCacheInventoryFile(dbConnectionPool &pool, db::LTAInventoryObject &s, int storageid) :
	dbIsOnCacheFile(pool, s, storageid) ,  
	_inventory(s),
...
void dbLTAIsOnCacheInventoryFile::start() 
{
    try {
        statN()["started"] += 1 ;
        Timer timer ;
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Start checking status for file \"" <<_inventoryName << "\"") ;
	    ...
	    SQLString sqlString = getSyncSMsForIsOnCacheQuery(getStorageId());
	    dbSet smset;
	    {
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Status of file \"" << _inventoryName << "\" can be checked from " << smsetsize << " synchronous SMs.") ;
        ...
        if ((0 == smsetsize) && (getStorageId() != 0)) {
            ACS_THROW(OnCacheException() << "Cannot check cache status for " << _inventoryName << "on SM id: " << getStorageId()) ;
        }
...
SQLString dbLTAIsOnCacheInventoryFile::getSyncSMsForIsOnCacheQuery(int storageid) const
{

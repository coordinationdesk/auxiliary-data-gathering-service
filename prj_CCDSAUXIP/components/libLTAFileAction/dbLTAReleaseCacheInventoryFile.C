...
namespace {
    StatNode &statN() { return lta::stat()["ReleaseCache"] ; }
}

ACS_CLASS_DEFINE_DEBUG_LEVEL(dbLTAReleaseCacheInventoryFile)

using acs::dbConnectionWrapper ;

// constructor
dbLTAReleaseCacheInventoryFile::dbLTAReleaseCacheInventoryFile(dbConnectionPool &pool, db::LTAInventoryObject &s, int storageid) :
	dbReleaseCacheFile(pool, s, storageid) ,  
	_inventory(s),
...
void dbLTAReleaseCacheInventoryFile::start() 
{
    try {
        statN()["started"] += 1 ;
        Timer timer ;
	    ...
	    ACS_LOG_PRIVINFO("Start releasing cache file \"" <<_inventoryName << "\"") ;
	    ...
	    SQLString sqlString = getSyncSMsForReleaseQuery(getStorageId());
	    dbSet smset;
	    {
	    ...
	    ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "The file \"" << _inventoryName << "\" can be released from " << smsetsize << " synchronous SMs.") ;
...
SQLString dbLTAReleaseCacheInventoryFile::getSyncSMsForReleaseQuery(int storageid) const
{

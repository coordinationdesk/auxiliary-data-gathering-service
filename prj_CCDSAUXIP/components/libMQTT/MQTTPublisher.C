...
MQTTPublisher::MQTTPublisher(const std::string& name, const MQTTWorker::Configuration& config, StopController const& stopController ) :
...
void MQTTPublisher::setTopic(const std::string& topic)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB)
	...
	{
		ACS_THROW ( MQTTPublisher::BadTopic () << "Trying to set an invalid publishing topic. Error is: " << mqtterr );
	}
 ...
 */
int MQTTPublisher::publishText (const std::string& message, MQTTWorker::MQTT_QOS qos)  
{
	return internalPublish (message.c_str(), static_cast<int>(message.size()), "", qos, false);
}
...
int MQTTPublisher::publishTextOnTopic (const std::string& message, const std::string& topic, MQTTWorker::MQTT_QOS qos)
{
	...
	return internalPublish (message.c_str(), static_cast<int>(message.size()), topic, qos, false);
}

int MQTTPublisher::publishBinary (void* payload, int payloadsize, MQTTWorker::MQTT_QOS qos)  // NOSONAR
{
	return internalPublish (payload, payloadsize, "", qos, false);
}

int MQTTPublisher::publishBinaryOnTopic (void* payload, int payloadsize, const std::string& topic, MQTTWorker::MQTT_QOS qos) // NOSONAR
{
	std::string errmsg;
	bool tpcValid = checkTopicWithError(topic, errmsg);
	if (!tpcValid) {
...
int MQTTPublisher::publishRetainText (const std::string& message, MQTTWorker::MQTT_QOS qos)  
{
	return internalPublish (message.c_str(), static_cast<int>(message.size()), "", qos, true);
}
...
int MQTTPublisher::publishRetainTextOnTopic (const std::string& message, const std::string& topic, MQTTWorker::MQTT_QOS qos)
{
	...
	return internalPublish (message.c_str(), static_cast<int>(message.size()), topic, qos, true);
}

int MQTTPublisher::publishRetainBinary (void* payload, int payloadsize, MQTTWorker::MQTT_QOS qos)  // NOSONAR
{
	return internalPublish (payload, payloadsize, "", qos, true) ;
}

int MQTTPublisher::publishRetainBinaryOnTopic (void* payload, int payloadsize, const std::string& topic, MQTTWorker::MQTT_QOS qos) // NOSONAR
{
	std::string errmsg;
	bool tpcValid = checkTopicWithError(topic, errmsg);
	if (!tpcValid) {
...
{
	return publishRetainBinary (nullptr, 0, QOS_2);
}
...
int MQTTPublisher::cleanRetainedMessageOnTopic (const std::string& topic)
{
	...
	return publishRetainBinaryOnTopic (nullptr, 0, topic, QOS_2);
}

int MQTTPublisher::internalPublish (const void* payload, int payloadsize, const std::string& topic, MQTTWorker::MQTT_QOS qos, bool retain) // NOSONAR
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_LOW_VERB)
#ifndef DISABLE_MQTT
...

	{
		...
		Mutex::Lock lock (_queueCondition);

		// Fire the message
		int mqResult = mosquitto_publish (getClient(), &messageId, tpc.c_str(), payloadsize, payload, qos, retain);
		if ( MOSQ_ERR_SUCCESS != mqResult )
		{
			ACS_THROW (ConnectionError () << "Publish message failed. Error is " << mosquitto_strerror (mqResult) );
		}

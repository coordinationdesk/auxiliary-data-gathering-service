...
ACS_CLASS_DEFINE_DEBUG_LEVEL(ObjStorageManager::Simulate);


ObjStorageManager::Simulate::Simulate()
{
}

ObjStorageManager::Simulate::~Simulate() throw() {}

bool ObjStorageManager::Simulate::simulate() const 
{
...
ObjStorageManager::ObjStorageManager(const string &smtype, dbPersistent::IdType smid,const dbPersistent* obj, dbConnectionPool &pool, bool ddl) : 
	...
	_invRelationTable(),
	_readConfiguration(true),
	_removeUnknownLinks(false),
	_ddl(ddl),
	_check()

{
    statN()["instances"]+=1 ;
	ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Type "<< smtype << " ID " << smid << " DDL " << boolalpha << _ddl ) ;
...
{	
	...
	if( _readConfiguration ) {
		getParameters();
	}
	else { // just for debug
		ACS_CLASS_WRITE_DEBUG( ACS_HIG_VERB, "Configuration already read" ) ;
	}
...
void ObjStorageManager::upload(const string &path, const dbPersistentOperatorIF& oper )
{
	...
	try {	
		...
		try {		
			...
			if( _ddl ) {
				...
				const string storedProc("p_inventory_inc_ddl_count") ;
				string command ;
				const std::string& dialectName = SQLDialectFactory::dialectName(dbKey);
				if( "Postgres8" == dialectName ) {
					ostringstream os;
					os << "CALL " << storedProc << '(' << this->getOper()->getId(*getObject()) << ')';
					command = os.str() ;
				}
				else {
					ACS_THROW ( exIllegalValueException("Unsupported sql dialect " + dialectName) );
				}
				SQLString sqlStringProc (dbKey,SQLString::COMMAND) ;
				sqlStringProc.forceSQLString(command) ;
				ACS_CLASS_WRITE_DEBUG(ACS_MID_VERB, "Going to execute the following command: " << sqlStringProc.getSQL()) ;
				
				dbQuery queryProc(myconn, sqlStringProc, "Execute " + storedProc ) ;	
				if ( not queryProc.execForStatus() ) {
				   ACS_THROW( dbQuery::QueryExecuteException("Cannot execute the following command: " + sqlStringProc.getSQL() + " Error : " + queryProc.errorString()) ) ;
				}
...
void ObjStorageManager::erase(dbPersistent::IdType sminvid, const dbPersistentOperatorIF& oper, string* p) 
{
	ACS_COND_THROW(getObject() == 0, ObjStorageManagerException("Base class _obj attr pointer attribute is NULL!")) ; // PRQA S 3081
	erase(sminvid, oper.getFileName(*getObject()), oper.getFileVersion(*getObject()), p);
}

void ObjStorageManager::erase(dbPersistent::IdType sminvid, const std::string &fname, const std::string &fvers, std::string* p)
{
	ACS_CLASS_ANNOUNCE_DEBUG(ACS_MID_VERB) ; // PRQA S 4631
	...
	bool simulate = _check.simulate() ;
	if (simulate) {
		ACS_LOG_WARNING("Obj SM in simulation mode in erase method. ") ;
	}
	...
	try {	
		...
		// APF-270
		ACS_LOG_MC_INFO( mc_param("filename", fname) << mc_param("fileversion", fvers) << mc_param("fileid", smpath) << mc_param("smid", SMId()) // PRQA S 3081
	...
	{
		// APF-270
		ACS_LOG_MC_ERROR( mc_param("filename", fname) << mc_param("fileversion", fvers) << mc_param("fileid", smpath) << mc_param("smid", SMId()) // PRQA S 3081
							<< mc_param("message", string("Erase error ") + ex.what()) 
							<< mc_metric(Metric_FAIL) <<  MCCode ("SM") << MCFunction ("Archive")) ;
        ACS_LOG_EVENT("ObjSM_IF",EXFE_ERROR) ;
		ACS_THROW(ObjStorageManagerException(ex, "Error erasing file \"" + fname + "\" version \"" + fvers + "\".")) ;
	}

	// get inventory id if exists
	dbSmartPersistent invsmSp(getInvSmTableName(), theConnection);
	invsmSp.load(theConnection, sminvid);

	bool nullInvId = false;
	dbPersistent::IdType invId = 0;
	invsmSp.get("inv_id", invId, nullInvId, theConnection);
	if (invId == 0 || nullInvId) {
		ACS_CLASS_WRITE_DEBUG(ACS_LOW_VERB, "Inventory id is null or 0. DDL management will not be executed.");
	}

	bool invSMDeleted = false ;	
	const string storedProc("p_inventory_dec_ddl_count") ;

	bool handleDDL = (_ddl && !nullInvId && invId != 0);
	try
	{
        db::Transaction transaction(theConnection) ;
		
		if( handleDDL ) { // start transaction just in this case
			transaction.begin_work("DEL InvRel") ;
		}
		...
		if( handleDDL ) {
			...
			dbQuery queryProc(theConnection, sqlStringProc, "Execute " + storedProc ) ;	
			if ( not queryProc.execForStatus() ) {
			   ACS_THROW( dbQuery::QueryExecuteException("Cannot execute the following command: " + sqlStringProc.getSQL() + " Error : " + queryProc.errorString()) ) ;
			}
			
			ACS_COND_THROW (simulate, ObjStorageManagerException("THIS IS A TEST simulating " + storedProc + " sp error")) ;
	...
	catch (exception &e)
	{
		...
		if( invSMDeleted ) {
			mcMessage << storedProc << " execution on inventory[" << invId << "] failed and consequently " << getInvSmTableName() << " record[" << sminvid << "] not deleted since in trasaction";
			exMsg << storedProc << " execution on inventory[" << invId << "] failed and consequently " << getInvSmTableName() << " record[" << sminvid 
					<< "] deletion failed since in trasaction. Inventory could be incoherent for SmId." << SMId() << " inventory["<< invId <<"]: " << fname << " version " << fvers ;
		}
		else {
			mcMessage << getInvSmTableName() << " record[" << sminvid << "] deletion failed ";
			exMsg << getInvSmTableName() << " record[" << sminvid << "] deletion failed. Inventory could be incoherent for SmId." << SMId() 
		   			<< " inventory["<< invId <<"]: " << fname << " version " << fvers ;
		}
		...
		ACS_THROW(ObjStorageManagerException(e, exMsg.str() )) ; // PRQA S 3081
	}
...
void ObjStorageManager::getParameters() 
{
	...
	rsResourceSet conf ;
	try {
		SMInterface::getParameters(dbConnectionWrapper( getPool() ), paramfile, conf, _invRelationTable, statN()) ;
	}
	catch(exception &e) {
	...
	//*********************   load parameters *********************************
	{
		...
		if ( not conf.getValue ("RemoveUnknownLinks", _removeUnknownLinks, false)) {
            ACS_CLASS_WRITE_DEBUG( ACS_LOW_VERB, "No ObjSMClient.RemoveUnknownLinks key found. Set to default value: " << boolalpha << _removeUnknownLinks );
        }
	...
	ACS_CLASS_BGN_DEBUG(ACS_LOW_VERB)
		...
		msg << "\tSignature Algo=\"" << _signatureAlgo << '\"'
			...
			<< "\n\tRemoveUnknownLinks=" << _removeUnknownLinks
...
bool ObjStorageManager::shallRemoveUnknownLinks()
{
	...
	return _removeUnknownLinks ;
}

bool ObjStorageManager::uploading() // PRQA S 4020
{
	if( running() )
	{
	...
	if( e.get() )
	{
		ObjStorageManagerException e2(*e,"Error in uploading thread");
		ACS_THROW(e2);
	}
...
bool ObjStorageManager::isUploadSynchronous() const 
{
	return true ;
}

bool ObjStorageManager::isDownloadSynchronous() const 
{
	return true ;
}


bool ObjStorageManager::isEraseSynchronous() const 
{
	return true ;
}

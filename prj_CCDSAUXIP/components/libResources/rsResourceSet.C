...
bool operator==(const rsResourceSet::rsValue &r1, const rsResourceSet::rsValue &r2) noexcept 
{
...
{
	v = static_cast<unsigned char>(strtoul(_value.c_str(), nullptr, 10)) ; // PRQA S 3081, 4400
}
...
{
	v = strtoul(_value.c_str(), nullptr, 10) ; // PRQA S 4400
}
...
{
	v = static_cast<unsigned short>(strtoul(_value.c_str(), nullptr, 10)) ; // PRQA S 3081, 4400
}
...
{
	v = strtoll(_value.c_str(), nullptr, 10) ;  // PRQA S 4400
}

template<>
void rsResourceSet::rsValue::get< long long unsigned int>(long long unsigned int &v) const
{
	v = strtoull(_value.c_str(), nullptr, 10) ;  // PRQA S 4400
}

template<>
void rsResourceSet::rsValue::get<unsigned int>(unsigned int &v) const
{
	v = size_t(strtoul(_value.c_str(), nullptr, 10)) ; // NOSONAR - you asked for a uint ... 
}
...
{
	ostringstream os ;
	os << std::setiosflags(std::ios::fixed) << std::setprecision(20) << v ; // PRQA S 4400
	setString(os.str()) ;
}
...
{
	ostringstream os ;
	os << std::setiosflags(std::ios::fixed) << std::setprecision(17) << v ; // PRQA S 4400
	setString(os.str()) ;
}
...
{
	ostringstream os ;
	os << std::setiosflags(std::ios::fixed) << std::setprecision(8) << v ; // PRQA S 4400
	setString(os.str()) ;
}
...
{
	copy(_comments.begin(), _comments.end(),std::ostream_iterator<string>(os,"\n")) ;
	if (os.rdstate()) { // PRQA S 3054
		ostringstream o ;
...
rsResourceSet::rsResourceSet(const rsResourceSet &rs) 
{
	...
	*this = rs ; // uses assignment
}

rsResourceSet::~rsResourceSet() noexcept 
{
// PRQA S 4631 L1
    try {
...
rsResourceSet &rsResourceSet::operator=(const rsResourceSet &rs)
{
	if (this != &rs) {
		...
		for (size_t i=0; i < rs.size(); ++i) {
			...
			rsValue *v = new rsValue(*rs._resources.find(k)->second) ; // NOSONAR
			_resources.insert(std::pair<string,std::unique_ptr<rsValue>>(k,v)) ;
...
bool rsResourceSet::isEqual(const rsResourceSet &rm) const noexcept // PRQA S 4020, 4214
{
	...
	for(const auto &key: _resOrdered) {
		auto r = rm._resources.find(key) ;
...
{
	Lock const lock(_mutex) ;
	auto p = _resources.find(composeArrayKey(res)) ;
    if (p == _resources.end()) {
        return nullptr ;
    }
    else {
...
{
	Lock const lock(_mutex) ;
	auto p = _resources.find(composeKey(res)) ;
	return p != _resources.end() ? p->second.get() : nullptr ;  // PRQA S 3380, 3382
}
...
rsResourceSet::rsValue *rsResourceSet::findOrInsertValue(const std::string &res) 
{
	...
	if (!value) { 
		value = new rsValue(composeKey(res)) ; // NOSONAR
		insert(value) ;
    }
    return value ; 
}
...
size_t rsResourceSet::getArraySize(const string &res, bool *isSingle) const // PRQA S 4020, 4214
{
	...
	if (!value) {
		...
		return (value ?  1 : 0) ; // 1 if a single value // PRQA S 3000, 3382
	}
...
{
	Lock const lock(_mutex) ;
	_resources.insert(std::pair<string,std::unique_ptr<rsValue>>(v->key(),v)) ;
...
{
	Lock const lock(_mutex) ;
	string kk = composeKey(res) ; // obtain absolute path of the key
    
    // from now on all the keys are absolute
	string key = kk + "[]" ;
	
	ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "deleting: " << key) ;
	auto atbd = _resources.find(key) ;
	if (atbd != _resources.end()) {
		_resOrdered.erase(find(_resOrdered.begin(),_resOrdered.end(),key)) ;
		_resources.erase(atbd) ;
	    std::vector<std::string> _new_resOrdered ; // prepare a new vector of contents
        string rootkey = kk + "[" ; // radix of the array as in aa[...
        for (const auto &k: _resOrdered) { // every key 
            if (0 == k.find (rootkey, 0)) { // rootkey is entirely at beginning: candidate to be removed
			    auto etbd = _resources.find(k) ;
			    ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "deleting: " << k) ;
			    if (etbd != _resources.end()) { // NOSONAR - nesting
			        ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, k << " deleted") ;
				    _resources.erase(etbd) ; // remove from the map
			    }
            }
            else {
                _new_resOrdered.push_back(k) ; // is not part of the array - keep it
            }
        }
        _resOrdered.swap(_new_resOrdered) ; // new vector just filled is swapped
	}
}
...
{
	Lock const lock(_mutex) ;
	if (!k && !v && !c) { return _resources.size() ; } // fast way out
		
	size_t i = 0 ;
	for (auto it =_resources.begin();it != _resources.end(); ++it,++i) {
		if (k) { k[i] = it->second->key() ; }
		if (v) { it->second->get(v[i]) ; }
		if (c) { c[i] = it->second->comments() ; }
	}
...
bool rsResourceSet::hasKey(const string &res, bool &isArray) const // PRQA S 4020, 4214
{
	...
	const rsValue *value = findValue(res) ;
	if (!value) {
		value = findValue(res+"[]") ; // test array
...
bool rsResourceSet::containsOnly(const string &s1, const string &s2)  // PRQA S 4020
{
	...
	return true ; // S1 is composed only by chars contained in S2 or empty
}
...
string &rsResourceSet::strip(string &s)
{
	...
	for (long i = len-1; i >= 0; --i) {
        if (! isspace(s[i])) { // PRQA S 3000, 3054
			end = i ;
			break ;
		}
    }
...
string &rsResourceSet::strip(string &s, const string &tbs)
{
	...
	long end = 0 ;
	for (long i = len-1; i >= 0; --i) {
        if (! isspace(tbs[i])) { // PRQA S 3000, 3054
			end = i ;
			break ;
		}
	}	
...
void rsResourceSet::merge(const rsResourceSet &other)
{
	...
	try {	
		...
		for (size_t i=0; i < len; ++i) { // every entry in the other rs
			...
			rsValue const &otherValue = *other._resources.find(other._resOrdered[i])->second.get() ;

			// check if present
			const string &otherKey = otherValue.key() ;
			auto myIter = _resources.find(otherKey) ;
			...
			if (myIter != _resources.end()) { // found
				// get the value 
				rsValue &myValue = *myIter->second.get() ;

				switch (_mergeMode) {
				case ModeUnion:
					ACS_CLASS_WRITE_DEBUG_TH(ACS_VLO_VERB, "checking isArray\"" << otherKey) ;
					if (isArray(otherKey)) {  // NOSONAR - nesting
                        ...
                        myValue.set(std::max(otherSize, mySize)) ;
				...
				default:
                    ACS_THROW(exIllegalValueException() << "Invalid MergeMode: " << int(_mergeMode)) ;  // PRQA S 3081
				}
			}
			else {
				insert(new rsValue(otherValue)) ;
...
std::istream &operator>>(std::istream &str, rsResourceSet &rs)
{
	...
	while (!str.eof()) {
		string k ;
		char line[4097] ; // NOSONAR - fast and cleaner 
		...
		lineno++ ;
		ACS_FUNC_WRITE_DEBUG_TH(rsResourceSet,ACS_VLO_VERB, "line: " << lineno << " \"" << line << "\"") ;
					
		const string & tmp=line;
		// empty ? go to next
		if (rsResourceSet::containsOnly(tmp,emptyLineContains)) { continue ; } 
        ...
        case '!':
        case ';':                
            comments.push_back(rsResourceSet::strip(cmt, tmp)) ;
            continue ;
        default:
            break ; /* nothing */ 
        }
		...
		if (0 == tmps) {
			...
			if (n == string::npos) {
				...
				n = tmp.find(s) ;
				if (n == string::npos) { // NOSONAR - nesting
					ACS_THROW(rsResourceSet::FileFormatException() << "Resource line: " << lineno << " \"" << tmp << "\"" ) ; // PRQA S 3081
				}
			}
			tmps = s ;
		}
		...
		rsResourceSet::rsValue *v = new rsResourceSet::rsValue(k) ; // NOSONAR
		v->_comments.swap(comments) ; // note: comments are stripped 
...
{
	Lock const lock(_mutex) ;
	auto tbd = find( _nameSpaces.begin(), _nameSpaces.end(),n) ;

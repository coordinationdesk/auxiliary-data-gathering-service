...
#ifndef _TrkFilterable_H_
#define _TrkFilterable_H_
...
public:
	TrkFilterable() ;
	explicit TrkFilterable(const std::string&);
	virtual ~TrkFilterable() throw() ;
	
	/// convenience function doing
	//
	// TrkFilterable m(originator) ;
	// m << msg ;
	// os << msg ;
	//
...
private:
	...
	TrkFilterable(const TrkFilterable &) ;
	TrkFilterable &operator=(const TrkFilterable &) ;
} ;

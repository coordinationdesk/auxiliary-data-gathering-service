...
public:
    ...
    StringUtils() = delete  ; // this is a container class, do not need to be instantiated
    ~StringUtils() = delete ;
...
// MP deprecated method to ber removed ASAP
    ...
    static std::string __attribute__((deprecated("use std::reverse instead"))) reverse(const std::string &source) ; // PRQA S 2502 3
...
template<typename T> inline __attribute__((deprecated("use std::to_string"))) std::string StringUtils::toString(const T& value) {
    std::ostringstream os;
    os << value;
    return os.str();
}

template<typename T> inline T __attribute__((deprecated("error prone - use std::stoXX family"))) StringUtils::fromString(const std::string& value) {
...
template <typename T> inline std::vector<T> __attribute__((deprecated("uses deprecated methods and its name is confusing"))) StringUtils::ExtractFromString(const std::string& in)
{

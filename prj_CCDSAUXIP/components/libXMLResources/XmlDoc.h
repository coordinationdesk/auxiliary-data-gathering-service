...
public:
	...
	/** npath is a compacted form of xpath to handle xml files with namespaces
	 * and section identification via child node value
	 * using 4 substitutions to be interpreted as follows :    
	 * { } -> ignore namespace
	 * $ -> with son named
	 * # -> equal to
	 * ## -> value
	...
	/** converts npath to xpath */
	static std::string XNpath(const std::string& Npath);

	/** converts xpath to more compact npath */
	static std::string Npath(const std::string& Xpath);
	...
	/** Get a value from xml npath (see Npath above)*/
	template < typename T >
	bool getValueN( std::string const& npath, T& val, bool mandatory = true ) const;
...
template < typename T >
bool XmlDoc::getValueN( std::string const& npath, T& val, bool mandatory) const{ // PRQA S 4020;

	try {
		bool found = getValue(XNpath(npath),val,mandatory);
		return found;

	} catch(std::exception& ex) {
		ACS_THROW( exXmlDocNotFoundException(ex, "Error getting xml npath ["+npath+"]" ) );
	}
}
